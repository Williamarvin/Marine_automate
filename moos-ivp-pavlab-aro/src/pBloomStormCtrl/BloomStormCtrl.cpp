...
bool BloomStormCtrl::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if ((key == "NODE_REPORT") || (key == "NODE_REPORT_LOCAL")) {
      ...
      std::cout << " Just got mail for Sample LOC = " << msg.GetString()
                << std::endl;
      ...
      std::map<std::string, double>::iterator iter =
          m_map_sampling_agents.find(msg.GetString());
...
bool BloomStormCtrl::Iterate() {
  ...
  std::cout << " ----------- After AppCastingMOOSApp::Iterate--------------- "
            << std::endl;
...
bool BloomStormCtrl::OnStartUp() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    if (param == "region_1_spec") {
      ...
      handled =
          setDoubleOnString(m_time_since_last_migration_resistance_val, value);
      ...
      handled =
          setDoubleOnString(m_time_since_no_samples_encouragement_slope, value);
...
bool BloomStormCtrl::buildReport() {
  ...
  actab2 << m_curr_option << doubleToString(m_odometry)
         << doubleToString(m_current_batt_voltage)
         << uintToString(m_bloom_found_counter);
  ...
  if (m_map_sampling_agents.find(m_vname) != m_map_sampling_agents.end()) {
    m_msgs << "                                                           "
           << endl;
    m_msgs << " Sampling now for : "
           << doubleToString(MOOSTime() - m_map_sampling_agents[m_vname])
           << std::endl;
  }

  double dt_no_samples = MOOSTime() - m_time_of_last_sample_available;
  m_msgs << "   Debug: time since no samples avail = "
         << doubleToString(dt_no_samples, 4) << endl;
  m_msgs << "   Debug:  m_time_of_last_sample_available = "
         << doubleToString(m_time_of_last_sample_available, 4) << endl;
  m_msgs << "                                                           "
         << endl;
  m_msgs << "=  Input Values  =========================================="
         << endl;
  ...
  for (it = m_opinion_inputs.begin(); it != m_opinion_inputs.end(); it++) {
    m_msgs << "  " << it->first << ": " << doubleToStringX(it->second, 4)
           << endl;
  }

  m_msgs << "                                                           "
         << endl;
  m_msgs << "=  Sample Points: "
         << "(" << uintToString(m_bloom_records.size()) << ")           "
         << endl;
  m_msgs << "=  Distance To Closest: "
         << doubleToString(m_dist_to_closest_sample) << "           " << endl;
  m_msgs << "==========================================================="
         << endl;
...
void BloomStormCtrl::updateSamplePoint() {
  ...
  std::cout << "  Checking status of sampling and building vector "
            << std::endl;
  for (it_nr = m_map_node_records.begin(); it_nr != m_map_node_records.end();
       it_nr++) {
    // continue if this vehicle is not actively sampling
    std::cout << "  -Checking vehicle: " << it_nr->second.getName()
              << std::endl;
    if (!m_sample_active.count(it_nr->second.getName()) &&
        (it_nr->second.getName() != m_vname)) {
      // This ensures that the distance vector is calculated for ownship
      // regardless of whether it is sampling or not.
    ...
    for (it_br = m_bloom_records.begin(); it_br != m_bloom_records.end();) {
      ...
      dist = pow((it_nr->second.getX() - it_br->get_vx()), 2) +
             pow((it_nr->second.getY() - it_br->get_vy()), 2);
      ...
      bool sample_point_captured =
          dist < m_sample_capture_radius * m_sample_capture_radius;
      // bool agent_sampling = m_sample_active.count(it_nr->second.getName());
      bool agent_sampling =
          m_map_sampling_agents.find(it_nr->second.getName()) !=
          m_map_sampling_agents.end();
      ...
      if (sample_point_captured && agent_sampling) {
        ...
        std::cout
            << "  ---Found vehicle is sampling here, removing from the list"
            << doubleToStringX(dist, 4) << std::endl;
        break;

      } else {
        // Add to vector if ownship
        if (it_nr->second.getName() == m_vname) {
          std::cout << "   ---Adding it to ownship distance vector"
                    << std::endl;
  ...
  // Step 2:
  // Complete a quick sanity check of the vehicles which have reported that they
  // are sampling This clears out old values in case of communication droppout.

  std::map<std::string, double>::iterator it_sa;
  for (it_sa = m_map_sampling_agents.begin();
       it_sa != m_map_sampling_agents.end();) {
  ...
  // Step 3:
  // Now pick the next sample pont. Try the naive greedy approach to the
  // multi-agent TSP Find the closest point
  std::set<unsigned int>
      omit_set; // set of indexes to omit from consideration.  These points
                // are better visited by other who are closer.
  double dist_to_closest;
  unsigned int index_to_closest;
  std::list<XYPoint>::iterator
      it_idx; // by the end this will point to the next XYPoint to visit
  bool continue_search = true;
  unsigned int count = 0;

  // loop until all points are allocated, or until the nearest node is found.
  while ((omit_set.size() < distance_from_own.size()) && continue_search &&
         (count < 100)) {

    std::cout << "In while loop:" << std::endl;
    std::cout << "  omit_set.size() = " << uintToString(omit_set.size())
              << std::endl;
    ...
    for (unsigned int i = 0; i < distance_from_own.size(); i++) {
      ...
      std::cout << "   checking through list, i = " << uintToString(i)
                << ", distance = " << doubleToStringX(distance_from_own[i], 4)
                << std::endl;

      if (omit_set.count(i) > 0) {
        // this point has already been marked as closer to another agent
        // skip it
        std::cout << "   skipping this point since it is already marked"
                  << std::endl;
        continue;
      }
    ...
    std::cout << "  Found closest valid point to be index = "
              << uintToString(index_to_closest) << std::endl;
    ...
    std::cout << "  Found the closest valid point at x="
              << doubleToStringX(ptx, 4) << ", y=" << doubleToStringX(pty, 4)
              << std::endl;
    ...
    if (m_map_sampling_agents.find(m_vname) != m_map_sampling_agents.end()) {
      ...
      own_time_delay =
          m_sample_time - (MOOSTime() - m_map_sampling_agents[m_vname]);
    }

    std::cout << "  own_time_delay = " << doubleToStringX(own_time_delay, 4)
              << std::endl;
    double cost_to_beat = dist_to_closest + own_time_delay;
    std::cout << "  cost_to_beat = " << doubleToStringX(cost_to_beat, 4)
              << std::endl;
    ...
    for (it_nr = m_map_node_records.begin(); it_nr != m_map_node_records.end();
      ...
      if (!m_sample_active.count(it_nr->second.getName()) ||
          (it_nr->second.getName() == m_vname)) {
        std::cout << "    -> " << it_nr->second.getName() << " not sampling now"
                  << std::endl;
        continue;
      }

      // calc distance for this vehicle
      proposed_dist = pow((it_nr->second.getX() - ptx), 2) +
                      pow((it_nr->second.getY() - pty), 2);
      proposed_dist = sqrt(proposed_dist);
      std::cout << "    -> calculated proposed_dist="
                << doubleToStringX(proposed_dist) << std::endl;

      // calc additional time for this vehicle
      proposed_time = 0.0;
      if (m_map_sampling_agents.find(it_nr->second.getName()) !=
          m_map_sampling_agents.end()) {
        // still waiting to finish sampling.  How much time is left?
        proposed_time =
            m_sample_time -
            (MOOSTime() - m_map_sampling_agents[it_nr->second.getName()]);
      }

      std::cout << "    -> proposed_time=" << doubleToStringX(proposed_time)
                << std::endl;

      if ((proposed_dist + proposed_time) < cost_to_beat) {
        // This vehicle is closer to this point

        // Is this vehicle closer to a different point, which it would
        // presumably go to first?
        bool better_point_for_other_vehicle = false;

        double alternate_point_dist = 0.0;
        for (it_br = m_bloom_records.begin(); it_br != m_bloom_records.end();
             it_br++) {
          std::cout << "     -->checking if this alternate point is closer: "
                    << it_br->get_spec() << std::endl;
          alternate_point_dist =
              pow((it_nr->second.getX() - it_br->get_vx()), 2) +
              pow((it_nr->second.getY() - it_br->get_vy()), 2);
          alternate_point_dist = sqrt(alternate_point_dist);

          std::cout << "      -->> alternate point dist = "
                    << doubleToStringX(alternate_point_dist) << std::endl;
          if (alternate_point_dist < proposed_dist) {
            // The other vehicle will go here and the added time cost will be
            // the same either way.  Exit
            std::cout << " This point is closer other vehicle will go here"
                      << std::endl;
        ...
        if (!better_point_for_other_vehicle) {
          std::cout << "    --> This vehicle is closer to the point and will "
                       "go there, adding it to the set"
                    << std::endl;
    ...
    std::cout << "   End of while loop, omit_set.size() = "
              << uintToString(omit_set.size()) << std::endl;
    std::cout << "   End of while loop, distance_from_own.size() = "
              << uintToString(distance_from_own.size()) << std::endl;
    std::cout << "   End of While loop continue_search = " << continue_search
              << std::endl;
    std::cout << "   End of While loop count = " << uintToString(count)
              << std::endl;
  } // end of while loop
  ...
  if (omit_set.size() == distance_from_own.size()) {
    ...
    std::cout << " * End of search updating travel point to be:" << spec
              << std::endl;
...
bool BloomStormCtrl::calculateInputs(double time) {
  ...
  if (m_got_real_batt_voltage) {
    ...
    m_current_batt_voltage =
        (m_min_batt - m_max_batt) * (m_odometry / max_heron_range) + m_max_batt;
  }
  ...
  double batt_state =
      (m_max_batt - m_current_batt_voltage) / (m_max_batt - m_min_batt);
  ...
  if (m_no_waypoints_available_to_sample) {
    ...
    if (m_dist_to_closest_sample < m_dist_to_next_sample_max_input_range) {
      ...
      // of the input
    } else if (m_dist_to_closest_sample <
               m_dist_to_next_sample_min_input_range) {

      double diff = (m_dist_to_next_sample_min_input_range -
                     m_dist_to_next_sample_max_input_range);
      double prop =
          (m_dist_to_closest_sample - m_dist_to_next_sample_max_input_range) /
          diff;
    ...
    std::map<std::string, double>::iterator iter =
        m_map_sampling_agents.find(m_vname);
    if (iter != m_map_sampling_agents.end()) {
  ...
  if (!m_migration_done) {
    ...
    for (it_nr = m_map_node_records.begin(); it_nr != m_map_node_records.end();
      ...
      if (!m_contacts.count(it_nr->second.getName()) &&
          (it_nr->second.getName() != m_vname)) {
        std::cout << " migration calc, rejecting nr = "
                  << it_nr->second.getName() << std::endl;
        continue;
      }
      std::cout << " migration calc, checking nr = " << it_nr->second.getName()
                << std::endl;
      ...
      if (m_next_region) {
        if (!m_region_2_poly.contains(it_nr->second.getX(),
                                      it_nr->second.getY())) {
          all_neighbors_migrated = false;
          std::cout << " found this one was not contained in poly2= "
                    << std::endl;
          break;
        }
      } else {
        if (!m_region_1_poly.contains(it_nr->second.getX(),
                                      it_nr->second.getY())) {
          all_neighbors_migrated = false;
          std::cout << " found this one was not contained in poly1= "
                    << std::endl;
          break;
        }
      }
  ...
  if ((2.0 < dt_migrate) && (dt_migrate < m_time_since_last_migration_thresh) &&
      !m_storm_detected) {
    migrate_input += m_time_since_last_migration_resistance_val *
                     (1.0 - dt_migrate / m_time_since_last_migration_thresh);
  }

  // Are there no samples detected here?
  double dt_no_samples = MOOSTime() - m_time_of_last_sample_available;
  if (dt_no_samples > m_time_since_no_samples_thresh) {
    migrate_input += (dt_no_samples - m_time_since_no_samples_thresh) *
                     m_time_since_no_samples_encouragement_slope;
  }
...
bool BloomStormCtrl::handleOptionMsg(const std::string &msg) {
  ...
  if (m_curr_option == "migrate") {
    clearSamplePoints();
    if ((MOOSTime() - m_time_of_last_migration_started) <
        m_time_since_last_migration_thresh)
      return (true);
