...
std::map<std::string, double> SimpleMDP::valueIteration(
    const std::map<std::string, double> &rewards,
    const std::vector<std::pair<std::string, std::string>> &edges,
    const double &discount, const double &epsilon) {
  ...
  for (it = rewards.begin(); it != rewards.end(); it++) {
    ...
    for (it2 = edges.begin(); it2 != edges.end(); it2++) {
      if (it2->first == it->first) {
        if (std::find(actions.begin(), actions.end(), it2->second) ==
            actions.end()) {
  ...
  int count = 0;
  int max_count = 50;
  while ((delta >= epsilon * (1.0 - discount) / discount) and
         (count <= max_count)) {
...
std::vector<double> SimpleMDP::stateTransitionProbability(
    const std::vector<std::string> &possible_next_state,
    const std::string current_state, const unsigned int &index) {
...
double getRewardUCB(double val, double var, double n_cells, double n_round,
                    double delta) {

  double beta =
      2.0 * log(n_cells * pow(n_round, 2) * M_PI * M_PI / (6.0 * delta));
...
std::vector<double> calculateZStarGumbel(unsigned int const &M,
  ...
  if (probf(left, vals, sigma) < 0.25) {
    ...
    right = *std::max_element(mean_plus_sigma_vec.begin(),
                              mean_plus_sigma_vec.end());
    ...
    for (int j = 0; j < M; j++) {
      double r = static_cast<double>(j) / static_cast<double>(M) +
...
double getRewardMVI(double depth, double var,
                    std::vector<double> const &zStar) {
  ...
  for (int i = 0; i < zStar.size(); i++) {
    ...
    double phi_gamma_z_star_i =
        1.0 / sqrt(2 * M_PI) * exp(-0.5 * pow(gamma_z_star_i, 2));
    double PHI_gamma_z_star_i = 0.5 * (erf(gamma_z_star_i / sqrt(2.0)) + 1.0);

    val += gamma_z_star_i * phi_gamma_z_star_i / (2.0 * PHI_gamma_z_star_i) -
           log(PHI_gamma_z_star_i);
  }
