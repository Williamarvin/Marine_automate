...
public:
  ...
  //                          edges   = a vector of directed edges between
  //                          nodes.
  //                                    These are treated like actions in
  //                                    the value iteration.
  //                                    edges[n] = {"node1","node2"} documents
  //                                    it is possible to move from node1 to
  //                                    node2
  ...
  //                                    ex U["node1"] = 13.1;
  //
  //                The transition model P(s'|s,a) must be defined before
  //                using!!
  //
  std::map<std::string, double>
  valueIteration(const std::map<std::string, double> &rewards,
                 const std::vector<std::pair<std::string, std::string>> &edges,
                 const double &discount, const double &epsilon);

  // stateTransistionProbability
  //                 Must be customized for every unique model format, but the
  //                 inputs and outputs should be the same. Inputs:
  //                 possible_next_state = a vector of possible next states
  //                                                       ex {"node2", "node3",
  //                                                       "node4"}
  //                                       current_state = the current state ex
  //                                       "node1"
  ...
  //                                                       example
  //
  //                 Outputs:    transistion probability = a vector of
  //                 probabilites for each state,
  //                                                       given the current
  //                                                       state and intended
  //                                                       action ex { 0.1, 0.8,
  //                                                       0.1} could be an
  ...
  //                                                       prob("node4") = 0.1
  //                                                       Should sum up to 1.
  std::vector<double> stateTransitionProbability(
...
// Bandit Setting https://ieeexplore.ieee.org/document/6138914
//
// Inputs:  val      =  expectation
//          var      =  variance
//          n_cells  =  number of cells in the grid
//          n_rounds =  number of rounds completed - Number of the times the
//          agent
...
double getRewardUCB(double val, double var, double n_cells, double n_rounds,
                    double delta);
