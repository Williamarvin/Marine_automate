...
SimpleAStar::SimpleAStar() { m_graph_preloaded = false; }
...
double SimpleAStar::calcCost(const SimpleAStar::vertex &Vi,
                             const SimpleAStar::vertex &q) {
  double distance = 0;
...
double SimpleAStar::calcHeuristic(const SimpleAStar::vertex &Vi,
                                  const SimpleAStar::vertex &q) {
  double factor = 1.0;
...
double SimpleAStar::minHeuristicToGoal(
    const std::map<SimpleAStar::index, SimpleAStar::vertex> &V,
    const int node_number, const std::set<SimpleAStar::index> &idx_final) {

  // Find the estimated cost to the closest goal.
  std::set<SimpleAStar::index>::const_iterator it;
  double h_min = std::numeric_limits<double>::infinity(); // reset the minimum
                                                          // cost for this list
...
SimpleAStar::path
SimpleAStar::recoverPath(const std::list<Node> &closed_list,
  ...
  // Loop through nodes following the parent node field to get back to the
  // start.
  ...
  while (idx_start.count(step) == 0) {
    // find the node in the closed list that is the parent of the node that is
    // equal to "step"
    ...
    for (it = closed_list.begin(); it != closed_list.end(); ++it) {
      if (it->getNumber() == step) {
        // found node at this step in the path. Get the node listed in the
        // parent field and set it to be the new step.  Add it to the front of
        // path_forward and continue on.
        step = it->getParent();
  ...
  /*
  std::cout << path_forward.size() << " nodes found between the start and the
  goal. "<< std::endl; std::cout << "They are: start config -> "; for (int k=0;
  k<=(path_forward.size()-1); k++) { std::cout << path_forward[k] << " -> ";
  }
...
SimpleAStar::path SimpleAStar::searchPath(
    const std::map<SimpleAStar::index, SimpleAStar::vertex> &V,
  ...
  // A* algorithm. Following the turtorial here:
  // http://web.mit.edu/eranki/www/tutorials/search/

  // Step 0: Book keeping
  // initialize the open list to hold all the nodes on the "frontier", or the
  // next ones to check.
  std::list<Node> open_list;

  // initialize the closed list to hold all the nodes that are on the
  // "interior", or all the nodes that have been checked already.
  std::list<Node> closed_list;

  // initialize list to hold all the nodes found near "q".  This gets erased and
  // filled up each time a new "q" is found.
  ...
  while (not open_list.empty()) {
    ...
    double f_min =
        std::numeric_limits<double>::infinity(); // reset the minimum cost for
                                                 // this list
    ...
    for (it = open_list.begin(); it != open_list.end(); ++it) {
      ...
      if (it->getF() < f_min) {
        ...
        f_min = it->getF();
        // set this node to "q" or the working node.  There is probably a better
        // way to do this...
        q = *it;
      }
    }
    ...
    // Loop through the vector E to find all the edges that start or end with
    // q.number
    ...
    for (it = neighbors_list.begin(); it != neighbors_list.end(); ++it) {
      ...
      if (idx_final.count(it->getNumber()) > 0) {
        ...
        double g_min_to_goal =
            std::numeric_limits<double>::infinity(); // min cost to goal
        std::list<Node>::iterator best_goal_node_itr;
        for (neighbor_goal_check_itr = neighbors_list.begin();
             neighbor_goal_check_itr != neighbors_list.end();
             ++neighbor_goal_check_itr) {

          if ((idx_final.count(neighbor_goal_check_itr->getNumber()) > 0) and
              (Obs.count(neighbor_goal_check_itr->getNumber()) == 0)) {
            // Compare the cost ("f") for this node to f_min
            double this_node_g =
                calcCost(V.at(neighbor_goal_check_itr->getNumber()),
                         V.at(q.getNumber()));
            if (this_node_g < g_min_to_goal) {
              // This node has a lower cost. Set this to the new min
              g_min_to_goal = this_node_g;
              // set this node to "q" or the working node.  There is probably a
        ...
        SimpleAStar::path path = recoverPath(closed_list, idx_start,
                                             best_goal_node_itr->getNumber());
        return path;
      }
      ...
      if (idx_start.count(neighbor_node.getNumber()) > 0) {
        ...
        neighbor_node.setG(q.getG() + calcCost(V.at(neighbor_node.getNumber()),
                                               V.at(q.getNumber())));
      }
      // find the min heuristic to reach any of the goal indices
      neighbor_node.setH(
          minHeuristicToGoal(V, neighbor_node.getNumber(), idx_final));
      neighbor_node.setF(neighbor_node.getG() + neighbor_node.getH());

      // Step 3.3
      // Check to see if this node is on the open list.  If it is, skip adding
      // it if the calculated "f" value is highter than what is in the open
      // list. This is like checking if any "sideways" movement on the frontier
      // is benificial.

      // loop through the open list
      std::list<Node>::iterator
          it2; // define another iterator for the open and closed list
      ...
      // Step 3.4
      // Check to see if this node is on the closed list.  If it is, skip adding
      // it if the calculated "f" value is higher than what is in the closed
      // list. This is like chcking if any "backtracking" into the interior is
      // benificial.

      // Only do this if we did not find the node in the open list, which is
      // flagged using the add_node boolean. Otherwise skip it for time.  The
      // node should never be on both the open and the closed list.
...
SimpleAStar::path SimpleAStar::recoverPathFast(index idx_final) {
  ...
  while (not m_node_map.at(step).isStart()) {
    // find the node that is the parent node of this node  set it to be the new
    // step.
...
bool SimpleAStar::preloadGraph(
    const std::map<SimpleAStar::index, SimpleAStar::vertex> &V,
    const std::vector<SimpleAStar::edge> &E,
    const std::set<SimpleAStar::index> &idx_start,
    const std::set<SimpleAStar::index> &idx_final, bool asc_cell_id) {
...
SimpleAStar::path SimpleAStar::searchPathFast(const std::set<index> &Obs) {
  ...
  // Step 0: Book keeping
  // initialize the open list to hold all the nodes on the "frontier", or the
  // next ones to check. Initialize it to be the starting values that are not
  // obstables.
  std::list<SimpleAStar::index> open_list = m_open_list_seed;
  std::list<SimpleAStar::index>::iterator itr_start_obs_check; //
  for (itr_start_obs_check = open_list.begin();
       itr_start_obs_check != open_list.end();) {
  ...
  // initialize list to hold all the nodes found near "q".  This gets erased and
  // filled up each time a new "q" is found.
  std::vector<SimpleAStar::index>
      neighbors_vec; // vec to hold all the nodes found near q.
  ...
  while (not open_list.empty()) {
    ...
    // Step 1.
    // find the node with the smallest "f" on the open list and call it q
    std::list<SimpleAStar::index>::iterator
        it; // define the iterator for the open list
    double f_min =
        std::numeric_limits<double>::infinity(); // reset the minimum cost for
                                                 // this list
    ...
    for (neighbor_itr = neighbors_vec.begin();
         neighbor_itr != neighbors_vec.end(); ++neighbor_itr) {
      ...
      if (m_node_map.at(*neighbor_itr).isGoal()) {
        ...
        for (neighbor_goal_check_itr = neighbors_vec.begin();
             neighbor_goal_check_itr != neighbors_vec.end();
             ++neighbor_goal_check_itr) {
          if ((m_node_map.at(*neighbor_goal_check_itr).isGoal()) and
              (Obs.count(*neighbor_goal_check_itr) == 0)) {
            // Compare the cost ("f") for this node to f_min
            double this_node_g =
                calcCost(m_node_map.at(*neighbor_goal_check_itr).getPos(),
                         m_node_map.at(q).getPos());
            if (this_node_g < g_min_to_goal) {
              // This node has a lower cost. Set this to the new min
              g_min_to_goal = this_node_g;
      ...
      if (m_node_map.at(*neighbor_itr).isStart()) {
        ...
        g_along_this_path = m_node_map.at(q).getG() +
                            calcCost(m_node_map.at(*neighbor_itr).getPos(),
                                     m_node_map.at(q).getPos());
        proposed_cost_to_this_node =
            g_along_this_path + m_node_map.at(*neighbor_itr).getH();
      }

      // Step 3.3
      // Check to see if this neighboring node is on the open list.  If it is,
      // only update with f value (and parent node) if the proposed cost is less
      // than what was estimated prevously. This is like checking if any
      // "sideways" movement on the frontier is benificial.
      bool check_closed_list_for_node = true;

      // first loop through the open list
      std::list<SimpleAStar::index>::iterator open_itr;
      for (open_itr = open_list.begin(); open_itr != open_list.end();
           ++open_itr) {
      ...
      // This is like chcking if any "backtracking" into the interior is
      // benificial.
      ...
      if (check_closed_list_for_node) {
        ...
        for (closed_itr = closed_list.begin(); closed_itr != closed_list.end();
             ++closed_itr) {

          if (*neighbor_itr == *closed_itr) {
            // neighbor is on the closed list
            if (proposed_cost_to_this_node <
                m_node_map.at(*closed_itr).getF()) {
              // the new path is better.
