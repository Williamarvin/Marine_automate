...
public:
  ...
  void setRewards(std::map<std::string, double> rewards) {
    m_rewards = rewards;
  };
  void setEdges(std::vector<std::pair<std::string, std::string>> edges) {
    m_edges = edges;
  };
  ...
  void addReward(std::string state, double reward) {
    m_rewards[state] = reward;
  }
