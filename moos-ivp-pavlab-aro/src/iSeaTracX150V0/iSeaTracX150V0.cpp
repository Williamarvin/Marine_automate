...
/*
  Notes
  This is the V0 HydroLink abstraction layer with the iSeaTracX150V0, which is
  aimed at demonstrating the most basic end-to-end one directional localization
  capabilities with the system
    - X150 Beacon periodically interrogates the X010 Beacon on the Sea Beaver
    - X150 obtains the depth, northing, and easting values with the extended
  acoustic nav message
    - Takes the local NAV_X and NAV_Y for HydroLink, and translates the vehicles
  position, and generates a node report for the Sea Beaver
    - This implementation will remain limited to PING protocol messages for the
  sake of testing - see 8.1 in dev manual
        ...
        - PSeatracPingResponseParams
          - obj.TSeatracAcoFix
              obj.TSeatracAcoFix.Msg
                obj.TSeatracAcoFix.Msg.DestId - for our use with one x150 - this
  should always be 15 obj.TSeatracAcoFix.Msg.SrcId - in the short term, this
  will always be one obj.TSeatracAcoFix.Msg.Type - defines the context for how
  to handle the response Type -> {ESeatracAcoMsgType} obj.TSeatracAcoFix.Info
                  Info -> {TSeatracAcoFixInfo}
                    Relevant fields
                    bool RangeValid;
  If this bit is set, it indicates the record contains the Range fields below.
                    uint32 RangeCount;                      				 The
  number of 16kHz timer intervals that were counted between Request message
  transmission and Response message reception. double RangeTime;
  The time in seconds derived from the RANGE_COUNT value, and with internal
  timing offsets and compensation applied. Values are in seconds.
                    ->float RangeDist;
  The resolved line-of-sight distance to the remote beacon, based on the
  RANGE_TIME and VOS values. Values are in metres.

                    //USBL Fields
                    bool UsblValid;                         				 If
  this bit is set, it indicates the record contains the USBL fields below. uint8
  UsblChannels;                     				 The number of USBL
  receiver channels being used to compute the signal angle. Typically this value
  is either 3 or 4.
                    float UsblRssi[ST_USBL_CHANNELS];       				 An
  array of the received signal strengths for each of the USBL receiver channels,
  where �x� is the value defined by the CHANNELS field. Values are deci-Bels
                    ->float UsblAzimuth;
  The incoming signal azimuth angle from 0� to 360�. Values are in degrees.
                    float UsblElevation;                    				 The
  incoming signal elevation angle from -90� to +90�. Values are in degrees.

                    //Position Fields
                    ->float PositionEasting;
  The Easting distance component of the relative position of the remote beacon
  to the local beacon computed from the range, incoming signal angle, local
  beacon depth, attitude and magnetic heading. Value in metres.
                    ->float PositionNorthing;
  The Northing distance component of the relative position of the remote beacon
  to the local beacon computed from the range, incoming signal angle, local
  beacon depth, attitude and magnetic heading. Value in metres.
                    ->float PositionDepth;
  ...
  Todo's
    ...
    [ ] Use the status messages for true heading of the X150 Beacon, and include
  an assembly heading offset [ ]
...
iSeaTracX150V0::~iSeaTracX150V0() { m_x150_beacon_interface.closeSerial(); }
...
bool iSeaTracX150V0::Iterate() {
  ...
  // TODO: 1) filter the samples and construct a model which estimates the AUV's
  // position, 2) from this model, approximate the speed, 3) will report the
  // AUV's heading in NAVX accom message which can provide X/Y to the AUV while
  // carrying a payload, where the vehicle can report its heading
...
bool iSeaTracX150V0::connectToSeatrac() {
  ...
  if (m_port_is_opened) {
    ...
    m_x150_object->OnCmdEncode.AttachMethod(
        this, &iSeaTracX150V0::writeSeaTracPort, nullptr);
    m_x150_object->OnCmdDecodeMsg.AttachMethod(
        this, &iSeaTracX150V0::messageDecoded, nullptr);
    m_x150_object->OnCmdDecodeLine.AttachMethod(
        this, &iSeaTracX150V0::lineDecoded, nullptr);

    m_x150_object->OnSysAlive.AttachMethod(
        this, &iSeaTracX150V0::seatracSysAlive, nullptr);
    m_x150_object->OnSysInfo.AttachMethod(this, &iSeaTracX150V0::seatracSysInfo,
                                          nullptr);
    m_x150_object->OnStatus.AttachMethod(this, &iSeaTracX150V0::seatracStatus,
                                         nullptr);

    // Handlers for receiving a specific signal, after we interrogate a response
    // from a remote beacon
    m_x150_object->OnPingResponse.AttachMethod(
        this, &iSeaTracX150V0::seatracPingResponse, nullptr);
    // m_x150_object->OnDatReceive.AttachMethod(this,
    // &iSeaTracX150V0::seatracReceiveData, nullptr);
    // m_x150_object->OnNavQueryResponse.AttachMethod(this, &iSeaTracX150V0::,
    // nullptr); m_x150_object->OnStatus.AttachMethod(this, &iSeaTracX150V0::,
    // nullptr); m_x150_object->OnXcvrAnalyseNoise.AttachMethod(this,
    // &iSeaTracX150V0::, nullptr);
  }
...
// void iSeaTracX150V0::sendNav(ESeatracBeaconId destId, uint8 queryFlags,
// puint8 data, uint8 dataLen)
// {
//   m_x150_object->NavStatusSend(id, ST_AMSG_REQU, queryFlags, data, dataLen);
// }

// void iSeaTracX150V0::sendData(ESeatracBeaconId destId, ESeatracAcoMsgType
// msgType, puint8 data, uint8 dataLen)
// {
...
void iSeaTracX150V0::writeSeaTracPort(pointer context,
...
void iSeaTracX150V0::readSeaTracPort() {
  ...
  if (err.size()) {
    // lib_serialdev returns an error message if there is one, and due to the
    // default non-blocking behavior
    //  If nothing is in the buffer, then it will report the resource is
    //  temporarily unavailable. With this, it is an insignificant warning and
    //  can be skipped for this application
    // TODO: Record exact error message output and use exact equals rather than
    // finding a substring (can't remember the exact output)
...
void iSeaTracX150V0::messageDecoded(pointer context,
                                    PSeatracCmdDecodeMsgParams params) {
  // Not used
}
void iSeaTracX150V0::lineDecoded(pointer context,
                                 PSeatracCmdDecodeLineParams params) {
  // Not used
}

// Sys messages
void iSeaTracX150V0::seatracSysAlive(pointer context,
                                     PSeatracSysAliveParams params) {
  // Not implemented
}
void iSeaTracX150V0::seatracSysInfo(pointer context,
                                    PSeatracSysInfoParams params) {
  // TODO
  /*
  Q_UNUSED(context)
...
void iSeaTracX150V0::seatracStatus(pointer context,
                                   PSeatracStatusParams params) {
  ...
  Q_UNUSED(context)
        ...
        emit onSeatracStatus(params->Status.EnvSupply, params->Status.EnvTemp,
                                                 params->Status.EnvPressure,
  params->Status.EnvDepth, params->Status.EnvVos);

        emit onSeatracEulerAngles(params->Status.AttitudeYaw,
  params->Status.AttitudePitch, params->Status.AttitudeRoll);

        if (IS_BIT_SET(flags, ST_STATUS_FLAGS_BNO055_STATUS_BIT)) {
                emit onSeatracAhrsStatus(params->Status.SysFlag,
  params->Status.AccFlag, params->Status.MagFlag, params->Status.GyroFlag);
        }
...
// Ping Response
void iSeaTracX150V0::seatracPingResponse(pointer context,
                                         PSeatracPingResponseParams params) {
  (void)(context);
...
bool iSeaTracX150V0::buildReport() {
  ...
  x150_state_tab << "Connected to Serial: "
                 << (m_port_is_opened ? "Connected" : "Disconnected");
  x150_state_tab << "Compass Heading (NI): " << doubleToString(m_x150_hdg, 2);
  // Include the IMU data and other sensors on the X150 (including power supply
  // levels)
  ...
  x150_comms_tab << "Last ping time: "
                 << doubleToString(m_ping_send_moos_time, 2);
  x150_comms_tab << "Last ping received: "
                 << doubleToString(m_ping_resp_moos_time, 2);
  ...
  x010_tab << "Has received response: "
           << (m_ping_resp_moos_time > 0 ? "Yes" : "No");
  x010_tab << "Voltage: "
           << "Not Implemented yet";
  x010_tab << "Last ping received: "
           << doubleToString(m_ping_resp_moos_time, 2);
