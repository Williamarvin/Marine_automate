...
*/
CSeatracCmdMsg::CSeatracCmdMsg(ESeatracCmdId id, puint8 data, uint16 len)
    : base() {
...
//------------------------------------------------------------------------------
bool CSeatracCmdMsg::AddAcoMsgType(ESeatracAcoMsgType value) {
  return this->WriteUint8((uint8)value);
}

bool CSeatracCmdMsg::AddBeaconId(ESeatracBeaconId value) {
  return this->WriteUint8((uint8)value);
}

bool CSeatracCmdMsg::AddCmdId(ESeatracCmdId value) {
  return this->WriteUint8((uint8)value);
}
...
bool CSeatracCmdMsg::AddSettings(TSeatracSettings &value) {
  ...
  if (value.StatusOutputEnv)
    SET_BIT(statusOutput, ST_STATUS_FLAGS_ENVIRONMENT_BIT);
  if (value.StatusOutputAttitude)
    SET_BIT(statusOutput, ST_STATUS_FLAGS_ATTITUDE_BIT);
  if (value.StatusOutputMagCal)
    SET_BIT(statusOutput, ST_STATUS_FLAGS_MAG_CAL_BIT);
  if (value.StatusOutputAccCal)
    SET_BIT(statusOutput, ST_STATUS_FLAGS_ACC_CAL_BIT);
  if (value.StatusOutputAhrsRaw)
    SET_BIT(statusOutput, ST_STATUS_FLAGS_AHRS_RAW_BIT);
  if (value.StatusOutputAhrsComp)
    SET_BIT(statusOutput, ST_STATUS_FLAGS_AHRS_COMP_BIT);

  uint8 envFlags = 0;
  if (value.EnvAutoPressureOffset)
    SET_BIT(envFlags, ST_ENV_FLAGS_AUTO_PRESSURE_BIT);
  if (value.EnvAutoCalcVos)
    SET_BIT(envFlags, ST_ENV_FLAGS_AUTO_VOS_BIT);

  uint8 ahrsFlags = 0;
  if (value.AhrsAutoCalMag)
    SET_BIT(ahrsFlags, ST_AHRS_FLAGS_AUTO_CALC_MAG_BIT);

  uint8 xcvrFlags = 0;
  if (value.XcvrOutputDiagMsgs)
    SET_BIT(xcvrFlags, ST_XCVR_FLAGS_DIAG_MSGS_BIT);
  if (value.XcvrOutputFixMsgs)
    SET_BIT(xcvrFlags, ST_XCVR_FLAGS_FIX_MSGS_BIT);
  if (value.XcvrOutputUsblMsgs)
    SET_BIT(xcvrFlags, ST_XCVR_FLAGS_USBL_MSGS_BIT);
  if (value.XcvrEnablePosFilter)
    SET_BIT(xcvrFlags, ST_XCVR_FLAGS_POSFLT_ENABLE_BIT);
  if (value.XcvrUsblUseAhrs)
    SET_BIT(xcvrFlags, ST_XCVR_FLAGS_USBL_USE_AHRS_BIT);
  xcvrFlags |=
      (uint8)(((uint8)value.XcvrTxMsgCtrl << ST_XCVR_FLAGS_TX_MSGCTRL_SHIFT) &
              ST_XCVR_FLAGS_TX_MSGCTRL_MASK);
  ...
  success &= this->WriteUint16(
      0); // NetMacAddr[0:1] - Not used - just a dummy placeholder
  success &= this->WriteUint16(
      0); // NetMacAddr[2:3] - Not used - just a dummy placeholder
  success &= this->WriteUint16(
      0); // NetMacAddr[4:5] - Not used - just a dummy placeholder
  success &=
      this->WriteUint32(0); // NetIpAddr - Not used - just a dummy placeholder
  success &=
      this->WriteUint32(0); // NetIpSubnet - Not used - just a dummy placeholder
  success &= this->WriteUint32(
      0); // NetIpGateway - Not used - just a dummy placeholder
  success &=
      this->WriteUint32(0); // NetIpDns - Not used - just a dummy placeholder
  success &=
      this->WriteUint16(0); // NetTcpPort - Not used - just a dummy placeholder
...
//------------------------------------------------------------------------------
bool CSeatracCmdMsg::ReadAcoMsgType(ESeatracAcoMsgType *value) {
  return this->ReadUint8((puint8)value);
}
bool CSeatracCmdMsg::ReadAcoMsgType(ESeatracAcoMsgType *value,
                                    ESeatracAcoMsgType defValue) {
  return this->ReadUint8((puint8)value, (uint8)defValue);
}

bool CSeatracCmdMsg::ReadAcoPayloadType(ESeatracAcoPayloadType *value) {
  return this->ReadUint8((puint8)value);
}
bool CSeatracCmdMsg::ReadAcoPayloadType(ESeatracAcoPayloadType *value,
                                        ESeatracAcoPayloadType defValue) {
  return this->ReadUint8((puint8)value, (uint8)defValue);
}

bool CSeatracCmdMsg::ReadAppType(ESeatracAppType *value) {
  return this->ReadUint8((puint8)value);
}
bool CSeatracCmdMsg::ReadAppType(ESeatracAppType *value,
                                 ESeatracAppType defValue) {
  return this->ReadUint8((puint8)value, (uint8)defValue);
}

bool CSeatracCmdMsg::ReadBeaconId(ESeatracBeaconId *value) {
  return this->ReadUint8((puint8)value);
}
bool CSeatracCmdMsg::ReadBeaconId(ESeatracBeaconId *value,
                                  ESeatracBeaconId defValue) {
  return this->ReadUint8((puint8)value, (uint8)defValue);
}

bool CSeatracCmdMsg::ReadCmdId(ESeatracCmdId *value) {
  return this->ReadUint8((puint8)value);
}
bool CSeatracCmdMsg::ReadCmdId(ESeatracCmdId *value, ESeatracCmdId defValue) {
  return this->ReadUint8((puint8)value, (uint8)defValue);
}

bool CSeatracCmdMsg::ReadCmdStatus(ESeatracCmdStatus *value) {
  return this->ReadUint8((puint8)value);
}
bool CSeatracCmdMsg::ReadCmdStatus(ESeatracCmdStatus *value,
                                   ESeatracCmdStatus defValue) {
  return this->ReadUint8((puint8)value, (uint8)defValue);
}
...
bool CSeatracCmdMsg::ReadAcoFix(PSeatracAcoFix value) {
  bool success = true;
  int16 attitudeYaw;   /*!< The yaw angle (relative to magnetic north) of the
                          local beacon when the fix was computed. Values are
                          encoded as deci-Degrees, so divide by 10 for just
                          degrees to a 0.1° resolution. */
  int16 attitudePitch; /*!< The pitch angle of the local beacon when the fix was
                          computed. Values are encoded as deci-Degrees, so
                          divide by 10 for just degrees to a 0.1° resolution. */
  int16 attitudeRoll;  /*!< The roll angle of the local beacon when the fix was
                          computed. Values are encoded as deci-Degrees, so divide
                          by 10 for just degrees to a 0.1° resolution. */
  uint16 depthLocal;   /*!< The reading from the local beacon depth sensor when
                          the fix was calculated. Values are encoded in
                          decimetres, so divide by 10 to obtain a value in metres
                          to a 0.1m resolution. */
  uint16 vos; /*!< The velocity of sound value used for the computation of the
                 remote beacon’s range based on timing information. Values are
                 encoded in decimetres-per-second, so divide by 10 for a value
                 in metres-per-second. */
  int16 rssi; /*!< The Received Signal Strength Indicator value for the received
                 message, encoded in centibels. To decode, divide this value by
                 10 for decibels to a 0.1 dB resolution. */
  uint32 rangeCount; /*!< The number of 16kHz timer intervals that were counted
                        between Request message transmission and Response
                        message reception. */
  int32 rangeTime; /*!< The time in seconds derived from the RANGE_COUNT value,
                      and with internal timing offsets and compensation applied.
                      Values are encoded in 100 nanosecond multiples, so divide
                      by 10000000 to obtain a value in seconds. */
  uint16
      rangeDist; /*!< The resolved line-of-sight distance to the remote beacon,
                    based on the RANGE_TIME and VOS values. Values are encoded
                    in decimetres, so divide by 10 for a value in metres. */
  uint8 usblChannels; /*!< The number of USBL receiver channels being used to
                         compute the signal angle. Typically this value is
                         either 3 or 4. */
  int16 usblRssi
      [ST_USBL_CHANNELS]; /*!< An array of the received signal strengths for
                             each of the USBL receiver channels, where “x” is
                             the value defined by the CHANNELS field. Values are
                             encoded in centi-Bels, so divide by 10 to obtain a
                             value in decibels to a resolution of 0.1dB. */
  int16 usblAzimuth; /*!< The incoming signal azimuth angle from 0° to 360°.
                        Values are encoded as deci-Degrees, so divide by 10 for
                        just degrees to a 0.1° resolution. */
  int16 usblEvelvation; /*!< The incoming signal elevation angle from -90° to
                           +90°. Values are encoded as deci-Degrees, so divide
                           by 10 for just degrees to a 0.1° resolution. */
  int16
      usblFitError; /*!< The fit error value returns a number that indicates the
                       quality of fit (or confidence) of the signal azimuth and
                       elevation values from the timing and phase-angle data
                       available. Smaller values towards 0.0 indicate a better
                       fit, while larger values (increasing above 2-3) indicate
                       poorer fits and larger error tolerances. Values are
                       dimensionless, but divide the value by 100 to obtain a
                       signed floating-point value to a resolution of 0.01. */
  int16 positionEasting;  /*!< The Easting distance component of the relative
  ...
  int16 positionNorthing; /*!< The Northing distance component of the relative
                             position of the remote beacon to the local beacon
                             computed from the range, incoming signal angle,
                             local beacon depth, attitude and magnetic heading.
                             Values are encoded in decimetres, so divide by 10
  ...
  int16
      positionDepth; /*!< The vertical Depth distance component of the remote
                        ...
                        magnetic heading. Values are encoded in decimetres, so
                        divide by 10 for a value in metres. NB: If the ‘Fix’ has
                        been obtained by a MSG_REQU (Usbl) type request, then
                        this value is computed from the beacon’s attitude and
                        incoming signal angle. If a MSG_REQX (Enhanced) type
  ...
  value->Local.DepthValid = true; //###TODO True at the moment, but allows
                                  //support for beacons without a depth sensor
  value->Local.Depth = depthLocal / 10.0F;
  value->Local.AttitudeValid = true; //###TODO True at the moment, add support
                                     //for beacons without an AHRX (X010=FALSE)
  ...
  value->Info.RangeValid =
      IS_BIT_SET(value->Flags, ST_ACOFIX_FLAGS_RANGE_VALID_BIT);
  value->Info.UsblValid =
      IS_BIT_SET(value->Flags, ST_ACOFIX_FLAGS_USBL_VALID_BIT);
  value->Info.PositionValid =
      IS_BIT_SET(value->Flags, ST_ACOFIX_FLAGS_POS_VALID_BIT);
  value->Info.PositionEnhanced =
      IS_BIT_SET(value->Flags, ST_ACOFIX_FLAGS_POS_ENHANCED_BIT);
  value->Info.PositionFilterError =
      IS_BIT_SET(value->Flags, ST_ACOFIX_FLAGS_POS_FLT_ERROR_BIT);
...
bool CSeatracCmdMsg::ReadAcoMsg(PSeatracAcoMsg value) {
  ...
  // success &= this->ReadArray(&value->Payload.Data, value->Payload.Length,
  // sizeof(value->Payload.Data[0]));
...
bool CSeatracCmdMsg::ReadBeaconBaud(puint32 value) {
  ...
  if (success) {
    switch (baud) {
    case ST_BEACON_BAUD_4800: {
      *value = 4800;
      break;
    }
    case ST_BEACON_BAUD_9600: {
      *value = 9600;
      break;
    }
    case ST_BEACON_BAUD_14400: {
      *value = 14400;
      break;
    }
    case ST_BEACON_BAUD_19200: {
      *value = 19200;
      break;
    }
    case ST_BEACON_BAUD_38400: {
      *value = 38400;
      break;
    }
    case ST_BEACON_BAUD_57600: {
      *value = 57600;
      break;
    }
    case ST_BEACON_BAUD_115200: {
      *value = 115200;
      break;
    }
    default: {
...
bool CSeatracCmdMsg::ReadNavPacket(PSeatracNavPacket value) {
  bool success = true;
  success &=
      this->ReadUint8(&value->Length, 0); // Supported in firmware 1.12 onwards
...
bool CSeatracCmdMsg::ReadSettings(PSeatracSettings value) {
  ...
  value->StatusOutputEnv =
      IS_BIT_SET(statusOutput, ST_STATUS_FLAGS_ENVIRONMENT_BIT);
  value->StatusOutputAttitude =
      IS_BIT_SET(statusOutput, ST_STATUS_FLAGS_ATTITUDE_BIT);
  value->StatusOutputMagCal =
      IS_BIT_SET(statusOutput, ST_STATUS_FLAGS_MAG_CAL_BIT);
  value->StatusOutputAccCal =
      IS_BIT_SET(statusOutput, ST_STATUS_FLAGS_ACC_CAL_BIT);
  value->StatusOutputAhrsRaw =
      IS_BIT_SET(statusOutput, ST_STATUS_FLAGS_AHRS_RAW_BIT);
  value->StatusOutputAhrsComp =
      IS_BIT_SET(statusOutput, ST_STATUS_FLAGS_AHRS_COMP_BIT);
  value->EnvAutoPressureOffset =
      IS_BIT_SET(envFlags, ST_ENV_FLAGS_AUTO_PRESSURE_BIT);
  ...
  value->AhrsAutoCalMag =
      IS_BIT_SET(ahrsFlags, ST_AHRS_FLAGS_AUTO_CALC_MAG_BIT);
  ...
  value->XcvrOutputDiagMsgs =
      IS_BIT_SET(xcvrFlags, ST_XCVR_FLAGS_DIAG_MSGS_BIT);
  value->XcvrOutputFixMsgs = IS_BIT_SET(xcvrFlags, ST_XCVR_FLAGS_FIX_MSGS_BIT);
  value->XcvrOutputUsblMsgs =
      IS_BIT_SET(xcvrFlags, ST_XCVR_FLAGS_USBL_MSGS_BIT);
  value->XcvrEnablePosFilter =
      IS_BIT_SET(xcvrFlags, ST_XCVR_FLAGS_POSFLT_ENABLE_BIT);
  value->XcvrUsblUseAhrs =
      IS_BIT_SET(xcvrFlags, ST_XCVR_FLAGS_USBL_USE_AHRS_BIT);
  value->XcvrTxMsgCtrl =
      (ESeaTracXcvrTxMsgCtrl)((xcvrFlags &
                               (uint8)ST_XCVR_FLAGS_TX_MSGCTRL_MASK) >>
                              ST_XCVR_FLAGS_TX_MSGCTRL_SHIFT);
...
bool CSeatracCmdMsg::ReadStatus(PSeatracStatus value) {
  ...
  value->AhrsCompValid =
      IS_BIT_SET(value->Flags, ST_STATUS_FLAGS_AHRS_COMP_BIT);
  ...
  if (value->AhrsRawValid) {
    ...
    value->AhrsRawMagY = 0;
    value->AhrsRawMagZ = 0;
    value->AhrsRawGyroX = 0;
    value->AhrsRawGyroY = 0;
    value->AhrsRawGyroZ = 0;
  }
  ...
  if (value->AhrsCompValid) {
    ...
    value->AhrsCompAccX = 0.0F;
    value->AhrsCompAccY = 0.0F;
    value->AhrsCompAccZ = 0.0F;
    value->AhrsCompMagX = 0.0F;
    value->AhrsCompMagY = 0.0F;
    value->AhrsCompMagZ = 0.0F;
    value->AhrsCompGyroX = 0.0F;
    value->AhrsCompGyroY = 0.0F;
    value->AhrsCompGyroZ = 0.0F;
  }
...
bool CSeatracCmdMsg::ReadUsbl(PSeatracUsbl value) {
  ...
  value->XcorLength =
      std::min(value->XcorLength, (uint16)ST_USBL_XCOR_DATA_MAX);
  success &= this->ReadArray(value->XcorData, value->XcorLength,
                             sizeof(value->XcorData[0]));
  success &= this->ReadUint8(&value->Channels, 0);
  value->Channels = std::min(value->Channels, (uint8)ST_USBL_CHANNELS);
  success &=
      this->ReadArray(channelRssi, value->Channels, sizeof(channelRssi[0]));
  success &= this->ReadUint8(&value->Baselines, 0);
  value->Baselines = std::min(value->Baselines, (uint8)ST_USBL_BASELINES);
  success &= this->ReadArray(value->BaselinePhaseAngle, value->Baselines,
                             sizeof(value->BaselinePhaseAngle[0]));
