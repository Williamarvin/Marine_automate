...
/*!-----------------------------------------------------------------------------
 */
void CSeatracDiagMsg::AddParamArgs(const char *name, const char *value,
                                   va_list args) {
  // Allocate a buffer for output characters
  char nameBuf[128];
  char valueBuf[1024];
...
/*!-----------------------------------------------------------------------------
 */
void CSeatracDiagMsg::AddParamsBuffer(const char *name,
                                      TSeatracAcoPayload &value) {
 ...
 */
void CSeatracDiagMsg::AddParamsBuffer(const char *name,
                                      TSeatracNavPacket &value) {
  this->AddParamsBuffer(name, value.Data, value.Length);
}

/*!-----------------------------------------------------------------------------
Function that adds the contents of a buffer (specified by a length and data
array) using hexadecimal values.
*/
void CSeatracDiagMsg::AddParamsBuffer(const char *name, puint8 data,
                                      uint8 len) {
  char nameLen[64];
  char nameData[64];
...
void CSeatracDiagMsg::AddParamsAcoFixInfo(TSeatracAcoFixInfo &value) {
  ...
  if (value.UsblValid) {
    this->AddParam("UsblRssi", "{%0.1fdB,%0.1fdB,%0.1fdB,%0.1fdB}",
                   value.UsblRssi[0], value.UsblRssi[1], value.UsblRssi[2],
                   value.UsblRssi[3]);
...
void CSeatracDiagMsg::AddParamsAcoFixMsg(TSeatracAcoFixMsg &value) {
  this->AddParamString("MsgSrcId",
                       CSeatracUtils::ToStringBeaconId(value.SrcId));
  this->AddParamString("MsgDestId",
                       CSeatracUtils::ToStringBeaconId(value.DestId));
  this->AddParamString("MsgType",
                       CSeatracUtils::ToStringAcoMsgType(value.Type));
...
void CSeatracDiagMsg::AddParamsAcoMsg(TSeatracAcoMsg &value) {
  this->AddParamString("HdrSrcId",
                       CSeatracUtils::ToStringBeaconId(value.SrcId));
  this->AddParamString("HdrDestId",
                       CSeatracUtils::ToStringBeaconId(value.DestId));
  this->AddParamString("HdrMsgType",
                       CSeatracUtils::ToStringAcoMsgType(value.MsgType));
  this->AddParam("HdrDepth", "%0.1fm", value.MsgDepth);
  this->AddParamString(
      "PayloadType", CSeatracUtils::ToStringAcoPayloadType(value.PayloadType));
...
void CSeatracDiagMsg::AddParamsNavQuery(TSeatracNavQuery &value) {
  ...
  if (value.AttitudeValid) {
    this->AddParam("AttitudeRemote", "{%0.1f°Y,%0.1f°P,%0.1f°R}",
                   value.AttitudeYawRemote, value.AttitudePitchRemote,
                   value.AttitudeRollRemote);
  }
}
...
void CSeatracDiagMsg::AddParamsPressureCal(TSeatracPressureCal &value) {
  ...
  this->AddParam("PressureCalDate", "%u-%02u-%02u", value.CalYear,
                 value.CalMonth, value.CalDay);
}

/*!-----------------------------------------------------------------------------
 */
void CSeatracDiagMsg::AddParamsSettings(TSeatracSettings &value) {
  this->AddParamString("StatusMode",
                       CSeatracUtils::ToStringStatusMode(value.StatusMode));
  ...
  this->AddParam("EnvPressureOffset", "{%s,%0.3fBar}",
                 (value.EnvAutoPressureOffset ? "AUTO" : "MANUAL"),
                 value.EnvPressureOffset);
  this->AddParam("EnvVos", "{%s,%0.1fms-1}",
                 (value.EnvAutoCalcVos ? "AUTO" : "MANUAL"), value.EnvVos);
  this->AddParam("EnvSalinity", "%0.1fppt", value.EnvSalinity);
  this->AddParamBool("AhrsAutoCalMag", value.AhrsAutoCalMag);
  this->AddParam("AhrsOffset", "{%0.1f°Y,%0.1f°P,%0.1f°R}", value.AhrsYawOffset,
                 value.AhrsPitchOffset, value.AhrsRollOffset);
  this->AddParamString("BeaconId",
                       CSeatracUtils::ToStringBeaconId(value.XcvrBeaconId));
  ...
  this->AddParamString("XcvrTxMsgCtrl", CSeatracUtils::ToStringXcvrTxMsgCtrl(
                                            value.XcvrTxMsgCtrl));
  this->AddParamBool("XcvrEnablePosFilter", value.XcvrEnablePosFilter);
  this->AddParamBool("XcvrUsblUseAhrs", value.XcvrUsblUseAhrs);
  this->AddParam("XcvrFixedAttitude", "{%0.1f°Y,%0.1f°P,%0.1f°R}",
                 value.XcvrFixedYaw, value.XcvrFixedPitch, value.XcvrFixedRoll);
  this->AddParam("XcvrPosFilterVelocity", "{%ums-1,%u°,%us}",
                 value.XcvrPosFilterVelocity, value.XcvrPosFilterAngularRate,
                 value.XcvrPosFilterTimeout);
}
...
void CSeatracDiagMsg::AddParamsStatus(TSeatracStatus &value) {
  ...
  if (value.AttitudeValid) {
    this->AddParam("Attitude", "{%0.1f°Y,%0.1f°P,%0.1f°R}", value.AttitudeYaw,
                   value.AttitudePitch, value.AttitudeRoll);
...
void CSeatracDiagMsg::AddParamsUsbl(TSeatracUsbl &value) {
  ...
  this->AddParam("ChannelRssi", "{%0.1fdB,%0.1fdB,%0.1fdB,%0.1fdB}",
                 value.ChannelRssi[0], value.ChannelRssi[1],
                 value.ChannelRssi[2], value.ChannelRssi[3]);
  this->AddParam("Baselines", "%u", value.Baselines);
  this->AddParam("PhaseAngles", "{%0.1f°,%0.1f°,%0.1f°,%0.1f°,%0.1f°,%0.1f°}",
                 value.BaselinePhaseAngle[0], value.BaselinePhaseAngle[1],
                 value.BaselinePhaseAngle[2], value.BaselinePhaseAngle[3],
                 value.BaselinePhaseAngle[4], value.BaselinePhaseAngle[5]);
...
void CSeatracDiagMsg::AddParamsXcvrTxMsgCtrl(ESeaTracXcvrTxMsgCtrl value) {
  this->AddParamString("XcvrTxMsgCtrl",
                       CSeatracUtils::ToStringXcvrTxMsgCtrl(value));
}
...
void CSeatracDiagMsg::MakeParams(TSeatracDiagLogParams &params) {
  ...
  snprintf(params.StatusStr, sizeof(params.StatusStr), "%s",
           CSeatracUtils::ToStringDiagMsgStatus(_status).c_str());
  params.NameStr[0] = 0; // To be filled in by the caller - null string.
  params.Type = _type;
  snprintf(params.TypeStr, sizeof(params.TypeStr), "%s",
           CSeatracUtils::ToStringDiagMsgType(_type).c_str());
  params.CmdIdValid = _cmdIdValid;
  params.CmdId = _cmdId;
  snprintf(params.CmdIdStr, sizeof(params.CmdIdStr), "%s",
           CSeatracUtils::ToStringCmdId(_cmdId).c_str());
...
/*!-----------------------------------------------------------------------------
 */
void CSeatracDiagMsg::SetType(ESeatracDiagMsgType type, ESeatracCmdId cmdId,
                              bool error) {
  _type = type;
  _cmdId = cmdId;
  _cmdIdValid = true;
