...
CSeatrac::CSeatrac() {
  ...
  _cmdProc = new CSeatracCmdProc(ST_ENCODE_HEADER_CHAR, ST_DECODE_HEADER_CHAR,
                                 ST_MSG_LEN);
...
void CSeatrac::CmdExecute(PSeatracCmdMsg msg) {
  ...
  if (!success) {
    ...
    paramsExe.Msg = msg;
    paramsExe.Id = id;
    paramsExe.Status = ST_EXE_OK; // The handling function should set this to
                                  // other values if an error occured.
    ...
    if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_EXECUTE_BIT)) {
      ...
      // diagMsg.AddParamString("Cmd",
      // CSeatracUtils::ToStringCmdId(paramsExe.Id));
      diagMsg.AddParam("Length", "%u", paramsExe.Msg->GetLength());
      this->DoDiagLog(diagMsg);
    }
#endif

    // Raise the external OnExecute event to allow outside code to process
    // commands
    this->OnCmdExecute.Call(&paramsExe);

    // Now call the internal code to execute event (if the command isn't marked
    // as handled)
  ...
  if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_EXECUTED_BIT)) {
    CSeatracDiagMsg diagMsg;
    diagMsg.SetType(ST_DIAG_TYPE_EXECUTED, paramsDone.Id,
                    (paramsDone.ExeStatus != ST_EXE_OK));
    // diagMsg.AddParamString("Cmd",
    // CSeatracUtils::ToStringCmdId(paramsDone.Id));
    diagMsg.AddParamString(
        "Status", CSeatracUtils::ToStringExecuteStatus(paramsDone.ExeStatus));
    this->DoDiagLog(diagMsg);
  }
#endif
...
/*!-----------------------------------------------------------------------------
 */
void CSeatrac::CmdProcDecodeLineEvent(pointer context,
                                      PSeatracCmdDecodeLineParams params) {
  ...
  if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_DECODE_BIT)) {
    ...
    diagMsg.AddParam("Length", "%u",
                     params->Length +
                         2); // Acount for <CR><LF> that have been stripped out
...
/*!-----------------------------------------------------------------------------
 */
void CSeatrac::CmdProcDecodeMsgEvent(pointer context,
                                     PSeatracCmdDecodeMsgParams params) {
  FUNC_PARAM_UNUSED(context);
...
*/
void CSeatrac::CmdProcEncodeEvent(pointer context,
                                  PSeatracCmdEncodeParams params) {
  FUNC_PARAM_UNUSED(context);

#ifdef SEATRAC_EN_DIAG
  // If diagnostic logging is enabled for encoding then raise an event
  if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_ENCODE_BIT)) {
    // Remove the <CR><LF> characters from the Tx buffer, and append a string
    // null char
    string encodeStr;
    encodeStr.append("'");
...
bool CSeatrac::CalAction(ESeatracCalAction action) {
  ...
  if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_CAL_BIT)) {
    ...
    diagMsg.AddParamString("CalAction",
                           CSeatracUtils::ToStringCalAction(action));
    this->DoDiagLog(diagMsg);
  }
#endif
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::CfgBeaconSet(uint16 destSerialNumber, ESeatracBeaconId beaconId,
                            uint16 rangeTimeout, uint16 responseTurnaround) {
  SEATRAC_MUTEX_CMD_LOCK;
  ...
  if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_CFG_BIT)) {
    ...
    diagMsg.AddParam("DestSerialNumber", "%06u", destSerialNumber);
    diagMsg.AddParamString("NewBeaconId",
                           CSeatracUtils::ToStringBeaconId(beaconId));
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::DatSend(ESeatracBeaconId destId, ESeatracAcoMsgType msgType,
                       puint8 data, uint8 dataLen) {
  ...
  if (!CSeatracUtils::IsAcoMsgTypeRequest(msgType) &&
      !CSeatracUtils::IsAcoMsgTypeOneWay(msgType))
    return false;
  ...
  if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_DAT_BIT)) {
    ...
    diagMsg.AddParamString("MsgType",
                           CSeatracUtils::ToStringAcoMsgType(msgType));
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::DatQueueSet(ESeatracBeaconId destId, puint8 data,
                           uint8 dataLen) {
  // Abort if the destId is invalid
  if (!CSeatracUtils::IsBeaconIdPhysicalOrAll(destId))
    return false;
...
*/
#ifdef SEATRAC_EN_DIAG
uint32 CSeatrac::DiagLogFlagsGet() { return _diagLogFlags; }
#endif
...
*/
#ifdef SEATRAC_EN_DIAG
void CSeatrac::DiagLogFlagsSet(uint32 value) { _diagLogFlags = value; }
#endif
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::EchoSend(ESeatracBeaconId destId, ESeatracAcoMsgType msgType,
                        puint8 data, uint8 dataLen) {
  // Abort if the destId is invalid
...
void CSeatrac::ExecuteCmd(PSeatracExecuteParams exe) {
  switch (exe->Id) {
  // System Messages
  case ST_CID_SYS_ALIVE: {
    this->ExecuteCmdSysAlive(exe);
    break;
  }
  case ST_CID_SYS_INFO: {
    this->ExecuteCmdSysInfo(exe);
    break;
  }
  case ST_CID_SYS_REBOOT: {
    this->ExecuteCmdSysReboot(exe);
    break;
  }
  // case ST_CID_SYS_ENGINEERING : { this->ExecuteXXX(exe); break; }
  //  Firmware Programming Messages
  case ST_CID_PROG_INIT: {
    this->ExecuteCmdProgInit(exe);
    break;
  }
  case ST_CID_PROG_BLOCK: {
    this->ExecuteCmdProgBlock(exe);
    break;
  }
  case ST_CID_PROG_UPDATE: {
    this->ExecuteCmdProgUpdate(exe);
    break;
  }
  // Status Messages
  case ST_CID_STATUS: {
    this->ExecuteCmdStatus(exe);
    break;
  }
  case ST_CID_STATUS_CFG_GET: {
    this->ExecuteCmdStatusCfgGet(exe);
    break;
  }
  case ST_CID_STATUS_CFG_SET: {
    this->ExecuteCmdStatusCfgSet(exe);
    break;
  }
  // Settings Messages
  case ST_CID_SETTINGS_GET: {
    this->ExecuteCmdSettingsGet(exe);
    break;
  }
  case ST_CID_SETTINGS_SET: {
    this->ExecuteCmdSettingsSet(exe);
    break;
  }
  case ST_CID_SETTINGS_LOAD: {
    this->ExecuteCmdSettingsLoad(exe);
    break;
  }
  case ST_CID_SETTINGS_SAVE: {
    this->ExecuteCmdSettingsSave(exe);
    break;
  }
  case ST_CID_SETTINGS_RESET: {
    this->ExecuteCmdSettingsReset(exe);
    break;
  }
  // Calibration Messages
  case ST_CID_CAL_ACTION: {
    this->ExecuteCmdCalAction(exe);
    break;
  }
  ...
  case ST_CID_XCVR_ANALYSE: {
    this->ExecuteCmdXcvrAnalyse(exe);
    break;
  }
  case ST_CID_XCVR_TX_MSG: {
    this->ExecuteCmdXcvrTxMsg(exe);
    break;
  }
  case ST_CID_XCVR_RX_ERR: {
    this->ExecuteCmdXcvrRxErr(exe);
    break;
  }
  case ST_CID_XCVR_RX_MSG: {
    this->ExecuteCmdXcvrRxMsg(exe);
    break;
  }
  case ST_CID_XCVR_RX_REQ: {
    this->ExecuteCmdXcvrRxReq(exe);
    break;
  }
  case ST_CID_XCVR_RX_RESP: {
    this->ExecuteCmdXcvrRxResp(exe);
    break;
  }
  case ST_CID_XCVR_RX_UNHANDLED: {
    this->ExecuteCmdXcvrRxUnhandled(exe);
    break;
  }
  case ST_CID_XCVR_USBL: {
    this->ExecuteCmdXcvrUsbl(exe);
    break;
  }
  case ST_CID_XCVR_FIX: {
    this->ExecuteCmdXcvrFix(exe);
    break;
  }
  case ST_CID_XCVR_STATUS: {
    this->ExecuteCmdXcvrStatus(exe);
    break;
  }
  case ST_CID_XCVR_TX_MSGCTRL_SET: {
    this->ExecuteCmdXcvrTxMsgCtrlSet(exe);
    break;
  }
  // PING Protocol Messages
  case ST_CID_PING_SEND: {
    this->ExecuteCmdPingSend(exe);
    break;
  }
  case ST_CID_PING_REQ: {
    this->ExecuteCmdPingReq(exe);
    break;
  }
  case ST_CID_PING_RESP: {
    this->ExecuteCmdPingResp(exe);
    break;
  }
  case ST_CID_PING_ERROR: {
    this->ExecuteCmdPingError(exe);
    break;
  }
  // ECHO Protocol Messages
  case ST_CID_ECHO_SEND: {
    this->ExecuteCmdEchoSend(exe);
    break;
  }
  case ST_CID_ECHO_REQ: {
    this->ExecuteCmdEchoReq(exe);
    break;
  }
  case ST_CID_ECHO_RESP: {
    this->ExecuteCmdEchoResp(exe);
    break;
  }
  case ST_CID_ECHO_ERROR: {
    this->ExecuteCmdEchoError(exe);
    break;
  }
  // NAV Protocol Messages
  case ST_CID_NAV_QUERY_SEND: {
    this->ExecuteCmdNavQuerySend(exe);
    break;
  }
  case ST_CID_NAV_QUERY_REQ: {
    this->ExecuteCmdNavQueryReq(exe);
    break;
  }
  case ST_CID_NAV_QUERY_RESP: {
    this->ExecuteCmdNavQueryResp(exe);
    break;
  }
  case ST_CID_NAV_ERROR: {
    this->ExecuteCmdNavError(exe);
    break;
  }
  case ST_CID_NAV_QUEUE_SET: {
    this->ExecuteCmdNavQueueSet(exe);
    break;
  }
  case ST_CID_NAV_QUEUE_CLR: {
    this->ExecuteCmdNavQueueClr(exe);
    break;
  }
  case ST_CID_NAV_QUEUE_STATUS: {
    this->ExecuteCmdNavQueueStatus(exe);
    break;
  }
  case ST_CID_NAV_STATUS_SEND: {
    this->ExecuteCmdNavStatusSend(exe);
    break;
  }
  case ST_CID_NAV_STATUS_RECEIVE: {
    this->ExecuteCmdNavStatusReceive(exe);
    break;
  }
  // DAT Protocol Messages
  case ST_CID_DAT_SEND: {
    this->ExecuteCmdDatSend(exe);
    break;
  }
  case ST_CID_DAT_RECEIVE: {
    this->ExecuteCmdDatReceive(exe);
    break;
  }
  case ST_CID_DAT_ERROR: {
    this->ExecuteCmdDatError(exe);
    break;
  }
  case ST_CID_DAT_QUEUE_SET: {
    this->ExecuteCmdDatQueueSet(exe);
    break;
  }
  case ST_CID_DAT_QUEUE_CLR: {
    this->ExecuteCmdDatQueueClr(exe);
    break;
  }
  case ST_CID_DAT_QUEUE_STATUS: {
    this->ExecuteCmdDatQueueStatus(exe);
    break;
  }
  ...
  case ST_CID_CFG_BEACON_GET: {
    this->ExecuteCmdCfgBeaconGet(exe);
    break;
  }
  case ST_CID_CFG_BEACON_SET: {
    this->ExecuteCmdCfgBeaconSet(exe);
    break;
  }
  case ST_CID_CFG_BEACON_RESP: {
    this->ExecuteCmdCfgBeaconResp(exe);
    break;
  }
...
void CSeatrac::ExecuteCmdCalAction(PSeatracExecuteParams exe) {
  ...
  if (!success) {
    // Indicate a decode error has occured, this will raise an ExecuteDoneEvent
    // with an error code
...
void CSeatrac::ExecuteCmdCfgBeaconResp(PSeatracExecuteParams exe) {
  ...
  if (!success) {
    ...
    if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_CFG_BIT)) {
      ...
      diagMsg.AddParamString("BeaconId",
                             CSeatracUtils::ToStringBeaconId(params.BeaconId));
  ...
  if (!success) {
    ...
    if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_DAT_BIT)) {
      ...
      diagMsg.SetType(ST_DIAG_TYPE_PROCESSING, exe->Id,
                      (params.CmdStatus != ST_CST_OK));
      diagMsg.AddParamsCmdStatus(params.CmdStatus);
      diagMsg.AddParamString("DestId",
                             CSeatracUtils::ToStringBeaconId(params.DestId));
      diagMsg.AddParam("PacketLen", "%u", params.Length);

      if (!destIdValid) {
        diagMsg.AddNote(
            "DestId missing (assumed BEACON_UNKNOWN) - upgrade firmware");
...
void CSeatrac::ExecuteCmdDatQueueStatus(PSeatracExecuteParams exe) {
  ...
  // if(!success) {
  //     //Indicate a decode error has occured, this will raise an
  //     ExecuteDoneEvent with an error code exe->Status = ST_EXE_PARAM_MISSING;
  // }
  // else {
#ifdef SEATRAC_EN_DIAG
  // Write a debug log entry if enabled
...
void CSeatrac::ExecuteCmdDatReceive(PSeatracExecuteParams exe) {
  ...
  bool localFlagValid = exe->Msg->ReadBool(
      &params.LocalFlag, false); // Supported in firmware 1.12 onwards
...
void CSeatrac::ExecuteCmdNavQueryReq(PSeatracExecuteParams exe) {
  ...
  bool packetValid = exe->Msg->ReadNavPacket(
      &params.Payload); // Supported in firmware 1.12 onwards
  bool localFlagValid = exe->Msg->ReadBool(
      &params.LocalFlag, false); // Supported in firmware 1.12 onwards
...
void CSeatrac::ExecuteCmdNavQueryResp(PSeatracExecuteParams exe) {
  ...
  if (IS_BIT_SET(params.QueryFlags, ST_NAVQUERY_FLAGS_DATA_BIT)) {
    packetValid = exe->Msg->ReadNavPacket(
        &params.Payload); // Supported in firmware 1.12 onwards
  } else {
...
void CSeatrac::ExecuteCmdStatusCfgGet(PSeatracExecuteParams exe) {
  ...
  if (!success) {
    ...
    if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_STATUS_BIT)) {
      CSeatracDiagMsg diagMsg;
      diagMsg.SetType(ST_DIAG_TYPE_PROCESSING, exe->Id);
      diagMsg.AddParamString("Mode",
                             CSeatracUtils::ToStringStatusMode(params.Mode));
...
void CSeatrac::ExecuteCmdSysInfo(PSeatracExecuteParams exe) {
  ...
  if (params.AppType == ST_APPTYPE_MAIN) {
    ...
    success &= exe->Msg->ReadUint8(
        &tmp8); // Should be 0x01, Indicates Pressure Sensor Flags
  ...
  if (!success) {
    ...
    uint32 curFirmware =
        (params.MainApp.VersionMajor * 1000) + params.MainApp.VersionMinor;
    uint32 minFirmware = (ST_FIRMWARE_MAIN_VERSION_MAJOR * 1000) +
                         ST_FIRMWARE_MAIN_VERSION_MINOR;
    ...
    if (IS_BIT_SET(_diagLogFlags, SEATRAC_DIAGLOG_SYS_BIT)) {
      ...
      diagMsg.SetType(ST_DIAG_TYPE_PROCESSING, exe->Id);
      diagMsg.AddParam("Seconds", "%us", params.Seconds);
      diagMsg.AddParamString("AppType",
                             CSeatracUtils::ToStringAppType(params.AppType));
      diagMsg.AddParamString(
          "BeaconType", CSeatracUtils::ToStringBeaconType(params.BeaconType));
      diagMsg.AddParam("HardwarePart", "BP%05u.%06u",
                       params.Hardware.PartNumber,
                       params.Hardware.SerialNumber);
      diagMsg.AddParam("HardwareRev", "%u", params.Hardware.PartRev);
      diagMsg.AddParam("HardwareFlags", "{0x%04X,0x%04X}",
                       params.Hardware.FlagsSys, params.Hardware.FlagsUser);
      ...
      if (params.BootApp.Valid) {
        ...
        diagMsg.AddParamString("BootVersion", CSeatracUtils::ToStringVersion(
                                                  params.BootApp.VersionMajor,
                                                  params.BootApp.VersionMinor,
                                                  params.BootApp.VersionBuild));
      ...
      if (params.MainApp.Valid) {
        ...
        diagMsg.AddParamString("MainVersion", CSeatracUtils::ToStringVersion(
                                                  params.MainApp.VersionMajor,
                                                  params.MainApp.VersionMinor,
                                                  params.MainApp.VersionBuild));
      ...
      // Set the message as a warning if the firmware isn't supported, or the
      // firmware isn't valid
      if (!params.BootApp.Valid || !params.MainApp.Valid ||
          !params.MainSupported)
        diagMsg.RaiseStatus(ST_DIAG_STATUS_WARNING);
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::NavQuerySend(ESeatracBeaconId destId, uint8 queryFlags,
                            puint8 data, uint8 dataLen) {
  // Abort if the destId is invalid
  if (!CSeatracUtils::IsBeaconIdPhysical(destId))
    return false;
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::NavQueueSet(ESeatracBeaconId destId, puint8 data,
                           uint8 dataLen) {
  // Abort if the destId is invalid
  if (!CSeatracUtils::IsBeaconIdPhysicalOrAll(destId))
    return false;
...
/*!-----------------------------------------------------------------------------
 */
bool CSeatrac::NavStatusSend(ESeatracBeaconId beaconId, puint8 data,
                             uint8 dataLen) {
  // Abort if the destId is invalid
...
*/
#ifdef SEATRAC_EN_DIAG
void CSeatrac::DiagNameSet(string &value) { _diagName.assign(value); }
#endif

/*!-----------------------------------------------------------------------------
 */
#ifdef SEATRAC_EN_DIAG
void CSeatrac::DiagNameSet(pchar buf) { _diagName.assign(buf); }
#endif
...
/*!-----------------------------------------------------------------------------
Function that requests the acoustic transceiver performs a background noise
analysis.
*/
