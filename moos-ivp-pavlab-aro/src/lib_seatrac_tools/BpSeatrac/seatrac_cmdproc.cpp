...
*/
CSeatracCmdProc::CSeatracCmdProc(char encodeHeader, char decodeHeader,
                                 uint32 bufLen) {
...
void CSeatracCmdProc::DoDecodeChar(uint8 ch) {
  uint8 hex; // Temporary values where hex characters are converted to binary
             // values
  ...
  if (ch == _decodeHeader) {
    ...
    if (ch == 13) {
      ...
      // Process the command on a<LF> (Line-Feed), as this is the last character
      // in the CR+LF sequence on half-duplex.
      uint16 csumRx;
      uint16 csumCalc;
      ...
      if (_decodeCsumEn) {
        ...
        // If we're not using Rx command checksums, set both to 0 so we always
        // execute...
        csumRx = 0;
        csumCalc = 0;
      }
...
*/
void CSeatracCmdProc::DoDecodeMsg(PSeatracCmdMsg msg,
                                  ECmdProcDecodeStatus status) {
...
@result true if the OnEncode event handler did not modify Success variable to
false.
*/
  ...
  if (msg) {
    ...
    if (msgLen > 0) {
      ...
      if (sendBuf) {
        ...
        // Populate the message executions params
        uint32 len =
            min((uint32)sizeof(params.Buffer), (uint32)sendBuf->GetLength());
...
@param args The variable args structure containing variables to insert in the
format string
@result true if the OnEncode event handler did not modify Success variable to
false.
*/
bool CSeatracCmdProc::EncodePrintArgs(pchar str, va_list args) {
  ...
  params.Success = true; // The OnEncode event handler can modify this to fail
                         // (i.e. if passing to a serial port etc)
