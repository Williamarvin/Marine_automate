...
@result True is the mutex has been locked and can be used, false is an error
occured and the mutex can't lock.
*/
bool CMutex::Lock(PMutexStatus status) {
  ...
  if (_valid) {
    ...
    switch (result) {
    case 0: {
      localStatus = MUTEX_SIGNALED;
      success = true;
      break;
    }
    case ETIMEDOUT: {
      localStatus = MUTEX_TIMEOUT;
      success = false;
      break;
    }
    case EBUSY: {
