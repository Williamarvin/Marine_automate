...
#define __MESSAGE(str)                                                         \
  PRAGMA(message(__FILE__ "(" STRINGIZE(__LINE__) "): " str))
...
// Function that should be placed and executed in RAM (and accessed with a
// Long-Call if it is far away from the calling statement) (see
//http://stackoverflow.com/questions/15137214/how-to-run-code-from-ram-on-arm-architecture
// and https://ez.analog.com/thread/10607)
// You may get an compiler warning that can be safely ignored: "Warning:
// ignoring changed section attributes for .data" here linker will place
// function in ram, but ideally linker requires seperatly defined ram function
// setion, but this is tedious. #define FUNC_RAM
//void __attribute__ (( long_call, section(".data")))

//#define FUNC_RAM_OPTIMIZE_OFF	void __attribute__ (( long_call,
//section(".data"), optimize("O0")))

/*! Macro that disables optimisation for a specific function - usage
 * "FUNC_OPTIMIZE_OFF foo(void) {}" */
...
//==============================================================================
#ifdef DEBUG
/*! Macro that sets optimisation off for all following functions in the file
 * until DEBUG_GLOBAL_OPTIMIZE_RESTORE is issued */
#define DEBUG_GLOBAL_OPTIMIZE_OFF                                              \
  PRAGMA(GCC push_options)                                                     \
  PRAGMA(GCC optimize("O0"))

#define DEBUG_GLOBAL_OPTIMIZE_RESTORE PRAGMA(GCC pop_options)

/*! Macro that disables optimisation for a specific function - usage "void
 * DEBUG_FUNC_OPTIMIZE_OFF foo(void) {}" */
...
//! Macro that removes compiler warning C4800 "forcing value to bool 'true' or
...
//==============================================================================
#define CSI_ERASE_LINE                                                         \
  "\x1b[2K" /*!< Erases the entire contents of the cursors line, does not move \
               cursor */
#define CSI_RESET_LINE                                                         \
  "\x1b[2K\r" /*!< Erases the entire contents of the cursors line, resets      \
                 cursor to start */

#define CSI_NORMAL "\x1b[30;0m"   /*!< Reset text parameters */
#define CSI_TEXT_BLACK "\x1b[30m" /*!< Text colour Black ANSI escape sequence  \
                                   */
#define CSI_TEXT_MAROON                                                        \
  "\x1b[31m" /*!< Text colour Dark Red ANSI escape sequence */
#define CSI_TEXT_FOREST                                                        \
  "\x1b[32m" /*!< Text colour Dark Green ANSI escape sequence */
...
#define CSI_TEXT_PURPLE                                                        \
  "\x1b[35m"                     /*!< Text colour Purple ANSI escape sequence */
#define CSI_TEXT_TEAL "\x1b[36m" /*!< Text colour Teal ANSI escape sequence */
#define CSI_TEXT_SILVER                                                        \
  "\x1b[37m" /*!< Text colour Silver ANSI escape sequence */
...
#define CSI_TEXT_GREEN                                                         \
  "\x1b[32;1m" /*!< Text colour Green ANSI escape sequence */
#define CSI_TEXT_YELLOW                                                        \
  "\x1b[33;1m" /*!< Text colour Yellow ANSI escape sequence */
#define CSI_TEXT_BLUE "\x1b[34;1m" /*!< Text colour Blue ANSI escape sequence  \
                                    */
#define CSI_TEXT_MAGENTA                                                       \
  "\x1b[35;1m" /*!< Text colour Magenta ANSI escape sequence */
#define CSI_TEXT_CYAN "\x1b[36;1m" /*!< Text colour Cyan ANSI escape sequence  \
                                    */
#define CSI_TEXT_WHITE                                                         \
  "\x1b[37;1m" /*!< Text colour White ANSI escape sequence */
