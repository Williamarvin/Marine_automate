...
bool FldBloomStormSim::handleConfigRegion(std::string opstr,
                                          unsigned int number) {
...
bool FldBloomStormSim::manageBlooms() {
  ...
  while (!good_point_found) {
    ...
    double rand_rad_scale =
        static_cast<double>(rand()) / static_cast<double>(RAND_MAX);
    new_pulse_rad =
        m_bloom_rad_min + rand_rad_scale * (m_bloom_rad_max - m_bloom_rad_min);
  ...
  double rand_dur_scale =
      static_cast<double>(rand()) / static_cast<double>(RAND_MAX);
  double new_pulse_dur =
      m_bloom_duration_min +
      rand_dur_scale * (m_bloom_duration_max - m_bloom_duration_min);
...
bool FldBloomStormSim::checkActiveBlooms() {
  ...
  for (it = m_active_blooms.begin(); it != m_active_blooms.end();) {
    ...
    if ((MOOSTime() - it->get_time()) > it->get_duration()) {
      ...
      Notify("UNSAMPLED_BLOOMS_COMPLETED",
             uintToString(m_unsampled_blooms_completed));
      Notify("SAMPLED_BLOOMS_COMPLETED",
             uintToString(m_sampled_blooms_completed));
...
bool FldBloomStormSim::notifyVehiclesOfBloom() {
  ...
  // Assume cost to assemble the polygon makes the order of this nested
  // iteration optimal.
...
Notify("TMP_DEBUG", "built polygon for " + it2->get_spec());
    */
    ...
    for (it = m_map_node_records.begin(); it != m_map_node_records.end();
         it++) {
  ...
  for (it3 = m_map_node_records.begin(); it3 != m_map_node_records.end();
       it3++) {
...
bool FldBloomStormSim::manageStorm() {
  ...
  */
  ...
  double min_cpa =
      polyRayCPA(sx, sy, m_storm_heading, m_storm_region, m_storm_x, m_storm_y);
  m_storm_active = true;
  m_storm_count += 1;
...
bool FldBloomStormSim::buildReport() {
  ...
  m_msgs << " Active Blooms: " << uintToString(m_active_blooms.size())
         << "             " << endl;
  m_msgs << "============================================" << endl;
  std::list<XYRangePulse>::iterator it;
  for (it = m_active_blooms.begin(); it != m_active_blooms.end(); it++) {
  ...
  for (it2 = m_vehicles_in_bloom.begin(); it2 != m_vehicles_in_bloom.end();
       it2++) {
    bloom_list_str += " " + *it2 + ",";
  }
  ...
  m_msgs << " Completed Blooms: Sampled: "
         << uintToString(m_sampled_blooms_completed)
         << " Unsampled: " << uintToString(m_unsampled_blooms_completed)
         << endl;
  ...
  if (m_storm_active) {
    ...
    for (it3 = m_vehicles_in_storm.begin(); it3 != m_vehicles_in_storm.end();
         it3++) {
      storm_list_str += " " + *it3 + ",";
    }
