...
BHV_DubinsPath::BHV_DubinsPath(IvPDomain domain) : IvPBehavior(domain) {
...
bool BHV_DubinsPath::setParam(string param, string val) {
  ...
  if ((param == "goal_heading") && isNumber(val)) {
    ...
    m_os_name = val;
    return (true);
  } else if ((param == "r") && isNumber(val) &&
             (double_val >=
              0)) { // Handle if both r and one of r1, r2, r3 are set
    m_r1 = double_val;
    m_r2 = double_val;
    m_r3 = double_val;
    return (true);
    ...
    return (true);
  } else if ((param == "visualize_path_idle") &&
             (val == "true" || val == "false")) {
    m_visualize_path_idle = (val == "true");
    return (true);
  } else if ((param == "slowdown_range") && isNumber(val) &&
             (double_val >= 0)) {
...
void BHV_DubinsPath::onIdleState() {
  ...
  output_seglist.set_label(m_os_name + "_dubin_" +
                           to_string(m_number_of_paths_generated + 1));
  ...
  // TODO: also potentially use compass heading here

  DubinsPath dp = DubinsPath();
  string trajectory_str = dp.findOptimalWaypoints(
      Point(m_osx, m_osy), os_heading_rad, Point(m_goal_x, m_goal_y),
      goal_heading_rad, m_r1, m_r2, m_r3, m_precision);
...
void BHV_DubinsPath::onRunToIdleState() {
  ...
  XYSegList output_seglist;
  output_seglist.add_vertex(m_osx, m_osy);
  output_seglist.set_label(m_us_name + "_dubin_" +
                           to_string(m_number_of_paths_generated));
...
IvPFunction *BHV_DubinsPath::onRunState() {
  ...
  if (!m_path_generated) {
    ...
    if (m_op_region.size() == 0) { // No op region
      ...
      string trajectory_str = dp.findOptimalWaypoints(
          Point(m_osx, m_osy), os_heading_rad, Point(m_goal_x, m_goal_y),
          goal_heading_rad, m_r1, m_r2, m_r3, m_precision, illegal_paths);
      ...
      while (path_outside_op_region) {
        ...
        for (int i = 0; i < new_trajectory.size(); i++) {
          if (!m_op_region.contains(new_trajectory.get_vx(i),
                                    new_trajectory.get_vy(i))) {
  ...
  double dist_to_nextpt =
      sqrt(pow(m_nextpt.x() - m_osx, 2) + pow(m_nextpt.y() - m_osy, 2));
  ...
  if (dist_to_nextpt < m_capture_radius) { // Captured the next point
  ...
  } else if ((dist_to_nextpt > m_current_cpa) &&
             (m_current_cpa <=
              m_slip_radius)) { // Increased distance to point and we are within
                                // the slip radius
    ...
    if (m_curr_ix + 1 >= m_trajectory.size()) {
      ...
      m_nextnextpt = m_trajectory.get_point(m_curr_ix + 1);
             ...
             m_drift_radius) { // No capture, no slip. Drifted more x meters
                               // from the next point
  ...
  //-------Send name, current position and remaining waypoints to a MOOS
  //variable:--------
  XYSegList output_seglist;
  output_seglist.add_vertex(m_osx, m_osy); // Add current position
  output_seglist.set_label(m_us_name + "_dubin_" +
                           to_string(m_number_of_paths_generated));
  for (int i = m_curr_ix; i < m_trajectory.size();
       i++) { // Add remaining waypoints
...
IvPFunction *BHV_DubinsPath::buildOF() {
  ...
  if (m_speed_desired > m_speed_previous) { // Accelerating --> LPF to smooth
    double next_speed = m_speed_LPF_alpha * m_speed_desired +
                        (1 - m_speed_LPF_alpha) * m_speed_previous;
  ...
  // TODO: MAKE SURE THAT IT IS NOT PHYSICALLY BLOCKED, BECAUSE THEN IT SHOULD
  // NOT HAVE MIN 0.1 SPEED
  if (m_slowdown_range > 0) {
    int slowdowon_range_points = round(m_slowdown_range / m_precision);
    if (m_trajectory.size() - m_curr_ix < slowdowon_range_points) {
      cruise_speed = max(0.1, m_speed_desired *
                                  (0 + 1.0 * (m_trajectory.size() - m_curr_ix) /
                                           slowdowon_range_points));
    }
  }
  ...
  // double rel_ang_to_nxt_pt = relAng(m_osx, m_osy, m_nextpt.x(),
  // m_nextpt.y());
  double rel_ang_to_nxt_pt =
      relAng(m_osx, m_osy, m_nextnextpt.x(), m_nextnextpt.y());
...
void BHV_DubinsPath::updateConfigParameters() {
  if (getBufferVarUpdated("DEMUSTER_CONFIG")) {
    ...
    if (new_config == "turning_radius_increase") {
      ...
      m_r2 += 1;
      m_r3 += 1;
      postMessage("AA_DEMUSTER_CONFIG_UPDATED",
                  "Turning radius: " + to_string(m_r1));
      m_path_generated = false;
      ...
      m_slowdown_range += 1;
      postMessage("AA_DEMUSTER_CONFIG_UPDATED",
                  "Slowdown range: " + to_string(m_slowdown_range));
