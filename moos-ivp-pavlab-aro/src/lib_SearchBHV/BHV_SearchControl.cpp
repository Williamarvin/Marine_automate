...
BHV_SearchControl::BHV_SearchControl(IvPDomain domain) : IvPBehavior(domain) {
...
bool BHV_SearchControl::setParam(string param, string param_val) {
  ...
  if (param == "speed") {
    ...
    handled = true;
    if (m_mode == "stochastic" || m_mode == "rotate_speed" ||
        m_mode == "stochastic" || m_mode == "stochastic_free_speed")
      m_rand_speed = true;
  }
