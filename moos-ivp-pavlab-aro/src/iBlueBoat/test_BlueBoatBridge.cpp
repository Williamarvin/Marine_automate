...
void showHelpAndExit() {
  cout << "Usage: " << endl;
  cout << "    BlueBoatBridge [-e | --example] [-h | --help] [-i | --ip] [-p | "
          "--port] [-t | --type]"
       << endl;
  cout << "Options: " << endl;
  ...
  cout << "    -t, --type       Set communication type (client or server)"
       << endl;
  exit(0);
}
...
int main(int argc, char *argv[]) {
  ...
  for (std::list<std::string>::const_iterator it = messages.begin();
       it != messages.end(); ++it) {
    cout << "Message: ";
    cout << *it << endl;
  }
