...
HydroMAN_IvPExtend::HydroMAN_IvPExtend() {
  // This is a fake number since BF doesnt give us sat info. But is required for
  // HydroMAN.
  m_current_gps.sat = 8;
}
...
bool HydroMAN_IvPExtend::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (m_current_actuator.thrust_left_rcvd &&
        m_current_actuator.thrust_right_rcvd) {
...
bool HydroMAN_IvPExtend::Iterate() {
  ...
  if (client_->connected()) {
    ...
    // Try to reconnect
    // TO DO - IF WE RECONNECTED, WE NEED TO MAKE SURE NAV SOLUTION CONTINUES
    // SMOOTHLY
  ...
  while (!client_->connected()) {
    ...
    try {
      ...
      if (!printed_loop_error) {
        ...
        std::cout << "Looping until I connect to HydroMAN gateway. Error = "
                  << ec << "\n";
...
void HydroMAN_IvPExtend::defineIncomingInterfaceMsges() {
  ...
  // hydroman::interface::DataFromHydroMAN protobuf message came through the
  // interface
  client_->read_callback<hydroman::interface::DataFromHydroMAN>(
      [this](const hydroman::interface::DataFromHydroMAN &msg,
...
void HydroMAN_IvPExtend::handle_DataFromHydroMAN_interfaceMsges(
  ...
  if (msg.has_nav()) {
    ...
    if (!std::isnan(msg.nav().lat()) && !std::isnan(msg.nav().lon())) {
      m_geodesy.LatLong2LocalUTM(msg.nav().lat(), msg.nav().lon(), veh_y,
                                 veh_x);
      Notify("NAV_X", veh_x);
      Notify("NAV_Y", veh_y);
    ...
    if (msg.nav().has_stdev_x() && msg.nav().has_stdev_y() &&
        msg.nav().has_cov_xy() && msg.nav().has_stdev_u() &&
        msg.nav().has_stdev_heading() && !std::isnan(veh_x) &&
        !std::isnan(veh_y)) {
      ...
      str_unitTrack << "vehicle=" << m_ownship_name
                    ...
                    << ",stdev_heading="
                    << std::to_string(msg.nav().stdev_heading());
  ...
  if (msg.has_target_nav()) {
    ...
    if (!std::isnan(msg.target_nav().lat()) &&
        !std::isnan(msg.target_nav().lon())) {
      m_geodesy.LatLong2LocalUTM(msg.target_nav().lat(), msg.target_nav().lon(),
                                 tgt_y, tgt_x);
      // Publish the tgt nav
      ...
      if (m_publish_tgt_node_rpt) {
        ...
        std::cout << "Target Node Report Published: " << node_report.getSpec()
                  << std::endl;
      } else {
        std::cout << "Target Node Report NOT Published: "
                  << node_report.getSpec() << std::endl;
      }
    }
  }
  ...
  if (msg.has_moos_publish()) {
    if (msg.moos_publish().has_msg_name() &&
        msg.moos_publish().has_msg_string()) {
      Notify(msg.moos_publish().msg_name(), msg.moos_publish().msg_string());
    } else if (msg.moos_publish().has_msg_name() &&
               msg.moos_publish().has_msg_double()) {
  ...
  if (msg.has_particle_nav()) {
    double veh_x{0}, veh_y{0};
    if (!std::isnan(msg.particle_nav().lat()) &&
        !std::isnan(msg.particle_nav().lon())) {
      m_geodesy.LatLong2LocalUTM(msg.particle_nav().lat(),
                                 msg.particle_nav().lon(), veh_y, veh_x);
      // Notify("NAV_X", veh_x);
      // Notify("NAV_Y", veh_y);
    ...
    if (msg.particle_nav().has_pos_source()) {
      if (msg.particle_nav().pos_source() ==
          hydroman::interface::PARTICLE_NAV::UNKNOWN)
        nav_source = "UNKNOWN";
      if (msg.particle_nav().pos_source() ==
          hydroman::interface::PARTICLE_NAV::GPS)
        nav_source = "GPS";
      if (msg.particle_nav().pos_source() ==
          hydroman::interface::PARTICLE_NAV::MODEL)
        nav_source = "MODEL";
    }
    ...
    if (msg.particle_nav().has_stdev_x() && msg.particle_nav().has_stdev_y() &&
        ...
        msg.particle_nav().has_stdev_heading() && !std::isnan(veh_x) &&
        !std::isnan(veh_y)) {
      std::stringstream str_unitTrack;
      str_unitTrack
          << "vehicle=" << m_ownship_name
          << ",time=" << std::to_string(msg.time())
          << ",x=" << std::to_string(veh_x) << ",y=" << std::to_string(veh_y)
          ...
          << ",stdev_heading="
          << std::to_string(msg.particle_nav().stdev_heading());
      if (m_use_p_nav_for_track) {
        Notify(m_var_tgt_track + "_LOCAL", str_unitTrack.str());
      }
    }
...
void HydroMAN_IvPExtend::handle_gps_data() {
  ...
  // try
  // {
  //   m_geodesy.UTM2LatLong(m_current_gps.x, m_current_gps.y, veh_lat,
  //   veh_lon);
  // }
  ...
  if (client_->connected() && gps_data_.has_gps_data()) {
    // std::cout << "Sending data to HydroMAN: GPS: " <<
    // gps_data_.ShortDebugString() << std::endl;
...
void HydroMAN_IvPExtend::handle_compass_data(double hdg_) {
  ...
  if (client_->connected() && imu_data_.has_imu_data()) {
    std::cout << "Sending data to HydroMAN: IMU: "
              << imu_data_.ShortDebugString() << std::endl;
...
void HydroMAN_IvPExtend::handle_actuator_data() {
  ...
  actuator_data_.mutable_actuator_data()->set_left_rpm(
      m_current_actuator.thrust_left);
  actuator_data_.mutable_actuator_data()->set_right_rpm(
      m_current_actuator.thrust_right);
...
void HydroMAN_IvPExtend::handle_tgt_track_data(std::string sval) {
  ...
  if (MOOSValFromString(vehicle, sval, "vehicle")) {
    track_report_.mutable_target_track_data()->set_vehicle_name(
        vehicle.c_str());
  }
  ...
  if (MOOSValFromString(nav_src, sval, "nav_source")) {
    if (nav_src == "UNKNOWN")
      track_report_.mutable_target_track_data()->set_pos_source(
          hydroman::interface::TARGET_TRACK::UNKNOWN);

    if (nav_src == "GPS")
      track_report_.mutable_target_track_data()->set_pos_source(
          hydroman::interface::TARGET_TRACK::GPS);

    if (nav_src == "MODEL")
      track_report_.mutable_target_track_data()->set_pos_source(
          hydroman::interface::TARGET_TRACK::MODEL);
  }
  ...
  if (client_->connected() &&
      track_report_.target_track_data().vehicle_name() != m_ownship_name &&
