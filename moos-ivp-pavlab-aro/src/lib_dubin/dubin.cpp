...
void DubinsPath::findOptimalPath(Point ps, double hs, Point pg, double hg,
                                 double r1, double r2, double r3,
                                 std::vector<std::string> illegal_paths) {
  m_ps = ps;
  m_hs = hs;
  m_pg = pg;
  m_hg = hg;
  ...
  // Check if each path has feasible solution (length > 0) AND if it is the
  // shortest path OR the first feasible path
  if (find(illegal_paths.begin(), illegal_paths.end(), "RSR") ==
      illegal_paths.end()) {
  ...
  if (find(illegal_paths.begin(), illegal_paths.end(), "LSL") ==
      illegal_paths.end()) {
  ...
  if (find(illegal_paths.begin(), illegal_paths.end(), "RSL") ==
      illegal_paths.end()) {
  ...
  if (find(illegal_paths.begin(), illegal_paths.end(), "LSR") ==
      illegal_paths.end()) {
  ...
  if (find(illegal_paths.begin(), illegal_paths.end(), "LRL") ==
      illegal_paths.end()) {
  ...
  if (find(illegal_paths.begin(), illegal_paths.end(), "RLR") ==
      illegal_paths.end()) {
  ...
  m_length = min_length;
  m_type = min_type;
  // Note: Segment lengths, circle centers and tangent points are updated in the
  // functions RSR, LSL, etc.
  //       and saved in the optimal variables in the updateOptimalVariables()
  //       function
  return;
};
...
string DubinsPath::findOptimalWaypoints(
    Point ps, double hs, Point pg, double hg, double r1, double r2, double r3,
    double precision, std::vector<std::string> illegal_paths) {
  findOptimalPath(
      ps, hs, pg, hg, r1, r2, r3,
      illegal_paths); // Calulates optimal path and stores the optimal variables
  ...
  // Add start point (NOTE: might not want first point as waypoint, since it is
  // OS position)
  ...
  if (number_of_points > 0) {
    ...
    m_waypoint_extra =
        "x=" + to_string(p_extra.x) + ", y=" + to_string(p_extra.y);
  } else {
...
double DubinsPath::RSR() {
  ...
  // Can be shown that c is the cosine of the angle between V1 and n_hat, simply
  // rotate V1_hat by this angle:
...
double DubinsPath::LRL() {
  ...
  double theta_temp = acos(
      (pow(D, 2) + pow(m_r1, 2) - pow(m_r2, 2) + 2 * m_r3 * (m_r1 - m_r2)) /
      (2 * D * (m_r1 + m_r3)));
...
double DubinsPath::RLR() {
  ...
  // Calculate theta and circle center 3
  double theta_temp = M_PI - acos((pow(D, 2) + pow(m_r1, 2) - pow(m_r2, 2) +
                                   2 * m_r3 * (m_r1 - m_r2)) /
                                  (2 * D * (m_r1 + m_r3)));
  double theta = atan2(V1.y, V1.x) + theta_temp;
  m_c3_temp = m_c1_temp - Point(cos(theta), sin(theta)) * (m_r1 + m_r3);
...
double Point::length() { return sqrt(pow(x, 2) + pow(y, 2)); };

Point Point::operator+(const Point &p) { return Point(x + p.x, y + p.y); };

Point Point::operator-(const Point &p) { return Point(x - p.x, y - p.y); };

Point Point::operator*(const double &s) { return Point(x * s, y * s); };
