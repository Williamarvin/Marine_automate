...
public:
   ...
   *
   * @param port A string which provides a path to a serial port i.e.
   * /dev/ttyACM0
   * @param baudRate Default 9600 bits/s
   * @param read_buffer_size Default 64 bytes
   */
  SerialDevice(const std::string &port, size_t baudRate,
  ...
  /**
   * @brief If not setup with the configuration constructor, can set
   * configuration settings before opening the port
   *
   * @param port A string which provides a path to a serial port i.e.
   * /dev/ttyACM0
   * @param baudRate Default 9600 bits/s
   * @param read_buffer_size Default 64 bytes
   */
  void config(const std::string &port, size_t baudRate,
              size_t read_buffer_size);
  ...
  /**
   * @brief Reads from the serial port, and returns the system level error
   * message. Can be used like readSerial, where a read is successful the size
   * is 0
   *
   *
   * @param contents A string buffer which the contents will be placed
   ...
   * level error message. Can be used like writeSerial, where if a write is
   * successful the length of the message is of size 0
   *
   * @param data Data to be written to the serial device
   * @return std::string An error message from strerror(errno)
   */
