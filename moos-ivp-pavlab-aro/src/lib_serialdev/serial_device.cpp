...
SerialDevice::SerialDevice() { init(); }

void SerialDevice::config(const std::string &port, size_t baudRate = B9600,
                          size_t read_buffer_size = 64) {
  m_port = port;
  if (!m_standard_bauds.count(baudRate))
    fprintf(stderr,
            "<serial_device.cpp> Warning: Non-standard baud rate selected\n");
...
SerialDevice::SerialDevice(const std::string &port, size_t baudRate = B9600,
                           size_t read_buffer_size = 64) {
  ...
  if (m_serial_fd == -1) {
    ...
    for (int i = 1; i < 6; i++) {
      if (m_reconnect_conditions.count(errno)) {
        ...
        // Wait for 200 milliseconds to try to reconnect, maybe waste a maximum
        // of 1 second
        usleep(200000);
        m_serial_fd = open(m_port.c_str(), O_RDWR | O_NOCTTY | O_NDELAY);
      } else {
...
int SerialDevice::readSerial(std::string &contents) {
  ...
  // Returns an error if there was one, or the total character count from what
  // was in the buffer
  return n;
}
...
int SerialDevice::closeSerial() {
  ...
  if (condition) {
    ...
    case EINTR: {
      fprintf(
          stderr,
          "<serial_device.cpp> ERR: [1] Closing interrupted - trying again\n");
      usleep(200000);
      for (int i = 2; i < 6; i++) {
        ...
        if (condition)
          break;
        fprintf(stderr,
                "<serial_device.cpp> ERR: [%d] Closing interrupted - trying "
                "again\n",
                i);
        usleep(200000);
      }
    ...
    case EBADF: {
      fprintf(stderr, "<serial_device.cpp> ERR: Unable to close serial port - "
                      "bad file descriptor\n");
      break;
    }
    case EIO: {
      fprintf(stderr,
              "<serial_device.cpp> ERR: Unable to close serial port [%s]\n",
              strerror(errno));
      break;
    }
    default: {
