...
/**
 * @brief A convoy point is an enhanced XYPoint with additional meta data to
 * support convoying, with convenience functions
 *
 */
class ConvoyPoint {
public:
...
public:
  ...
  /**
   * @brief Get the total approximated distance between all the points in the
   * point queue
   *
   * @return double
   */
  ...
  /**
   * @brief Seriaize the ConvoyPoint queue's representation into a string data
   * structure with a default delimeter
   *
   * @return std::string
   */
  std::string repr();
  ...
  /**
   * @brief Seriaize the ConvoyPoint queue's representation into a string data
   * structure with a chosen delimeter
   *
   * @return std::string
   */
