...
BHV_ConvoyPD::BHV_ConvoyPD(IvPDomain domain) : IvPBehavior(domain) {
  ...
  // Ownship gains
  m_kp_spd_1 = 0.1, m_kp_spd_2 = 0, m_kp_spd_1 = 0.1, m_kp_spd_2 = 0,
  ki_spd = 0;
  m_kp_hdg_1 = 0.1, m_kp_hdg_2 = 0, m_kp_hdg_1 = 0.1, m_kp_hdg_2 = 0,
  ki_hdg = 0;

  // Coupling gains - Must be at least less than or equal to one half of the
  // ownship gains, i.e. kp - 2 * kpc >= 0
  m_kpc_spd = 0.0;
  m_kdc_spd = 0.0;
  ...
  m_max_lag_length = 20; // maximum tail length to be left behind - we have this
                         // so the leaders trajectory will mostly be followed
  m_color = "yellow";
  m_redudant_update_interval =
      2; // by default, every two seconds, we will deliberately post a redundant
  ...
  string infovars =
      m_nav_h_k + "," + m_nav_y_k + "," + m_nav_h_k + "," + m_nav_spd_k + "," +
      m_leader_k + "," + m_contact_k + "," + m_agent_info_k + "," + m_nrl_k +
      "," + m_lead_point_k + "," + m_contact_list_k + "," + m_task_state_k +
      "," + m_whotowho_k + "," + m_ext_ordering_k + "," + m_updates_var_k;
...
void BHV_ConvoyPD::onIdleState() { updateMessages(); }
...
void BHV_ConvoyPD::scheduleRepost(std::string field) {
  m_next_redundant_update[field] =
      m_latest_buffer_time + m_redudant_update_interval;
}

//---------------------------------------------------------------
// Procedure: postStateMessages()
//   Purpose: Invoked when idle and when running publish generally maintained
//   state variables
...
void BHV_ConvoyPD::postOrdering() {
  ...
  // When we have a mapping of all known agents, we generate our ordering, and
  // share it to all other nodes
...
void BHV_ConvoyPD::seedPoints() {
  if ((abs(m_osx - m_osx_prv) > m_eps) && (abs(m_osy - m_osy_prv) > m_eps)) {
...
void BHV_ConvoyPD::updateMessages() {
  ...
  // If update our own state variables on this iteration pulling from the latest
  ...
  // If we received an update to our task state bid, we know who we are
  // following
  if (getBufferVarUpdated(m_task_state_k))
    updateCheckForContact();

  // If we receive a new lead point from whom we are follwoing, we add this to
  // our queue
  ...
  // We check to see if we received another node message where someone
  // broadcasted who they are following, to update our follower to leader and
  // leader to follower mapping
  if (getBufferVarUpdated(m_whotowho_k))
    updateFtoLMapping();

  // Ourselves and all other agents publish their known ordering, since it is
  // possible some or all agents are missing some information or updates. If we
  // recieve information which conveys we are missing information, we definitely
  // are, so we simply update our ordering with this information (redundant but
  // a fail safe)
  ...
  // Ourselves and all other agents distribute our own agent info, for some
  // local useage in ConvoyPD, but later to mostly be used for synchronization
...
void BHV_ConvoyPD::updateOwnshipState() {
  // Not to be continuously reset, but the name wasnt available after setting
  // parameters
  m_debug_fname = "debug_" + m_us_name + ".txt";
  ...
  // TODO: Should find a better place for this and be more consistent with our
  // usage
...
void BHV_ConvoyPD::updateCapturePoint() {
  ...
  if (!m_is_leader && m_cpq.size() > 0) {
    ...
    // If our distance to the target is less than our capture radius, then we
    // captured it If the point is essentially behind us but close enough to
    // cause problems, we let is slip i.e. The point is > +- 90 degrees off our
    // current heading, and may cause inconveniences, so we don't need to
    // consider it and should discard it
    if (dist < m_capture_radius ||
        (dist < m_slip_radius && abs(phi) > M_PI / 2)) {
      ...
      // If we are not the tail agent, then we can propagate this point to our
      // follower
      if (!m_is_tail) {
...
void BHV_ConvoyPD::updateIsLeader() {
  ...
  m_is_leader =
      tolower(getBufferStringVal(m_leader_k)) == "true" ? true : false;
}
  ...
  if (ext_ordering.size() > m_ordering_str.size()) {
    ...
    for (int i = 0; i < ext_order_vector.size() - 1; i++) {
      ...
      if (ext_order_vector[i] != "" && ext_order_vector[i + 1] != "") {
        ...
        // REM: this was a better place to re-register for known agents other
        // than the contacts list - need to look into this when making this code
        // more lean
...
void BHV_ConvoyPD::updateContactList() {
  ...
  // We received our total contact list, and update our list of contacts to
  // consider
...
void BHV_ConvoyPD::updateCheckForContact() {
  // We use the task state to see if we won a convoy bid, if we have, then we
  // broadcast who we are following for our other contacts
  ...
  if ((idx1 != std::string::npos) && (idx2 != std::string::npos) &&
      !m_has_broadcast_contact && !m_is_leader) {
...
IvPFunction *BHV_ConvoyPD::onRunState() {
  ...
  // If we are the leader, we don't do any of the driving for this convoypd
  // behavior, - it returns a weight of 0, but is a placeholder here for
  // synchronization
  if (m_is_leader) {
    seedPoints();
    ZAIC_PEAK spd_zaic(m_domain, "speed");

    bool has_tail = m_leader_to_follower_mapping[m_us_name] != "" ||
                    m_leader_to_follower_mapping[m_us_name] != "NULL";
    double set_spd = m_desired_speed;
    if (m_coupled && has_tail) {
      double tail_dist_err =
          m_contacts_lookup[m_leader_to_follower_mapping[m_us_name]].dist_err;
      double tail_spd_err =
          m_contacts_lookup[m_leader_to_follower_mapping[m_us_name]].spd_err;
      double tail_comp =
          0.5 * (m_kpc_spd * tail_dist_err + m_kdc_spd * tail_spd_err);
    ...
    // Check to see if we captured our current point if we have a target, if we
    // did, execute a procedure
    updateCapturePoint();

    // We build a speed function, and our speed policy is governed by a PD
    // controller
    ZAIC_PEAK spd_zaic(m_domain, "speed");

    double dist_to_target;

    // Obtain our total  distance to the target, and if our total distance is
    // passed our max lead length, we drop and prioritize points to catchup
    ...
    do {
      ...
      // If the distance exceeds our max lead length, we cut the lead to the max
      // length
      if (dist_to_target > (m_ideal_follow_range + m_max_lag_length) &&
          m_cpq.size() > 0) {
    ...
    } while (dist_to_target > (m_ideal_follow_range + m_max_lag_length) &&
             m_cpq.size() > 0);
    ...
    // Obtain the encoced leader speed of the lead point - if there are no
    // points in this queue, we maintain our desired speed
    if (m_cpq.size() != 0) {
    ...
    double set_spd =
        m_desired_speed +
        (m_kp_spd_1 + m_kp_spd_2 * abs(m_dist_err)) * (m_dist_err) +
        (m_kd_spd_1 + m_kd_spd_2 * abs(m_spd_err)) * (m_spd_err);

    // If we are coupled, then add on the coupling terms
    if (m_coupled) {
      double tail_dist_err, tail_spd_err, trail_dist_err, trail_spd_err;
      bool has_trail = m_follower_to_leader_mapping[m_us_name] != "" ||
                       m_follower_to_leader_mapping[m_us_name] != "NULL";
      bool has_tail = m_leader_to_follower_mapping[m_us_name] != "" ||
                      m_leader_to_follower_mapping[m_us_name] != "NULL";
      ...
      if (has_trail) {
        trail_dist_err =
            m_contacts_lookup[m_follower_to_leader_mapping[m_us_name]].dist_err;
        trail_spd_err =
            m_contacts_lookup[m_follower_to_leader_mapping[m_us_name]].spd_err;
      } else {
        trail_dist_err = 0;
        trail_spd_err = 0;
      }
      ...
      double trail_comp =
          m_kpc_spd * trail_dist_err + m_kdc_spd * trail_spd_err;
    ...
    // Clamp the result to be between zero and the max speed (assume no reverse
    // thrust)
    clamp(set_spd, 0, m_max_speed);
    ...
    // Now that we have our angle to to our target, which is either a waypoint
    // or our contact, we now incorporate a PD controller for trajectory
    // tracking
    double ang_to_target = relAng(m_osx, m_osy, next_convoy_point.p.get_vx(),
                                  next_convoy_point.p.get_vy());
    // We have our current heading, we have the angle to the target, and we have
    // the heading which was dropped by the leader. The heading dropped by the
    // leader is the "trajectory" with an optimal turning rate

    // TODO: This currently does nothing - we only only have a PD controller for
    // the speed policy
    double hdg_err, hdg_err_dot;
    if (m_cpq.size() != 0) {
      hdg_err =
          ((next_convoy_point.leader_heading + ang_to_target) / 2) - m_osh;
    ...
    double desired_course =
        ang_to_target +
        (m_kd_hdg_1 + m_kd_hdg_2 * abs(hdg_err_dot)) * (hdg_err_dot);
