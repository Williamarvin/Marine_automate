...
ConvoyPoint::ConvoyPoint(std::string strrep) { unpack(strrep); }
...
void ConvoyPoint::add_meta(std::string k, std::string v) { meta[k] = v; }

double ConvoyPoint::dist(ConvoyPoint trg) { return dist(trg.p); }
...
void ConvoyPoint::unpack(std::string cp_str) {
  ...
  for (int i = 0; i < kv_pairs.size(); i++) {
    ...
    v = kv_pairs[i];
    // We populate both the dictionary and the core variables - we may want to
    // propagate variables later for analysis which aren't used in smoothing
    meta[k] = v;
    if (k == "seed_time") {
      seed_time = stod(v);
...
void ConvoyPointQueue::add_point(ConvoyPoint cp) { m_points.push_back(cp); }
...
size_t ConvoyPointQueue::size() { return m_points.size(); }
...
double ConvoyPointQueue::get_dist_to_target() {
  ...
  // (os)  x x x x x x x  (cs)
  // If there are no points in the queue, we consider the straight distance to
  // the target
  if (!samples) {
    total_convoy_dist = 0;
  } else {
    // If there are  points in the queue, we consider the distance from the
    // ownship to the first point, the distance between all the points, and the
    // distance from the end of the list of points to the target
...
std::string ConvoyPointQueue::repr() { return repr("^"); }
