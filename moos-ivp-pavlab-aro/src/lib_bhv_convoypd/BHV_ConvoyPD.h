...
/* **************************************************************
  ...
  DESC:
    ...
    contain state information for the leader at the time in which it seeds the
point. A follower agent follows this trajectory, while trying to match the
leaders state at the time in which the point was seeded, i.e. if an agent is
generally lagging from its desired follow distance, a strength in the P
component of the controller increases the speed in which the agent pursues its
target, however if the leader was taking a turn and slowed its speed along this
trajectory the agent will slow down on this turn to prevent overshoot or large
    deviations from the trajectory. Imagine virtual springs and dampers, where
one directional springs connect an agent to its target along a trajectory, and a
virtual damper bound between an agent speed and the leaders speed at that point
in the trajectory.

    Algorithmically, the leader is seeding/encoding and sending the point to its
follower, where this point is added to the followers queue. While it captures
points in this queue, points are passed sequentially to its follower until it
reaches the tail of the convoy, and is then destroyed.

    Due to frequent message drops in simulation/on the water, there are
redundant broadcasting protocol, in which all agents certain expected
information to fill in parts of what an agent may have missed.
...
protected: // Local Utility functions
  ...
  /**
   * @brief Updates our ownship state information which we are tracking
   * internally
   *
   */
  ...
  /**
   * @brief If we receive an ordering posting from another agent which is more
   * complete than our observations, we update our ordering
   *
   */
  void updateExtOrdering();

  /**
   * @brief If the contact manager is running and posting contacts, we update
   * our contact list to use redundant information for error checking
   *
   */
  void updateContactList();

  /**
   * @brief When a bid is won, it tells us whom we are following, or whom
   * another agent is following. With this, we update whom we are following and
   * also broadcast it for others in the convoy to reference.
   *
   */
  void updateCheckForContact();

  /**
   * @brief When we receive a lead point from whom we are following, we add it
   * to our convoy point queue
   *
   */
  void updateLeadPoint();

  /**
   * @brief If we obtain new information on the ordering of agents in the
   * convoy, we update our follower to leader/leader to follower mappings.
   *
   */
  void updateFtoLMapping();

  /**
   * @brief If we receive an update in real time to our udpates variable, we
   * handle it
   *
   */
  ...
  /**
   * @brief If we receive a node report, we extract the information we need from
   * it. Currently, we are only using a node report for obtaining our vehicle
   * color. Probably a better source for this.
   *
   */
  ...
  /**
   * @brief Herein we serialize and broadcast our realtime agent information,
   * related to managing the convoy
   *
   */
  void postAgentInfo();
  ...
  /**
   * @brief If we are the leader, we encode a convoy point with our state
   * information and send it to our follower
   *
   */
  void seedPoints();
  ...
  /**
   * @brief We schedule broadcasts on a common interval, and if we go through
   * our cycles beyond this rate, we check to see whether or not we should
   * repost a message
   *
   ...
   * obtain a debug print query, you format your message as you would with
   * printf and this opens the file, writes your message, and then closes the
   * file.
   *
   * @param format
   * @param ...
...
protected: // Configuration parameters
  ...
  double m_max_lag_length;  // Max allowable length from our target before we
                            // start making comprimises
  double m_full_stop_range; // Safety - how close can we get to our target
                            // before saying we need to stop moving
  ...
  double m_kp_hdg_1, m_kp_hdg_2, m_kd_hdg_1, m_kd_hdg_2,
      ki_hdg; // Heading Gains
