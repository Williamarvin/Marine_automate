...
bool OpinionInput::calculateInputs(double time) {
  ...
  double dist_to_center = distPointToPoint(
      m_nav_x, m_nav_y, m_danger_pulse.get_x(), m_danger_pulse.get_y());

  double danger_pulse_rad = (time - m_danger_time) /
                            m_danger_pulse.get_duration() *
                            m_danger_pulse.get_radius();
  ...
  if (!stale && close && recieved_one) {
    m_opinion_inputs["DANGER_LEVEL"] =
        (m_danger_max - m_danger_min) *
            (m_danger_pulse.get_radius() - dist_to_center) /
            m_danger_pulse.get_radius() +
        m_danger_min;
  ...
  m_opinion_inputs["COVERAGE_INPUT"] =
      50.0 * battery_exhausted * battery_exhausted + 50.0;
