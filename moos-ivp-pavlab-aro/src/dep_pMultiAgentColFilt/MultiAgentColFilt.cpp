...
bool MultiAgentColFilt::Iterate() {
  ...
  bool ok = m_own_fwd_reach_set.updateSet(m_nav_x, m_nav_y, m_nav_hdg,
                                          m_nav_spd, MOOSTime());
...
bool MultiAgentColFilt::buildReport() {
  ...
  if (!m_emergency_posted) {
    ...
    m_msgs << "  !Emergency Stop for " << m_emergency_stop_contact_name
           << "!     " << endl;
  }
  m_msgs << "============================================" << endl;
  m_msgs << "                                            " << endl;
  ...
  for (it = m_estop_record.begin(); it != m_estop_record.end(); it++) {
    ...
    m_msgs << "(" << doubleToStringX((MOOSTime() - it->first), 2)
           << " sec ago)";
    m_msgs << endl;
    count += 1;
  }
