...
BHV_VectorField::BHV_VectorField(IvPDomain domain) : IvPBehavior(domain) {
...
IvPFunction *BHV_VectorField::buildOF() {
  ...
  double mag1 =
      ((m_osy - m_region_center_y)); //*sqrt(abs(m_osy-m_region_center_y)));
  double mag2 =
      ((m_osx - m_region_center_x)); //*sqrt(abs(m_osx-m_region_center_x)));
  double mag = sqrt((mag1 * mag1) + (mag2 * mag2));
  double mag1n =
      ((m_max_y - m_region_center_y)); //*sqrt(abs(m_max_y-m_region_center_y)));
  double mag2n =
      ((m_max_x - m_region_center_x)); //*sqrt(abs(m_max_x-m_region_center_x)));
