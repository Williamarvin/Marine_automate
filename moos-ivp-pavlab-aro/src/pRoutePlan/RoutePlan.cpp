    ...
    if (key == "VIEW_GRID_CONS_LOCAL") {
      ...
      if (cond) {
        ...
        if (!m_first_grid_received) {
          ...
          bool ok = m_astar.preloadGraph(m_vertices, m_edges, m_start_ids,
                                         m_end_ids, sort_by_asc_cell_id);
          if (not ok)
      ...
      if (!m_first_grid_received)
        reportRunWarning("Mail Error: Recieved a grid delta message before "
                         "receiving the full grid.");

      bool ok1 = m_grid.processGridDelta(sval);
      if (not ok1)
        reportRunWarning("Mail Error: Was not able to process grid delta" +
                         sval);
...
bool RoutePlan::Iterate() {
  ...
  double time_elapsed_since_last_grid =
      MOOSTime() - m_time_last_grid_was_received;
  bool cond1 = (m_new_grid_received and
                (time_elapsed_since_last_grid < (m_consensus_wait_time + 0.5)));
  ...
  m_final_obstacles =
      m_grid.getFinalObstacles(m_depth_threshold, m_var_threshold);

  string obs_out = "";
  for (set<size_t>::const_iterator it = m_final_obstacles.cbegin();
       it != m_final_obstacles.cend();) {
  ...
  bool found_path = (m_final_path.size() > 0) and
                    (not m_sent_final_path_found); // found a path
  bool timed_out =
      m_timeout and ((MOOSTime() - m_start_time) > m_timeout_time); // timed out

  // There are two end conditions, we found a path or we timed out.
  if (found_path) {

    string route_str = "";
    for (vector<size_t>::const_iterator it = m_final_path.cbegin();
         it != m_final_path.cend();) {
  ...
  bool iterations_less_than_max =
      m_proposal_iterations < m_max_proposal_iterations;

  // the time since the last proposal will be large when a new grid comes in
  // thus we can use it as a criteria here.
  bool enough_time_elapsed_since_last_proposal =
      (MOOSTime() - m_last_proposal_time) > m_proposal_wait_time;
  // The third condition is whether we have won a proposal.
  if (iterations_less_than_max and enough_time_elapsed_since_last_proposal and
      (not m_won_a_proposal)) {
    ...
    if (m_won_a_proposal) {
      ...
      string obs_spec =
          m_grid.getObstacleGridSpec(m_depth_threshold, m_var_threshold);
      ...
      double cost_to_path =
          calcCost(current_position, path_vertices, index_of_closest_vertex);
...
bool RoutePlan::OnStartUp() {
  ...
  m_grid =
      EsriBathyGrid(temp_grid, m_lat_origin, m_lon_origin, m_no_data_value);
...
bool RoutePlan::buildReport() {
  ...
  m_msgs << "Number of grid deltas recieved = " << m_full_grid_deltas_rcvd
         << endl;
  ...
  double time_elapsed_since_last_grid =
      MOOSTime() - m_time_last_grid_was_received;
  bool cond1 = (m_new_grid_received and
                (time_elapsed_since_last_grid < (m_consensus_wait_time + 2.0)));
  if (!m_surveying or !m_new_grid_received or cond1 or m_final_path_found) {
    m_msgs << "Searching for a final path" << endl;
  } else {
    m_msgs << "Not Searching for final path" << endl;
  }
  m_msgs << "Depth threshold = " << m_depth_threshold
         << ", Dynamic variance thresh = " << m_var_threshold << endl;
  m_msgs << "Estimated number of obstacles in grid = "
         << m_final_obstacles.size() << endl;
  m_msgs << "Number of start cells = " << m_start_ids.size()
         << " Number of end cells = " << m_end_ids.size() << endl;
  m_msgs << "Final path found = " << boolToString(m_final_path_found) << endl;
  m_msgs << "Time to find final path: " << (m_time_to_first_path - m_start_time)
         << endl;
  ...
  m_msgs << "Number of neighbors that submitted proposals = "
         << m_proposed_paths.size() << endl;
  m_msgs << "Number of proposal iterations = " << m_proposal_iterations << endl;
  m_msgs << "Time since last proposal = " << MOOSTime() - m_last_proposal_time
         << endl;
  m_msgs << "Won a proposal = " << m_won_a_proposal << endl;
  m_msgs << "Number of maintained paths (hysteresis) = "
         << m_path_maintained_cnt << endl;
  m_msgs << "Number of times A* found no paths = " << m_no_path_counter << endl;
  m_msgs << "Number of times proposal timed out = " << m_proposal_timeout_cnt
         << endl;
  ...
  for (it = m_proposed_paths.begin(); it != m_proposed_paths.end(); it++) {
    ...
    for (vector<size_t>::const_iterator it2 = path.cbegin();
         it2 != path.cend();) {
      m_msgs << *it2;
      ++it2;
      if (it2 != path.cend())
    ...
    m_msgs << ": Time Exp = " << MOOSTime() - m_proposal_arrival_time.at(vname)
           << "]" << endl;
  }

  m_msgs << "------Proposal Won: -------------------" << endl;
  m_msgs << "Fwd Path: ";
  for (vector<size_t>::const_iterator it2 = m_fwd_path.cbegin();
       it2 != m_fwd_path.cend();) {
    m_msgs << *it2;
    ++it2;
  ...
  m_msgs << "Rev Path: ";
  for (vector<size_t>::const_iterator it2 = m_rev_path.cbegin();
       it2 != m_rev_path.cend();) {
    m_msgs << *it2;
    ++it2;
...
void RoutePlan::plotPath(std::vector<size_t> path) {
  ...
  for (vector<XYCircle>::iterator it = m_active_circles.begin();
       it != m_active_circles.end(); it++) {
    it->set_active(false);
  ...
  // Draw the new path
  for (vector<size_t>::const_iterator it = path.cbegin(); it != path.cend();
       ++it) {
    double curr_x = 0;
    double curr_y = 0;
...
void RoutePlan::plotStartIds() {
  ...
  for (vector<XYCircle>::iterator it = m_active_start.begin();
       it != m_active_start.end(); it++) {
    it->set_active(false);
    Notify("VIEW_CIRCLE", it->get_spec());
  ...
  for (set<size_t>::const_iterator it = m_start_ids.cbegin();
       it != m_start_ids.cend(); ++it) {
    double curr_x = 0;
    double curr_y = 0;
    m_grid.getCellXY(*it, curr_x, curr_y);
...
void RoutePlan::plotEndIds() {
  ...
  //  Clear all previous markers
  for (vector<XYCircle>::iterator it = m_active_end.begin();
       it != m_active_end.end(); it++) {
    it->set_active(false);
    Notify("VIEW_CIRCLE", it->get_spec());
  ...
  // Draw the new markers
  for (set<size_t>::const_iterator it = m_end_ids.cbegin();
       it != m_end_ids.cend(); ++it) {
    double curr_x = 0;
    double curr_y = 0;
    m_grid.getCellXY(*it, curr_x, curr_y);
...
bool RoutePlan::handlePathProposal(std::string msg, double msg_time) {
  ...
  if (svector[1] == "") {
    ...
    for (unsigned int i = 0; i < path_vstring.size(); i++) {
      ...
      if (ok) {
        std::size_t temp_val =
            static_cast<size_t>(std::stoull(path_vstring[i], nullptr));
...
//                  the cost is 123.4
//                  the vehicle name is vname;
std::string RoutePlan::getPathProposalSpec(std::vector<size_t> path,
                                           double cost) {
...
bool RoutePlan::checkProposalsForWin() {
  ...
  if ((MOOSTime() - m_proposal_arrival_time.at(m_vname)) >
      (m_proposal_wait_time + 1.0)) {
  ...
  for (it = m_proposed_paths.begin(); it != m_proposed_paths.end(); it++) {
    ...
    // check if this proposal is fresh, or if this is the first proposal we have
    // done, we can include stale proposals.  This is needed when the state
    // transitions from an initial waypoint path to EXPLORE mode, which does not
    // necessarily co-incide with a new consensus grid.  As a result, two
    // vehicles could temporarily have the same path
    bool valid_proposal =
        ((MOOSTime() - m_proposal_arrival_time.at(it->first)) <
         ((m_proposal_wait_time * static_cast<double>(m_proposal_iterations)) +
          1.0));
...
double RoutePlan::calcCost(const std::vector<double> &v1,
  ...
  for (int i = 0; i < (V2.size() - 1); i++) {
    ...
    double distance = distPointToSeg(V2[i][0], V2[i][1], V2[i + 1][0],
                                     V2[i + 1][1], v1[0], v1[1]);
    ...
    if (distance < min_distance) {
      ...
      double dist_i_plus_one =
          hypot(V2[i + 1][0] - v1[0], V2[i + 1][1] - v1[1]);
...
std::vector<std::size_t> RoutePlan::findBestPathDirection() {
  ...
  it_closest =
      std::find(own_path.begin(), own_path.end(), index_of_closest_vertex);
  ...
  // goal vertices
  if ((number_of_start_or_goal_vertices_fwd > 0) or
      (number_of_start_or_goal_vertices_rev > 0)) {
    cout << "checking direction from # start/goal - forward "
         << number_of_start_or_goal_vertices_fwd << " reverse "
         << number_of_start_or_goal_vertices_rev << endl;
    if (number_of_start_or_goal_vertices_fwd >
        number_of_start_or_goal_vertices_rev) {
      m_rev_path = rev_path;
      return (fwd_path);
    } else if (number_of_start_or_goal_vertices_fwd <
               number_of_start_or_goal_vertices_rev) {
      m_rev_path = fwd_path;
      return (rev_path);
    }
  ...
  cout << "Variance forward: " << to_string(total_variance_fwd)
       << " Variance backward: " << to_string(total_variance_rev) << endl;
...
std::vector<size_t> RoutePlan::findInitialExplorePath() {
  ...
  for (unsigned int i = 0; i < path_seglist.size(); i++) {
    ok = m_grid.getCellID(path_seglist.get_vx(i), path_seglist.get_vy(i),
                          cell_id);
    if (ok) {
...
void RoutePlan::updateStartOrEndPaths() {
  bool cond1 =
      ((m_vehicle_n == 0) or (m_vehicle_n == (m_number_of_vehicles - 1)));
...
bool RoutePlan::isPathShorter(std::vector<size_t> const &path1,
