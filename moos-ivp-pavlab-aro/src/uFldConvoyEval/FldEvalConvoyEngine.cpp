...
void FldEvalConvoyEngine::updateMetrics() { updateCoreMetrics(); }
...
//   Example:
//   convoy_rng=17.4,vname=abe,rng_delta=-7.5,tail_rng=4.5,tail_ang=3.44,
...
void FldEvalConvoyEngine::updateRanges() {
  ...
  double convoy_rng_delta_running_avg =
      getRunningAvg(vname, convoy_rng_delta, msg_time);
  ...
  if (itr != m_prev_vehicle_range_delta_val.end()) {
    ...
    if (convoy_rng_delta_running_avg < itr->second) {
      ...
      if (m_prev_vehicle_range_delta_peak_time.find(vname) !=
        ...
        // Calculate frequency
        double osc_period =
            msg_time - m_prev_vehicle_range_delta_peak_time.at(vname);
...
bool FldEvalConvoyEngine::calculateConvoyLength() {
  ...
  std::reverse(convoy_names.begin(),
               convoy_names.end()); // Returned in reverse order
...
double FldEvalConvoyEngine::getRunningAvg(string vname, double val,
                                          double time) {
  ...
  while (!done && !m_range_delta_hist_tstamp[vname].empty()) {
    if ((curr_time - m_range_delta_hist_tstamp[vname].back()) >
        max_age_of_queue) {
