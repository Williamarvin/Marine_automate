...
bool M300Health::Iterate() {
  ...
  for (int i = 0; i < m_nav_speed_vals.size();) {
    if ((MOOSTime() - m_nav_speed_times[i]) < m_time_average_length) {
      m_nav_speed_running_ave =
          ((m_nav_speed_vals[i] + m_nav_speed_running_ave * count1) /
           (count1 + 1.0));
      count1 = count1 + 1.0;
      i++;
    } else {
  ...
  for (int i = 0; i < m_ave_nav_speed_vals.size();) {
    if ((MOOSTime() - m_ave_nav_speed_times[i]) < m_time_average_length) {
      m_ave_nav_speed_running_ave =
          ((m_ave_nav_speed_vals[i] + m_ave_nav_speed_running_ave * count2) /
           (count2 + 1.0));
      count2 = count2 + 1.0;
      i++;
    } else {
  ...
  if ((count1 > 0.0) and (count2 > 0.0)) {
    ...
    m_deviation_running_ave =
        m_nav_speed_running_ave - m_ave_nav_speed_running_ave;
...
bool M300Health::buildReport() {
  ...
  if (m_imu_gps_heading_agreement and !m_stale_nav and !m_stale_gps)
    m_msgs << "IMU and GPS heading within " << m_max_heading_delta
           << " degrees." << endl;
  else if (!m_stale_nav and !m_stale_gps)
    m_msgs << "Warning: IMU and GPS differ by "
           << abs(m_nav_heading - m_gps_heading) << " degrees." << endl;
  ...
  if (!m_stale_nav)
    actab << "IMU" << m_nav_heading << m_nav_heading_aux
          << (MOOSTime() - m_nav_heading_last_msg_time) << "       ";
  else
    actab << "IMU" << m_nav_heading << m_nav_heading_aux
          << (MOOSTime() - m_nav_heading_last_msg_time) << " STALE ";

  if (!m_stale_gps)
    actab << "GPS" << m_gps_heading << m_gps_heading_aux
          << (MOOSTime() - m_gps_heading_last_msg_time) << "       ";
  else
    actab << "GPS" << m_gps_heading << m_gps_heading_aux
          << (MOOSTime() - m_gps_heading_last_msg_time) << " STALE ";
  ...
  m_msgs << "                                                      " << endl;
  m_msgs << "------------------------------------------------------" << endl;
  m_msgs << "  Faults Detected: " << m_number_of_faults_detected
         << "             " << endl;
  m_msgs << "  Deviation of Estimate From Truth = " << m_deviation_running_ave
         << " m/s               " << endl;
  m_msgs << "                       (threshold is "
         << m_speed_est_deviation_thresh << "m/s)          " << endl;
