...
bool LegRun::setLegByCtr1(string str) {
  ...
  if (!isNumber(xs) || !isNumber(ys) || !isNumber(langs) || !isNumber(llens))
    return (false);
...
bool LegRun::setLegAux(double cx, double cy, double lang, double llen) {
  if (llen <= 0)
    return (false);
...
bool LegRun::modTurnRad(double dval) {
  return (setTurn1Rad(m_turn1_rad + dval) && setTurn2Rad(m_turn2_rad + dval));
}
...
bool LegRun::modTurnExt(double dval) {
  clearTurnSegls();
  return (setTurn1Ext(m_turn1_ext + dval) && setTurn2Ext(m_turn2_ext + dval));
}
...
double LegRun::getLegLen() const { return (distPointToPoint(m_p1, m_p2)); }
...
double LegRun::getLegAng() const { return (relAng(m_p1, m_p2)); }

//---------------------------------------------------------
// Procedure: getCenterPt()

XYPoint LegRun::getCenterPt() const { return (getSegCenter(m_p1, m_p2)); }
...
XYSegList LegRun::initTurnPoints1(bool adjust, double osx, double osy) {
  XYSegList turn_segl;
...
XYSegList LegRun::initTurnPoints2(bool adjust, double sx, double sy) {
  XYSegList turn_segl;
