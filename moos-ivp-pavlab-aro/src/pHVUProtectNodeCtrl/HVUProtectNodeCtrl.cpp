...
bool HVUProtectNodeCtrl::Iterate() {
  ...
  if ((m_send_new_ignore_set) ||
      ((MOOSTime() - m_last_ignore_list_sent) > 20.0)) {
    publishIgnoreLists();
  }
...
bool HVUProtectNodeCtrl::buildReport() {
  ...
  actab2 << doubleToString(m_odometry)
  ...
  if (m_cur_mission_tasks.size() > 0) {
    ...
    for (lit = m_cur_mission_tasks.begin(); lit != m_cur_mission_tasks.end();
         lit++) {
...
bool HVUProtectNodeCtrl::calculateInputs(double time) {
  ...
  if (m_got_real_batt_voltage) {
    ...
    m_current_batt_voltage_filtered +=
        alpha * (m_m300_batt_voltage - m_current_batt_voltage_filtered);
    m_time_last_got_batt = MOOSTime();
  ...
  double battery_exhausted =
      (m_max_batt - m_current_batt_voltage) / (m_max_batt - m_min_batt);

  // when battery is exhausted
  // m_opinion_inputs["OUTSIDE_ZONE_INPUT"] = 50.0 * battery_exhausted *
  // battery_exhausted + 50.0;
  ...
  std::string metric_spec = "name=" + m_host_community +
                            ",odom=" + doubleToString(m_odometry, 2) +
                            ",batt=" + doubleToString(m_current_batt_voltage);
  ...
  if (closest_intruder_dist > m_intercept_input_range_of_min_val) {
    ...
    double span =
        m_intercept_input_range_of_min_val - m_intercept_input_range_of_max_val;
    if (span < 0)
      span *= -1.0;

    double delta =
        closest_intruder_dist -
        m_intercept_input_range_of_max_val; // will always be non negative
    double proportion = delta / span;

    // proportion from [0 1] where zero is really close

    intercept_input =
        proportion * (m_intercept_input_min_val - m_intercept_input_max_val) +
        m_intercept_input_max_val;
  }
...
bool HVUProtectNodeCtrl::handleNodeReport(std::string msg) {
  ...
  dist_inside =
      distPointToPoint(m_inside_zone_centriod.get_vx(),
                       m_inside_zone_centriod.get_vy(), node_x, node_y);
  dist_outside =
      distPointToPoint(m_outside_zone_centriod.get_vx(),
                       m_outside_zone_centriod.get_vy(), node_x, node_y);
...
void HVUProtectNodeCtrl::handleMissionTask(std::string msg) {
  ...
  if (!stop) {
    ...
    for (it1 = m_cur_mission_tasks.begin(); it1 != m_cur_mission_tasks.end();) {
      ...
      parseMissionTask(this_task_type, this_task_stop, this_task_name,
                       this_task_x, this_task_y);
...
void HVUProtectNodeCtrl::parseMissionTask(std::string spec, bool &stop,
