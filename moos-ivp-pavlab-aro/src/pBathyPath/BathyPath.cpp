  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (key == "VIEW_GRID_CONS_LOCAL") {
      ...
      if (cond) {
        ...
        reportRunWarning("Mail Error: Grid received from VIEW_GRID_CONS_LOCAL "
                         "was not labeled 'cons'");
      }

    } else if (key == "VIEW_GRID_CONS_LOCAL_DELTA") {
      if (!m_first_grid_received)
        reportRunWarning("Mail Error: Received a grid delta message before "
...
bool BathyPath::Iterate() {
  ...
  if (!m_lawnmower) {
    ...
    if (m_grid.getCellCenter(m_x, m_y, curr_id, center_x,
                             center_y)) { // in the grid
      ...
      if (we_just_started or we_reached_last_goal) {
        ...
        getNextCellLookahead(center_x, center_y, m_heading, new_x, new_y,
                             m_new_id);
...
bool BathyPath::buildPathTree(int iterations, double curr_x, double curr_y,
                              double curr_heading, PathTree &path) {
  // Step 0. Bookkeeping
  ...
  for (int i = 0; i <= iterations; i++) {
    ...
    while (it != cells_to_process.end()) {
      ...
      if (i < iterations) {
        for (int j = 0; j < 3; j++) {
          ...
          if (ahead_i[j] != -1) { // if not out of bounds
            ...
            string ahead_state =
                to_string(ahead_i[j]) + "," + to_string(directions[j]);
      ...
      if (m_use_UCB_reward) {
        ...
        reward = getRewardUCB(temp_depth, temp_var * m_prob_scale_factor,
                              n_cells, m_round_number, delta);
      } else {
        reward =
            getRewardMVI(temp_depth, temp_var * m_prob_scale_factor, zStar);
      }
...
string BathyPath::getBestPath(PathTree pathtree) {
  ...
  double epsilon = 0.1;
  SimpleMDP mdp;
  m_utilities = mdp.valueIteration(pathtree.getRewards(), pathtree.getEdges(),
                                   discount_MDP, epsilon);
  ...
  for (int j = 0; j < m_tree_depth; j++) {
    ...
    for (int i = 0; i < pathtree.getEdges().size(); i++) {
      if (pathtree.getEdges()[i].first == node_to_expand) {
        if (pathtree.getRewards()[pathtree.getEdges()[i].second] >=
            best_utility_so_far) {
          best_utility_so_far =
              pathtree.getRewards()[pathtree.getEdges()[i].second];
    ...
    if (best_next_path_vertex == "") {
      ...
      } else if (j > 0) { // j is incremented below, j>0 means at least one step
                          // was found
        Notify("VIEW_SEGLIST", best_path_seglist.get_spec());
...
//
bool BathyPath::getNextCellLookahead(double curr_x, double curr_y,
                                     double curr_heading, double &new_x,
                                     double &new_y, int &new_id) {
...
bool BathyPath::buildReport() {
  ...
  m_msgs << "Average depth of grid = "
         << doubleToString(m_grid.getGridAverageDepth(), 3) << endl;
  m_msgs << "Maximum grid variance (adjusted) "
         << doubleToString(m_prob_scale_factor * m_grid.getMaxVar(), 4) << endl;
  m_msgs << "Path found from pRoutePlan = " << boolToString(m_path_found)
         << endl;
  m_msgs << "------------------------------------------------" << endl;
  if (m_use_UCB_reward)
  ...
  m_msgs << "------------------------------------------------" << endl;
  m_msgs << "\n-- Running MDP = "
         << boolToString(not(!m_surveying or m_path_found)) << endl;
  ...
  ","; if (count >= max_count){ m_msgs << endl; count = 0; } else { count ++;
    }
  }
  m_msgs << endl;
  */
