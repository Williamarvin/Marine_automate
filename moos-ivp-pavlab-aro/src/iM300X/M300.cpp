...
M300::~M300() { m_ninja.closeSockFDs(); }
  ...
  for (p = incoming_msgs.begin(); p != incoming_msgs.end(); p++) {
    ...
    if (m_ignore_msgs.count(msg.substr(0, 6)) >= 1)
      ...
      if (m_use_nvg_msg_for_nav_x_nav_y)
        ...
        handled = handleMsgCPNVG_heading(
            msg); // Only using NVG for low speed heading!
      else        // ignore it
        handled = true;
...
bool M300::handleMsgCPNVG(string msg) {
  ...
  if (m_publish_body_vel) {
    ...
    std::string dest = "aux";
    string n_msg = "src_node=" + m_vname + ",dest_node=" + dest +
                   ",var_name=NAV_FULL_POS,string_val=" + nav_full_pos_msg;
...
bool M300::handleMsgCPNVR(string msg) {
  ...
  std::string end_char = ")";
  std::string nav_full_state = unav_header + unav_string + vnav_header +
                               vnav_string + rnav_header + rnav_string +
                               end_char;
  ...
  std::string dest = "aux";
  string n_msg = "src_node=" + m_vname + ",dest_node=" + dest +
                 ",var_name=NAV_FULL_STATE,string_val=" + nav_full_state;
  Notify("NODE_MESSAGE_LOCAL", n_msg);
...
bool M300::buildReport() {
  ...
  m_msgs << "           min_thr: " << smin_th << "   max_thr: " << smax_th
         << endl;
  m_msgs << "------------------------------------------------------" << endl;
