...
bool FalconRunMgr::OnStartUp() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    if (param == "midpoint_flag_var_name") {
      handled =
          setNonWhiteVarOnString(m_midpoint_flag_var_name, toupper(value));
...
bool FalconRunMgr::buildReport() {
  ...
  m_msgs << "============================================" << endl;
  m_msgs << "             Loops started: " << intToString(m_curr_loop_id + 1)
         << endl;
  m_msgs << "        Loops before fault: "
         << intToString(m_loops_before_induce_fault);
  if (m_sent_fault) {
    m_msgs << " : FAULT SENT" << endl;
  } else {
    m_msgs << endl;
  }
  m_msgs << "Loops before radius change: "
         << intToString(m_loops_before_change_rad);
  if (m_sent_rad_change) {
    m_msgs << " : RAD CHANGE SENT" << endl;
  } else {
    m_msgs << endl;
  }
  m_msgs << "       Loops before return: "
         << intToString(m_loops_before_return);
  if (m_sent_return) {
