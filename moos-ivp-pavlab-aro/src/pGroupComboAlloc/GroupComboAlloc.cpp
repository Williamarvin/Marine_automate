...
bool GroupComboAlloc::buildReport() 
{
  m_msgs << "                                                          " << endl;
  m_msgs << "==========================================================" << endl;
  m_msgs << "=  Intercept Estimates:                                   " << endl;
...
void GroupComboAlloc::buildCostMap(){
  ...
  for (it2 = m_pop_state_map["intercept"].begin(); it2 != m_pop_state_map["intercept"].end(); it2++){
    agent_costs.clear();
...
std::map<std::string, double>  GroupComboAlloc::getAgentCosts(std::string vname)
{
...
std::vector<std::vector<double>> GroupComboAlloc::getDistMatrixFromCostMap()
{
  ...
  std::copy(m_intruder_set.begin(), m_intruder_set.end(), new_intruder_vec.begin());
  ...
  std::set<std::string>::iterator it;
  for (it = m_pop_state_map["intercept"].begin(); it != m_pop_state_map["intercept"].end(); it++) {
