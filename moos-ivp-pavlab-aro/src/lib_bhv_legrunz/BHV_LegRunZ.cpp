...
void BHV_LegRunZ::setMode(string mode) {
  if ((mode != "leg1") && (mode != "turn1") && (mode != "leg2") &&
      (mode != "turn2") && (mode != "init") && (mode != "idle"))
    return;
...
void BHV_LegRunZ::initLegMode() {
  ...
  if (m_init_leg_mode == "far_turn") {
    ...
    postEMessage(
        "Leg dir not initialized properly, check init_leg_mode param.");
  }
}
...
void BHV_LegRunZ::setTurnCoordSpd(double gpa) {
  ...
  if (m_mode == "leg2") {
    ...
    // cout << "cc: M:leg2 ToP2, rd=" << raw_dist << ", gd=" << glr_dist <<
    // endl;
  } else if (m_mode == "turn2") {
    ...
    glr_dist = glrDistToP1(gpa);
    // cout << "cc: M:leg1 ToP1, rd=" << raw_dist << ", gd=" << glr_dist <<
    // endl;
  } else if (m_mode == "turn1") {
    // raw_dist = rawDistToP2(raw_degs);
    // raw_dist = rawDistToP2(gpa);
    ...
    glr_dist = glrDistToP2(gpa);
    // cout << "cc: M:turn1 ToP2, rd=" << raw_dist << ", gd=" << glr_dist <<
    // endl;
  }
...
double BHV_LegRunZ::distToP1(double gpa, double leg_len, double turn1_len,
                             double turn2_len) {
  // Sanity checks
  if ((gpa < 0) || (gpa > 360))
    return (0);
...
double BHV_LegRunZ::distToP2(double gpa, double leg_len, double turn1_len,
                             double turn2_len) {
  // Sanity checks
  if ((gpa < 0) || (gpa > 360))
    return (0);
