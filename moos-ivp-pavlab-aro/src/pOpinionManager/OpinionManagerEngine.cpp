...
bool OpManagerEngine::handleOptConfig(std::string filename,
                                      std::string &warn_msg) {
    ...
    while (std::getline(file, line)) {
      ...
      if (stripped_line == "social_option") {
        ...
        if (found_config_start) {
          warn_msg = "Opinion Manager Error: Encountered a config block that "
                     "was not closed";
          file.close();
          return (false);
        }
      ...
      } else if ((stripped_line != "{") && (stripped_line != "}") &&
                 found_config_start) {
        // must be a valid line, add this line to the vector of strings to
        // process
        ...
        std::cout << "  Error here, found an opening bracket, but no start"
                  << std::endl;
        warn_msg = "Opinion Manager Error: Encountered a config block that was "
        ...
        warn_msg = "Opinion Manager Error: Encountered a config block that was "
                   "not started properly";
        file.close();
        std::cout << "  Error here encountered a config block that was not "
                     "started properly"
                  << std::endl;
        return (false);
        ...
        std::cout << "  Found a valid closing bracket, processing this block"
                  << std::endl;
        std::string temp_warn_msg = "";
        bool ok = false;
        Option new_option =
            buildOptionFromSpec(config_lines, ok, temp_warn_msg);

        std::cout << "   Processed option ok = " << std::to_string(ok)
                  << std::endl;
        if (ok) {
          m_options.push_back(new_option);
          std::string new_option_name = new_option.getOptionName();
          m_option_names_hash[new_option_name] = static_cast<unsigned int>(
              m_options.size() - 1); // will always be last
        } else {
  ...
  if (m_options.size() < 1) {
    warn_msg =
        "Opinion Manager Error: Opened file but did not find any config blocks";
    return (false);
  }
...
bool OpManagerEngine::setupOpinionManager(double time) {
  ...
  for (int i = 0; i < m_options.size(); i++) {
    setup_opinions.push_back(1.0 /
                             total_opinions); // already checked for divide by 0
...
bool OpManagerEngine::receiveOpinionMessage(std::string optn_msg,
                                            double msg_time) {
  ...
  if (ok) {
    ...
    m_neighbor_opinion_records[vname] =
        new_record; // overwrites old, assumes unique names
    ...
    std::string option_with_strongest_opinion =
        new_record.getStrongestOpinion();
    std::map<std::string, std::set<std::string>>::iterator it;
    bool need_to_add_option = true;
    for (it = m_population_status.begin(); it != m_population_status.end();
         it++) {
...
bool OpManagerEngine::iterateOwnOpinion(double time) {
  ...
  for (unsigned int j = 0; j < options_status.size(); j++) {
    ...
    std::cout << "  -> Updating option j=" << uintToString(j)
              << " with name =" << option_name << std::endl;
    ...
    // Step 2.3  computes the summation sum( S_2 ( ... from Step 2.1 + from
    // Step 2.2 ...) ) Step 3    computes the active attention u_i Step 4
    // computes the input term b_ij Step 5    adds all the terms to compute F_ij
    ...
    for (k = m_neighbor_opinion_records.begin();
         k != m_neighbor_opinion_records.end(); k++) {
      ...
      if (k->second.isOptionExist(option_name)) {
        ...
        std::cout << "  This option is not in the neighbors record"
                  << std::endl;
        continue;
      }
      ...
      std::cout << "  Found neighbor opinion of z_kj = " << doubleToString(z_kj)
                << " and the gamma = "
                << doubleToString(m_options[j].getGamma()) << std::endl;
    ...
    for (l = betaMap.begin(); l != betaMap.end(); l++) {
      ...
      std::cout << "  -> Found other option name = " << other_option_name
                << " with beta_il = " << doubleToString(beta_il) << std::endl;
      // Is this option active on own vehicle?
      if (options_status[m_option_names_hash[other_option_name]]) {
        std::cout
            << "     This other option is active on own vehicle with opinion = "
            << doubleToString(
                   m_opinions[m_option_names_hash[other_option_name]])
            << std::endl;
        // add it to the sum
        sum3 += beta_il * m_opinions[m_option_names_hash[other_option_name]];
        std::cout << "     Added it to the sum3, which now ="
                  << doubleToString(sum3) << std::endl;
      }
      ...
      if (deltaMap.count(other_option_name)) {
        ...
        std::cout << "  -> Found other option on other vehicle = "
                  << other_option_name
                  << " with delta_ik = " << doubleToString(delta_ik)
                  << std::endl;
        ...
        for (k = m_neighbor_opinion_records.begin();
          ...
          std::cout << "     This other option is active on other vehicle with "
                       "opinion z_kl = "
                    << doubleToString(z_kl) << std::endl;
      ...
      } // end of if delta exists.  If it does not, then all the contributions
        // from
      // the neighbors opinions are zero anyways, so they are skipped.

      // Step 2.3  Now add this influence to the outer sum
      std::cout << "     The sigmoid of sum3 is  ="
                << doubleToString(sigmoid(sum3)) << std::endl;
      term2 += sigmoid(sum3);

      std::cout << "     Adding it to the term2, which now ="
                << doubleToString(term2) << std::endl;
    ...
    if (m_info_buff.isKnown(input_var_for_this_option)) {
      ...
      dbl_buffer_val =
          m_info_buff.dQuery(input_var_for_this_option, found_in_buffer);
      if ((found_in_buffer)) {
        std::cout << "   -> Found input variable in buffer = "
                  << doubleToString(dbl_buffer_val) << std::endl;
        b_ij = m_options[j].computeInputB(dbl_buffer_val);

        std::cout << "      And computed input b_ij = " << doubleToString(b_ij)
                  << std::endl;
      } else {
    ...
    std::cout << "    *-> -m_options[j].getDi() * m_opinions[j] = "
              << doubleToString(-m_options[j].getDi() * m_opinions[j])
              << std::endl;
    std::cout << "    *-> b_ij = " << doubleToString(b_ij) << std::endl;
    std::cout << "    *-> m_u_i * ( sigmoid(term1) + term2 ) = "
              << doubleToString(m_u_i * (sigmoid(term1) + term2)) << std::endl;

    // Step 5:  All terms are calcuated, add together
    F_ij = -m_options[j].getDi() * m_opinions[j] + b_ij +
           m_u_i * (sigmoid(term1) + term2);
...
void OpManagerEngine::updateAttention(double time,
                                      std::vector<bool> options_status) {
  ...
  std::cout << " Updating attention:  Normalized square of own opinions is = "
            << doubleToString(sum1) << std::endl;

  std::map<std::string, OpinionRecord>::iterator it;
  for (it = m_neighbor_opinion_records.begin();
       it != m_neighbor_opinion_records.end(); it++) {
    ...
    std::cout << " -> Checked vehicle " << it->second.getVname()
              << " and running sum of squares is now = " << doubleToString(sum1)
              << std::endl;
  }
  ...
  std::cout << " Updating attention:  m_u_min + (m_u_max - m_u_min) * "
               "satFunctionHill(sum1) = "
            << m_u_min + (m_u_max - m_u_min) * satFunctionHill(sum1)
            << std::endl;

  double du_dt =
      (-1.0 * m_u_i + m_u_min + (m_u_max - m_u_min) * satFunctionHill(sum1)) /
      m_tau_u;
  std::cout << " Updating attention: satFunctionHill(sum1) = "
            << doubleToString(satFunctionHill(sum1)) << std::endl;
  m_u_i = m_u_i + du_dt * dt;
  std::cout << " Updating attention:  m_u_i = " << doubleToString(m_u_i)
            << std::endl;
...
bool OpManagerEngine::getOutputsForLargestOpinion(
  ...
  unsigned int numb_outputs =
      m_options[best_index].getOptionOutputs(names_temp, vals_temp);
...
std::list<std::string> OpManagerEngine::getReport(double time_now) {
  ...
  report.push_back("Name: " + pd_vname + " Group: " + pd_vgroup + " Atten: " +
                   pd_attention + " Opinion Iter: " + str_opinion_iter_count);
  report.push_back(
      "=============================================================");
  report.push_back(
      " Option              |  Own Opinion   |  Input  | Active? ");
  report.push_back(
      "-------------------------------------------------------------");
  ...
  if (m_options.size() == m_opinions.size()) {
    for (unsigned int i = 0; i < m_options.size(); i++) {
      std::string pd_option_name =
          padString(m_options[i].getOptionName(), 20, false);
      std::string str_opinion_val = doubleToString(m_opinions[i], 4);
      std::string pd_opinion_val = padString(str_opinion_val, 10, false);
      std::string str_opinion_input_val =
          doubleToString(m_opinion_inputs[i], 3);
      std::string pd_opinion_input_val =
          padString(str_opinion_input_val, 6, false);
      ...
      report.push_back(" " + pd_option_name + ":  " + pd_opinion_val +
                       "    :  " + pd_opinion_input_val + " : " +
                       str_op_status);
    }
  } else {
  ...
  for (p_it = m_population_status.begin(); p_it != m_population_status.end();
       p_it++) {
    ...
    for (sp_it = set_of_agents.begin(); sp_it != set_of_agents.end(); sp_it++) {
      ...
      if ((time_now - m_neighbor_opinion_records[*sp_it].getTime()) <
          m_stale_thresh)
        line += *sp_it + ",";
    }
  ...
  for (rec_it = m_neighbor_opinion_records.begin();
       rec_it != m_neighbor_opinion_records.end(); rec_it++) {
    std::string neighbor_name = rec_it->second.getVname();
    std::string neighbor_count =
        uintToString(m_neighbor_opinion_rec[neighbor_name]);
    ...
    std::string pd_last_msg =
        padString(rec_it->second.getLastRecordMsg(), 20, false);
...
bool OpManagerEngine::getUpdatedOptionStateLists(
    std::vector<std::string> &opt_names, std::vector<std::string> &agent_list,
    double time_now) {
