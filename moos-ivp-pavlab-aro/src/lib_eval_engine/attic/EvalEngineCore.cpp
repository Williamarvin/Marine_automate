...
EvalEngineCore::EvalEngineCore() { m_eval_collection_type = "EvalEngine"; }
...
bool EvalEngineCore::UpdateVariables(
    std::map<std::string, EEMessage> msg_buffer) {

  std::map<std::string, EEMessage>::iterator p;

  // Unpack the collection of MOOS Messages within this timeframe of receiving
  // new mail
  ...
  // Now that we have all the mail since the last cycle, we distribute this mail
  // to the kernels
  ...
  for (it_info = m_info_buffer.begin(); it_info != m_info_buffer.end();
    ...
    for (kernel_it = m_subscriptions_to_kernels[it_info->first].begin();
      ...
      if (successful_update == false) {
        ...
        // Msg<key=" + it_info->first + ",val=" + it_info->second.msg() + ">");
      }
    }
  }
...
EvalEngineCore::ComputeEvaluations() {
  ...
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
    ...
    if ((kernel_it->second->Evaluate(updates)) == false) {
      ...
      // warning message reportRunWarning("Error in Evaluation: Kernel <" +
      // kernel_it->first + ">");
    }
  }
  return updates;
...
std::string EvalEngineCore::genEngineExampleConfig() {
  ...
  // cout << "//--------------------------------------------------------" <<
  // endl; cout << "// " << m_eval_collection_type << " Example MOOS
  // Configuration" << endl
  //      << endl;
  ...
  ecp << "//Kernel-specific configuration parameters" << endl << endl;
  ...
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
       ++kernel_it) {
    // Kernel name
    ecp << "//    > " << kernel_it->second->m_identity.c_str()
    ...
    for (param_it = parameters.begin(); param_it != parameters.end();
         ++param_it)
      ecp << "        " << std::right << std::setw(longest_name)
