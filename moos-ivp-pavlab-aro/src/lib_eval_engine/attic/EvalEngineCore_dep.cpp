...
EvalEngineCore::~EvalEngineCore() {
  ...
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
       ++kernel_it) {
...
bool EvalEngineCore::OnNewMail(MOOSMSG_LIST &NewMail) {
  /*
    Here, when we receive new mail, we accumulate an updates buffer and do
    nothing with it yet
  */
...
bool EvalEngineCore::Iterate() {
  ...
  // Let all the kernels receive new mail, do error checking, and update their
  // variables for all the updates we have, we go through the names of the
  // topics which have new updates Each kernel should be designed to handle one
  // piece of mail at a time, in order to enable high-level error checking

  m_info_buffer["MOOSTime"] = EEMessage("MOOSTime", MOOSTime(), MOOSTime());

  std::map<std::string, EEMessage>::iterator it_info;
  for (it_info = m_info_buffer.begin(); it_info != m_info_buffer.end();
       ++it_info) {
    // For each topic, we go through all the kernels which have subscribed to
    // this topic
    std::set<std::string>::iterator kernel_it;
    for (kernel_it = m_subscriptions_to_kernels[it_info->first].begin();
         kernel_it != m_subscriptions_to_kernels[it_info->first].end();
         ++kernel_it) {
      dbg_print("Updating: %s with topic %s\n", (*kernel_it).c_str(),
                it_info->first.c_str());
      bool successful_update = m_eval_kernels[*kernel_it]->UpdateVariable(
          m_info_buffer[it_info->first]);
      if (successful_update == false) {
        ...
        dbg_print("Unhandled Mail: Kernel <%s> -> Msg<key=%s,val=%s>\n",
                  ...
                  it_info->second.msg().c_str());
        reportRunWarning("Unhandled Mail: Kernel <" + *kernel_it +
                         "> -> Msg<key=" + it_info->first +
                         ",val=" + it_info->second.msg() + ">");
      }
    }
  }
  ...
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
    ...
    if ((kernel_it->second->Evaluate(updates)) == false) {
      // TODO: Get rid of only a bool evaluation and let the kernel generate a
      // warning message/runtime error which gets displayed at this level i.e.
      // !it_kernel->second.Evaluate(&updates, &warning_msg) -> print the
      // warning message
  ...
  // All the kernels would have completed a single evaluation, now we post all
  // their updates
...
bool EvalEngineCore::OnStartUp() {
  /*
    On Startup, we are going to collect all the parameters which are included in
    the configuration block. We also initially populate the time in the buffer
  */
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    if (param == "debug") {
      ...
      if (m_debug) {
        ...
        std::map<std::string, EvalEngineKernel *>::iterator kernel_it;
        for (kernel_it = m_eval_kernels.begin();
             kernel_it != m_eval_kernels.end(); ++kernel_it)
          kernel_it->second->set_debug_stream(m_fname);

        dbg_print("Opened file\n");
      }
    } else if (m_parameters_all.count(param)) {
      // If it matches a parameter one of the kernels will use, then we assume
      // that it is valid and will be handled by the kernel
      m_parameters_all[param] = value;
    } else {
      // If it doesn't match a parameter that a kernel will use, then it is
      // invalid
      reportUnhandledConfigWarning(orig);
    }
  }

  // Now that we have all the parameters, simply let the kernels update
  // themselves
  std::map<std::string, EvalEngineKernel *>::iterator kernel_it;
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
       ++kernel_it) {
  ...
  // We don't register for variables if we are processing an alogfile that
  // already exists
  registerVariables();
  return (true);
}
...
void EvalEngineCore::registerVariables() {
  ...
  for (subscription_it = m_subscriptions_all.begin();
       subscription_it != m_subscriptions_all.end(); ++subscription_it)
...
void EvalEngineCore::insertKernel(EvalEngineKernel *kernel) {
  // We can traverse and navigate our collection of kernels with iterators and
  // by looking them up by their identity

  std::map<std::string, std::string> kernel_parameters =
      kernel->get_parameters();
  std::map<std::string, std::string>::iterator param_it;
  for (param_it = kernel_parameters.begin();
       param_it != kernel_parameters.end(); param_it++)
    m_parameters_all[param_it->first] = param_it->second;

  m_eval_kernels[kernel->m_identity] = kernel;

  std::unordered_set<std::string> kernel_subscriptions =
      kernel->get_subscriptions();

  // We update our master list of all our subscriptions
  // We also construct a map from a topic to all the kernels which are
  // subscribed - we do this so when we receive new mail, we only update those
  // kernels
  std::unordered_set<std::string>::iterator sub_it;
  for (sub_it = kernel_subscriptions.begin();
       sub_it != kernel_subscriptions.end(); sub_it++) {
...
void EvalEngineCore::showExampleConfigAndExit() {
  ...
  cout << "//--------------------------------------------------------" << endl;
  cout << "// " << m_eval_collection_type << " Example MOOS Configuration"
       << endl
       << endl;
  ...
  cout << "    CommsTick = 10" << endl << endl;
  cout << "        debug = false" << endl << endl;
  cout << "//Kernel-specific configuration parameters" << endl << endl;

  // For each kernel which is included is built into this EvalEngine, generate a
  // configuration file template with default values
  std::map<std::string, EvalEngineKernel *>::iterator kernel_it;
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
       ++kernel_it) {
    // Kernel name
    cout << "//    > " << kernel_it->second->m_identity.c_str()
         << " (default values as shown)" << endl;
    // First find the longest parameter name, and format the width of each line
    // accordinly so everything is nicely aligned
    std::map<std::string, std::string> parameters =
        kernel_it->second->get_parameters();
    uint16_t longest_name = 0;
    std::map<std::string, std::string>::iterator param_it;
    for (param_it = parameters.begin(); param_it != parameters.end();
         ++param_it)
    ...
    // For each parameter if they exist, print out the parameter name and the
    // default value
    for (param_it = parameters.begin(); param_it != parameters.end();
         ++param_it)
      cout << "        " << std::right << std::setw(longest_name)
           << param_it->first << " = " << param_it->second << endl;
    cout << endl;
  }
  cout << "}" << endl;
