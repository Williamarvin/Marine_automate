...
EvalEngine::EvalEngine() { m_engine_name = "EvalEngine"; }
...
bool EvalEngine::Initialize() {
  ...
  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
    ...
    dbg_print("<%s> Set Params on: %s\n", m_engine_name.c_str(),
              kernel_it->second->m_identity.c_str());
    kernel_it->second->SetParams(m_parameters_all);
  }

  for (kernel_it = m_eval_kernels.begin(); kernel_it != m_eval_kernels.end();
       ++kernel_it) {
    // Since we allow remappable subscriptions, after the parameters are
    // updated, we then collect the final list of subscriptions
    std::set<std::string> kernel_subscriptions =
        kernel_it->second->get_subscriptions();
...
std::vector<std::pair<std::string, std::string>> EvalEngine::ComputeEvaluations(
