...
xSurfaceOdometryKernel::xSurfaceOdometryKernel(std::string identity,
                                               std::string suffix) {
  ...
  // The identity can be renamed, include it as parameter options with default
  // values
  m_parameters[m_identity_key] = m_identity;

  // The variables which we listen to can be remapped, include these as
  // parameter options with default values
...
bool xSurfaceOdometryKernel::hasMoved() {
  // TODO: This threshold should be a parameter if we are only working in
  // long/lat instead of x/y. This is temporary for now.
...
bool xSurfaceOdometryKernel::UpdateVariable(const EEMessage &message) {
  ...
  if (m_first_reading == false && ((m_x != m_x_prv) || (m_y != m_y_prv))) {
    // The actual first reading just means that it is seeded with an intial
    // condition - we should not consider the distance between the arbitrary
    // initial *_prv variables and the location we just received
    m_x_prv = m_x;
    m_y_prv = m_y;
...
bool xSurfaceOdometryKernel::Evaluate(
  ...
  if (m_first_reading && hasMoved()) {
    ...
    m_total_distance +=
        sqrt(pow(delta_x, 2) + pow(delta_y, 2)) * m_conversion_factor;
    m_num_sample_points++;
    updates.push_back(
        std::make_pair(m_eval_odometry_key, to_string(m_total_distance)));
