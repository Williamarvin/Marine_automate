...
/*
    ...
    Considerations
        - Timer can be cummulative or reset when it exits a state
            - i.e. Total time deployed, or how long since the last time deploy
   was published to
        - Timer can be cummulative and receive force a reset condition. This
   condition can also be applied to a non-cummulative state timer
        ...
        - The state which it is tracking can be collection of conditions
            - VAR=NAME
            - NOT IMPLEMENTED - VAR1=NAME1 && VAR2=NAME2, VAR1=NAME1 ||
   VAR2=NAME2 (Downstream)
            - NOT IMPLEMENTED - VAR > NUM, VAR < NUM, VAR >= NUM, VAR <= NUM
   (Downstream)
*/
...
xStateTimer::xStateTimer(std::string identity, std::string suffix) {
  ...
  // Include the subscriptions which describe the needs of this kernel to the
  // EvalEngine
  m_subscriptions.insert(m_sub_var_to_time);
  m_subscriptions.insert(m_sub_var_reset);
  m_subscriptions.insert(m_sub_moos_time);

  // Include the publications which describe the needs of this kernel to the
  // EvalEngine
  m_publications.insert(m_pub_time_to_var);
  m_publications.insert(m_pub_nresets_to_var);

  // Include the parameters which describe the needs of this kernel to the
  // EvalEngine, and their default values
...
bool xStateTimer::SetParams(const map<string, string> &params) {
  ...
  if (params.count(m_pub_nresets_to_var_key)) {
    updatePublications(m_pub_nresets_to_var,
                       params.at(m_pub_nresets_to_var_key));
...
bool xStateTimer::Evaluate(
  ...
  if (tolower(m_var_to_time_latest_val) == tolower(m_var_condition)) {
    // We check to see if this is our first satisfying condition, and if we are
    // transitioning into a timing state
    if (m_transitioned_into_timing == false) {
      // If we are transitioning into a valid state, then all we can do is save
      // the last valid time
      m_transitioned_into_timing = true;
      m_last_time = m_timenow;
    }
    // If we already transitioned into a timing state and have a valid last
    // saved time since when the condition has been met
    ...
    // Otherwise, the condition is not met, and we are transitioning out of a
    // timing state or holding the timer at 0 if it is not cummulative
    m_transitioned_into_timing = false;
...
bool xStateTimer::AppCastingNotice(ACTable &actab) {
  ...
  snprintf(message, msg_buff_len,
           "Timer status: %s %s %s for %0.2f seconds with %d resets\n",
           m_sub_var_to_time.c_str(), m_sub_state_expr.c_str(),
           m_var_condition.c_str(), m_ctimer, m_nresets);
