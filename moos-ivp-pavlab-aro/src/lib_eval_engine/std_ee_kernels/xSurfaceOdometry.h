...
public:
  // Sets the subscriptions and the parameters that it will consume from the
  // configuration file. The EvalEngine core will reference these variables
  xSurfaceOdometryKernel(std::string identity = "xSurfaceOdometry",
                         std::string suffix = "");
  ...
  // On a pass over, it can post updates to an object maintained by the
  // EvalEngine
  bool Evaluate(std::vector<std::pair<std::string, std::string>> &updates,
