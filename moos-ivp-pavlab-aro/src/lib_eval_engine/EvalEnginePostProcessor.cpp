  ...
  if (dir) {
    ...
    while ((entity = readdir(dir)) != nullptr) {
      ...
      if (filename.size() > alog_suffix.size() &&
          filename.compare(filename.size() - alog_suffix.size(),
                           alog_suffix.size(), alog_suffix) == 0) {
        closedir(dir);
        return true;
      }
    }
...
std::string getAlogFileName(std::string dirname) {
  // To get this far, we already confirmed that an alog exists, so now we just
  // extract is generic name
  DIR *dir = opendir(dirname.c_str());
  std::string alog_suffix = ".alog";
  struct dirent *entity;
  while ((entity = readdir(dir)) != nullptr) {
...
EvalEnginePostProcessor::EvalEnginePostProcessor(EvalEngine *engine,
                                                 std::string appName) {
  ...
  // When outputting a final Evaluation/latest kernel publications, we exclude
  // visual artifacts, as a convenience
...
bool EvalEnginePostProcessor::setParam(std::string param_name,
  ...
  if (param_name == m_verbose_key) {
    ...
    handled =
        setParam(m_input_alog_file_key, getAlogFileName(m_target_directory));
    ...
    if (fileExists(m_input_alogf)) {
      ...
      std::cerr << "ERR: .alog file does not exist < " << m_input_alogf << " >"
                << std::endl;
    }
    ...
    if (m_MissionReader.SetFile(m_input_moosf)) {
      ...
      std::cerr << "ERR: Mission file not found <" << m_input_moosf << ">"
                << std::endl;
    }
  }
  return handled;
...
void EvalEnginePostProcessor::process() {
  ...
  // Use the mission reader and extract the configuration block for the
  // associated EvalEngine
  getConfigParams();
  ...
  // The half the lesser of the two tick rates is the frequency in which we'll
  // run our state machine
  double write_period_s = apptick_time_s < commtick_time_s
                              ? apptick_time_s / 2.0
                              : commtick_time_s / 2.0;

  m_engine->Initialize();

  // If rewriting, make the directory and the file pointer, also dump the
  // configuration block for the eval engine
  if (m_rewrite) {
    // Get the absolute time in which the log file was opened
    m_logstart = getLogStartStr();
    // We do this since you are not making a new mission, just rewriting what
    // would have been realtime calculations, so it is still part of the
    // original mission directory
    m_output_directory =
        m_target_directory +
        m_target_directory.substr(0, m_target_directory.size() - 1) + "_" +
        m_tag + "/";
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, line)) {
      ...
      if (m_verbose) {
        // Verbose mode should only be used if the user is looking for a line
        // number which is corrupt
        printf("%d/%d\n", on_line, fileLength);
      }
      ...
      // We check the message to see if it came from our EvalEngine process name
      // argument, if it does, we ignore it

      if (msg.src == m_process_name) {
        // This line was published by the previous EvalEngine process, so we
        // ignore it
        continue;
      }
      ...
      if (m_rewrite) {
        ...
        if (file.is_open()) {
          ...
          std::cerr << "Cannot open output alog file: " << m_output_alogf
                    << std::endl;
          exit(1);
        }
        file.close();
      ...
      // If it is time to handle new mail which has built up in the queue if we
      // have new mail, then we update all the kernel variables and then
      // schedule the next time in which we update variables The state machine
      // time also has to at least be in sync with the alog files previous logs
      // in order to see if there have been viable updates

      // With this latest update in the buffer, let the state machine timer run
      // until it is time to handle the new mail while processing back end
      // updates

      // When running the approximate mailupdate, and if we have new mail, we
      // let the kernels update all their variables
      ...
      do {
        ...
        // If it is time to write/publish another evaluation, then we write the
        // latest updates to the file while scheduling the next evaluation time
        ...
        if (c_time >= next_evaluation) {
          ...
          // If we have runtime warnings and we are not in debug mode, then we
          // need to alert the user of the runtime warnings
          if (kernel_level_warnings.size() && !m_debug) {
            std::vector<std::string>::iterator warning_it;
            for (warning_it = kernel_level_warnings.begin();
                 warning_it != kernel_level_warnings.end(); ++warning_it) {
              std::cerr << "ERR: Kernel Level Runtime Warning - " << *warning_it
                        << std::endl;
            }
          }
          ...
          if (m_rewrite) {
            ...
            if (file.is_open()) {
              for (it = updates.begin(); it != updates.end(); ++it) {
                ...
                newentry << std::setw(15) << std::fixed << std::setprecision(5)
                         << std::left << c_time << " " << std::setw(20)
                         << std::left << it->first << " " << std::setw(15)
                         << std::left << m_process_name << "  " << it->second
                         << std::endl;
                file << newentry.str();
                prev_updates[it->first] = it->second;
              }
            } else {
              std::cerr << "ERR: Cannot open output alog file: "
                        << m_output_alogf << std::endl;
              exit(1);
            }
            file.close();
  ...
  // At this point, we now maintain the latest updates from all the kernels, and
  // we write this to the stdout
  if (latest_updates_exclusive.size() != 0) {
    std::map<std::string, std::string>::iterator it;
    for (it = latest_updates_exclusive.begin();
         it != latest_updates_exclusive.end(); ++it) {
    ...
    std::cout << "Error: Kernel rendered no updates throughout Alog file"
              << std::endl;
    exit(1);
  }
}
...
int EvalEnginePostProcessor::asAscii(char c) { return static_cast<int>(c); };
...
std::string EvalEnginePostProcessor::makeAlogFile() {
  ...
  std::string alog_output =
      m_output_directory + m_process_name + "_" + m_tag + ".alog";
  ...
  if (alogheader.size()) {
    // Extract the header - Terminate on an absense of '%'
    for (std::string line;
         std::getline(alogheaderss, line) && line[0] == '%';) {
      if (line.find("LOG FILE: ") != std::string::npos) {
        ...
        new_alogheader += lead + "<Include new time here> " +
                          "POST PROCESSED REWRITTEN FILE - Originally from " +
                          line + "\n";
      } else {
        // Otherwise we need express that this is a manipulated and revised file
        // Format the new time in which it was opened
...
std::string EvalEnginePostProcessor::makeMOOSFile() {
  ...
  std::string output_moos_fname = m_output_directory +
                                  m_input_moosf.substr(0, pos) + "_" + m_tag +
                                  "._moos";
  ...
  if (m_cached_moosf.size()) {
    ...
    ofile << "// Automatically generated by: <" << m_appName
          << "> when referencing the configuration for <" << m_process_name
          << ">" << std::endl;
    ofile << "//" << xChars("-", 59) << std::endl;
...
std::string EvalEnginePostProcessor::getMOOSConfigAsStr(std::string moosfile) {
  ...
  int stack_max = 0;
  if (file.is_open()) {
    // Extract only the related configuration block in the .moos file to save it
    // when rewriting
    while (std::getline(file, line)) {
      if ((line.find("ProcessConfig") != std::string::npos) &&
          (line.find(m_process_name) != std::string::npos)) {
      ...
      // The stack must exceed one at least once and the next time it reaches
      // zero, we've reached a full configuration block closed in brackets at
      // the same level
      for (int i = 0; i < line.size(); ++i) {
        if (line[i] == '{')
          stack_height++;
...
void EvalEnginePostProcessor::getConfigParams() {
  ...
  if (!m_MissionReader.GetConfiguration(m_process_name, sParams))
    std::cerr << "Unhandled configuration parameter: " << m_process_name
              << std::endl;
