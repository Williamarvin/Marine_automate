...
bool BathyGrider::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (key == "NAV_X") {
      ...
      if (ok) {
        m_consensus.receiveConsEstMessage(vname, mtime, msg_estimate,
                                          msg_variance, msg_deg_connectedness);
...
bool BathyGrider::Iterate() {
  ...
  if (m_apptick_counter < m_appticks_to_skip) { // keep skipping
    ...
    for (unsigned int idx = 0; idx < m_grid_gpr.size(); idx++) {
      ...
      double depth = (cell_depth * m_iterations_completed + val) /
                     (m_iterations_completed + 1);
      double variance = (cell_var * pow(m_iterations_completed, 2) + covar) /
                        (pow((m_iterations_completed + 1), 2));
      ...
      if ((m_iterations_completed == (m_iterations_to_do - 1)) and
          (variance <= m_variance_threshold)) {
    ...
    if (m_iterations_completed == m_iterations_to_do) {
      ...
      string delta_spec_gpr =
          m_grid_gpr.getDeltaSpec(m_delta_thresh, found_delta);
      if (found_delta) {
        Notify("VIEW_GRID_GPR_LOCAL_DELTA", delta_spec_gpr);
        m_grid_gpr
            .setOldGridToNew(); // no more delta, we have sent all the updates
      }
  ...
  bool cond2 = (MOOSTime() - m_consensus.getNewConsensusRequestTime()) >=
               (static_cast<double>(m_max_iterations) * m_cons_waittime + 1.0);
  if (cond1 and cond2) {
  ...
  if (new_cons_requested) {
    ...
    if (m_consensus.readyToPerformConsensus(MOOSTime(), m_max_iterations)) {
      ...
      ok_consensus = m_consensus.newConsensusItr(
          MOOSTime(), m_multivariate_states,
          use_running_consensus_when_neighbor_est_available,
          use_running_consensus_if_no_neighbor_est_available, consensus_val,
          consensus_var);
      if (ok_consensus) {
        ...
        bool found_delta = false;
        string delta_spec_gpr =
            m_grid_cons.getDeltaSpec(m_delta_thresh, found_delta);
        if (found_delta) {
...
bool BathyGrider::buildReport() {
  ...
  m_msgs << "    " << m_iterations_completed << " of " << m_iterations_to_do
         << " iterations to do." << endl;
  m_msgs << "    Number of cells to omit = " << m_omit_list.size() << endl;
  m_msgs << " -------------Consensus---------------------------------------  "
         << endl;
  m_msgs << "  Last consensus was performed " << MOOSTime() - m_cons_time
         << " seconds ago" << endl;
  m_msgs << "  Last consensus was requested "
         << MOOSTime() - m_consensus.getNewConsensusRequestTime()
         << " seconds ago" << endl;
  m_msgs << "  Last consensus took " << m_cons_calc_timer << " seconds" << endl;
  m_msgs << "New Consensus requested = " << m_consensus.newConsensusRequested()
         << endl;
  m_msgs << "    Total own requests initiated  = " << m_total_requests_initiated
         << endl;
  m_msgs << "    Total other requests recieved = " << m_total_requests_recieved
         << endl;
  m_msgs << "    Total own submissions   = " << m_total_own_submissions << endl;
  m_msgs << "    Total other submissions = " << m_total_other_submissions
         << endl;
