...
VECMAT_PAIR sigmaPoints(arma::vec x, arma::mat P) {
  ...
  if (!P.is_symmetric())
    ...
    std::cout << "ERROR: covariance matrix must have the same number of rows "
                 "and columns as the state vector has rows."
              << std::endl;
  ...
  arma::mat xcopies =
      arma::repmat(x, 1, N); //(nxn) matrix of (nx1) state copies
...
// ********************************************
// PREDICT
VECMAT_PAIR predict(arma::vec x, arma::mat P, arma::mat Q, dseconds dt,
                    double a, double r) {
  ...
  // a priori covariance
  arma::mat Pbar = (Y.each_col() - xbar) * arma::diagmat(sigmas.first) *
                   (Y.each_col() - xbar).t();
  Pbar += Q;
...
// ********************************************
// CORRECT
VECMAT_PAIR correct(VECMAT_PAIR a_priori, arma::vec z, arma::mat H,
                    arma::mat R) {
  ...
  // Joseph form
  arma::mat P =
      (arma::eye(4, 4) - K * H) * Pbar * (arma::eye(4, 4) - K * H).t() +
      K * R * K.t();
