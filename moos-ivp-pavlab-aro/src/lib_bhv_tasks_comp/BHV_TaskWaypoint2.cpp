...
double BHV_TaskWaypoint2::getTaskBid() {
  ...
  // Identify the angle between our current heading and the target - is between
  // -pi and pi
  ...
  // The bid is the distance scaled by how complementary the angles are. In the
  // two cases, the distance is scaled [dist*0.5, dist*1] on whether or not the
  // agent is facing that direction, or away from, respectively
...
vector<VarDataPair>
BHV_TaskWaypoint2::applyFlagMacros(vector<VarDataPair> flags) {
