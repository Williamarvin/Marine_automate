  ...
  for (int i = 0; i < portList.size(); i++) {
    ...
    if (m_vname == "floatie") {
      for (int i = 0; i < num_bytes; ++i) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, vehicle_buffer[i], &msg,
                               &status)) {
  ...
  for (int i = 0; i < num_bytes; ++i) {
    ...
    if (mavlink_parse_char(MAVLINK_COMM_0, vehicle_buffer[i], &msg, &status)) {
      ...
      case MAVLINK_MSG_ID_GLOBAL_POSITION_INT: {
        ...
        m_nav_hdg = hdg;
        hdg_found = true;
        // cout << "heading: " << packets.hdg << "relativealt" <<
        // packets.relative_alt << endl;
      }
      ...
      //     cout << "thruster: " << packet.servo1_raw << " " <<
      //     packet.servo3_raw << endl;
      // }
...
void M300::sendServo(uint8_t servoNumber, float pwmValue) {
  ...
  mavlink_msg_command_long_pack(1, 0, &msg, 0, 0, MAV_CMD_DO_SET_SERVO, 0,
                                servoNumber, pwmValue, 0, 0, 0, 0, 0);
...
// checks the global variable of automate, remote, and on board, and overrides
...
void M300::ThrustOutputPriority() {
  ...
  // on board control
  if ((o_Thrust_L >= 1525 && o_Thrust_L <= 2000) ||
      (o_Thrust_R >= 1525 && o_Thrust_R <= 2000) ||
      (o_Thrust_L <= 1475 && o_Thrust_R >= 1000) ||
      (o_Thrust_R <= 1475 && o_Thrust_R >= 1000)) {
    sendServo(3, o_Thrust_L);
    sendServo(1, o_Thrust_R);
  }

  // Remote
  else if ((f_Thrust_L >= 1525 && f_Thrust_L <= 2000) ||
           (f_Thrust_R >= 1525 && f_Thrust_R <= 2000) ||
           (f_Thrust_L <= 1475 && f_Thrust_R >= 1000) ||
           (f_Thrust_R <= 1475 && f_Thrust_R >= 1000)) {
    sendServo(3, f_Thrust_L);
    sendServo(1, f_Thrust_R);
  }

  // Automation
  else if ((a_Thrust_L >= 1525 && a_Thrust_L <= 2000) ||
           (a_Thrust_R >= 1525 && a_Thrust_R <= 2000) ||
           (a_Thrust_L <= 1475 && a_Thrust_R >= 1000) ||
           (a_Thrust_R <= 1475 && a_Thrust_R >= 1000)) {
...
bool M300::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (key == "IVPHELM_ALLSTOP")
      ...
      if (ok2)
        Notify("ROTATE_HDG_TARGET",
               m_rot_ctrl.getHeadingTarget()); // for debugging.
...
void M300::sendMessagesToSocket() {
  ...
  bool ok_to_rotate =
      m_rot_ctrl.checkClearToRotate(m_nav_x, m_nav_y, m_curr_time);
...
void M300::readMessagesFromSocket() {
  ...
  if (m_ignore_msgs.count(msg.substr(0, 6)) >= 1)
    ...
    handled =
        handleMsgGNRMC(msg); // Added on 06-01-2022 by Supun for 2022 Herons
  else if (strBegins(msg, "$GPGGA"))
    handled = handleMsgGPGGA(msg);
  else if (strBegins(msg, "$GNGGA"))
    handled =
        handleMsgGNGGA(msg); // Added on 06-01-2022 by Supun for 2022 Herons
    ...
    if (m_use_nvg_msg_for_nav_x_nav_y)
      ...
      handled =
          handleMsgCPNVG_heading(msg); // Only using NVG for low speed heading!
    else                               // ignore it
      handled = true;
...
bool M300::handleMsgCPNVG(string msg) {
  ...
  if (m_publish_body_vel) {
    ...
    string nav_full_pos_msg =
        "x(" + str_nav_x + ")y(" + str_nav_y + ")hdg(" + str_state_hdg + ")";
...
bool M300::handleMsgCPNVG_heading(string msg) {
  ...
  bool stale_gps =
      ((MOOSTime() - m_last_gps_msg_time) > m_stale_gps_msg_thresh);
  if ((m_heading_source == "imu") or
      ((m_heading_source == "auto") and stale_gps)) {
    m_nav_hdg = dbl_hdg;
    Notify(m_nav_prefix + "_HEADING", dbl_hdg, "CPNVG");
  }
  Notify(m_compass_prefix + "_HEADING", dbl_hdg, "CPNVG");
...
//  3 [Heading]      Raw reading from compass for degrees clockwise from true
//  north 4 [Pitch]        Raw reading from compass for degrees of pitch 5
//  [Roll]         Raw reading from compass for degrees of roll 6 [NavTimestamp]
//  Timestamp for time compass reported this data. If blank, use [Timestamp]
...
bool M300::handleMsgCPNVR(string msg) {
  ...
  if ((m_tstamp_compass_msg - MOOSTime()) > m_stale_compass_thresh)
    return (reportBadMessage(msg, "Stale compass heading, not able to convert "
                                  "NVR message to body coordinates "));
  ...
  double vel_surge = dbl_vel_north * cos(m_compass_hdg * deg_to_rad) +
                     dbl_vel_east * sin(m_compass_hdg * deg_to_rad);
  double vel_sway = dbl_vel_north * -1.0 * sin(m_compass_hdg * deg_to_rad) +
                    dbl_vel_east * cos(m_compass_hdg * deg_to_rad);
  ...
  string end_char = ")";
  string nav_full_state = unav_header + unav_string + vnav_header +
                          vnav_string + rnav_header + rnav_string + end_char;
...
//            reference.
void M300::calculateFaultyThrust(double thrustL, double thrustR,
                                 double &thrustL_faulty,
                                 double &thrustR_faulty) {
  ...
  thrustL_faulty =
      thrustL * m_fault_factor_thr_L + m_fault_bias_thr_L + m_rudder_bias_L;
  thrustR_faulty =
      thrustR * m_fault_factor_thr_R + m_fault_bias_thr_R + m_rudder_bias_R;
...
bool M300::buildReport() {
  ...
  m_msgs << "           nav_y:   " << pd_navy << "   nav_spd: " << str_nav_spd
         << endl;
  m_msgs << "------------------------------------------------------" << endl;
  m_msgs << "System:    voltage: " << pd_volt << "   satellites: " << str_sats
         << endl;
  m_msgs << "------------------------------------------------------" << endl;
  ...
  if (m_vname == "floatie") {
    m_msgs << "On Board: "
           << "thrust " << o_Thrust_L << " " << o_Thrust_R
           << " available: " << onBoard << endl;
    m_msgs << "Remote: "
           << "thrust: " << f_Thrust_L << " " << f_Thrust_R << endl;
    m_msgs << "Automation: " << a_Thrust_L << " " << a_Thrust_R << endl;
    m_msgs << "gps found: " << gpsFound << " heading found: " << hdg_found
           << endl;
  }

  m_msgs << "beaconMode: " << beaconMode << endl;
  m_msgs << "vehicle name: " << m_vname << " Vehicle port: " << checkVehicle
         << endl;
  ...
  if (m_add_thruster_fault) {
    ...
    string str_total_bias_thr_L =
        doubleToStringX(m_rudder_bias_L + m_fault_bias_thr_L, 2);
    string str_total_bias_thr_R =
        doubleToStringX(m_rudder_bias_R + m_fault_bias_thr_R, 2);
    ...
    string pd_fault_factor_rudder =
        padString(str_fault_factor_rudder, 10, false);
    ...
    m_msgs << "Fault factors: L = " << pd_fault_factor_thr_L
           << "   R = " << pd_fault_factor_thr_R << endl;
    m_msgs << "Fault biases : L = " << pd_fault_bias_thr_L
           << "   R = " << pd_fault_bias_thr_R << endl;
    m_msgs << "Fault rudder : G = " << pd_fault_factor_rudder << endl;
    m_msgs << "______________________________________________________" << endl;
    m_msgs << "Total biases : L = " << pd_total_bias_thr_L
           << "   R = " << pd_total_bias_thr_R << endl;
