...
bool M300::ValidateTCP(string addr, int port) {
  ...
  if (server == NULL) {
    ...
    bcopy((char *)server->h_addr, &server_addr.sin_addr.s_addr,
          server->h_length);
...
bool M300::Connect() {
  int status =
      connect(m_socketfd, (struct sockaddr *)&server_addr, sizeof(server_addr));
...
bool M300::diffThrust(const double &desired_rudder,
                      const double &desired_thrust, double &desired_thrustL,
