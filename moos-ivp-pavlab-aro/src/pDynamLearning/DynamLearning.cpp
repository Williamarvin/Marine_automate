...
bool DynamLearning::Iterate() {
  ...
  bool cond5 =
      ((MOOSTime() - m_heading_received_time) <= m_time_between_estimates);
  bool cond6 =
      ((MOOSTime() - m_thrustR_received_time) <= m_time_between_estimates);
  bool cond7 =
      ((MOOSTime() - m_thrustL_received_time) <= m_time_between_estimates);
  bool cond8 = (m_num_readings_received >= 2);

  if (cond4 and cond5 and cond6 and cond7 and
      cond8) { // This could be an else if...
    ...
    // RLS ////////////////
    arma::Col<double> rls_inputs_est = {m_pydir_thrust_L,
                                        pow(m_pydir_thrust_L, 2),
                                        pow(m_pydir_thrust_L, 3),
                                        m_pydir_thrust_R,
                                        pow(m_pydir_thrust_R, 2),
                                        pow(m_pydir_thrust_R, 3),
                                        sin(m_nav_heading * PI / 180),
                                        cos(m_nav_heading * PI / 180),
                                        abs(heading_rate)};
  ...
  double max_time =
      std::max({m_speed_received_time, m_heading_received_time,
                m_thrustR_received_time, m_thrustL_received_time});
  double min_time =
      std::min({m_speed_received_time, m_heading_received_time,
                m_thrustR_received_time, m_thrustL_received_time});
  double time_span = max_time - min_time;

  bool all_recieved = m_speed_received and m_heading_received and
                      m_thrustR_received and m_thrustL_received;
...
void DynamLearning::initializeEstimators() {
  ...
  ok2 =
      m_SimpleRNN_obj.setLearningRateAdam(m_alpha, m_beta1, m_beta2, m_epsilon);
  ok2_1 =
      m_SimpleRNN_obj.setLearningRateContraction(m_contraction_error_weight);
  ...
  if (!(loadedW && loadedB && loadedA)) {
    ...
    reportRunWarning(
        "Warning: Failed to load latest RNN params, using original weights");
  ...
  if (pretrainedAIDParams.load((m_file_path + "AIDParams.csv"), csv_ascii)) {
    ...
    pretrained_AID_params =
        conv_to<std::vector<double>>::from(pretrainedAIDParams);
  } else {
    reportRunWarning(
        "Warning: Failed to load latest AID params, using original params");
  ...
  // RLS ////////////
  bool ok8;
  m_rls_model_speed = new identification::rls(m_num_of_params_rls, m_order_rls,
                                              m_forgetting_factor);
  ...
  if (pretrainedRLSParams.load((m_file_path + "RLSParams.csv"), csv_ascii)) {
    // if ( false ) {
    //  Loading was successful
    pretrained_RLS_params =
        conv_to<std::vector<double>>::from(pretrainedRLSParams);
  } else {
    reportRunWarning(
        "Warning: Failed to load latest RLS params, using original params");
...
bool DynamLearning::checkDataSet() {
  ...
  if (m_state_t[0] > 1.0 / m_scale_input_1)
    return false;
  // if (m_state_t[1] > 1.0 / m_scale_input_2)  // heading check no longer
  // required
  //   return false;
...
bool DynamLearning::updateEstimators() {
  ...
  RNN_inputs_t_minus_2.push_back(0.5 +
                                 0.5 * sin(m_state_t_minus_2[1] * PI / 180.0));
  RNN_inputs_t_minus_2.push_back(0.5 +
                                 0.5 * cos(m_state_t_minus_2[1] * PI / 180.0));
  ...
  RNN_inputs_t_minus_1.push_back(0.5 +
                                 0.5 * sin(m_state_t_minus_1[1] * PI / 180.0));
  RNN_inputs_t_minus_1.push_back(0.5 +
                                 0.5 * cos(m_state_t_minus_1[1] * PI / 180.0));
  ...
  double vel_estimate_t_minus_1 =
      m_SimpleRNN_obj.forwardProp(RNN_inputs_t_minus_2);
  ...
  bool ok1 = m_SimpleRNN_obj.backPropRecurrentAdam(
      error_t_minus_1, RNN_inputs_t_minus_2, error_t, RNN_inputs_t_minus_1);
  ...
  arma::Col<double> rls_inputs = {m_state_t[2],
                                  pow(m_state_t[2], 2),
                                  pow(m_state_t[2], 3),
                                  m_state_t[3],
                                  pow(m_state_t[3], 2),
                                  pow(m_state_t[3], 3),
                                  sin(m_state_t[1] * PI / 180),
                                  cos(m_state_t[1] * PI / 180),
                                  abs(heading_rate_RLS)};
...
bool DynamLearning::buildReport() {
  ...
  double average =
      ((m_vel_est_rnn / m_scale_input_1) + m_vel_est_aid + m_vel_est_rls) / 3.0;
  actab << (m_vel_est_rnn / m_scale_input_1) << m_vel_est_aid << m_vel_est_rls
        << average << m_nav_speed;

  m_msgs << actab.getFormattedString() << endl;
  m_msgs << "Variance in Estimation = " << m_nav_speed_est_var
         << "             " << endl;
  m_msgs << "                                            " << endl;
  m_msgs << "============================================" << endl;
  m_msgs << "Number of Estimations Made = " << m_num_est_made << "             "
         << endl;
  m_msgs << "Number of Readings Received = " << m_num_readings_received
         << "             " << endl;
  m_msgs << "============================================" << endl;
...
bool DynamLearning::saveParams() {
  ...
  ok10 = dataAIDParams.save((m_file_path + time_stamp + "_AIDParams.csv"),
                            csv_ascii);
  ok11 = dataRLSParams.save((m_file_path + time_stamp + "_RLSParams.csv"),
                            csv_ascii);
