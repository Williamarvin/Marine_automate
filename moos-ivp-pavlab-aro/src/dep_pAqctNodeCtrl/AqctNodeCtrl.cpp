...
void AqctNodeCtrl::calculateInputs() {
  ...
  m_dist_to_own_flag = distPointToPoint(
      m_nav_x, m_nav_y, m_own_flag_pt.get_vx(), m_own_flag_pt.get_vy());
  ...
  // Do some cleanup.  If an enemy agent is no longer in our zone, we need to
  // remove the tasking data the map to reassign

  /*
  // Now clear out any old node reports
  std::map<std::string, double>::iterator it2;
  for (it2 = m_time_to_reassign_intruder_map.begin(); it2 !=
  m_time_to_reassign_intruder_map.end();){ if (
  m_names_of_enemy_agents_in_our_zone.count(it2->first) ) {
        ++it2;
      } else {
...
void AqctNodeCtrl::updateBlockingBehavior() {
  ...
  if (m_node_rec_map.count(m_name_of_our_teammate_who_grabbed_their_flag) ==
      0) {
  ...
  double protect_x =
      m_node_rec_map[m_name_of_our_teammate_who_grabbed_their_flag].getX();
  double protect_y =
      m_node_rec_map[m_name_of_our_teammate_who_grabbed_their_flag].getY();
  ...
  for (it = m_node_rec_map.begin(); it != m_node_rec_map.end(); it++) {
    ...
    if (m_opposing_team_names.count(it->first)) {
      ...
      dist_enemy =
          distPointToPoint(protect_x, protect_y, close_enemy_x, close_enemy_y);
  ...
  m_best_trail_angle =
      relAng(protect_x, protect_y, close_enemy_x, close_enemy_y);
  ...
  Notify("TRAIL_INFO",
         "contact=" + m_name_of_our_teammate_who_grabbed_their_flag);
...
void AqctNodeCtrl::updateDefense() {
  ...
  if (m_they_grabbed_our_flag) {
    ...
    double enemy_x =
        m_node_rec_map[m_name_of_the_opposing_player_who_grabbed_our_flag]
            .getX();
    double enemy_y =
        m_node_rec_map[m_name_of_the_opposing_player_who_grabbed_our_flag]
            .getY();
    ...
    if (dist_to_enemy > 25.0)
      ...
      new_trail_range = ((dist_to_enemy - 15.0) / 10.0) *
                            (max_trail_range - min_trail_range) +
                        min_trail_range;
    ...
    if (m_node_rec_map[m_name_of_the_opposing_player_who_grabbed_our_flag]
            .isSetSpeed()) {
      enemy_speed =
          m_node_rec_map[m_name_of_the_opposing_player_who_grabbed_our_flag]
              .getSpeed();
    ...
    bool moving_perpendicular =
        ((30.0 < heading_diff) && (heading_diff < 120.0));
    ...
    XYPoint trail_point =
        projectPoint(enemy_hdg, new_trail_range, enemy_x, enemy_y);
    ...
    if (!m_home_zone_poly.contains(trail_point)) {
      ...
      new_trail_range = distPointToPoint(
          trail_point.get_vx(), trail_point.get_vy(), enemy_x, enemy_y);
    }

    double filt_trail_range =
        0.5 * (new_trail_range + m_last_grabber_intercept_trail_range);
    Notify("INTERCEPT_FLAG_GRABBER_UPDATES",
           "trail_range=" + doubleToString(new_trail_range));
    m_last_grabber_intercept_trail_range = new_trail_range;
  }

  // Is the flag burglar still in our zone?
  bool still_in_zone =
      (m_names_of_enemy_agents_in_our_zone.count(
           m_name_of_the_opposing_player_who_grabbed_our_flag) > 0);
  ...
  Notify("ENEMY_IN_ZONE_INTERCEPT_ONES_UPDATES",
         "contact=" + best_intruder_to_persecute);
  ...
  Notify("ENEMY_IN_ZONE_INTERCEPT_ONES_UPDATES",
         "trail_range=" + doubleToString(new_gen_trail_range));
  ...
  XYPoint delta_shift =
      projectPoint(m_loiter_offset_angle + 90.0, m_loiter_offset, 0.0, 0.0);
  ...
  if (number_defending == 1) {
    ...
    if (closest_to_near_loiter == m_vname) {
      Notify("DEFEND_FWD_UPDATES",
             "station_pt = " + near_adj_loiter_pt.get_spec());
    } else {
      Notify("DEFEND_FWD_UPDATES",
             "station_pt = " + far_adj_loiter_pt.get_spec());
    }
  }
...
std::string AqctNodeCtrl::determineBestIntruderToPursue() {
  ...
  for (it_vec = m_pop_state_map["defend"].begin();
       it_vec != m_pop_state_map["defend"].end(); it_vec++) {
  ...
  // m_vname == agent_vec[0]
  for (it_lv1 = m_cost_map[m_vname].begin();
       it_lv1 != m_cost_map[m_vname].end(); it_lv1++) {
    ...
    std::cout << " *teammates_2_and_3_best_cost ="
              << doubleToString(teammates_2_and_3_best_cost) << std::endl;
    ...
    if (number_defending >= 2) {
      ...
      std::cout << " **agent_lv2_lowest_cost"
                << doubleToString(agent_lv2_lowest_cost) << std::endl;

      // what is the minimum cost to intercept the other (1-2) agents?
      for (it_lv2 = m_cost_map[agent_vec[1]].begin();
           it_lv2 != m_cost_map[agent_vec[1]].end(); it_lv2++) {
        ...
        std::cout << " **possible_teammate_2_cost = "
                  << doubleToString(possible_teammate_2_cost) << std::endl;
        ...
        std::cout << " **teammate_3_cost = " << doubleToString(teammate_3_cost)
                  << std::endl;

        if (number_defending == 3) {

          for (it_lv3 = m_cost_map[agent_vec[2]].begin();
               it_lv3 != m_cost_map[agent_vec[2]].end(); it_lv3++) {
            ...
            std::cout << " ***intruder_level_3 = " << intruder_level_3
                      << std::endl;
            std::cout << " ***possible_teammate_3_cost = "
                      << doubleToString(possible_teammate_3_cost) << std::endl;
            ...
            std::cout << " **teammate_3_cost = "
                      << doubleToString(teammate_3_cost) << std::endl;
          }
        } // by now we have the third agent's cost for this choice at level 2

        double total_additional_cost =
            possible_teammate_2_cost + teammate_3_cost;
        std::cout << " **total_additional_cost = "
                  << doubleToString(total_additional_cost) << std::endl;
      ...
      // by now agent_lv2_lowest_cost is the lowest posible combo for the
      // remaining two agents and intruders.
    ...
    double population_cost_with_this_option =
        my_cost + teammates_2_and_3_best_cost;

    std::cout << " *population_cost_with_this_option ="
              << doubleToString(population_cost_with_this_option) << std::endl;
    if (population_cost_with_this_option < lowest_population_cost) {
      ...
      std::cout << " *lowest_population_cost ="
                << doubleToString(lowest_population_cost) << std::endl;
      std::cout << " *best_intruder_to_persecute ="
                << best_intruder_to_persecute << std::endl;
    }
  }

  std::cout << " --FINAL -- best_intruder_to_persecute ="
            << best_intruder_to_persecute << std::endl;
...
bool AqctNodeCtrl::calcIfClosestTo(std::string contact_name) {
  ...
  double dist_from_ownship =
      distPointToPoint(m_nav_x, m_nav_y, contact_x, contact_y);
...
std::string AqctNodeCtrl::getContactClosestToFlag() {
  ...
  std::set<std::string>::iterator it;
  for (it = m_names_of_enemy_agents_in_our_zone.begin();
       it != m_names_of_enemy_agents_in_our_zone.end(); it++) {
    ...
    dist = distPointToPoint(contact_x, contact_y, m_own_flag_pt.get_vx(),
                            m_own_flag_pt.get_vy());
...
std::string AqctNodeCtrl::getTeammateClosestToNearLoiter() {
  ...
  if (m_curr_option == "defend") {
    smallest_dist = distPointToPoint(
        m_near_loiter_pt.get_vx(), m_near_loiter_pt.get_vy(), m_nav_x, m_nav_y);
  ...
  std::set<std::string>::iterator it;
  for (it = m_pop_state_map["defend"].begin();
       it != m_pop_state_map["defend"].end(); it++) {
    ...
    dist = distPointToPoint(m_near_loiter_pt.get_vx(),
                            m_near_loiter_pt.get_vy(), contact_x, contact_y);
...
double AqctNodeCtrl::getCostToIntercept(std::string teammate,
                                        std::string contact) {
  ...
  // Compute dist
  double dist = distPointToPoint(friendly_x, friendly_y, future_point.get_vx(),
                                 future_point.get_vy());
...
void AqctNodeCtrl::buildCostMap() {
  ...
  for (it2 = m_pop_state_map["defend"].begin();
       it2 != m_pop_state_map["defend"].end(); it2++) {
    agents_costs.clear();
...
std::string AqctNodeCtrl::buildInterceptMissionTask(std::string target_name)
{
    ...
    // It should look like:
    //  MISSION_TASK_ALL =
type=waypoint,id=wpt$[BIX],waypt_x=$[XPOS],waypt_y=$[YPOS],exempt=rex
    ...
    if (m_pop_state_map.count("attack") > 0){
      if (m_pop_state_map["attack"].size() > 0) {
        std::set<std::string>::iterator it2;
        for (it2 = m_pop_state_map["attack"].begin(); it2 !=
m_pop_state_map["attack"].end(); it2++){ task_desc += ":" + *it2;
        }
...
bool AqctNodeCtrl::buildReport() {
  m_msgs << "============================================" << endl;
  m_msgs << "Team Color: " << m_team_color
         << ",  Size: " << uintToString(m_team_size) << endl;
  ...
  for (it = m_opposing_team_names.begin(); it != m_opposing_team_names.end();
       it++) {
    other_line += *it + ",";
  }
  ...
  m_msgs << "============================================" << endl;
  if (m_we_grabbed_their_flag) {
    m_msgs << " * !We grabbed their flag! Agent "
           << m_name_of_our_teammate_who_grabbed_their_flag << " has it! "
           << endl;
    m_msgs << "  -> Enemy agent " << m_name_of_closest_enemy_to_player_with_flag
           << " is "
           << doubleToString(m_dist_of_closest_enemy_to_player_with_flag, 2)
           << "m away" << endl;
    m_msgs << "  -> Best trail range = "
           << doubleToStringX(m_best_trail_dist, 2)
           << ", angle = " << doubleToStringX(m_best_trail_angle, 2) << endl;
  } else {
    m_msgs << " * We do not have their flag                 " << endl;
  }

  if (m_they_grabbed_our_flag) {
    m_msgs << " * !They grabbed our flag! Agent "
           << m_name_of_the_opposing_player_who_grabbed_our_flag << " has it! "
           << endl;
  } else {
  ...
  std::string intruder_line =
      stringSetToString(m_names_of_enemy_agents_in_our_zone);
  ...
  actab << doubleToStringX(m_dist_to_own_flag, 2)
        << doubleToStringX(m_agents_in_our_zone, 2) << intruder_line
        << tagged_line;
  ...
  for (it2 = m_opinion_inputs.begin(); it2 != m_opinion_inputs.end(); it2++) {
    m_msgs << "  " << it2->first << ": " << doubleToStringX(it2->second, 4)
           << endl;
  }
  m_msgs << "                                            " << endl;
  m_msgs << "=  Node Reports   =========================================="
...
bool AqctNodeCtrl::handleFlagSummary(std::string msg) {
  ...
  for (it = m_opposing_team_names.begin(); it != m_opposing_team_names.end();
    ...
    if (strContains(msg, common_preamble + *it)) {
      ...
      Notify("INTERCEPT_FLAG_GRABBER_UPDATES",
             "contact=" + m_name_of_the_opposing_player_who_grabbed_our_flag);
      break;
    }
  }
...
bool AqctNodeCtrl::handleIntruderTasked(std::string msg){
  ...
  double random_interval = randomDouble(m_min_reallocate_interval,
m_max_reallocate_interval); m_time_to_reassign_intruder_map[name] = MOOSTime() +
random_interval;
