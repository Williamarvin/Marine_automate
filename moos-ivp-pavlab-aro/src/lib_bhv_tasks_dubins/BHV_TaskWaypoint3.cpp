...
double BHV_TaskWaypoint3::getTaskBid() {
  ...
  double cw =
      -1; // a logical representation as a double - are we turning 'clockwise'
  if (psi < 0) {
    ang_to_cor -= M_PI / 2;
    cw = -1;
  } else {
  ...
  // If the point is within the radius of our center of rotation, we need to
  // scoot forward and update our center of rotation, adding this to our
  // odometer
  ...
  // If we did have to move forward at all, then just add the new point to the
  ...
  double ang_step_size =
      5 * M_PI /
      180; // k degrees to radians (5 degree step size for evaluation)
  while (true) {
    ...
    // If the angle between these vectors is interior to the circle, i.e. it
    // passes the circle, then we need to step to this next point Otherwise, if
    // we have a straight shot to the target, then we completed the dubins turn

    // Or If we have done this over for over 1000 steps, we messed up and the
    // algorithm is faulty
...
vector<VarDataPair>
BHV_TaskWaypoint3::applyFlagMacros(vector<VarDataPair> flags) {
