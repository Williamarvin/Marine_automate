...
bool GridSwitcher::Iterate() {
  ...
  if (m_label_switched) {
    ...
    if ((m_current_grid_label == "cons") and (m_want_variance)) {
      grid_spec =
          m_grids_received.at(m_current_grid_label).getVarianceGridSpec();
    } else {
    ...
    if (m_need_to_update_full_grid[m_current_grid_label]) {
      ...
      if ((m_current_grid_label == "cons") and (m_want_variance)) {
        m_grids_received.at(m_current_grid_label).setOldGridToNew();
        std::string var_grid_spec =
            m_grids_received.at(m_current_grid_label).getVarianceGridSpec();
        std::string relabeled_grid_spec =
            replaceGridLabel(var_grid_spec, m_vname);
        Notify(m_output_var, relabeled_grid_spec);
        m_need_to_update_full_grid[m_current_grid_label] = false;
        m_need_to_update_grid_delta[m_current_grid_label] = false;
        ...
        std::string delta_grid_spec = m_grids_received.at(m_current_grid_label)
                                          .getDeltaSpec(0.01, found_delta);
...
void GridSwitcher::registerVariables() {
  ...
  for (itr = m_input_vars_delta.begin(); itr != m_input_vars_delta.end();
       itr++) {
    if (*itr != "")
      Register(*itr, 0);
  }
...
std::string GridSwitcher::replaceGridLabel(std::string grid_spec,
                                           std::string vname) {
...
std::string GridSwitcher::replaceDeltaGridLabel(std::string grid_spec,
                                                std::string vname) {
  std::vector<std::string> svector = parseString(grid_spec, '@');
  unsigned int length = svector.size();
  if (length < 1) {
...
bool GridSwitcher::populateFromFile(double lat, double lon) {
  ...
  if (newfile.is_open()) {
    ...
    string border_str =
        poly_spec.substr(border_start, border_end - border_start);

    string convex_spec = "pts=" + border_str + "," + cell_size_str + ",";

    convex_spec += "cell_vars=depth:0:var:0,cell_min=depth:0,cell_max=depth:30,"
                   "cell_min=var:0.0001,cell_max=var:100,";
    ...
    while ((pos = cell_data.find(',')) != std::string::npos) {
      ...
      convex_spec += token.substr(0, colon_pos) + ":depth" +
                     token.substr(colon_pos) + "var:100,";
      cell_data.erase(0, pos + 1);
    }

    colon_pos = token.find(":");
    convex_spec += cell_data.substr(0, colon_pos) + ":depth" +
                   cell_data.substr(colon_pos) + "var:100";
...
bool GridSwitcher::recalcGtGrid(std::string grid_spec) {
  ...
  for (idx = 0; idx < size; idx++) {
    ...
    m_gt_grid_from_file.getCellDataXY(full_grid_x, full_grid_y, depth_gt,
                                      var_gt);
...
void GridSwitcher::mirrorPoint(double p1x, double p1y, double &p2x,
                               double &p2y) {
  double dx = m_x2 - m_x1;
  double dy = m_y2 - m_y1;
