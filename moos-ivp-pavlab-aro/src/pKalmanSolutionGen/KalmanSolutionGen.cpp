...
//---------------------------------------------------------
// Procedure: initModel
void KalmanSolutionGen::initModel(double x, double y, double vx, double vy,
                                  double time) {
  m_x = x;
  m_xo = x;
  m_y = y;
...
void KalmanSolutionGen::executeKalmanX() {
  ...
  double vk[2][2] = {{m_meas_noise, 0},
                     {0, 5 * m_meas_noise / (delta_tc * delta_tc)}};
