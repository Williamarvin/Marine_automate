...
bool BHV_ConvoyV21X::setParam(string param, string param_val) {
  ...
  if ((param == "nm_radius") || (param == "slip_radius"))
    ...
    handled = m_marker_tail.setInterMarkRange(param_val);
           ...
           (param == "full_lag_convoy_range") || (param == "lag_speed_delta"))
...
bool BHV_ConvoyV21X::handleConfigHoldingPolicy(string policy) {
  if ((policy != "zero") && (policy != "curr_hdg") && (policy != "off") &&
      (policy != "setpt_hdg"))
    return (false);
...
IvPFunction *BHV_ConvoyV21X::onRunState() {
  ...
  m_set_speed = m_spd_policy.getSpdFromPolicy(m_cnv_avg_2sec, m_contact_range,
                                              m_convoy_range);
    ...
    for (unsigned int i = 0; i < msgs.size(); i++) {
      ...
      if (!new_marker.valid())
        ...
        if (tolower(new_marker.getVName()) == tolower(m_contact)) {
          ...
          list<ConvoyMarker> cleared_markers =
              m_marker_tail.getClearedMarkers();
          list<ConvoyMarker>::iterator p;
          for (p = cleared_markers.begin(); p != cleared_markers.end(); p++)
            eraseMarker(*p);

          marker_added = true;
          // cout << "active marker_added:" << boolToString(marker_added) <<
          // endl;
...
bool BHV_ConvoyV21X::checkDropAftMarker() {
  ...
  if (!marker_dropped && (dist < m_slip_radius)) {
    ...
    double angle =
        angleFromThreePoints(m_wptx, m_wpty, m_osx, m_osy, next_x, next_y);
...
void BHV_ConvoyV21X::onRunToIdleState() { clearMarkerTail(); }
...
void BHV_ConvoyV21X::drawMarker(ConvoyMarker marker, int vsize, string color) {
  if (!marker.valid())
    return;
