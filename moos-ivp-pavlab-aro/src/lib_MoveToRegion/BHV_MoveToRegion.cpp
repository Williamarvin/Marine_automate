...
BHV_MoveToRegion::BHV_MoveToRegion(IvPDomain domain) : IvPBehavior(domain) {
...
bool BHV_MoveToRegion::updateOwnshipPosition() {
  ...
  if ((tstamp_osx > m_stale_nav_thresh) || (tstamp_osy > m_stale_nav_thresh)) {
...
bool BHV_MoveToRegion::updateSetPoint() {
  ...
  bool ok = m_poly_region.closest_point_on_poly(m_osx, m_osy, new_setpt_x,
                                                new_setpt_y);
  if (!ok)
    return (false);
...
void BHV_MoveToRegion::postSetPoint() {
  ...
  if (commsPolicy() == "lean") {
    double dist_to_prev =
        distPointToPoint(m_setpt_x_prev, m_setpt_y_prev, m_setpt_x, m_setpt_y);
    if (dist_to_prev < 1)
      return;
  }
