...
bool DynamicTrafficLight::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (key == "TRAJECTORY_MSG_LOCAL") {
      ...
      string label = temp_seglist.get_label(); // label set previously to
                                               // "osname_dubin_#oftrajectories"
      ...
      string traj_label =
          temp_seglist
              .get_label(); // label on the form "osname_dubin_#oftrajectories"
      ...
      //  Check if sender is in proximity, position is the first point in the
      //  trajectory
      XYPoint position = temp_seglist.get_first_point();
      double dist =
          sqrt(pow(position.x() - m_nav_x, 2) + pow(position.y() - m_nav_y, 2));
      if (dist < m_proximity_range &&
          find(m_nodes_in_proximity.begin(), m_nodes_in_proximity.end(),
               sender_name) == m_nodes_in_proximity.end()) {
        m_nodes_in_proximity.push_back(sender_name);
      } else if (dist >= m_proximity_range &&
                 find(m_nodes_in_proximity.begin(), m_nodes_in_proximity.end(),
                      sender_name) != m_nodes_in_proximity.end()) {
        m_nodes_in_proximity.erase(remove(m_nodes_in_proximity.begin(),
                                          m_nodes_in_proximity.end(),
                                          sender_name),
                                   m_nodes_in_proximity.end());
      }
      ...
      bool sender_in_handshakes = find(m_handshakes.begin(), m_handshakes.end(),
                                       sender_name) != m_handshakes.end();
      bool traj_number_most_recent =
          (traj_number >=
           m_os_trajectory_number); // should not be possible to be greater than
...
bool DynamicTrafficLight::Iterate() {
  ...
  for (auto const &node :
       m_known_traj_numbers) { // TODO: could reduce the frequency of this
    NodeMessage node_msg;
    node_msg.setSourceNode(m_name_os);
  ...
  if (m_trajectory_map.find(m_name_os) ==
      m_trajectory_map.end()) { // If no trajectory, stop
    break_early = true;
  } else if (m_trajectory_map[m_name_os].size() == 0) { // No os trajectory,
                                                        // stop
    break_early = true;
  }
  // else if (m_handshakes.size() < m_known_traj_numbers.size()) { //Wait until
  // all handshook
  //   break_early = true;
  // }
  ...
  for (auto const &node : m_nodes_in_proximity) {
    if (find(m_handshakes.begin(), m_handshakes.end(), node) ==
        m_handshakes.end()) {
      break_early = true;
      break;
    }
  ...
  if (m_deadlock_timer != -1) {
    if (MOOSTime() - m_deadlock_timer < 10) {
      ...
      // XYPolygon poly = string2Poly("format=wedge, x=0, y=0, lang=40, rang=60,
      // range=10, snap=1"); XYPolygon poly = string2Poly("format=wedge, x=" +
      // to_string(m_nav_x) + ", y=" + to_string(m_nav_y) + ", range=" +
      // to_string(poly_radius) + ", lang=" + to_string(lang) + ", rang=" +
      // to_string(rang) + ", pts=10");
      ...
      if (!poly.is_convex()) {
        // Will not be convex if relative angle is >=90 (= beacuse of rounding
        // errors)
  ...
  //  If total status is temp_blocked, check all other vehicles to make final
  //  decision
  ...
  for (auto const &vehicle_i : m_trajectory_map) { // For all vehicles...
    ...
    double dynamic_safe_dist_i =
        m_min_safety_dist + min(pow((speed_i / m_default_speed), 2), 1.0) *
                                (m_max_safety_dist - m_min_safety_dist);
    double dynamic_horizon_i_meters =
        m_min_horizon_meters +
        min(speed_i / m_default_speed, 1.0) *
            (m_max_horizon_meters - m_min_horizon_meters);
    int dynamic_horizon_i_points =
        round(dynamic_horizon_i_meters / m_precision);
    int horizon_i =
        min(dynamic_horizon_i_points, int(m_trajectory_map[name_i].size()));

    for (auto const &vehicle_j :
         m_trajectory_map) { //..check block status with all other vehicles
      string name_j = vehicle_j.first;
      if (name_i == name_j) {
        m_block_matrix[name_i][name_j] =
            UNBLOCKED; // Cannot be blocked by itself
        continue;
      }
      XYSegList seglist_j = vehicle_j.second;
      double speed_j = m_speed_map[name_j];
      double dynamic_safe_dist_j =
          m_min_safety_dist + min(pow((speed_j / m_default_speed), 2), 1.0) *
                                  (m_max_safety_dist - m_min_safety_dist);
      double largest_safe_dist = max(dynamic_safe_dist_i, dynamic_safe_dist_j);
      double dynamic_horizon_j_meters =
          m_min_horizon_meters +
          min(speed_j / m_default_speed, 1.0) *
              (m_max_horizon_meters - m_min_horizon_meters);
      int dynamic_horizon_j_points =
          round(dynamic_horizon_j_meters / m_precision);
      int horizon_j =
          min(dynamic_horizon_j_points, int(m_trajectory_map[name_j].size()));
      ...
      for (int i = 0; i < horizon_i; i++) { // For all points in trajectory i
        ...
        double dist = sqrt(pow(point_i.x() - position_j.x(), 2) +
                           pow(point_i.y() - position_j.y(), 2));
      ...
      //----------2. Check if vehicle i is temporarily blocked by vehicle
      //j----------
      ...
      for (int i = 0; i < horizon_i; i++) { // For all points in trajectory i
        ...
        for (int j = 0; j < horizon_j; j++) { // For all points in trajectory j
          ...
          double dist = sqrt(pow(point_i.x() - point_j.x(), 2) +
                             pow(point_i.y() - point_j.y(), 2));
          if (dist < largest_safe_dist) {
            temp_blocked = true;
            break;
          }
      ...
      if (temp_blocked) {
        ...
        if (horizon_color != "red" &&
            name_i == m_name_os) { // Phys. blocked has priority
          horizon_color = "orange";
        }
        // Decide on the actual blockage here...
      } else {
  ...
  for (auto const &block_map : m_block_matrix) {
    ...
    for (auto const &blockage : block_map.second) {
      ...
      if (m_block_matrix[other][current] ==
          PHYSICALLY_BLOCKED) { // other one is physically blocked (or
                                // previously set to blocked bc of priority)
        // Current one is unblocked by the other one because it is physically
        // blocking it
      } else {
  ...
  // TODO: ONLY NEED TO FIND MY OWN TOTAL STATUS? COULD REMMOVE FROM MATRIX AND
  // JUST HAVE A VARIABLE?
  for (auto const &block_map : m_block_matrix) {
  ...
  // if (m_block_matrix[m_name_os]["total"] == TEMP_BLOCKED) {
  //   m_block_matrix[m_name_os]["total"] = UNBLOCKED; //assume unblocked until
  //   proven otherwise for (auto const& blockage : m_block_matrix[m_name_os]) {
  //     if (blockage.first == "total") {
  //       continue;
  //     }
  //     if (blockage.second != TEMP_BLOCKED) {
  ...
  //     if (m_block_matrix[blockage.first][m_name_os] == PHYSICALLY_BLOCKED) {
  //     //they are physically blocked by me
  //       //continue assuming unblocked
  //     }
  //     else {
  //       int decision_var = m_name_os.compare(blockage.first);
  //       if (decision_var < 0) {
  //         // continue assuming unblocked
  //       } else {
  //         m_block_matrix[m_name_os]["total"] = PHYSICALLY_BLOCKED;
  //         m_block_matrix[m_name_os][blockage.first] = PHYSICALLY_BLOCKED;
  //         //Also set the other to physically blocked for deadlock detection
  //         // m_block_matrix[blockage.first][m_name_os] = PHYSICALLY_BLOCKED;
  //         break;
  //       }
  //     }
  //   }
  ...
  string viz_color =
      ""; // TODO: MOVE THIS UP SO ITS GLOBAL AND DECIDE IN THE MAIN LOOP. START
          // GREEN, IF TEMP-ORANGE, IF PHYSICALLY-RED
  if (m_block_matrix[m_name_os]["total"] == PHYSICALLY_BLOCKED) {
    ...
    Notify("DUBIN_UPDATE",
           "speed=" +
               doubleToString(m_default_speed * synch_factor)); // Unblocked
    viz_color = "green";
  }
  ...
  if (m_block_matrix[m_name_os]["total"] == PHYSICALLY_BLOCKED) {
    ...
    potential_deadlock =
        depthFirstSearch(m_name_os, m_block_matrix, visited, recursive_stack);
    potential_permablock =
        isPermablocked(m_name_os, m_block_matrix, m_trajectory_map);
  }
  ...
  if (potential_permablock) {
    ...
    if (m_potential_permablock_timer == -1) {
      ...
      output_seglist.set_label(m_name_os + "_dubin_" +
                               to_string(m_os_trajectory_number + 1));
  ...
  if (m_show_visualization) {
    ...
    string label = "safety_" + m_name_os;
    double dynamic_safety_dist =
        m_min_safety_dist + min(pow((m_nav_speed / m_default_speed), 2), 1.0) *
                                (m_max_safety_dist - m_min_safety_dist);
    XYPolygon poly =
        XYPolygon(m_nav_x, m_nav_y, dynamic_safety_dist, 10, label);
    poly.set_edge_color(viz_color);
    poly.set_vertex_color(viz_color);
    poly.set_label_color("transparent");
    Notify("VIEW_POLYGON", poly.get_spec());
    ...
    double dynamic_horizon_os_meters =
        m_min_horizon_meters +
        min(m_nav_speed / m_default_speed, 1.0) *
            (m_max_horizon_meters - m_min_horizon_meters);
    int dynamic_horizon_os_points =
        round(dynamic_horizon_os_meters / m_precision);
    int horizon =
        min(dynamic_horizon_os_points, int(m_trajectory_map[m_name_os].size()));
    XYSegList viz_seglist = XYSegList("traj_" + m_name_os);
...
bool DynamicTrafficLight::buildReport() {
  ...
  double dynamic_horizon_os_meters =
      m_min_horizon_meters + min(m_nav_speed / m_default_speed, 1.0) *
                                 (m_max_horizon_meters - m_min_horizon_meters);
  ...
  m_msgs << "============================================" << endl;
  m_msgs << "Own Trajectory: " << m_trajectory_map[m_name_os].get_spec()
         << endl;
  ...
  m_msgs << "============================================" << endl;
  m_msgs << "Potential permablock timer: " << m_potential_permablock_timer
         << endl;
...
bool depthFirstSearch(string name,
                      map<string, map<string, BlockStatus>> &block_matrix,
                      vector<string> &visited,
                      vector<string> &recursive_stack) {
  ...
  for (auto const &blockage : block_matrix[name]) {
    ...
    if (blockage.first == "name") { // Should not be necessary, as a node cannot
    ...
    if (blockage.second == PHYSICALLY_BLOCKED) {
      bool already_visited =
          find(visited.begin(), visited.end(), blockage.first) != visited.end();
      if (!already_visited && depthFirstSearch(blockage.first, block_matrix,
                                               visited, recursive_stack)) {
        return true;
      } else if (find(recursive_stack.begin(), recursive_stack.end(),
                      blockage.first) != recursive_stack.end()) {
        return true;
      }
    }
  }
  ...
  recursive_stack.erase(
      remove(recursive_stack.begin(), recursive_stack.end(), name),
      recursive_stack.end());
...
bool isPermablocked(
    std::string name,
    std::map<std::string, std::map<std::string, BlockStatus>> &block_matrix,
    std::map<std::string, XYSegList> &trajectory_map) {
