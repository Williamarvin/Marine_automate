...
protected:
  ...
  unsigned int
      _n_order; // order per input value (how many past values are relevant)
...
public:
  rls(unsigned int n_inputs, unsigned int n_order, double lambda = 0.5)
    ...
    _P = inv(0.0001 *
             eye<mat>(_n_rlsorder + _n_inputs, _n_rlsorder + _n_inputs)) *
         _P;
...
public:
  nlms(unsigned int n_inputs, unsigned int n_order, double mu = 0.5) : _mu(mu) {
    _n_inputs = n_inputs;
    _n_order = n_order;
