...
//                  expected value
bool SimpleKalmanConsensus::receiveConsEstMessage(
    std::string vname, double time, std::vector<double> estimate,
    std::vector<double> variance, double msg_deg_connectedness) {
...
bool SimpleKalmanConsensus::receiveRequestMessage(double msg_time,
                                                  double time) {
...
bool SimpleKalmanConsensus::replyToRequestSpec(
    std::string &own_vname, std::vector<double> &own_estimate,
...
bool SimpleKalmanConsensus::submitEstimate(double time,
...
bool SimpleKalmanConsensus::readyToPerformConsensus(
    double time, unsigned int max_iterations) {
  // Check 0:
  ...
  time_to_wait_accounting_for_iterations =
      m_wait_time * (static_cast<double>(m_consensus_iteration_number + 1));
  if ((time - m_new_consensus_request_time) <
      time_to_wait_accounting_for_iterations) {
    return (false);
  }
  ...
  cout << "Number of valid Neighbors found = " << count
       << " and invalid found = " << old_msg << endl;
  ...
  // and we should stop
  /*
  if ( m_previous_consensus_was_fully_connected and
  (m_consensus_iteration_number > 0) ) {
    // we are done with this request
    m_new_consensus_requested = false;
    m_consensus_timed_out = false;
  ...
  */
  ...
  if ((count == 0) and (m_first_consensus_completed)) {
    ...
    max_time_to_wait_accounting_for_iterations =
        m_wait_time * (static_cast<double>(max_iterations));
    bool cond1 = ((time - m_new_consensus_request_time) <
                  max_time_to_wait_accounting_for_iterations);

    if (cond1) {
      cout << "False:  No data from neigbors and not first consensus and we "
              "have not timed out"
           << endl;
      return (false);
    } else {
...
//
//    returns false if error
bool SimpleKalmanConsensus::newConsensusItr(
    double time, std::vector<std::vector<double>> multivariate_states,
    bool use_running_consensus_when_neighbor_est_available,
    bool use_running_consensus_if_no_neighbor_est_available,
    std::vector<double> &consensus_val, std::vector<double> &consensus_var) {
  ...
  if (m_consensus_timed_out and
      not use_running_consensus_if_no_neighbor_est_available) {
    ...
    if (m_consensus_iteration_number > 0) {
      consensus_val =
          conv_to<std::vector<double>>::from(m_current_consensus_estimate);
      consensus_var =
          conv_to<std::vector<double>>::from(m_current_consensus_variance);
  ...
  m_unweighted_covariance_matrix =
      calcUnweightedCovarMarix(multivariate_states);
  ...
  for (it = m_est_times.begin(); it != m_est_times.end(); it++) {
    ...
    if ((time - it->second) < m_timeout) {
      ...
      Col<double> temp_vars =
          conv_to<Col<double>>::from(m_est_variances[vname]);
  ...
  // For simplicity/readability determine if we are in a special end condition,
  // i.e. if we have timed out, and we have requested to fuse the previous
  // consensus with the current own estimate for the final output
  bool in_special_end_condition =
      (m_consensus_timed_out and
  ...
  if (((x_vals.size() == 1) or (x_covariance.size() == 1)) and
      not in_special_end_condition) {
  ...
  // Bound own connectedness greater than 0.0 otherwise we will be dividing by
  // zero. This also prevents the own_connectedness from becoming 0 if in
  // "special end condition"
  if (m_own_connectedness < .0001) {
    m_own_connectedness = 1.0;
  };
  ...
  bool cond3 = (m_first_consensus_completed and
                use_running_consensus_when_neighbor_est_available and
                (m_consensus_iteration_number < 1));
  ...
  if (cond3 or cond4) {
    ...
    posterior_covar =
        posterior_covar + Q_process_noise * m_process_noise_variance;
...
bool SimpleKalmanConsensus::calcFusedMeanVariance(
    std::vector<Col<double>> x_vals, std::vector<Mat<double>> x_covariance,
    std::vector<double> x_connectedness, Col<double> &calc_val,
    Mat<double> &calc_covar) {
  ...
  for (unsigned int j = 1; j < n_agents; j++) {
    ...
    if (connectedness_scale_factor < 1.0) {
      ...
      cout << "  Warning in Kalman Update:  Had to reset scale factor to 1 "
           << endl;
      cout << "  Check that messages have connectedness numbers of at least 1 "
           << endl;
    }

    bool ok1 =
        arma::inv_sympd(temp, connectedness_scale_factor * x_covariance[j]);

    if (not ok1) {
      cout << "   Con error:  The j=" << j
           << " covariance matrix has a bad condition number="
  ...
  if (not ok3) {
    cout << "   Con error:  The covariance inverse matrix has a bad condition "
            "number = "
...
double SimpleKalmanConsensus::covarianceFun(std::vector<double> v1,
...
//            variance known
Mat<double> SimpleKalmanConsensus::calcUnweightedCovarMarix(
  ...
  for (unsigned int i = 0; i < length; ++i) {
    for (unsigned int j = i; j < length; ++j) {
      double val =
          covarianceFun(multivariate_states[i], multivariate_states[j]);
      mat(i, j) = val;
      mat(j, i) = val;
    }
  }
...
Mat<double> SimpleKalmanConsensus::calcCovarMatrix(Col<double> variance) {
  ...
  for (unsigned int i = 0; i < length; ++i) {
    for (unsigned int j = i; j < length; ++j) {
      double val = m_unweighted_covariance_matrix(i, j) *
                   std::sqrt(variance[i]) * std::sqrt(variance[j]);
      mat(i, j) = val;
      mat(j, i) = val;
    }
  }
