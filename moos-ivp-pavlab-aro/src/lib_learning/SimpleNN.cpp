...
//-----------------------------------------------------------
// Initialize
bool SimpleNN::initialize(long long unsigned int num_of_neurons,
                          long long unsigned int num_of_inputs) {
...
//--------------------------------------------------------------
// Set learning Rates
bool SimpleNN::setLearningRateAdam(double alpha, double beta1, double beta2,
                                   double epsilon) {
  m_alpha = alpha;
  m_beta1 = beta1;
  m_beta2 = beta2;
...
bool SimpleNN::backPropAdam(double error, std::vector<double> inputs) {
  ...
  Col<double> grad_B =
      error * g_prime_G_dot_A_dble * m_A % activationFunDerivative(m_Z);
...
bool SimpleNN::backPropRecurrentAdam(double error_t1,
                                     std::vector<double> inputs_t1,
                                     double error_t2,
                                     std::vector<double> inputs_t2) {
  ...
  Col<double> grad_B_error_t2 =
      error_t2 * g_prime_G_dot_A_dble_t2 * m_A % activationFunDerivative(m_Z);
  ...
  // Calculate the gradient of B at this timestep with this error
  Col<double> grad_B_error_t1 =
      error_t1 * g_prime_G_dot_A_dble_t1 * m_A % activationFunDerivative(m_Z);
  // Calculate the gradient of W at this timestep with this error
  ...
  // Calculate the gradient of B at this timestep with this error
  Col<double> grad_B_error_3 =
      error_3 * g_prime_G_dot_A_dble_t1 * m_A % activationFunDerivative(m_Z);
  // Calculate the gradient of W at this timestep with this error
...
// Update weights using Adam
// Refs:
// https://srdas.github.io/DLBook/GradientDescentTechniques.html#ParameterUpdate
//       https://machinelearningmastery.com/adam-optimization-from-scratch/
// Returns true if successful

bool SimpleNN::updateWeightsAdam(Mat<double> grad_W, Col<double> grad_B,
                                 Col<double> grad_A) {
  ...
  Mat<double> mom1_W_hat =
      m_mom1_W / (1.0 - pow(m_beta1, (m_num_back_props + 1)));
  Mat<double> mom2_W_hat =
      m_mom2_W / (1.0 - pow(m_beta2, (m_num_back_props + 1)));
  ...
  // Normalize the first and second moments
  Col<double> mom1_B_hat =
      m_mom1_B / (1.0 - pow(m_beta1, (m_num_back_props + 1)));
  Col<double> mom2_B_hat =
      m_mom2_B / (1.0 - pow(m_beta2, (m_num_back_props + 1)));
  ...
  // Normalize the first and second moments
  Col<double> mom1_A_hat =
      m_mom1_A / (1.0 - pow(m_beta1, (m_num_back_props + 1)));
  Col<double> mom2_A_hat =
      m_mom2_A / (1.0 - pow(m_beta2, (m_num_back_props + 1)));
