...
public:
  ...
  bool initialize(long long unsigned int num_of_neurons,
  ...
  bool setLearningRateAdam(double alpha, double beta1, double beta2,
                           double epsilon);
  bool setLearningRateContraction(double c_w) {
    m_contraction_weight = c_w;
    return (true);
  };
  ...
  bool backPropRecurrentAdam(double error_t1, std::vector<double> inputs_t1,
                             double error_t2, std::vector<double> inputs_t2);

  bool isRNNContracting();
  std::vector<double> getEqualibPoints();

protected:
  bool updateWeightsAdam(Mat<double> grad_W, Col<double> grad_B,
                         Col<double> grad_A);
  ...
  Mat<double>
      m_W_grad_cont; // Matrix to hold any contraction errors for weights in W
  Col<double>
      m_A_grad_cont; // Vector to hold any contraction errors for weights in A
