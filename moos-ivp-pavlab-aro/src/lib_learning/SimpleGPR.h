...
public:
  ...
  bool sampleAndBuild(std::vector<std::vector<double>> omit_sample_list,
                      double omit_dist_thresh);
  bool estimateFastGPRFromSampleSet(std::vector<double> state, double &val,
                                    double &covar);
...
protected:
  double covarianceFun(const std::vector<double> &v1,
                       const std::vector<double> &v2);
  double covarianceFun(const double &dist); // if you already have the distance
                                            // between two states
