...
public:
  ...
  void setProcessNoiseVariance(double Q_diag_var) {
    m_process_noise_variance = Q_diag_var;
    return;
  };
  void setKernelLengthScale(double val) {
    m_kernel_length_scale = val;
    return;
  };
  ...
  bool submitEstimate(double time, std::vector<double> own_estimate,
                      std::vector<double> own_variance);
  ...
  double getNewConsensusRequestTime() {
    return (m_new_consensus_request_time);
  };
...
protected:
  ...
  Mat<double> calcUnweightedCovarMarix(
      std::vector<std::vector<double>> multivariate_states);
