...
bool SimpleGPR::calcSampleSizeAndIterations() {
  ...
  double N_s_dbl =
      std::floor(5.0 * pow(N_dbl, delta) / pow(m_error_thresh, 0.1));
...
//                             at x=1, y=2.
//
bool SimpleGPR::sampleAndBuild(
    std::vector<std::vector<double>> omit_sample_list,
    double omit_dist_thresh) {
  ...
  std::vector<bool> alreadly_sampled_one_on_omitted_list(
      omit_sample_list.size(), false);
  ...
  // while loop.
  while ((m_sampled_index.size() < m_fastGPR_sample_size) and
         (rejected_count < (m_observed_values.size() - m_sampled_index.size() -
                            omit_sample_list.size()))) {
    // Randomly sample
  ...
  // initialize
  uword sample_size =
      m_fastGPR_sample_size; // "convert" from unsigned int to uword
  ...
  m_alpha = arma::solve(arma::trimatu(arma::trans(m_L_cholesky)),
                        arma::solve(arma::trimatl(m_L_cholesky), m_y));
...
double SimpleGPR::covarianceFun(const std::vector<double> &v1,
...
bool SimpleGPR::estimateFastGPRFromSampleSet(std::vector<double> state,
                                             double &val, double &covar) {
...
//            a grid.
bool SimpleGPR::estimateFastGPR(std::vector<double> state, double &val,
                                double &covar) {
  ...
  for (int j = 1; j <= m_fastGPR_iterations; j++) {
    ...
    running_average_val =
        (jth_val + running_average_val * (j_dbl - 1.0)) / j_dbl;
    running_average_covar =
        (running_average_covar * pow((j_dbl - 1.0), 2) + jth_covar) /
        (pow((j_dbl), 2));
  }
...
bool SimpleGPR::estimateGPR(std::vector<double> state, double &val,
                            double &covar) {
  // Clear the matricies
  uword sample_size =
      m_observed_values.size(); // "convert" from unsigned int to uword
  m_K = zeros(sample_size, sample_size);
  m_y = zeros(sample_size);
  ...
  m_K =
      m_K + arma::eye(sample_size, sample_size) * m_variance_in_observed_values;
