...
public:
  ...
  // Set own name
  void setVName(std::string vname) {
    m_vname = vname;
    return;
  };
  void setOwnNumberOfNeighbors(unsigned int n) {
    m_number_of_own_neighbors = n;
    return;
  };
  ...
  bool submitOwnEstimate(std::vector<double> estimate, double time);
  ...
  //    readyToPerformConsensus() - (we have recieved enough messages to
  //    generate the another consensus iteration). If ready, generate a new
  //    consensus from all messages and own estimate using newConsensusItr().
