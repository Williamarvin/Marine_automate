...
bool TrajectTranslate::Iterate() {
  ...
  m_des_traject_vel_x_last +=
      alpha * (des_traject_vel_x - m_des_traject_vel_x_last);

  // Vel_y
  alpha = 1.0 - exp(-1.0 * delta_t / m_time_const_vel_y);
  m_des_traject_vel_y_last +=
      alpha * (des_traject_vel_y - m_des_traject_vel_y_last);

  // Ang_z
  alpha = 1.0 - exp(-1.0 * delta_t / m_time_const_ang_z);
  m_des_traject_ang_z_last +=
      alpha * (des_traject_ang_z - m_des_traject_ang_z_last);

  if (m_include_depth) {
    // Vel_z
    alpha = 1.0 - exp(-1.0 * delta_t / m_time_const_vel_z);
    m_des_traject_vel_z_last +=
        alpha * (des_traject_vel_z - m_des_traject_vel_z_last);
  }
...
bool TrajectTranslate::buildReport() {
  if (checkReadyToGo()) {
    ...
    if (dt_hdg > m_time_window)
      m_msgs << " ** Late DESIRED_HEADING msg, " << doubleToStringX(dt_hdg, 3)
             << "s since last. " << endl;

    double dt_spd = abs(m_desired_speed_time - MOOSTime());
    if (dt_spd > m_time_window)
      m_msgs << " ** Late DESIRED_SPEED msg, " << doubleToStringX(dt_spd, 3)
             << "s since last. " << endl;

    double dt_comp = abs(m_compass_heading_raw_time - MOOSTime());
    if (dt_comp > m_time_window)
      m_msgs << " ** Late " << m_body_heading_var << " msg, "
             << doubleToStringX(dt_comp, 3) << "s since last. " << endl;

    if (m_include_depth) {
      double dt_depth = abs(m_cur_depth_time - MOOSTime());
      if (dt_depth > m_time_window)
        m_msgs << " ** Late " << m_cur_depth_var << " msg, "
               << doubleToStringX(dt_depth, 3) << "s since last. " << endl;

      double dt_des_depth = abs(m_desired_depth_time - MOOSTime());
      if (dt_des_depth > m_time_window)
        m_msgs << " ** Late DESIRED_DEPTH msg, "
               << doubleToStringX(dt_des_depth, 3) << "s since last. " << endl;
    }
  }
  m_msgs << "                                                              "
         << endl;
  m_msgs << "First-Order Filter Time Constants:                            "
         << endl;
  m_msgs << "=============================================================="
         << endl;
  ...
  actab2 << "Last Helm Decision:" << doubleToString(m_desired_speed, 2)
         << doubleToString(m_desired_heading, 2)
         << doubleToString(m_compass_heading_raw, 2);
  if (m_include_depth) {
  ...
  m_msgs << "Time since last output: "
         << doubleToString(MOOSTime() - m_last_publish_time) << "s" << endl;
...
bool TrajectTranslate::checkReadyToGo() {
  ...
  if (!m_got_desired_heading || !m_got_desired_speed ||
      !m_got_compass_heading_raw)
    return (false);
...
void TrajectTranslate::publishTraj() {
  ...
  Notify("DESIRED_VEL_TWIST_ANGULAR_Z",
         m_des_traject_ang_z_last * M_PI / 180.0);
