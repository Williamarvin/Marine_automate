...
public:
  EsriBathyGrid();
  EsriBathyGrid(XYConvexGrid new_grid, double y_origin_lat, double x_origin_lon,
                double no_data_value);
  ~EsriBathyGrid() {}

  bool loadGridFromFile(std::string filename, double x_origin_lon,
                        double y_origin_lat);
  bool saveGridToFile(std::string filename);

  bool updateCellValueXY(double x, double y, std::vector<std::string> cell_vars,
                         std::vector<double> cell_vals);
  bool updateCellValueIDX(unsigned int idx, std::vector<std::string> cell_vars,
                          std::vector<double> cell_vals);

  // Getters
  bool getCellData(unsigned int idx, double &x, double &y, double &depth,
                   double &var) const; // given id; get x, y, depth, variance
  bool getCellXY(unsigned int idx, double &x,
                 double &y) const; // given id; get x, y
  bool getCellDataXY(double x, double y, double &depth,
                     double &var) const; // given xy; get depth, variance
  bool getCellDataXYI(double x, double y, double &depth, double &var,
                      double &i_out) const; // given xy; get id, depth, variance
  bool getCellID(double x, double y, int &cell_id) const; // given xy; get id
  bool getCellCenter(double x, double y, int &cell_id, double &center_x,
                     double &center_y) const; // given xy; get id, center xy

  unsigned long int getNCols() {
    calculateRowsCols();
    return (m_n_cols);
  }
  unsigned long int getNRows() {
    calculateRowsCols();
    return (m_n_rows);
  }
  ...
  std::set<size_t> getFinalObstacles(double depth_threshold,
  ...
  bool getNextCell(double curr_x, double curr_y, double curr_heading,
                   double &new_x, double &new_y, int &new_id);
  bool getAheadIDs(double curr_x, double curr_y, int curr_direction,
                   int (&ahead_i)[3], int (&directions)[3]);
  bool getBehindIDs(double curr_x, double curr_y, int curr_direction,
                    int (&behind_i)[5]);
  ...
  std::set<unsigned int> getCellsBetween(double x1, double y1, double x2,
                                         double y2);
  ...
  // Setters
  void setNCols(unsigned long int ncols) {
    m_n_cols = ncols;
    return;
  }
  void setNRows(unsigned long int nrows) { m_n_rows = nrows; }

  void setXllCorner(double x_crnr) {
    m_x_ll_corner = x_crnr;
    return;
  }
  void setYllCorner(double y_crnr) {
    m_y_ll_corner = y_crnr;
    return;
  }
  ...
  void setNoDataValue(double no_val) {
    m_no_data_value = no_val;
    return;
  }
  void setDefaultVarianceValue(double d_var) {
    m_default_variance = d_var;
    return;
  }
...
private:
  ...
  bool readEsriLine(std::string lineIn, double &lat, double &lon,
                    double &depth);
