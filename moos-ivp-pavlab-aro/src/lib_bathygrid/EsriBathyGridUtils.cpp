...
string getLawnmowerString(EsriBathyGrid grid, int vnum, int v_N,
                          double degrees) {
  // points = format=lawnmower, label=foxtrot, x=$(LAWNMIDDLEX), y=-135,
  // height=170, width=$(LAWNWIDTH), lane_width=10, rows=north-south,
  // startx=$(LAWNSTARTX), starty=$[Y], degs=0
  ...
  cout << "minx " << min_x << " maxx " << max_x << " miny " << min_y << " maxy "
       << max_y << endl;
  ...
  if (remainder == 0) {
    ...
    if (v_N !=
        (vnum - 1)) { // not last vehicle, round columns to get 0 remainder
      ...
      // find x and width
      x = left_bound +
          (max_x - left_bound) / 2; // min_x+(max_x-min_x)*(1+2*v_N)/(2*vnum);
  ...
  lawn += ", rows=north-south, startx=" + to_string(start_x) +
          ", starty=$[Y], degs=0";
...
string getLawnmowerString2(EsriBathyGrid grid, int vnum, int v_N) {
  ...
  double mid_dist =
      sqrt(pow((x_mid_32 - x_mid_01), 2) + pow((y_mid_32 - y_mid_01), 2));
  ...
  if (remainder == 0) {
    ...
    dist_from_left_edge_to_center =
        lm_width / 2.0 + lm_width * static_cast<double>(v_N);
  } else {
    // adjustment needed
    // pretend that max is higher/lower so it is an even split
    ...
    if (v_N != (vnum - 1)) { // not last vehicle,
      ...
      dist_from_left_edge_to_center =
          new_lm_width / 2.0 + new_lm_width * static_cast<double>(v_N);
      lm_width = new_lm_width; // reset the lm width;

    } else { // last vehicle,
      // just cover the remaining space
      double dist_from_left_edge_to_right_edge_of_second_to_last_area =
          new_lm_width * (static_cast<double>(vnum) - 1.0);
      dist_from_left_edge_to_center =
          (dist_from_left_edge_to_right_edge_of_second_to_last_area +
           mid_dist) /
          2.0;
      // reset the lm width to be whatever is left over
      lm_width =
          mid_dist - dist_from_left_edge_to_right_edge_of_second_to_last_area;
    }
  }

  double x_center = x_mid_01 + dist_from_left_edge_to_center / mid_dist *
                                   (x_mid_32 - x_mid_01);
  double y_center = y_mid_01 + dist_from_left_edge_to_center / mid_dist *
                                   (y_mid_32 - y_mid_01);
  ...
  double x_at_dist_03 =
      seg_list.get_vx(0) + dist_from_left_edge_to_center / mid_dist *
                               (seg_list.get_vx(3) - seg_list.get_vx(0));
  double y_at_dist_03 =
      seg_list.get_vy(0) + dist_from_left_edge_to_center / mid_dist *
                               (seg_list.get_vy(3) - seg_list.get_vy(0));
  double x_at_dist_12 =
      seg_list.get_vx(1) + dist_from_left_edge_to_center / mid_dist *
                               (seg_list.get_vx(2) - seg_list.get_vx(1));
  double y_at_dist_12 =
      seg_list.get_vy(1) + dist_from_left_edge_to_center / mid_dist *
                               (seg_list.get_vy(2) - seg_list.get_vy(1));
  double lm_height = sqrt(pow((x_at_dist_03 - x_at_dist_12), 2) +
                          pow((y_at_dist_03 - y_at_dist_12), 2));
  ...
  // double dist_from_left_edge_to_right_lm_edge = lm_width *
  // static_cast<double>(v_N);
  double dist_from_left_edge_to_right_lm_edge =
      dist_from_left_edge_to_center - lm_width / 2.0;
  double start_x =
      seg_list.get_vx(0) + dist_from_left_edge_to_right_lm_edge / mid_dist *
                               (seg_list.get_vx(3) - seg_list.get_vx(0));
  double start_y =
      seg_list.get_vy(0) + dist_from_left_edge_to_right_lm_edge / mid_dist *
                               (seg_list.get_vy(3) - seg_list.get_vy(0));

  // Step 4.  Get the rotation of the grid
  //          Average the slope of the top and bottom
  //          edge
  double angle_top = atan2((seg_list.get_vy(3) - seg_list.get_vy(0)),
                           (seg_list.get_vx(3) - seg_list.get_vx(0)));
  double angle_bottom = atan2((seg_list.get_vy(2) - seg_list.get_vy(1)),
                              (seg_list.get_vx(2) - seg_list.get_vx(1)));
  double angle = (angle_top + angle_bottom) / 2.0;

  // Step 5.  Build the string
  // points = format=lawnmower, label=foxtrot, x=XXXXX, y=XXXXX, height=XXXXXX,
  // width=XXXXXX, lane_width=10, rows=north-south, startx=XXXXX, starty=XXXX,
  // degs=XXXX

  string lawn = "format=lawnmower, label=survey, x=";

  lawn += to_string(x_center) + ", y=" + to_string(y_center);
  lawn += ", height=" + to_string(lm_height) +
          ", width=" + to_string(lm_width - cell_size);
  lawn += ", lane_width=" + to_string(cell_size);
  lawn += ", rows=north-south, startx=" + to_string(start_x) +
          ", starty=" + to_string(start_y);
