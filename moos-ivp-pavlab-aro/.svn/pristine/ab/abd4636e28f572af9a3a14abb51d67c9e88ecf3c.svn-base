...
bool DemusterAssign::Iterate()
{
  ...
  if (!m_calculate_formation) {
    ...
    if ((m_complete_nodes.size()+m_node_permablocks.size()) >= m_node_poses.size() && m_complete_nodes.size() > 0){
  ...
  m_formation = calculateFormation(points, m_distance, m_heading, m_margin, m_circle_radius, m_arrow_angle, m_type, m_CG, m_anchor);
  ...
  if (m_assignment_algorithm == "random"){
    ...
    for (int i = 0; i < m_formation.size(); i++) {
      ...
      for(auto const& node : unassigned_nodes) {
        ...
        dp.findOptimalPath(start_point, start_heading, end_point, end_heading, r, r, r, illegal_paths);
