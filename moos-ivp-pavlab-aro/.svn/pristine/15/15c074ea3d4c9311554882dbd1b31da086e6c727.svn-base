...
vector<Pose> calculateFormation(vector<Point> points, double distance, double heading, double margin, double circle_radius, double arrow_angle, string type, Point CG, Point anchor) {
    ...
    if (type == "line"){
        ...
        // for (int i = 0; i < N; i++) {
        //     while (true) {
        //         //Generate random values for x and y within +/- x_range and y_range from the anchor point
        //         double random_x = anchor_point.x + ((rand() % (int)(2*x_range)) - x_range);
        //         double random_y = anchor_point.y + ((rand() % (int)(2*y_range)) - y_range);

        //         bool valid = true;
        //         for (auto pose : formation) {
        //             if (sqrt(pow(random_x - pose.p.x, 2) + pow(random_y - pose.p.y, 2)) < min_distance) {
        //                 valid = false;
        //                 break;
        //             }
