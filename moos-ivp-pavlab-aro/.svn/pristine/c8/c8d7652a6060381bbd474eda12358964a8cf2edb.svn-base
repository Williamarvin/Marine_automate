...
void BHV_DubinsPath::onIdleState()
{
  ...
  string trajectory_str = dp.findOptimalWaypoints(Point(m_osx, m_osy), os_heading_rad, Point(m_goal_x, m_goal_y), goal_heading_rad, m_r1, m_r2, m_r3, m_precision);
...
IvPFunction* BHV_DubinsPath::onRunState()
{
  ...
  if (!m_path_generated){
    ...
    if (m_op_region.size() == 0){ //No op region
      ...
      string trajectory_str = dp.findOptimalWaypoints(Point(m_osx, m_osy), os_heading_rad, Point(m_goal_x, m_goal_y), goal_heading_rad, m_r1, m_r2, m_r3, m_precision, illegal_paths);
...
IvPFunction *BHV_DubinsPath::buildOF() {
  ...
  //TODO: MAKE SURE THAT IT IS NOT PHYSICALLY BLOCKED, BECAUSE THEN IT SHOULD NOT HAVE MIN 0.1 SPEED
