...
int main() {
  ...
  std::vector<std::string> formation_types = {"vee", "echelon", "ncolumn",
                                              "wedge", "circle"};
      ...
      formation_arguments = {
          ...
          {"echelon",
           {{"echelon_angle", "45"},
            ...
            {"fingered", "true"}}},
          {"ncolumn",
           {{"num_cols", "5"}, {"col_spacing", "5"}, {"row_spacing", "5"}}},
  ...
  for (const auto &formation_type : formation_types) {
    ...
    for (const auto &num_vehicle : num_agents) {
      formationGeometry.generateFormationGeometry(
          formation_type, num_vehicle, formation_arguments[formation_type]);
      std::string line = std::to_string(num_vehicle) + " - " +
                         formationGeometry.repr() + " - " +
                         formationGeometry.getCentroid().repr();
