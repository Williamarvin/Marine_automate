...
bool GenGreedyRescue::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (key == "NAV_X") {
      ...
      string found_id = tokStringParse(
          visit_swimmer, "id", ',', '='); // parses  "id= #" from found message
      m_found_list.insert(found_id); // creates a set of all found ID's. Sets do
                                     // not include duplicates like vectors
      m_swimmer_list.erase(found_id); // removes ID's that have been found from
                                      // the map of swimmers in the water
    }

    else if (key == "SWIMMER_ALERT") {
      string visit_point = msg.GetString();
      string swimmer_id = tokStringParse(
          visit_point, "id", ',', '='); // parses "id= #" from swimmer alert

      // Compares swimmer ID to list of found ID's
      bool found_it = false;
      for (set<string>::iterator it = m_found_list.begin();
           it != m_found_list.end(); it++) {
        if (swimmer_id == *it) {
          found_it = true;
          break;
        }
      }
      if (!found_it) // if the ID is not in the set of found ID's, the ID and
                     // XYPoint are added to map of swimmers in the water
      {
...
bool GenGreedyRescue::Iterate() {
  ...
  if (m_swimmer_list.size() != 0) {
    ...
    for (map<string, XYPoint>::iterator it = m_swimmer_list.begin();
         it != m_swimmer_list.end(); ++it) {
