...
bool SendPoints::Iterate() {
  ...
  for (int i = 0; i < m_number_of_vehicles; i++) {
    if (m_vehicle_awake[m_vehicle_names[i]] == true &&
  ...
  // if all bools in m_initial_points_needed are false, set
  ...
  if ((m_need_to_send) && (m_all_initial_points_sent)) {
    cout << "Sending Requested Point to " + toupper(m_requesting_vehicle)
         << endl;
    XYPoint point = m_region_fld.generatePoint();

    // check to make sure point is at least m_min_dist away from all other
    // points in m_visit_points
      ...
      for (int i = 0; i < m_number_of_vehicles; i++) {
        ...
        for (unsigned int j = 0; j < m_visit_points[vname].size(); j++) {
          double dist = distPointToPoint(point.get_vx(), point.get_vy(),
                                         m_visit_points[vname][j].getX(),
                                         m_visit_points[vname][j].getY());
          if (dist < m_min_dist) {
            good_point = false;
    ...
    Notify("SWIMMER_ALERT_" + toupper(m_requesting_vehicle),
           vpoint.getSwimmerAlert());
    for (int i = 0; i < m_number_of_vehicles; i++) {
      if (m_vehicle_names[i] == m_requesting_vehicle) {
  ...
  for (int i = 0; i < m_number_of_vehicles; i++) {
    string vname = m_vehicle_names[i];
    for (unsigned int j = 0; j < m_visit_points[vname].size(); j++) {
      double dist = distPointToPoint(m_nav_xy_points[vname][0].get_vx(),
                                     m_nav_xy_points[vname][0].get_vy(),
                                     m_visit_points[vname][j].getX(),
                                     m_visit_points[vname][j].getY());
      if (dist < m_capture_radius) {
        Notify("FOUND_SWIMMER_" + toupper(vname),
               "id=" + m_visit_points[vname][j].getID());
