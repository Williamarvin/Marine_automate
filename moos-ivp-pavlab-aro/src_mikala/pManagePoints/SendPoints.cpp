...
bool SendPoints::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (key == "NODE_REPORT") {
      ...
      if (find(m_vehicle_names.begin(), m_vehicle_names.end(), vname) ==
          m_vehicle_names.end()) {
...
bool SendPoints::Iterate() {
  ...
  // Check if a vehicle is awake. If it is, generate m_initial points number of
  // initial points and create an XYSeglist of those points
  for (int i = 0; i < m_number_of_vehicles; i++) {
    string vname = m_vehicle_names[i];
    if (m_vehicle_awake[vname] == true &&
        m_needs_initial_points[vname] == true) {
      ...
      for (unsigned int j = 0; j < m_visit_points[vname].size(); j++) {
        m_visit_point_seglists[vname].add_vertex(
            m_visit_points[vname][j].getX(), m_visit_points[vname][j].getY());
      }
      if (m_visit_point_seglists[vname].get_spec() != "") {
        Notify("SURVEY_UPDATE_" + toupper(vname),
               "points=" + m_visit_point_seglists[vname].get_spec());
        m_needs_initial_points[vname] = false;
        debug1 = "points sent to " + vname;
      }
    }
  }
  // if all vehicles have received their initial points, set
  // m_all_initial_points_sent to true
        ...
        if (checked == true) {
          ...
          for (unsigned int k = 0; k < m_visit_points[vname].size(); k++) {
            if (m_visit_points[vname][k].getState() == "unvisited") {
              unvisited_seglist.add_vertex(m_visit_points[vname][k].getX(),
                                           m_visit_points[vname][k].getY());
              m_map_unvisited_points[vname].push_back(m_visit_points[vname][k]);
            }
          }
          if (unvisited_seglist.get_spec() != "") {
            Notify("SURVEY_UPDATE_" + toupper(vname),
                   "points=" + unvisited_seglist.get_spec());
          }
        }
      }
    }
  ...
  for (int i = 0; i < m_number_of_vehicles; i++) {
    ...
    if (m_map_need_points[vname] == true) {
      for (int j = 0; j < m_minimum_points; j++) {
        ...
        XYSegList unvisited_seglist;
        for (unsigned int k = 0; k < m_map_unvisited_points[vname].size();
             k++) {
          unvisited_seglist.add_vertex(m_map_unvisited_points[vname][k].getX(),
                                       m_map_unvisited_points[vname][k].getY());
        }
        if (unvisited_seglist.get_spec() != "") {
          // Notify("SURVEY_UPDATE_" + toupper(vname), "points=" +
          // unvisited_seglist.get_spec());
...
bool SendPoints::OnStartUp() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    if (param == "initial_points") {
      ...
      handled = true;
      cout << "minimum distance enforced between points (default 10): "
           << m_min_dist << endl;
...
bool SendPoints::buildReport() {
  ...
  m_msgs << "============================================" << endl;
  // generate a table of the vehicle names, if they are awake, if they need
  // initial points, and how many points they have received
  ACTable actab(6);
  actab << "Vehicle"
        << "Awake"
        << "Needs Initial"
  ...
  for (int i = 0; i < m_number_of_vehicles; i++) {
    string vname = m_vehicle_names[i];
    actab << vname << boolToString(m_vehicle_awake[vname])
          << boolToString(m_needs_initial_points[vname])
          << intToString(m_id_counter[vname])
          << intToString(m_unvisited_count[vname])
          << intToString(m_visited_count[vname]);
  }
  ...
  m_msgs << "============================================" << endl;
  m_msgs << "All Intial points received? "
         << boolToString(m_all_initial_points_sent) << endl;
  m_msgs << "============================================" << endl;
  m_msgs << "Minimum Distance" << m_min_dist << endl;
  m_msgs << "============================================" << endl;
  m_msgs << debug1 << endl;
  m_msgs << "Message here if a vehicle has visited all points: " << debug2
         << endl;
...
// generates a new point within the region and ensures the point is atleast
// m_minimum_distance away from all unvisted points
XYPoint SendPoints::generateNewPoint() {
  ...
  for (int i = 0; i < m_number_of_vehicles; i++) {
    ...
    for (unsigned int j = 0; j < m_map_unvisited_points[vname].size(); j++) {
      double dist = distPointToPoint(point.get_vx(), point.get_vy(),
                                     m_map_unvisited_points[vname][j].getX(),
                                     m_map_unvisited_points[vname][j].getY());
      if (dist < m_min_dist) {
...
// checks if a point is within m_capture_radius of a vehicles m_nav_xy_points
// vector
bool SendPoints::checkVisited(VisitPoint point, string vname) {
  bool visited = false;
  for (unsigned int i = 0; i < m_nav_xy_points[vname].size(); i++) {
    double dist = distPointToPoint(point.getX(), point.getY(),
                                   m_nav_xy_points[vname][i].get_vx(),
                                   m_nav_xy_points[vname][i].get_vy());
