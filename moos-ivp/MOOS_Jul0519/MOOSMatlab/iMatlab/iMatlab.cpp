...
// called the fist time iMatlab runs or when 'init' is passed as teh first
// parameter
bool Initialise(int nlhs, mxArray *plhs[], const mxArray *prhs[], int nrhs) {
  ...
  if (!GetParam("CONFIG_FILE", P)) {
    ...
    if (!gConfigReader.SetFile(P.sVal)) {
      ...
      if (!gConfigReader.GetConfiguration(gConfigReader.GetAppName(),
                                          ConfigFileParams)) {
  ...
  // tes 2012-06-20 - write iMatlab configuration to return value [config] =
  // iMatlab('init', ... );
  if (nlhs == 1) {
    // transform STRING_LIST into map of key onto values
    std::map<std::string, std::vector<std::string>> params;
    for (STRING_LIST::const_iterator q = ConfigFileParams.begin();
         q != ConfigFileParams.end(); ++q) {
    ...
    for (std::map<std::string, std::vector<std::string>>::const_iterator
      ...
      mxSetFieldByNumber(
          plhs[0], 0, field,
          mxCreateCharMatrixFromStrings(values.size(), &values[0]));
    }
  }
  ...
  if (gConfigReader.GetConfigurationParam("MOOSComms", sBool)) {
    if (MOOSStrCmp(sBool, "TRUE")) {
      ...
      if (!gConfigReader.GetValue("SERVERHOST", sServerHost)) {
        MOOSTrace("Warning Server host not read from mission file: assuming "
                  "LOCALHOST\n");
  ...
  if (gConfigReader.GetConfigurationParam("SerialComms", sBool)) {
    if (MOOSStrCmp(sBool, "TRUE")) {
      ...
      if (!gPort.Configure(ConfigFileParams)) {
        ...
        MOOSTrace(
            "Beware - this release only supports receiving ASCII strings\n");
      }
    }
  }
...
void iMatlab(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (MOOSStrCmp(sCmd, "INIT")) {
    ...
    if (!bInitialised) {
      MOOSTrace(
          "iMatlab is not initialised - must call \"iMatlab('init')\" first\n");
    }
    /// SENDING MOOS MAIL
    else if (MOOSStrCmp(sCmd, "MOOS_MAIL_TX")) {
      if (nrhs < 3) {
        MOOSTrace("Incorrect format : 'MOOS_MAIL_TX','VAR_NAME'.VarVal (string "
                  "or double)\n");
        return;
      }
      std::string sKey;
      if (!Matlab2String(sKey, prhs[1])) {
        mexErrMsgTxt(
            "Param 2 (key) must be a string name of the data being sent\n");
      }
      ...
      if (pComms && pComms->IsConnected()) {
        ...
        if (Matlab2String(sTmp, prhs[2])) {
          ...
          mexErrMsgTxt("MOOS transmit failed parameter 3 must be a string or "
                       "double data value\n");
        }
      } else {
        ...
        if (pComms->Fetch(NewMail)) {
          ...
          if (nlhs == 1) {
            ...
            plhs[0] = mxCreateStructArray(
                2, DimArray, sizeof(FieldNames) / sizeof(char *), FieldNames);
            ...
            for (p = NewMail.begin(); p != NewMail.end(); p++, i++) {
              ...
              mxSetFieldByNumber(plhs[0], i, nKeyField,
                                 mxCreateString(p->m_sKey.c_str()));

              // copy in the type
              char *pType =
                  (char *)(p->IsDataType(MOOS_DOUBLE) ? "DBL" : "STR");
              mxSetFieldByNumber(plhs[0], i, nTypeField, mxCreateString(pType));

              // copy in time
              mxSetFieldByNumber(plhs[0], i, nTimeField,
                                 mxCreateDoubleScalar(p->GetTime()));

              // copy in sVal
              mxSetFieldByNumber(plhs[0], i, nStrField,
                                 mxCreateString(p->m_sVal.c_str()));

              // copy in dfVal
              mxSetFieldByNumber(plhs[0], i, nDblField,
                                 mxCreateDoubleScalar(p->GetDouble()));

              // copy in src process
              mxSetFieldByNumber(plhs[0], i, nSrcField,
                                 mxCreateString(p->m_sSrc.c_str()));

              // copy in originating community
              mxSetFieldByNumber(
                  plhs[0], i, nCommunityField,
                  mxCreateString(p->m_sOriginatingCommunity.c_str()));
            }
          } else {
            MOOSTrace("Picked up %d MOOSMsgs - but no output variables to "
                      "return them in!\n",
                      nMsgs);
          }
    ...
    // REGISTERING FOR MAIL
      ...
      if (nrhs != 3) {
        MOOSTrace(
            "incorrect format. Use iMatlab('MOOS_REGISTER','WHAT',HOW_OFTEN\n");
        return;
      } else {
        ...
        if (!Matlab2Double(dfHowOften, prhs[2])) {
          MOOSTrace("incorrect format parameter 3 must be a double (min period "
                    "between messages)\n");
          return;
        }
    ...
    // PAUSING
      ...
      if (!Matlab2Double(dfT, prhs[1])) {
        MOOSFail(
            "incoorect MOOS_PAUSE format - param 2 must be  numeric seconds\n");
        return;
      }
    ...
    // SENDING SERIAL DATA
      ...
      if (GetParam("SERIAL") == 0.0) {
        ...
        default:
          MOOSTrace("Problem: cannot send data of type \"%s\" must be UINT8 ot "
                    "CHAR\n",
    ...
    // RECEIVING SERIAL DATA
      ...
      if (GetParam("SERIAL") == 0.0) {
        ...
        if (gPort.IsStreaming()) {
          ...
          // just read one
          if (gPort.GetTelegram(sRx, GetParam("SERIAL_TIMEOUT").dfVal,
                                &dfTime)) {
            RxL.push_back(TG(sRx, dfTime));
          }
        }
        ...
        if (nlhs > 0) {
          ...
          for (unsigned int i = 0; i < RxL.size(); i++) {
            ...
            mxSetFieldByNumber(plhs[0], i, nStrField,
                               mxCreateString(RxL[i].first.c_str()));
