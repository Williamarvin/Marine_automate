...
bool CMOOSGeodesy::Initialise(double lat, double lon) {
  ...
  char utmZone[4];
  LLtoUTM(m_iRefEllipsoid, m_dOriginLatitude, m_dOriginLongitude, tempNorth,
          tempEast, utmZone);
...
double CMOOSGeodesy::GetMetersNorth() { return m_dNorth; }

double CMOOSGeodesy::GetMetersEast() { return m_dEast; }
...
void CMOOSGeodesy::SetMetersNorth(double North) { m_dNorth = North; }

void CMOOSGeodesy::SetMetersEast(double East) { m_dEast = East; }
...
int CMOOSGeodesy::GetRefEllipsoid() { return m_iRefEllipsoid; }
...
char *CMOOSGeodesy::GetUTMZone() { return m_sUTMZone; }
 ...
 *Therefore, at Lat/Lon values that are significantly far enough (~300km) away
 *from the origin of the UTM grid (0,0), a shift in one dimension, i.e. just
 *along Latitude, or just along Longitude, does not map to a corresponding one
 ...
 *observed that moving just .0001 degrees in Longitude (~1m in local) results in
 *both a deltaX that is coupled to a deltaY.
 *
...
bool CMOOSGeodesy::LatLong2LocalUTM(double lat, double lon, double &MetersNorth,
                                    double &MetersEast) {
  ...
  // This does not need to be split like into before and after first reading,
  // but makes the calculation clearer.  Plus, can add other features like
  // logging or publishing of value
...
bool CMOOSGeodesy::LatLong2LocalGrid(double lat, double lon,
  ...
  double dfRadius =
      dfb * sqrt(1 + dftanlat2) / sqrt((pow(dfb, 2) / pow(dfa, 2)) + dftanlat2);
...
void CMOOSGeodesy::SetLocalGridX(double X) { m_dLocalGridX = X; }

void CMOOSGeodesy::SetLocalGridY(double Y) { m_dLocalGridY = Y; }

double CMOOSGeodesy::GetLocalGridX() { return m_dLocalGridX; }

double CMOOSGeodesy::GetLocalGridY() { return m_dLocalGridY; }
 ...
 *@param dfEast  The current local grid distance in meters traveled East (X dir)
 *wrt to Origin
 *@param dfNorth The current local grid distance in meters traveled North (Y
 *dir) wrt to Origin
 *@param dfLat the calculated latitude out
 *@param dfLon the calculated longitude out
 */

bool CMOOSGeodesy::LocalGrid2LatLong(double dfEast, double dfNorth,
  ...
  double dfXArcRad =
      asin(dfEast / (dfRadius * cos(GetOriginLatitude() * deg2rad)));
  ...
  // fix to segfault issue if you get diverging values (BUGFIX! Sep1822 Jared
  // Silbermann)
...
bool CMOOSGeodesy::UTM2LatLong(double dfX, double dfY, double &dfLat,
                               double &dfLong) {
