...
void PrintHelpAndExit() {
  std::cerr << "\nA program which captures times of dispatch and arrival of "
               "named messages to file.\n\n";
...
std::string GetTimeStampString() {
  ...
  // 14_5_1993_____9_30
  sprintf(sTmp, "_%d_%d_%d_____%.2d_%.2d.%2d", Now->tm_mday, Now->tm_mon + 1,
...
void LogMessage(CMOOSMsg &M) {
  ...
  if (dfDelay > 30e-3) {
    std::cerr << GetTimeStampString() << " " << MOOSTime() - dfStartTime
              << " ouch! delay is " << dfDelay << "\n";
  }
  if (bLog) {
    std::stringstream ss;
