...
void PrintHelpAndExit() {
  std::cout << "This program simply send/receives a hundred or so bytes over a "
               "vanilla tcp/ip socket. "
               "It is provided as a baseline for MOOS communications "
               "performance. MOOS will never be faster than this. "
               "All, transit times (Tx->Rx) or latencies, are logged to file "
  ...
  std::cout << "	--port                port to send to or receive on "
               "(default 9007)\n";
  std::cout
      << "	--threshold=<float>   latency (s) above which print alert\n";
  std::cout
      << "	--log_file=string     file to log to (default cmp2sock.txt)\n";
  std::cout << "Example:\n";
