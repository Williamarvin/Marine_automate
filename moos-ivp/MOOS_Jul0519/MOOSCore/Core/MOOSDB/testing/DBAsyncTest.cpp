...
bool _OnConnectNULL(void * /*pParam*/) { return true; }
...
bool _OnMail(void *pParam) {
  ...
  for (q = M.begin(); q != M.end(); ++q) {
    if (!q->IsSkewed(dfNow) && bEnableCapture) {
      ...
      // gPrinter.Print(MOOSFormat("%s [%3d]
      // lag:%.3f",pC->GetMOOSName().c_str(),k++,dfLagMS));
    }
  }
...
void PrintHelpAndExit() {
  ...
  MOOSTrace("  -p=<numeric>              : test period in seconds (20 seconds "
            "default)\n");
  MOOSTrace("  -m=<numeric>              : send test data every m milliseconds "
            "(default 100 ms) \n");
  MOOSTrace("  -c=<numeric>              : number of clients to instantiate "
            "(default 40)\n");
  MOOSTrace("  -s=<numeric>              : size of data to send default "
            "(default 1024 bytes) \n");

  MOOSTrace("\n\nExample Usage:\n");
  MOOSTrace(
      " test for 15 seconds with 20 clients and sending 100K every 50 ms\n");
...
int main(int argc, char *argv[]) {
  ...
  while ((MOOSLocalTime() - dfStart) < dfTestPeriod) {
    if (i % 10 == 0)
      MOOSTrace("%4d messages sent %.1f seconds to go\r", i,
                dfTestPeriod - (MOOSLocalTime() - dfStart));
  ...
  std::cerr << std::setw(15) << "Client" << std::setw(15) << "mean latency (ms)"
            << std::setw(15) << "#msgs" << std::endl;
  ...
  for (q = LagLogs.begin(); q != LagLogs.end(); ++q) {
    ...
    logfile << q->first << " ";
    std::copy(rV.begin(), rV.begin() + umin,
              std::ostream_iterator<double>(logfile, " "));
    logfile << std::endl;

    std::cerr << std::setw(15) << q->first << std::setw(15) << std::fixed
              << std::setprecision(2) << dfAv << std::setw(15) << rV.size()
              << std::endl;
  }

  std::cout << "detailed log written to "
            << "asynctest.log\n";
  std::cout << num_clients << " clients handling  " << payload_size
            << " byte packets at " << 1000.0 / message_period << " Hz\n";
}
