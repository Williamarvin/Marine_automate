...
void PrintStatistics() {
  ...
  for (q = stat_list.begin(); q != stat_list.end(); ++q) {
    ...
    ss << std::setw(15) << "\"" << q->name << "\""
       << " from " << q->source << " to " << q->destination << "\n";
    std::cerr << ss.str();
  }
}
...
void AccummulateStatistic(MOOS::EndToEndAudit::MessageStatistic &ms) {
  std::string key =
      ms.message_name + ":" + ms.source_client + ":" + ms.destination_client;
  ...
  if (q == atm_stats.end()) {
    ...
    stat.average_bandwidth =
        (1.0 - kBandwidthFIRConstant) * stat.average_bandwidth +
...
void PrintHelpAndExit() {
  ...
  std::cerr
      << "--channel=<address>     address to listen  for audit comms on \n";
...
void OpenLogFile(std::ofstream &ofs, const std::string &logging_directory,
  ...
  if (logfile_name.empty()) {
    log_file_path =
        logging_directory + "/" + MOOSGetTimeStampString() + ".moos_audit.log";
  } else {
...
void PrettyPrint(std::ostream &output_stream,
                 const MOOS::EndToEndAudit::MessageStatistic &ms) {
  static double last = ms.source_time;
  output_stream << std::left << std::setw(7) << std::setprecision(12)
                << ms.source_time - last << " ";
  output_stream << std::left << std::setw(7) << std::setprecision(4);
  output_stream << (ms.receive_time - ms.source_time) / 1000.0 << " ms delay ";
  output_stream << ms.source_client << " |----  " << ms.message_name << "["
                << ms.message_size << "]  ----> " << ms.destination_client
                << std::endl;
  last = ms.source_time;
}

void LogToFile(std::ofstream &output_stream,
               const MOOS::EndToEndAudit::MessageStatistic &ms) {
  ...
  if (lines_logged++ % kFlusheveryN == 0) {
    output_stream.flush();
    std::cerr << "\r" << std::left << std::setw(15) << lines_logged
...
bool ShouldBeLogged(
    const MOOS::EndToEndAudit::MessageStatistic &message_statistic,
    int64_t ignore_delays_below) {

  int64_t delta =
      message_statistic.receive_time - message_statistic.source_time;
...
int main(int argc, char *argv[]) {
  ...
  if (logging) {
    OpenLogFile(output_stream, logging_directory, log_file_name);
  ...
  while (1) {
    ...
    if (MOOSTime() - last_display_time > 3.0) {
      ...
      std::cerr << MOOSFormat("processed %.8d messages and logged %.6d. "
                              "Mean delay %.2fuS\n",
                              number_messages_rxed, number_messages_logged,
                              average_latency_us);
