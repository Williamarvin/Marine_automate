...
public:
  ...
  bool Iterate() {
    ...
    if (time_expired > kSamplePeriod) {
      ...
      std::cerr << "sampling....please  wait " << std::setprecision(1)
                << 5.0 - time_expired << " seconds\r";
    }
  ...
  void UpdateTopology() {
    ...
    for (q = statuses_.begin(); q != statuses_.end(); ++q) {
      ...
      for (s = qStatus.subscribes_.begin(); s != qStatus.subscribes_.end();
           ++s) {
        message &rmessage = known_messages_[*s];
  ...
  void PrintMessageRoute(std::ofstream &f, const std::string &src,
                         const std::string &dest, const std::string &name,
                         bool dotted, bool ommit_leg_1) {
    ...
    if (!ommit_leg_1) {
      f << "\"" << src << "\""
        ...
        << "\"" << name << "\""
        << "->"
        << "\"" << dest << "\"" << (dotted ? "[style=dotted]" : "[style=solid]")
        << "\n";
    } else {
      f << "\"" << name << "\""
        << "->"
  ...
  void PrintTopologyToFile() {
    ...
    for (q = statuses_.begin(); q != statuses_.end(); ++q) {
      ...
      for (s = qStatus.publishes_.begin(); s != qStatus.publishes_.end(); ++s) {
        ...
        for (w = rmessage.consumers_.begin(); w != rmessage.consumers_.end();
             ++w) {
    ...
    for (m = known_messages_.begin(); m != known_messages_.end(); ++m) {
      message &rmessage = m->second;
      if (rmessage.consumers_.empty()) {
        rmessage.has_no_consumer_ = true;
        std::set<std::string>::iterator w;
        for (w = rmessage.producers_.begin(); w != rmessage.producers_.end();
             w++) {
          ...
          PrintMessageRoute(output_file, *w, kNoConsumer, m->first, true,
                            false);
        }
      }
      ...
      if (rmessage.producers_.empty()) {
        ...
        for (w = rmessage.consumers_.begin(); w != rmessage.consumers_.end();
          ...
          PrintMessageRoute(output_file, kNoProducer, *w, m->first, true,
                            false);
        }
      }
    }
    ...
    for (mm = known_messages_.begin(); mm != known_messages_.end(); ++mm) {
...
protected:
  ...
  struct message {
    ...
    explicit message(const std::string &s)
        : name_(s), has_no_consumer_(false), has_no_producer_(false) {}
    std::string name_;
