...
unsigned int GetScreenWidth() { return 80; }
...
public:
  ...
  bool Worker() {
    ...
    while (!thread_.IsQuitRequested()) {
      ...
      iterations = std::max(
          (int64_t)0, iterations + (int64_t)(gain * (target_load_ - load_now)));
      double dummy = 0;
...
void PrintHelp() {
  ...
  MOOSTrace("  -s=<string>            : list of subscriptions in form "
            "var_name@period eg -s=x,y,z\n");
  MOOSTrace(
      "  -w=<string>            : list of wildcard subscriptions in form "
      "var_pattern:app_pattern@frequency_hz eg -w='x*:*@0.1,*:GPS@0.0' \n");
  MOOSTrace(
      "  -p=<string>            : list of publications in form "
      "var_name[:optional_binary_size]@frequency_hz eg -p=x@0.5,y:2048@2.0\n");
  MOOSTrace(
      "  -b=<string>            : bounce or reflect variables back under new "
      "name using syntax name:new_name,{name:new_name] eg -b=x:y,u:v\n");

  MOOSTrace(
      "  --num_tx=<integer>     : only send \"integer\" number of messages\n");
  MOOSTrace("  --latency              : show latency (time between posting and "
            "receiving)\n");
  MOOSTrace("  --bandwidth            : print bandwidth\n");
  MOOSTrace("  --skew                 : print timing adjustment relative to "
            "the MOOSDB\n");
  ...
  MOOSTrace("  --spy_proc=<string>    : spy on all variables from a named "
            "processes\n");

  MOOSTrace("\n\nNetwork failure simulation:\n");
  MOOSTrace("  --simulate_network_failure             : enable simulation of "
            "network/app failure\n");
  MOOSTrace("  --network_failure_prob=<numeric>       : probability of each DB "
            "interaction having network failure [0.1]\n");
  MOOSTrace("  --network_failure_time=<numeric>       : duration of network "
            "failure [3s]\n");
  MOOSTrace("  --application_failure_prob=<numeric>   : probability of "
            "application failing during DB-communication [0]\n");
  MOOSTrace("  --cpu_load=<numeric>                   : percentage cpu loading "
            "to simulate (400% takes 4 cores etc.. )  [0]\n");

  MOOSTrace("\n\nExample Usage:\n");
  MOOSTrace("  ./umm --moos_name=C1 --moos_apptick=30  -s=x -p=y@0.5,z@2.0\n");
  MOOSTrace(
      "  ./umm C2 --simulate_network_failure --network_failure_prob=0.05 "
      "--network_failure_time=10.0 --application_failure_prob=0.05 -s=z \n");
  MOOSTrace(
      "  ./umm --moos_name=C1 --moos_apptick=10  -s=x -p=y:4567@0.5,z@2.0\n");
...
public:
  ...
  bool OnProcessCommandLine() {
    ...
    m_CommandLineParser.GetVariable("--network_failure_prob",
                                    _NetworkStallProb);

    _NetworkStallTime = 0.0;
    m_CommandLineParser.GetVariable("--network_failure_time",
                                    _NetworkStallTime);

    _ApplicationExitProb = 0.0;
    m_CommandLineParser.GetVariable("--application_failure_prob",
                                    _ApplicationExitProb);
    ...
    _SimulateNetworkFailure =
        m_CommandLineParser.GetFlag("-N", "--simulate_network_failure");
    ...
    for (q = vPublish.begin(); q != vPublish.end(); ++q) {
      ...
      if (!sEntry.empty()) {
        if (!MOOSIsNumeric(sEntry)) {
          ...
          std::cerr << MOOS::ConsoleColours::red()
                    << "badly formatted publish directive name1@frequency "
                       "name2@frequency 2\n"
      ...
      if (nArraySize == 0) {
        ...
        if (!m_bQuiet)
          std::cout << MOOS::ConsoleColours::Green() << "+Publishing " << sName
                    << " at " << 1.0 / dfPeriod << " Hz\n"
                    << MOOS::ConsoleColours::reset();
      } else {
  ...
  bool OnStartUp() {
    if (_SimulateNetworkFailure) {
      ...
      m_Comms.ConfigureCommsTesting(_NetworkStallProb, _NetworkStallTime,
                                    _ApplicationExitProb);
    }
  ...
  bool OnNewMail(MOOSMSG_LIST &NewMail) {
    ...
    for (q = NewMail.begin(); q != NewMail.end(); ++q) {
      ...
      if (_bVerbose) {
        ...
        std::cout << std::left << std::setw(20) << std::setprecision(15)
                  << q->GetTime();
      ...
      if (_bShowLatency) {
        std::cout << MOOS::ConsoleColours::cyan()
                  << "\nMOOS::Time Latency: " << std::setprecision(3)
                  << dfLatencyMS << " ms\n";
        std::cout << MOOS::ConsoleColours::cyan()
                  << "                Tx: " << std::setw(20)
                  << std::setprecision(14) << q->GetTime() << "\n";
        std::cout << MOOS::ConsoleColours::cyan()
                  << "                Rx: " << std::setw(20)
                  << std::setprecision(14) << MOOS::Time() << "\n";
        std::cout << MOOS::ConsoleColours::cyan()
        ...
        if (std::fabs(GetMOOSTimeWarp() - 1.0) > 1e-3) {
          std::cout << MOOS::ConsoleColours::cyan()
                    << "Abs::Time  Latency: " << std::setprecision(3)
                    << dfLatencyMS / GetMOOSTimeWarp() << " ms\n";
          std::cout << MOOS::ConsoleColours::cyan()
                    << "                Tx: " << std::setw(20)
                    << std::setprecision(14) << q->GetTime() / GetMOOSTimeWarp()
                    << "\n";
          std::cout << MOOS::ConsoleColours::cyan()
                    << "                Rx: " << std::setw(20)
                    << std::setprecision(14) << MOOS::Time() / GetMOOSTimeWarp()
                    << "\n";
          std::cout << MOOS::ConsoleColours::cyan()
                    << "              mean: " << std::setprecision(3)
                    << _dfMeanLatency / GetMOOSTimeWarp() << " ms\n\n";
        }
      ...
      if (_LogFile) {
        ...
        _LogFile << std::left << std::setw(20) << std::setprecision(14)
                 << q->GetTime();
        _LogFile << std::left << std::setw(20) << std::setprecision(14)
                 << MOOS::Time();
      ...
      if (_bPing && q->GetKey().find("__ping") == 0) {
        if (!q->IsSkewed(MOOS::Time())) {
          ...
          std::cerr << "time=" << std::setprecision(3) << dfLatencyMS
                    << " ms \n";
    ...
    if (MOOSLocalTime(false) - dfT > 1.0) {
      ...
      if (_bShowBandwidth) {
        ...
        std::cout << MOOS::ConsoleColours::green()
                  << "Incoming: " << std::setw(8)
                  << 8 * (bi - nByteInCounter) / (1024.0 * 1024.0)
                  << "  Mb/s  ";
        std::cout << MOOS::ConsoleColours::Green()
                  << "Outgoing: " << std::setw(8)
                  << 8 * (bo - nByteOutCounter) / (1024.0 * 1024.0)
                  << "  Mb/s\r";
        std::cout << MOOS::ConsoleColours::reset();
        std::flush(std::cout);
      }
      ...
      if (_bShowTimingAdjustment) {
        std::cout << MOOS::ConsoleColours::magenta()
                  << "timing correction : " << std::left << std::setw(8)
                  << 1e6 * GetMOOSSkew() << "us \n";
        std::cout << MOOS::ConsoleColours::reset();
      }
    ...
    for (w = _vWildSubscriptions.begin(); w != _vWildSubscriptions.end(); ++w) {
      ...
      if (!sEntry.empty()) {
        if (MOOSIsNumeric(sEntry)) {
          ...
          std::cerr
              << MOOS::ConsoleColours::red() << *w
              << " does not have format var_pattern:app_pattern[@period]\n";
          continue;
        }
      } else {
    ...
    while (!Scheduler.IsQuitRequested()) {
      while (!_Jobs.empty() && _Jobs.top().isActive(MOOS::Time())) {
        ...
        if (Active.IsBinary()) {
          ...
          Notify(Active._sName, &_BinaryArray[0], Active._DataSize,
                 MOOS::Time());
        } else {
...
private:
  ...
  struct Job {
    Job(double dfPeriod, const std::string &sName)
        : _dfPeriod(dfPeriod), _sName(sName), _nCount(0), _DataSize(0) {
      _dfTimeScheduled = MOOSLocalTime() + _dfPeriod;
    }

    Job(double dfPeriod, const std::string &sName, unsigned int nSize)
        : _dfPeriod(dfPeriod), _sName(sName), _nCount(0) {
      _DataSize = nSize;
      _dfTimeScheduled = MOOSLocalTime() + _dfPeriod;
    }
    ...
    bool IsBinary() { return _DataSize != 0; }
    ...
    bool isActive(double TimeNow) const { return _dfTimeScheduled < TimeNow; }
