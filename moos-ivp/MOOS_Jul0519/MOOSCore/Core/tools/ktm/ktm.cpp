...
std::string multicast_address =
    MOOS::SuicidalSleeper::GetDefaultMulticastAddress();
...
void PrintHelpAndExit() {
  ...
  std::cerr << "--phrase=<string>      pass phrase which Suicide listeners are "
               "keyed to\n";
...
int main(int argc, char *argv[]) {
  ...
  if (P.GetFlag("--all")) {
    std::cerr << MOOS::ConsoleColours::Red()
              << "THIS WILL KILL ALL APPS WITHIN 1 HOP...[N/y]\n";
  ...
  if (bQuery) {
    std::cerr << "searching for suicidal sleepers on " << multicast_address
              << ":" << multicast_port << "\n";
  } else {
    // std::cerr<<"sending suicide instruction to "<<multicast_address<<":"<<
    // multicast_port<<"\n";
    //  Change by mikerb Apr 09, 2021
    std::cerr << "sending self-exit instruction to " << multicast_address << ":"
              << multicast_port << "\n";
  }
  ...
  while (FDUPA.Read(sReply, 1000)) {
    ...
    std::cout << (bQuery ? MOOS::ConsoleColours::Yellow()
                         : MOOS::ConsoleColours::Red())
              << sReply;
  }
