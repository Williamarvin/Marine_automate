...
bool CMOOSSerialPort::StartThreads() {
  ...
  m_hCommsThread = ::CreateThread(NULL, 0, CommsLoopProc, this,
                                  CREATE_SUSPENDED, &m_nCommsThreadID);
...
void CMOOSSerialPort::SetIsCompleteReplyCallBack(bool (*pfn)(char *pData,
                                                             int nLen,
                                                             int nRead)) {
  ...
  while (!m_bQuit) {
    ...
    if (nRead != -1) {
      ...
      do {
        ...
        pTelegramEnd =
            (char *)memchr(pAccumulator, GetTermCharacter(), nInStore);
        ...
        if (pTelegramEnd != NULL) {
          ...
          // shuffle accumulator
          memmove(pAccumulator, pAccumulator + nTelegramLength,
...
bool CMOOSSerialPort::IsStreaming() { return m_bStreaming; }
 ...
 */
char CMOOSSerialPort::GetTermCharacter() { return m_cTermCharacter; }

int CMOOSSerialPort::ReadNWithTimeOut(char *pData, int nLen, double dfTimeOut,
                                      double *pTime) {
...
int CMOOSSerialPort::ReadNWithTimeOut2(char *pData, int nLen, double dfTimeOut,
                                       double *pTime) {
...
bool CMOOSSerialPort::GetTelegram(std::string &sTelegram, double dfTimeOut,
                                  double *pTime) {
  if (IsStreaming()) {
  ...
  while ((dfTimeWaited < dfTimeOut) && nRead < TELEGRAM_LEN) {
    ...
    if (nGrabbed == 0) {
      ...
      if (IsCompleteReply(pData, TELEGRAM_LEN, nRead)) {
        ...
        // MOOSTrace("Required %d retries and %d
...
std::string CMOOSSerialPort::GetPortName() { return m_sPort; }
