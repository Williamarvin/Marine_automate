...
std::string TimeToDate(double dfTime, bool bDate, bool bTime) {
  ...
  if (!nowtm) // This happens e.g. when this is called with dfTime<0 (it really
              // happens in MOOSDB!)
  {
  ...
  snprintf(stimeall, sizeof stimeall, "%s.%03d", stime,
...
std::vector<std::string> StringListToVector(std::string L,
...
double StringToDouble(const std::string &sNum) {
  if (!MOOSIsNumeric(sNum))
    throw std::runtime_error("MOOS::StringToDouble: " + sNum +
                             " is not a number");
...
double GetMOOSSkew() { return gdfMOOSSkew; }
...
double MOOSLocalTime(bool bApplyTimeWarping) {
  ...
  if (gbWin32HPTiming) {
    ...
    if (!bHPTimeInitialised) {
      ...
      double T = dfMOOSStart + (double)(liNow.QuadPart - liStart.QuadPart) /
                                   ((double)(liPerformanceFreq.QuadPart));
...
double GetMOOSTimeWarp() { return gdfMOOSTimeWarp; }
...
size_t MOOSStrFind(const std::string &sSource, const std::string &sToken,
                   bool bInsensitive) {
  if (sToken.empty())
    return std::string::npos;
  if (bInsensitive) {
    std::string::const_iterator q =
        std::search(sSource.begin(), sSource.end(), sToken.begin(),
                    sToken.end(), CompareInsensitive());
    if (q == sSource.end())
      return std::string::npos;
    else {
...
bool MOOSValFromString(string &sVal, const string &sStr, const string &sTk,
  ...
  while ((nPos = MOOSStrFind(sStr.substr(k), sTk, bInsensitive)) !=
    ...
    // we need to be carefull here = there could be many spaces between token
    // and =
    /*unsigned int*/
    ...
    if (nLastComma == std::string::npos) {
      ...
      // starting from previous comma was when is the first non white space
      // char?
    ...
    if (nEqualsPos != string::npos) {
      ...
      std::string t =
          sStr.substr(nPos + sTk.size(), nEqualsPos - (nPos + sTk.size()));
...
bool MOOSValFromString(unsigned int &nVal, const string &sStr,
                       const string &sTk, bool bInsensitive) {
  int nIntVal;
...
// bool MOOSValFromString(long  & nVal,const string & sStr,const string &
// sTk,bool bInsensitive)
//{
//     string sVal;
//
//     if(MOOSValFromString(sVal,sStr,sTk,bInsensitive))
...
bool MOOSVectorFromString(const string &sStr, std::vector<double> &dfValVec,
                          int &nRows, int &nCols) {
...
bool MOOSVectorFromString(const string &sStr, std::vector<float> &fValVec,
                          int &nRows, int &nCols) {
...
bool MOOSVectorFromString(const string &sStr,
                          std::vector<unsigned int> &nValVec, int &nRows,
                          int &nCols) {
...
bool MOOSValFromString(std::vector<double> &dfValVec, int &nRows, int &nCols,
                       const std::string &sStr, const std::string &sToken,
                       bool bInsensitive) {
...
bool MOOSValFromString(std::vector<unsigned int> &nValVec, int &nRows,
                       int &nCols, const std::string &sStr,
                       const std::string &sToken, bool bInsensitive) {
...
bool MOOSValFromString(int64_t &nVal, const string &sStr, const string &sTk,
                       bool bInsensitive) {
  std::string sVal;
...
bool GetNextAlogLineByMessageName(std::istream &Input,
                                  ...
                                  double &dfTime, std::string &sSource,
                                  std::string &sPayload) {
...
string MOOSGetTimeStampString(double t) {
  ...
  // 14_5_1993_____9_30
  sprintf(sTmp, "_%d_%d_%d_____%.2d_%.2d", Now->tm_mday, Now->tm_mon + 1,
...
void MOOSTrace(string sStr) { MOOSTrace("%s", sStr.c_str()); }
...
void MOOSTrace(const char *FmtStr, ...) {
  ...
  if (strlen(FmtStr) < MAX_TRACE_STR) {
    ...
    // arh changed this because if you wanted to add a percent character in the
    // string, it would first be processed by the _vsnprintf above, then placed
    // in 'buf'. Problem is that fprintf finds the '%' in buf and expects us to
...
double MOOSDeg2Rad(double dfDeg) { return dfDeg * PI / 180.0; }

double MOOSRad2Deg(double dfRad) { return dfRad * 180.0 / PI; }

bool MOOSGetValueFromToken(STRING_LIST &sParams, const string &sToken,
                           string &sVal) {
...
string MOOSThirdPartyActuationString(double *pdfRudder, double *pdfElevator,
                                     double *pdfThrust) {
  ostringstream os;
...
double MOOSNormalInv(double dfArea) {
  double dfNormInv[] = {
      0,       -2.3263, -2.0537, -1.8808, -1.7507, -1.6449, -1.5548, -1.4758,
      -1.4051, -1.3408, -1.2816, -1.2265, -1.1750, -1.1264, -1.0803, -1.0364,
      -0.9945, -0.9542, -0.9154, -0.8779, -0.8416, -0.8064, -0.7722, -0.7388,
      -0.7063, -0.6745, -0.6433, -0.6128, -0.5828, -0.5534, -0.5244, -0.4959,
      -0.4677, -0.4399, -0.4125, -0.3853, -0.3585, -0.3319, -0.3055, -0.2793,
      -0.2533, -0.2275, -0.2019, -0.1764, -0.1510, -0.1257, -0.1004, -0.0753,
      -0.0502, -0.0251, 0,       0.0251,  0.0502,  0.0753,  0.1004,  0.1257,
      0.1510,  0.1764,  0.2019,  0.2275,  0.2533,  0.2793,  0.3055,  0.3319,
      0.3585,  0.3853,  0.4125,  0.4399,  0.4677,  0.4959,  0.5244,  0.5534,
      0.5828,  0.6128,  0.6433,  0.6745,  0.7063,  0.7388,  0.7722,  0.8064,
      0.8416,  0.8779,  0.9154,  0.9542,  0.9945,  1.0364,  1.0803,  1.1264,
      1.1750,  1.2265,  1.2816,  1.3408,  1.4051,  1.4758,  1.5548,  1.6449,
      1.7507,  1.8808,  2.0537,  2.3263,  0};
...
bool MOOSFileParts(std::string sFullPath, std::string &sPath,
                   std::string &sFile, std::string &sExtension) {
  /*unsigned int*/ size_t nBreak;
  /*unsigned int*/ size_t nFS = sFullPath.find_last_of("/");

#ifdef _WIN32
  // Windows user use either forward ot backslash to delimit (or even a
  // combination...)
