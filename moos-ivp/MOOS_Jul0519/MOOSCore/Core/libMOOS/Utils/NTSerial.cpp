...
CNTSerial::EPort CNTSerial::CheckPort(LPCTSTR lpszDevice) {
  ...
  HANDLE hFile = ::CreateFile(lpszDevice, GENERIC_READ | GENERIC_WRITE, 0, 0,
...
LONG CNTSerial::Open(LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue) {
  ...
  // Open the device
  m_hFile = ::CreateFile(lpszDevice, GENERIC_READ | GENERIC_WRITE, 0, 0,
                         OPEN_EXISTING, FILE_FLAG_OVERLAPPED, 0);
...
LONG CNTSerial::ClosePort(void) {
  ...
  if (m_hFile == 0) {
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::Close - Method called when device is not open\n");
    return m_lLastError;
  }
...
LONG CNTSerial::Setup(EBaudrate eBaudrate, EDataBits eDataBits, EParity eParity,
                      EStopBits eStopBits) {
  // Reset error state
  m_lLastError = ERROR_SUCCESS;
...
LONG CNTSerial::SetEventChar(BYTE bEventChar, bool fAdjustMask) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::SetEventChar - Unable to obtain DCB information\n");
    return m_lLastError;
  }
  ...
  if (!::SetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::SetEventChar - Unable to set DCB information\n");
    return m_lLastError;
  }
...
LONG CNTSerial::WaitEvent(LPOVERLAPPED lpOverlapped, DWORD dwTimeout) {
  ...
  if (!::WaitCommEvent(m_hFile, LPDWORD(&m_eEvent), lpOverlapped)) {
    ...
    if (lLastError != ERROR_IO_PENDING) {
      ...
      // Issue an error and quit
      _RPTF0(_CRT_WARN,
             "CNTSerial::WaitEvent - Unable to wait for COM event\n");
      return m_lLastError;
    }
    ...
    if (lpOverlapped == &ovInternal) {
      ...
      default:
        ...
        // Issue an error and quit
        _RPTF0(_CRT_WARN, "CNTSerial::WaitEvent - Unable to wait until COM "
                          "event has arrived\n");
        return m_lLastError;
      }
    }
...
LONG CNTSerial::SetupHandshaking(EHandshake eHandshake) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::SetupHandshaking - Unable to obtain DCB information\n");
    return m_lLastError;
  }
  ...
  if (!::SetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::SetupHandshaking - Unable to set DCB information\n");
    return m_lLastError;
  }
...
LONG CNTSerial::SetupReadTimeouts(EReadTimeout eReadTimeout) {
  ...
  if (!::GetCommTimeouts(m_hFile, &cto)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN, "CNTSerial::SetupReadTimeouts - Unable to obtain timeout "
                      "information\n");
    return m_lLastError;
  }
  ...
  if (!::SetCommTimeouts(m_hFile, &cto)) {
    ...
    _RPTF0(
        _CRT_WARN,
        "CNTSerial::SetupReadTimeouts - Unable to set timeout information\n");
    return m_lLastError;
  }
...
CNTSerial::EBaudrate CNTSerial::GetBaudrate(void) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetBaudrate - Unable to obtain DCB information\n");
    return EBaudUnknown;
  }
...
CNTSerial::EDataBits CNTSerial::GetDataBits(void) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetDataBits - Unable to obtain DCB information\n");
    return EDataUnknown;
  }
...
CNTSerial::EParity CNTSerial::GetParity(void) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetParity - Unable to obtain DCB information\n");
    return EParUnknown;
  }
...
CNTSerial::EStopBits CNTSerial::GetStopBits(void) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetStopBits - Unable to obtain DCB information\n");
    return EStopUnknown;
  }
...
DWORD CNTSerial::GetEventMask(void) {
  ...
  if (!::GetCommMask(m_hFile, &dwMask)) {
    ...
    // Issue an error and quit
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetEventMask - Unable to get the event mask\n");
    return 0;
  }
...
BYTE CNTSerial::GetEventChar(void) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetEventChar - Unable to obtain DCB information\n");
    return 0;
  }
...
CNTSerial::EHandshake CNTSerial::GetHandshaking(void) {
  ...
  if (!::GetCommState(m_hFile, &dcb)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetHandshaking - Unable to obtain DCB information\n");
    return EHandshakeUnknown;
  }
...
LONG CNTSerial::Write(const void *pData, size_t iLen, DWORD *pdwWritten,
    ...
    if (lpOverlapped == &ovInternal) {
      ...
      case WAIT_OBJECT_0:
        ...
        if (!::GetOverlappedResult(m_hFile, lpOverlapped, pdwWritten, FALSE)) {
          ...
          _RPTF0(_CRT_WARN,
                 "CNTSerial::Write - Overlapped completed without result\n");
          return m_lLastError;
        }
        break;
      ...
      default:
        ...
        // Issue an error and quit
        _RPTF0(_CRT_WARN,
               "CNTSerial::Write - Unable to wait until data has been sent\n");
        return m_lLastError;
      }
    }
  } else {
...
LONG CNTSerial::Write(LPCSTR pString, DWORD *pdwWritten,
                      LPOVERLAPPED lpOverlapped, DWORD dwTimeout) {
  // Determine the length of the string
  return Write(pString, strlen(pString), pdwWritten, lpOverlapped, dwTimeout);
}

LONG CNTSerial::NTRead(void *pData, size_t iLen, DWORD *pdwRead,
                       LPOVERLAPPED lpOverlapped, DWORD dwTimeout) {
    ...
    if (lpOverlapped == &ovInternal) {
      ...
      case WAIT_OBJECT_0:
        ...
        if (!::GetOverlappedResult(m_hFile, lpOverlapped, pdwRead, FALSE)) {
          ...
          _RPTF0(_CRT_WARN,
                 "CNTSerial::Read - Overlapped completed without result\n");
          return m_lLastError;
        }
        break;
      ...
      default:
        ...
        // Issue an error and quit
        _RPTF0(_CRT_WARN,
               "CNTSerial::Read - Unable to wait until data has been read\n");
        return m_lLastError;
      }
    }
  } else {
...
bool CNTSerial::GetCTS(void) {
  ...
  if (!::GetCommModemStatus(m_hFile, &dwModemStat)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetCTS - Unable to obtain the modem status\n");
    return false;
  }
...
bool CNTSerial::GetDSR(void) {
  ...
  if (!::GetCommModemStatus(m_hFile, &dwModemStat)) {
    ...
    // Display a warning
    _RPTF0(_CRT_WARN,
           "CNTSerial::GetDSR - Unable to obtain the modem status\n");
    return false;
  }
