...
bool CMOOSFileReader::GetTokenValPair(std::string sLine, std::string &sTok,
                                      std::string &sVal,
...
bool CMOOSFileReader::GetValue(std::string sName, bool &bResult) {
  ...
  if (GetValue(sName, sT)) {
    bResult =
        (MOOSStrCmp(sT, "TRUE") || (MOOSIsNumeric(sT) && atof(sT.c_str()) > 0));
...
bool CMOOSFileReader::eof() { return IsOpen() && GetFile()->eof(); }
  ...
  do {
    ...
    if (!sExpand.empty()) {
      ...
      if (p == m_LocalShellVariables.end()) {
        ...
        if (pShellVal != NULL) {
          // MOOSTrace("using system variable expansion ${%s} ->
          ...
          MOOSTrace("Error in mission file\n\t no shell or mission file "
                    "expansion fouind for ${%s}\n",
                    sExpand.c_str());
        }
      } else {
        // OK we have been told about this in file scope
        // MOOSTrace("using local variable expansion ${%s} ->
...
bool CMOOSFileReader::MakeOverloadedCopy(
    const std::string &sCopyName,
      ...
      if (!ignoreLine) {
        ...
        if (GetTokenValPair(sLine, sVarName, sVarVal)) {
          if (OverLoads.find(sVarName) != OverLoads.end()) {
            of << "//----  Next Line was commented and replaced with a command "
                  "line overload ---- //"
               << std::endl;
