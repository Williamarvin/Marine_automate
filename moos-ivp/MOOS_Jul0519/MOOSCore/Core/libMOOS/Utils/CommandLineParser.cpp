...
bool CommandLineParser::GetOption(const std::string &option,
                                  std::string &result) {
  if (!IsAvailable())
    return false;
...
bool CommandLineParser::GetVariable(const std::string &var,
                                    std::string &result) {
  if (!IsAvailable() || !VariableExists(var))
    return false;
...
bool CommandLineParser::IsAvailable() { return pcl_.get() != NULL; }
...
std::string
CommandLineParser::GetFreeParameter(unsigned int ndx,
