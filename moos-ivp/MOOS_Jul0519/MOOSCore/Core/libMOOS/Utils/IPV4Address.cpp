...
IPV4Address::IPV4Address(const std::string &host, uint16_t p)
    : host_(host), port_(p){
...
IPV4Address::IPV4Address(const std::string &host_and_port) {
  ...
  if (tmp.find(':') == std::string::npos)
    throw std::runtime_error("IPV4Address::IPV4Address " + tmp +
                             " is not of host:port format");
...
void IPV4Address::set_host(const std::string &host) { host_ = host; }

void IPV4Address::set_port(uint16_t port) { port_ = port; }

std::string IPV4Address::host() const { return host_; }

uint16_t IPV4Address::port() const { return port_; }
...
bool IPV4Address::ConvertHostToNumeric() {
  try {
    ...
    std::cerr << "IPV4Address::ConvertHostToNumeric issue" << e.what()
              << std::endl;
    return false;
  }
  return true;
