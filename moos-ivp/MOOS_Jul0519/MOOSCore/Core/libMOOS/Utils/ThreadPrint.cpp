...
void ThreadPrint::Print(const std::string &sMessage, const std::string &sPrompt,
                        color_t color, bool bAppendNewLine) {
  ...
  _Impl->_outstream << std::setprecision(4) << MOOS::Time() << " "
                    << MOOS::ConsoleColours::yellow() << std::left
                    << std::setw(15) << std::setfill(' ') << Me << "- ";
...
void ThreadPrint::Enable() { _Impl->_Enable = true; }

void ThreadPrint::Disable() { _Impl->_Enable = false; }
