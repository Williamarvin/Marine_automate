...
std::string CProcessConfigReader::GetAppName() { return m_sAppName; }

std::string CProcessConfigReader::GetFileName() { return m_sFileName; }

bool CProcessConfigReader::GetConfigurationAndPreserveSpace(
    std::string sAppName, STRING_LIST &Params) {
  Params.clear();
  ...
  if (GoTo(sKey)) {
    ...
    if (MOOSStartsWith(sBracket, "{")) {
      while (!GetFile()->eof()) {
        ...
        if (MOOSStartsWith(sLine, "{")) {
          MOOSTrace("CProcessConfigReader::GetConfiguration() missing \"}\" "
                    "syntax error in mission file\n");
        }
      }
    }
  }
...
std::list<std::string>
CProcessConfigReader::GetSearchedParameters(const std::string &sAppName) {
  std::list<std::string> L;
  std::map<std::string, std::set<std::string>>::iterator q =
      m_Audit.find(sAppName);
  if (q != m_Audit.end()) {
