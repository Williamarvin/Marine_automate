...
#if !defined(                                                                  \
    AFX_MOOSFILEREADER_H__B355D791_3CC0_4612_B755_020A269204F2__INCLUDED_)
...
public:
  ...
  /**static helper which splits a line into token = value and by deafult removes
   * white space*/
  static bool GetTokenValPair(std::string sLine, std::string &sTok,
                              std::string &sVal,
                              bool bPreserveWhiteSpace = false);

  /** returns a string of the next non comment line (and removs trailing
   * comments)*/
  ...
  bool MakeOverloadedCopy(const std::string &sCopyName,
                          std::map<std::string, std::string> &OverLoads);
  void EnableVerbatimQuoting(bool bEnable = true) {
    m_bEnableVerbatimQuoting = bEnable;
  };

protected:
  /** iterates through filemap freeing up resources then calls filemap's clear
   * method */
  void ClearFileMap() {
    for (THREAD2FILE_MAP::iterator iter = m_FileMap.begin();
         iter != m_FileMap.end(); iter++) {
  ...
  // true if quoted strings are treated as literals - quotes will be removed and
  // comment "//" characters not treated as comments
