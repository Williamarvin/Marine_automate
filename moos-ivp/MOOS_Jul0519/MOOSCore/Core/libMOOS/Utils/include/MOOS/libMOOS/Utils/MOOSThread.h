...
public:
  ...
  inline void RequestQuit() { SetQuitFlag(true); }

  inline bool IsQuitRequested() { return GetQuitFlag(); }

  inline bool IsThreadRunning() { return GetRunningFlag(); }

  inline void Name(const std::string &sName) { m_sName = sName; }

  inline std::string Name() { return m_sName; }

  //! Starts the thread running (as long as the class has been properly
  //! initialised!)
...
#ifdef _WIN32
    m_hThread = ::CreateThread(NULL, 0, CallbackProc, this, CREATE_SUSPENDED,
                               &m_nThreadID);
  ...
  thread_id GetNativeThreadHandle() { return m_nThreadID; }
...
private:
  ...
  void Verbose(bool bV) { m_bVerbose = bV; }
