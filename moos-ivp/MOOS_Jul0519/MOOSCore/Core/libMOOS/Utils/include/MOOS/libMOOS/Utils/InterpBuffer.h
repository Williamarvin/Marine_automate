...
template <class Key, class Data, class InterpFunc,
...
public:
  ...
  Data operator()(const Key &interp_time) const {
    ...
    if (hi != this->begin() && hi != this->end()) {
      typename BASE_TYPE::const_iterator low = hi;
      --low;
      return m_interpFunc(*low, *hi, interp_time);
    } else {
      if (hi == this->begin()) {
        typename BASE_TYPE::const_iterator hi2 = hi;
        ++hi2;
...
template <class T> class CTimeNumericInterpolator {
public:
  // return object with time closest to requested time
  typedef std::pair<double, T> TIME_DOUBLE_NUM_PAIR;

  T operator()(const TIME_DOUBLE_NUM_PAIR &loPair,
               const TIME_DOUBLE_NUM_PAIR &hiPair, double dfInterpTime) const {
...
template <class T> class CTimeGenericInterpolator {
public:
  typedef std::pair<double, T> TIME_DOUBLE_VAL_PAIR;

  T operator()(const TIME_DOUBLE_VAL_PAIR &loPair,
               const TIME_DOUBLE_VAL_PAIR &hiPair, double dfInterpTime) const {
...
template <class T> class ClosestInterpFunc {
  typedef std::pair<double, T> val_type;

public:
  // return object with time closest to requested time
  T operator()(const val_type &loPair, const val_type &hiPair,
               double interp_time) const {
    return fabs(interp_time - hiPair.first) > fabs(interp_time - loPair.first)
               ? loPair.second
               : hiPair.second;
  }
};
