...
public:
  ...
  void Tock(const std::string &S) {
    if (m_T.find(S) == m_T.end()) {
      ...
      std::string R = MOOSFormat("%s%-40s %.3f seconds", space.c_str(),
                                 S.c_str(), HPMOOSTime() - m_T[S]);
      m_L.push_back(R);
  ...
  void NewLevel(int nL = -1) { m_nLevel = nL > -1 ? nL : m_nLevel + 1; }

  void Dump() {
    m_File << "Level:" << m_nLevel << std::endl;
    std::copy(m_L.rbegin(), m_L.rend(),
