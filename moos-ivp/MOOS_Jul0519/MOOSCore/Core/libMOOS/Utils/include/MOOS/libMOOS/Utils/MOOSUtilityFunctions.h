...
std::string Chomp(std::string &sStr, const std::string &sTk,
...
// find the location of sToken in a string sSource with or without case
// sensitivity
...
bool MOOSGetValueFromToken(STRING_LIST &sParams, const std::string &sToken,
                           std::string &sVal);

// following functions extract a value from a string containing comma seperated
// pairs of Token = Val.... by default case sensitivity on the Token is off.
// Thus  given two strings S1="X=AbCd" and S2="x=AbCd", invoking
// MOOSValFromString(s,S1,"x",true) and MOOSValFromString(s,S2,"x",true) with
// both insert "AbCd" into s
bool MOOSValFromString(std::string &sVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);
bool MOOSValFromString(double &dfVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);
bool MOOSValFromString(float &fVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);
// bool MOOSValFromString(long   &  nVal,const std::string & sStr,const
// std::string & sTk,bool bInsensitive=false);
bool MOOSValFromString(int &nVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);
bool MOOSValFromString(bool &bVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);
bool MOOSValFromString(unsigned int &nVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);
...
bool MOOSValFromString(std::vector<unsigned int> &nValVec, int &nRows,
                       ...
                       const std::string &sToken, bool bInsensitive = false);
bool MOOSValFromString(int64_t &nVal, const std::string &sStr,
                       const std::string &sTk, bool bInsensitive = false);

// the following simply parse a MOOSFormated vector [nxm]{a,b,c...}
bool MOOSVectorFromString(const std::string &sStr,
                          std::vector<double> &dfVecVal, int &nRows,
                          int &nCols);
bool MOOSVectorFromString(const std::string &sStr, std::vector<float> &fValVec,
                          int &nRows, int &nCols);
bool MOOSVectorFromString(const std::string &sStr,
                          std::vector<unsigned int> &dfVecVal, int &nRows,
                          int &nCols);

// the following a sim[lpe line extractor from a file possessing an alog format
...
/**return time as a double (time since unix in seconds). This will
 ...
 MOOSCommsServer (often in the shap of a DB) will have a unified time. Of course
 if your process isn't using MOOSComms at all this funtion works just fine and
...
are on and always used in calls to MOOSTime()*/
