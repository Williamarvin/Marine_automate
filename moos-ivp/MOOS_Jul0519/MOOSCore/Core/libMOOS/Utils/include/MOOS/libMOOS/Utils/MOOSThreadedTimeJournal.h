...
public:
  ...
  void Open(const std::string &sFile) { m_file.open(sFile.c_str()); }

  void SetStartTime(double startTime) { m_dfStartTime = startTime; }
  ...
  bool Tock(const H_THREADLOG handle, const std::string &sTimerName) {
    ...
    if (pThreadInfo != m_threadInfoMap.end()) {
      ...
      if (pTimer != threadinfo.timermap.end()) {
        std::string msg = MOOSFormat(
            "\t\t%-15s : %-20s %.3f seconds", threadinfo.name.c_str(),
            sTimerName.c_str(), HPMOOSTime() - threadinfo.timermap[sTimerName]);
  ...
  void Log(const H_THREADLOG handle, const std::string &str) {
    ...
    if (pThreadInfo != m_threadInfoMap.end()) {
      ...
      std::string msg =
          MOOSFormat("\t%.3f %-15s : %s", HPMOOSTime() - m_dfStartTime,
                     threadinfo.name.c_str(), str.c_str());
      m_logList.push_back(msg);
    }
  ...
  void Dump() {
    ...
    std::copy(m_logList.begin(), m_logList.end(),
              std::ostream_iterator<std::string>(m_file, "\n"));
    m_file.flush();
...
private:
  CMOOSThreadedTimeJournal(
      const CMOOSThreadedTimeJournal &); // Deliberately not implemented
  void
  operator=(const CMOOSThreadedTimeJournal &); // Deliberately not implemented
};
