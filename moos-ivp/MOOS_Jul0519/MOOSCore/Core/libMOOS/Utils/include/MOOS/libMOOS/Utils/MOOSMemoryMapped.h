...
public:
  ...
  std::string GetFileName() { return m_sName; }
  ...
  bool Open(const std::string &sfName) {
    ...
    // Windows land
    m_hFileHandle =
        CreateFile(m_sName.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
                   OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

    if (m_hFileHandle != INVALID_HANDLE_VALUE) {
      m_hMapHandle =
          CreateFileMapping(m_hFileHandle, NULL, PAGE_READONLY, 0, 0, NULL);
    ...
    if (m_hMapHandle != -1) {
      ...
      m_pBuffer = (char *)mmap(NULL, StatusBuffer.st_size, PROT_READ,
                               MAP_SHARED, m_hMapHandle, 0);
    } else {
      m_hMapHandle = 0;
    }
#endif
    ...
    // report out success
    if (IsOpen()) {
      std::cout << "Successfully Memory Mapped " << double(m_nSize) / (1048576)
                << "MB file \n";
      return true;
    } else {
  ...
  bool IsOpen() { return m_hMapHandle != 0 && GetContents() != NULL; }

  /** returns pointer to start of file contents */
  char *GetContents() { return m_pBuffer; }
...
/** Specialised memory mapped file class to handle text files.
  ...
  Default template parameter is type TextLineInfo. Other template types must
  support the following functions: T::T(char * pStart, char * pEnd)
  //constructor char * T.Start(); //return start of line char * T.End();
  //return end of line bool IsWanted();  //return true if this line is wanted
...
public:
  ...
  bool BuildLineIndex(int nNumLines = -1) {
    ...
    std::cout << "Pruning "
              << std::distance(m_LineIndex.begin() + n, m_LineIndex.end())
              << " elements from LineIndex (comments)\n";
...
struct ALogLineInfo : public TextLineInfo {
  ...
  static bool Printable(char c) { return (isalnum(c) != 0); }
  ...
  bool IsNumFirst() {
    // This does not detect lines with negative time stamps (as we seem to get
    // when we use wildcard logging).  I'm not sure if this is a problem or not.
    // ARH
    return (isdigit(*pLineStart) != 0);
  }

  bool IsNumFollowedByWhiteSpace() {
    char *pFirstNonNumeric =
        std::find_if(pLineStart, pLineEnd, std::not1(std::ptr_fun(isnumeric)));
...
inline bool ALogLineInfoLessThan(const ALogLineInfo &L1,
                                 const ALogLineInfo &L2) {
  /*static int nCalls = 0;
  if(nCalls++%10000==0)
...
public:
  ...
  bool Open(const std::string &sName, bool bSummary = true,
            int nMaxLines = -1) {
  ...
  int SeekToFindTime(double dfT) {
    ...
    // p = std::lower_bound(m_LineIndex.begin(),m_LineIndex.end(),dfT -
    // m_dfLogStart,TimePredicate);
    p = std::lower_bound(m_LineIndex.begin(), m_LineIndex.end(), Q,
                         TimePredicate2);
  ...
  // simply extract a space delimeted token from a string startign at position
  // nPos
  ...
  /** This function builds two sets , one conataining all the unique messgae
  names and the other containing the set of message sources (processes)*/
  bool ReadSourceAndTypeSets() {
    ...
    std::cout << "\nThere are " << m_MessageNames.size()
              << " different message types:\n\t";
    std::copy(m_MessageNames.begin(), m_MessageNames.end(),
              std::ostream_iterator<std::string>(std::cout, "\n\t"));

    std::cout << "\nThere are " << m_SourceNames.size()
              << " different sources:\n\t";
    std::copy(m_SourceNames.begin(), m_SourceNames.end(),
              std::ostream_iterator<std::string>(std::cout, "\n\t"));
