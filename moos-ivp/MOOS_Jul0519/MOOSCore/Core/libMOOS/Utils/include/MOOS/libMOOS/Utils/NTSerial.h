...
public:
  ...
  virtual LONG Open(LPCTSTR lpszDevice, DWORD dwInQueue = 2048,
  ...
  virtual LONG Setup(EBaudrate eBaudrate = EBaud9600,
                     EDataBits eDataBits = EData8, EParity eParity = EParNone,
  ...
  // enabled (see SetMask).
  virtual LONG WaitEvent(LPOVERLAPPED lpOverlapped = 0,
  ...
  virtual LONG NTRead(void *pData, size_t iLen, DWORD *pdwRead = 0,
