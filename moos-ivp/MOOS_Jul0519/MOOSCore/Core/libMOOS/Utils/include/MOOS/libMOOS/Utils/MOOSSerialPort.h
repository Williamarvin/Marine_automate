...
public:
  ...
  virtual bool Create(const char *pPortNum = DEFAULT_PORT,
                      int nBaudRate = DEFAULT_BAUDRATE) = 0;
  virtual int ReadNWithTimeOut(char *pBuff, int nBufferLen,
                               double Timeout = 0.5, double *pTime = NULL);
  ...
  bool GetTelegram(std::string &sTelegram, double dfTimeOut,
                   double *pTime = NULL);
  ...
  bool GetTelegramOrAccumulate(std::string &sTelegram, double dfTimeOut,
                               double *pTime = NULL);

  // This version of ReadNWithTimeOut returns a negative value to signal an
  // error
  virtual int ReadNWithTimeOut2(char *pBuff, int nBufferLen,
                                double Timeout = 0.5, double *pTime = NULL);

  void SetIsCompleteReplyCallBack(bool (*pfn)(char *pData, int nLen,
                                              int nRead));
