...
public:
  ...
  /** returns the name of the application an instance of this class is concerned
   * with*/
  ...
  /** set the name of the application (MOOSProcess) that this class shoud
   * concern itself with (unless directed otherwise)*/
  void SetAppName(std::string sAppName);

  /** read a string parameter for a named process*/
  bool GetConfigurationParam(std::string sAppName, std::string sParam,
                             std::string &sVal);
  ...
  /** read a vector<double> parameter for a Process "m_sName" (can be
   * interprested as a matrix with (rows x cols) */
  bool GetConfigurationParam(std::string sParam, std::vector<double> &Vec,
                             int &nRows, int &nCols);

  /** return a list of strings of Token = Val for the specfied named application
   * configuration block*/
  bool GetConfiguration(std::string sAppName, STRING_LIST &Params);

  bool GetConfigurationAndPreserveSpace(std::string sAppName,
                                        STRING_LIST &Params);

  std::list<std::string> GetSearchedParameters(const std::string &sAppName);

  /** the name of process an instance this class will handle unless told
   * otherwise */
