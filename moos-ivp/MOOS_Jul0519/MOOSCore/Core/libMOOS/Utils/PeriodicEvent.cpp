...
public:
  ...
  static bool DefaultCallback(double TimeNow, double TimeLastRun,
                              double TimeScheduled, void *pParamCaller) {
  ...
  void SetCallback(bool (*pfn)(double TimeNow, double TimeLastRun,
                               double TimeScheduled, void *pParamCaller),
                   void *pCallerParam) {
  ...
  bool DoWork() {
    ...
    while (!Thread_.IsQuitRequested()) {
      ...
      int MillisecondsSleep =
          static_cast<int>((TimeScheduled - MOOS::Time()) * 1000.0);
  ...
  CMOOSThread Thread_;
  bool (*pfn_)(double TimeNow, double TimeLastRun, double TimeScheduled,
               void *pParam);
  void *pParamCaller_;
  double Period_;
};

void PeriodicEvent::SetCallback(bool (*pfn)(double Now, double LastRun,
                                            double Scheduled,
                                            void *pParamCaller),
                                void *pCallerParam) {
...
bool PeriodicEvent::Start() { return Impl_->Start(); }

bool PeriodicEvent::Stop() { return Impl_->Stop(); }
