 ...
 * main() will typically end with a call to MOOSAppDerivedClass::Run(). It
 * provides automatic connection to the MOOSDB, provides slots for Mail
 * Processing and application work, callbacks for connection/disconnection to
 * MOOSDB, Configuration file reading and dynamic (runtime) variables.
...
public:
  ...
  */
  bool Run(const std::string &sName, const std::string &sMissionFile,
           const std::string &sSubscribeName);
...
protected:
  /** called when the application should iterate. Overload this function in a
  derived class and within it write all the application specific code. It will
  be called at approximately nFreq = 1/AppTick Hz*/
  ...
  /** called when new mail has arrived. Overload this method in a derived class
  to process new mail. It will be called at approximately 1/CommsTick Hz. In
  this function you'll most likely interate over the collection of mail message
  received or call a m_Comms::PeekMail() to look for a specific named message.
  @param NewMail a list of new mail messages*/
  virtual bool OnNewMail(MOOSMSG_LIST &NewMail);

  /** called just before the main app loop is entered. Specific initialisation
  code can be written in an overloaded version of this function */
  ...
  /** optionally (see ::EnableCommandMessageFiltering() ) called when a command
  message (<MOOSNAME>_CMD) is recieved by the application.
  @param a copy of CmdMsg the message purporting to be a "command" - i.e. has
  the name <MOOSNAME>_CMD */
  virtual bool OnCommandMsg(CMOOSMsg Msg);

  /** make a status string - overload this in a derived class if you want to
   * modify or what the statuts string looks like */
  virtual std::string MakeStatusString();

  /** called before OnStartUp and before communications have been established to
   * give users option of processing command line*/
...
public:
  /** Called when the class has succesfully connected to the server. Overload
  this function and place use it to register for notification when variables of
  interest change */
  virtual bool OnConnectToServer();

  /** Called when the class has disconnects from  the server. Put code you want
   * to run when this happens in a virtual version of this method*/
...
protected:
   ...
   * @param dfTime time valid (if not specified this is filled in for you as
   * MOOS::Time())
   * @return
   */
  bool Notify(const std::string &sVar, const std::string &sVal,
              double dfTime = -1);

  /** notify the MOOS community that something has changed (string) with an
   * auxiliary string payload
   *
   * @param sVar Name of variable being notified /posted
   * @param sVal string contents (data payload)
   ...
   * @param sVar Name of variable being notified /posted
   * @param sVal string contents (data payload)
   * @param dfTime dfTime time valid (if not specified this is filled in for you
   * as MOOS::Time())
   * @return
   */
  bool Notify(const std::string &sVar, const char *sVal, double dfTime = -1);

  /** notify the MOOS community that something has changed (const char *) with
   * an auxiliary string paylad
   *
   * @param sVar Name of variable being notified /posted
   * @param sVal string contents (data payload)
   ...
   * @return
   */
  bool Notify(const std::string &sVar, const char *sVal,
              const std::string &sSrcAux, double dfTime = -1);
  ...
  /** notify the MOOS community that something has changed (double) with an
   * auxiliary string paylad
   *
   * @param sVar Name of variable being notified /posted
   * @param dfVal double value of data being sent
   * @param sSrcAux  additional string data
   ...
   * @return
   */
  bool Notify(const std::string &sVar, double dfVal, const std::string &sSrcAux,
              double dfTime = -1);
   ...
   * @return
   */
  bool Notify(const std::string &sVar, void *pData, unsigned int nDataSize,
              double dfTime = -1);

  /** notify the MOOS community that something has changed  ( binary data ) with
   * an auxiliary string paylad
   *
   * @param sVar Name of variable being notified /posted
   * @param pData pointer to a chunk of data
   ...
   * @return
   */
  bool Notify(const std::string &sVar, const std::vector<unsigned char> &vData,
              double dfTime = -1);
  ...
  /** Register for notification in changes of variables which match variable and
   * source patterns
   *
   ...
   */
  bool Register(const std::string &sVarPattern, const std::string &sAppPattern,
   ...
   * @param pYourParam a void * pointer to the thing we want passed as pParam
   * above
   * @return true on success
   */
  ...
  /**
   * Register a custom call back for a particular message. This call back will
   * be called from its own thread.
   * @param sQueueName
   ...
   */
  template <class T>
  bool AddActiveQueue(const std::string &sQueueName, T *Instance,
   ...
   * @param pfn  pointer to your function should be type bool func(CMOOSMsg &M,
   * void *pParam)
   * @param pYourParam a void * pointer to the thing we want passed as pParam
   * above
   * @return true on success
  ...
  template <class T>
  bool AddMessageRouteToActiveQueue(const std::string &sQueueName,
                                    const std::string &sMsgName, T *Instance,
                                    bool (T::*memfunc)(CMOOSMsg &));

  /**
   * Add a callback to ::OnMessage() for a particular message. This will cause
   * OnMessage() to be called from its own thread as soon as a message named as
   * sMsgName arrives. You do need to be careful about thread safety if you use
   * this facilty. remember OnMessage could be called simultaneously by N
   * threads if you have N callbacks registered - don't get hurt by this.
   * @param sMsgName
   * @return true on success
   */
  ...
  /** Set the time between calls into the DB - can be set using the CommsTick
   * flag in the config file*/
  bool SetCommsFreq(unsigned int nFreq);

  /** Set the time  between calls of ::Iterate (which is where you'll probably
   * do Application work)- can be set using the AppTick flag in the config
   * file*/
  ...
  /**a very useful object that lets us retrieve configuration information from
   * the mission file using calls like ::GetConfigurationParam() */
  CProcessConfigReader m_MissionReader;

  /** A function which Run eventually calls which itself  calls on NewMail and
   * Iterate*/
  bool DoRunWork();

  /** sets the error state of the app and a comment  - this is published as a
   * field in <PROCNAME>_STATUS */
  ...
  /** Called to set the MOOS server info used rarely usually this info will be
  picked up by the MOOSApp automatically when it ::Run is called specifying the
  configuration file (which contains the DB's coordinates)
  ...
  /** By default MOOSDB comms are on - but you may want to use the structuire of
  MOOSApp as a standalone application - if so call this function with a false
  parameter*/
  ...
  /** set to true to make the App sensitive (and quit) to Iterate() returning
   * false*/
  ...
  /** Add a dynamic (run time) variable
      ...
      @param sSubscribeName if you call RegisterMOOSVariables() the variable
     will be updated with mail called <sSubscribeName> if and when you call
     UpdateMOOSVariables()
      @param sPublishName  if you call PublishFreshMOOSVariables() (and you've
     written to the dynamic varible since the last call) the variable will be
     published under this name.
      @param CommsTime - if sSubscribeName is not empty this is the minimum time
     between updates which you are interested in knowing about, so if
     CommsTime=0.1 then the maximum update rate you will see on the variable
     from the DB is 10HZ. */
  bool AddMOOSVariable(std::string sName, std::string sSubscribeName,
                       std::string sPublishName, double dfCommsTime);

  /** Sets the value of a previously added dynamic variable to the given
   * CMOOSVariable.
   * @return \c true if the variable's type and value were set, \c false
   * otherwise.   */
   ...
   * variables. If they are interested (mail name matches their subscribe name)
   * they will update themselves automatically */
  bool UpdateMOOSVariables(MOOSMSG_LIST &NewMail);

  /** Set  value in a dynamic variable if the variable is of type double (type
   * is set on first write )*/
  bool SetMOOSVar(const std::string &sName, const std::string &sVal,
                  double dfTime);

  /** Set  value in a dynamic variable if the variable is of type string (type
   * is set on first write ) */
  bool SetMOOSVar(const std::string &sVarName, double dfVal, double dfTime);

  /** Send any variables (under their sPublishName see AddMOOSVariable)  which
   * been written too since the last call of PublishFreshMOOSVariables()*/
  bool PublishFreshMOOSVariables();

  /** a map of dynamic/run time moos variables that may be set by comms - avoid
  messy long if else if statements */
  MOOSVARMAP m_MOOSVars;

  /** Returns true if Simulate = true is found in the mission/configuration file
   * (a global flag) - the mission file is not re-read on each call */
  bool IsSimulateMode();
  ...
  /** flag to say whether or not App should quit after an Iterate returns
   * false*/
  ...
  /** string that should be written to the status string if the App Error flag
   * is true */
  ...
  /** Return time at which the Run loop last ran (called Iterate) - this is a
   local time - you need to add GetMOOSSKew to produce a unified system time**/
  ...
  /** returns the string which constitutes a command string for this
  application. if CommandFiltering is enabled (see
  EnableCommandMessageFiltering() ) the application will filter incoming mail
  and Call OnCommandMsg() (which can be overiden) if a message with this command
  string as a name is received. Command strings look like APPNAME_CMD */
...
protected:
  ...
  /**
   * look for  a parameter in the mission file and on the command line. If found
   * in both command line wins. It is (default) assumed "--" is prepended on
   * command line. So foo=xx in mission file appears as
   * --foo=xx on command line
   * @param sOption name of parameter
   * @param var variable to be returned
   * @param bPrependMinusMinusForCommandLine if true then add "--" to command
   * line
   */
  template <class T>
  bool GetParameterFromCommandLineOrConfigurationFile(
      std::string sOption, T &var,
      bool bPrependMinusMinusForCommandLine = true);

  bool GetFlagFromCommandLineOrConfigurationFile(
      std::string sOption, bool bPrependMinusMinusForCommandLine = true);
...
private:
  ...
  /* and this is a private iterate - we may need to regularly do things behind
   * the scenes */
  void IteratePrivate();
  ...
  /** called before starting the Application running. If parameters have not
  beedn set correctly it prints a help statement and returns false */
  bool CheckSetUp();
