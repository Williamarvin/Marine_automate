...
bool CMOOSInstrument::InitialiseSensor() {
  MOOSTrace("warning base class CMOOSInstrument::InitialiseSensor() called - "
            "NULL action\n");
  return false;
}
...
double CMOOSInstrument::GetMagneticOffset() {
  ...
  if (m_MissionReader.GetValue("MAGNETICOFFSET", dfTmp)) {
    ...
    MOOSTrace("WARNING: No magnetic offset specified  in Mission file (Field "
              "name = \"MagneticOffset\")\n");
    m_dfMagneticOffset = 0;
  }
...
void CMOOSInstrument::SetPrompt(string sPrompt) { m_sPrompt = sPrompt; }
