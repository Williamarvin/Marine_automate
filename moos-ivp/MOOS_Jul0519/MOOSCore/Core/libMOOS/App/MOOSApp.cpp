...
//   http://www.gnu.org/licenses/lgpl.txt  This program is distributed in the
...
CMOOSApp::CMOOSApp() {
  ...
  m_dfMaxAppTick = DEFAULT_MOOS_APP_FREQ; // we can respond to mail very quickly
                                          // but by default we are cautious
...
void CMOOSApp::OnPrintInterfaceAndExit() {
  std::cout
      << " sadly, there is no description of the interface for this app\n";
  exit(0);
}
...
void CMOOSApp::PrintDefaultCommandLineSwitches() {
  ...
  std::cout << "  --moos_name=<string>        : name with which to register "
               "with MOOSDB\n";
  ...
  std::cout << "  --moos_app_tick=<number>    : frequency of application (if "
               "relevant) \n";
  std::cout << "  --moos_max_app_tick=<number>: max frequency of application "
               "(if relevant) \n";
  std::cout
      << "  --moos_comms_tick=<number>  : frequency of comms (if relevant) \n";
  std::cout << "  --moos_tw_delay_factor=<num>: comms delay as % of time warp "
               "(if relevant) \n";

  std::cout << "  --moos_iterate_Mode=<0,1,2> : set app iterate mode \n";
  std::cout << "  --moos_time_warp=<number>   : set time warp \n";
  std::cout << "  --moos_suicide_channel=<str>: suicide monitoring channel (IP "
               "address) \n";
  ...
  std::cout
      << "  --moos_quiet                : don't print banner information \n";
  ...
  std::cout << "\nhelp:\n";
  std::cout << "  --moos_print_example        : print an example configuration "
               "block \n";
  std::cout << "  --moos_print_interface      : describe the interface "
               "(subscriptions/pubs) \n";
  std::cout
      << "  --moos_print_version        : print the version of moos in play \n";
  std::cout << "  --moos_help                 : print help on moos switches\n";
  std::cout << "  --moos_configuration_audit  : print configuration terms "
               "searched for\n";
  std::cout << "  --help                      : print help on moos switches "
               "and custom help\n";
}
...
// this is an overloaded 3 parameter version which allows explicit setting of
// the registration name
bool CMOOSApp::Run(const std::string &sName, const std::string &sMissionFile,
...
bool CMOOSApp::Run(const std::string &sName, const std::string &sMissionFile) {
  ...
  if (m_bUseMOOSComms) {
    ...
    // give iostream time to write comms start details up to screen..this is not
    // really necessary as the code is thread safe...it is aesthetic only
    MOOSPause(500);
  }
...
bool CMOOSApp::GetFlagFromCommandLineOrConfigurationFile(
    std::string sOption, bool bPrependMinusMinusForCommandLine) {
  bool bC, bF, bFlag;
...
bool CMOOSApp::Configure() {
  ...
  EnableIterateWithoutComms(
      GetFlagFromCommandLineOrConfigurationFile("moos_iterate_no_comms"));

  // are we having our suicide address being set?
  std::string sSuicideAddress;
  if (GetParameterFromCommandLineOrConfigurationFile("moos_suicide_channel",
                                                     sSuicideAddress)) {
  ...
  int nSuicidePort = -1;
  if (GetParameterFromCommandLineOrConfigurationFile("moos_suicide_port",
                                                     nSuicidePort)) {
  ...
  if (GetParameterFromCommandLineOrConfigurationFile("moos_suicide_phrase",
                                                     sSuicidePhrase)) {
  ...
  if (GetParameterFromCommandLineOrConfigurationFile("moos_time_warp",
                                                     dfTimeWarp) ||
  ...
  if (GetParameterFromCommandLineOrConfigurationFile("moos_tw_delay_factor",
                                                     dfTimeWarpCommsFactor)) {
  ...
  GetParameterFromCommandLineOrConfigurationFile("moos_max_app_tick",
                                                 m_dfMaxAppTick);
  ...
  // alternative
  m_MissionReader.GetConfigurationParam("CatchCommandMessages",
                                        m_bCommandMessageFiltering);
...
void CMOOSApp::PrintSearchedConfigurationFileParameters() {
  std::list<std::string> L =
      m_MissionReader.GetSearchedParameters(GetAppName());
  std::cout << MOOS::ConsoleColours::Green();
  std::cout << "\nThis application has searched the configuration file for the "
...
void CMOOSApp::DoBanner() {
  ...
  if (m_Comms.IsAsynchronous()) {
    ...
    case REGULAR_ITERATE_AND_MAIL:
      std::cout
          << " -Iterate Mode 0 :\n   |-Regular iterate and message delivery at "
          << m_dfFreq << " Hz\n";
      break;
    case COMMS_DRIVEN_ITERATE_AND_MAIL:
      std::cout << " |--Iterate Mode 1 :\n   |-Dynamic iterate speed driven by "
                   "message delivery ";
...
void CMOOSApp::SleepAsRequired(bool &bIterateShouldRun) {
  ...
  int nElapsedTime_ms =
      static_cast<int>(1000.0 * (MOOSLocalTime() - m_dfLastRunTime));
  ...
  switch (m_IterationMode) {
  case REGULAR_ITERATE_AND_MAIL:
    // we always to sleep - this behaves like old MOOS did - AppTick governs it
    // all std::cerr<<"sleeping for "<<nSleep<<"\n";
    MOOSPause(nSleep);
    break;
  case REGULAR_ITERATE_AND_COMMS_DRIVEN_MAIL:
  ...
  case COMMS_DRIVEN_ITERATE_AND_MAIL:
    ...
    // to a limiting speed.
    if (m_Comms.GetNumberOfUnreadMessages() > 0 ||
...
bool CMOOSApp::AddMessageRouteToActiveQueue(
...
bool CMOOSApp::AddActiveMessageQueueCallback(
    const std::string &sQueueName, const std::string &sMsgName,
    bool (*pfn)(CMOOSMsg &M, void *pYourParam), void *pYourParam) {
...
bool CMOOSApp::AddMessageRouteToOnMessage(const std::string &sMsgName) {
  return AddMessageRouteToActiveQueue(sMsgName + "_CB", sMsgName,
                                      MOOSAPP_OnMessage, this);
}
// deprecated version
...
////////////////////// DEFAULT HANDLERS //////////////////////
bool CMOOSApp::OnNewMail(MOOSMSG_LIST &) { return true; }
...
bool CMOOSApp::OnMessage(CMOOSMsg &M) {
  MOOSTrace("- default handler for %s invoked. Did you mean to write your own?",
            M.GetKey().c_str());
  return true;
}
...
/* this block of functions simply notifies the DB that a string variable has
 * changed*/
bool CMOOSApp::Notify(const std::string &sVar, const std::string &sVal,
                      double dfTime) {
...
bool CMOOSApp::Notify(const std::string &sVar, const char *sVal,
                      double dfTime) {
  return m_Comms.Notify(sVar, sVal, dfTime);
}
...
bool CMOOSApp::Notify(const std::string &sVar, double dfVal,
                      const std::string &sSrcAux, double dfTime) {
  return m_Comms.Notify(sVar, dfVal, sSrcAux, dfTime);
}

/** notify the MOOS community that something has changed binary data*/
bool CMOOSApp::Notify(const std::string &sVar, void *pData,
                      unsigned int nDataSize, double dfTime) {
  return m_Comms.Notify(sVar, pData, nDataSize, dfTime);
}
bool CMOOSApp::Notify(const std::string &sVar, void *pData,
                      unsigned int nDataSize, const std::string &sSrcAux,
                      double dfTime) {
  return m_Comms.Notify(sVar, pData, nDataSize, sSrcAux, dfTime);
}

bool CMOOSApp::Notify(const std::string &sVar,
                      const std::vector<unsigned char> &vData, double dfTime) {
...
/** Register with wildcards based on application and variable patterns (* and ?
 * allowed)*/
bool CMOOSApp::Register(const std::string &sVarPattern,
                        const std::string &sAppPattern, double dfInterval) {
...
/** this is a call back from MOOSComms and its use is specialised (not for
 * general consumption)*/
...
bool CMOOSApp::ConfigureComms() {
  ...
  if (!m_CommandLineParser.GetVariable("--moos_host", m_sServerHost)) {
    if (!m_MissionReader.GetValue("SERVERHOST", m_sServerHost)) {
      // MOOSTrace("Warning Server host not read from mission file or command
      // line: assuming %s\n",m_sServerHost.c_str());
    }
  }
  ...
  if (!m_CommandLineParser.GetVariable("--moos_port", m_lServerPort)) {
    if (!m_MissionReader.GetValue("SERVERPORT", m_lServerPort)) {
      // MOOSTrace("Warning Server port not read from mission file or command
      // line: assuming %d\n",m_lServerPort);
    }
  }
...
bool CMOOSApp::OnStartUp() { return true; }

int CMOOSApp::GetIterateCount() { return m_nIterateCount; }

double CMOOSApp::GetAppStartTime() { return m_dfAppStartTime; }

/** return the application frequency*/
double CMOOSApp::GetAppFreq() { return m_dfFreq; }

/** get the comms frequency*/
unsigned int CMOOSApp::GetCommsFreq() { return m_nCommsFreq; }

void CMOOSApp::SetAppFreq(double dfFreq, double dfMaxFreq) {
  if (m_dfFreq <= MOOS_MAX_APP_FREQ) {
    m_dfFreq = dfFreq;
  } else {
    std::cout << "Setting baseline apptick to allowable maximum of "
              << MOOS_MAX_APP_FREQ << std::endl;
  }
...
bool CMOOSApp::IsSimulateMode() { return m_bSimMode; }
...
bool CMOOSApp::AddMOOSVariable(string sName, string sSubscribeName,
...
bool CMOOSApp::SetMOOSVar(const string &sVarName, const string &sVal,
                          double dfTime) {
  MOOSVARMAP::iterator p = m_MOOSVars.find(sVarName);
...
bool CMOOSApp::CanIterateWithoutComms() { return m_bIterateWithoutComms; }
...
double CMOOSApp::GetLastIterateTime() { return m_dfLastRunTime; }

std::string CMOOSApp::GetMissionFileName() { return m_sMissionFile; }

string CMOOSApp::GetAppName() { return m_sAppName; }
...
std::string CMOOSApp::MakeStatusString() {
  ...
  if (m_ProcessMonitor.GetMemoryUsage(memory_now, memory_max)) {
    ssStatus << "memory_kb=" << std::setprecision(4) << memory_now / (1024.0)
             << ",";
    ssStatus << "memory_max_kb=" << std::setprecision(4)
             << memory_max / (1024.0) << ",";
  }
  ...
  std::copy(Published.begin(), Published.end(),
            std::ostream_iterator<string>(ssStatus, ","));
  ssStatus << "\",";

  ssStatus << "Subscribing=\"";
  std::copy(Registered.begin(), Registered.end(),
            std::ostream_iterator<string>(ssStatus, ","));
  ssStatus << "\"";
...
void CMOOSApp::OnDisconnectToServerPrivate() { return; }
