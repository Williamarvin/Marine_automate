...
public:
  ...
  SharedPtr(const RP &releasePolicy)
      : _releasePolicy(releasePolicy), _pCounter(new RC), _ptr(0) {}

  SharedPtr(C *ptr, const RP &releasePolicy)
      : _releasePolicy(releasePolicy), _pCounter(new RC), _ptr(ptr) {}

  template <class Other, class OtherRP>
  SharedPtr(const SharedPtr<Other, RC, OtherRP> &ptr)
      : _releasePolicy(ptr._releasePolicy), _pCounter(ptr._pCounter),
...
private:
  ...
  SharedPtr(RC *pCounter, const RP &releasePolicy, C *ptr)
      : _releasePolicy(releasePolicy), _pCounter(pCounter), _ptr(ptr)
  /// for cast operation
  {
    moos_poco_assert_dbg(_pCounter);
    _pCounter->duplicate();
