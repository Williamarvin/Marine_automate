...
bool AppCastingMOOSInstrument::OnStartUpDirectives(string directives) {
  ...
  if (m_MissionReader.GetValue("TERM_REPORTING", term_reporting)) {
    if (MOOSStrCmp(term_reporting, "false")) {
      ...
      reportConfigWarning("Invalid value for TERM_REPORTING: " +
                          term_reporting);
  }
...
void AppCastingMOOSInstrument::reportUnhandledConfigWarning(
    const string &orig) {
...
unsigned int
AppCastingMOOSInstrument::getWarningCount(const string &filter) const {
