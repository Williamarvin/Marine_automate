...
bool func_alt(CMOOSMsg &M, void * /*pParam*/) {
  gPrinter.Print(
      MOOSFormat("in alternate callback for %s", M.GetKey().c_str()));
  return true;
}
...
void PrintHelpAndExit() {
  ...
  std::cerr << " c) di appearing in callback \"func_wild\" because it is "
               "caught by the wildcard queue\n";
  std::cerr << " c) la appearing in callback \"class-method-callback\" because "
               "it is caught by a class member\n";
  exit(0);
}
...
public:
  bool HandleMessageA(CMOOSMsg &M) {
    gPrinter.Print(
        MOOSFormat("in class::HandleMessageA for %s", M.GetKey().c_str()));
    return true;
  }
  bool HandleMessageB(CMOOSMsg &M) {
    gPrinter.Print(
        MOOSFormat("in class::HandleMessageB for %s", M.GetKey().c_str()));
    return true;
  }
  bool HandleMessageC(CMOOSMsg &M) {
    gPrinter.Print(
        MOOSFormat("in class::HandleMessageC for %s", M.GetKey().c_str()));
    return true;
  }
  bool HandleMessageSet(std::vector<CMOOSMsg> &Mvec) {
    ...
    for (unsigned int k = 0; k < Mvec.size(); k++) {
      gPrinter.Print(MOOSFormat("     %s %10.3f", Mvec[k].GetKey().c_str(),
                                Mvec[k].GetTime()));
    }
    return true;
  }
...
int main(int argc, char *argv[]) {
  ...
  // C.AddMessageRouteToActiveQueue("ClassMember","la",
  // &aClass,&InterestedParty::HandleMessageA);
  // C.AddMessageRouteToActiveQueue("ClassMember","di",
  // &aClass,&InterestedParty::HandleMessageB);
  ...
  Acc.Configure(Names);
  C.AddMessageRouteToActiveQueue("Accumulator", "di", &Acc,
                                 &MOOS::MessageQueueAccumulator::AddMessage);
  C.AddMessageRouteToActiveQueue("Accumulator", "la", &Acc,
                                 &MOOS::MessageQueueAccumulator::AddMessage);
  ...
  while (++j) {
    if (j % 100 == 0) {
      C.AddWildcardActiveQueue("WCB", "d*", &aClass,
                               &InterestedParty::HandleMessageC);
      C.PrintMessageToActiveQueueRouting();
    }
    MOOSPause(100);
    continue;
