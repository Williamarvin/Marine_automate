...
CMOOSMsg::CMOOSMsg()
    : m_cMsgType(MOOS_NULL_MSG), m_cDataType(MOOS_DOUBLE), m_nID(-1),
      m_dfTime(-1), m_dfVal(-1), m_dfVal2(-1), m_sSrc(""), m_sSrcAux("") {}

CMOOSMsg::~CMOOSMsg() {}

CMOOSMsg::CMOOSMsg(char cMsgType, const std::string &sKey, double dfVal,
                   double dfTime)
    : m_cMsgType(cMsgType), m_cDataType(MOOS_DOUBLE), m_sKey(sKey), m_nID(-1),
      m_dfTime((dfTime == -1) ? MOOSTime() : dfTime), m_dfVal(dfVal),
      m_dfVal2(-1) {}

CMOOSMsg::CMOOSMsg(char cMsgType, const std::string &sKey,
                   const std::string &sVal, double dfTime)
    : m_cMsgType(cMsgType), m_cDataType(MOOS_STRING), m_sKey(sKey), m_nID(-1),
      m_dfTime((dfTime == -1) ? MOOSTime() : dfTime), m_dfVal(-1), m_dfVal2(-1),
      m_sVal(sVal) {}

CMOOSMsg::CMOOSMsg(char cMsgType, const std::string &sKey,
                   unsigned int nDataSize, const void *Data, double dfTime)
    : m_cMsgType(cMsgType), m_cDataType(MOOS_BINARY_STRING), m_sKey(sKey),
      m_nID(-1), m_dfTime((dfTime == -1) ? MOOSTime() : dfTime), m_dfVal(-1),
      m_dfVal2(-1) {
  m_sVal.assign((char *)Data, nDataSize);
}

bool CMOOSMsg::operator==(const CMOOSMsg &M) const {
  return m_cMsgType == M.m_cMsgType && m_cDataType == M.m_cDataType &&
         m_sKey == M.m_sKey &&
         m_sOriginatingCommunity == M.m_sOriginatingCommunity &&
         m_sSrcAux == M.m_sSrcAux && m_sSrc == M.m_sSrcAux &&
         fabs(m_dfVal - M.m_dfVal) <
             2 * std::numeric_limits<double>::epsilon() &&
         fabs(m_dfVal2 - M.m_dfVal2) <
             2 * std::numeric_limits<double>::epsilon() &&
         fabs(m_dfTime - M.m_dfTime) <
             2 * std::numeric_limits<double>::epsilon() &&
         m_nID == M.m_nID;
}

void CMOOSMsg::MarkAsBinary() { m_cDataType = MOOS_BINARY_STRING; }

/** copies data to a buffer reversing byte order if needed */
template <class T>
void CopyToBufferAsLittleEndian(T Var, unsigned char *pBuffer) {
  int nSize = sizeof(T);
  if (!IsLittleEndian()) {
...
unsigned int CMOOSMsg::GetSizeInBytesWhenSerialised() const {
  ...
  unsigned int nString = sizeof(int) + m_sSrc.size() + sizeof(int) +
                         m_sSrcAux.size() + sizeof(int) +
                         m_sOriginatingCommunity.size() + sizeof(int) +
                         m_sKey.size() + sizeof(int) + m_sVal.size();
...
int CMOOSMsg::Serialize(unsigned char *pBuffer, int nLen, bool bToStream) {
  ...
  if (bToStream) {
    try {
      ...
      // MOOSTrace("Packing Msg with community
      ...
      // how many bytes in total have we written (this includes an int at the
      // start)?
      ...
      MOOSTrace("perhaps accummulated messages exceeded available buffer space "
                "of %d bytes\n",
...
int CMOOSMsg::GetLength() { return m_nLength; }

bool CMOOSMsg::CanSerialiseN(int N) {
  return (m_pSerializeBuffer - m_pSerializeBufferStart) + N <=
         m_nSerializeBufferLen;
}

bool CMOOSMsg::IsType(char cType) const { return m_cMsgType == cType; }

char CMOOSMsg::GetType() const { return m_cMsgType; }

void CMOOSMsg::Trace() {
  MOOSTrace("Type=%c DataType=%c Key =%s ", m_cMsgType, m_cDataType,
            m_sKey.c_str());
...
bool CMOOSMsg::IsYoungerThan(double dfAge) const { return m_dfTime >= dfAge; }

/**
 *A method to check the timestamping of a MOOSMsg.
 *Does so by checking the <code>TimeNow</code> passed to it, and gives the
 *requesting class an idea about how out of sync this message is by comparing
 *the MOOSMsg's time stamp (<code>m_dfTime</code>) to SKEW_TOLERANCE.
 *@return true if a MOOSMsg's time stamp is either SKEW_TOLERANCE seconds ahead
 *or behind the MOOSDB clock.  Will also pass you the <code>pdfSkew</code>, or
 *amount of time difference between the MOOSDB and MOOSMsg timestamp if desired.
 */
