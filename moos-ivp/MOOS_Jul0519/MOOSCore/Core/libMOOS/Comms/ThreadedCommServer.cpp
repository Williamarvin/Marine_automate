...
/**
 * called when a new client connects. Our job is to make a new thread which will
 * handle all communications with the client. This will stop slow comms slowing
 * everything up
 * @param pNewClient
 * @param sName
 * @return
 ...
 * @return
 */
bool ThreadedCommServer::AddAndStartClientThread(XPCTcpSocket &NewClientSocket,
  ...
  if (q != m_ClientThreads.end()) {
    gPrinter.SimplyPrintTimeAndMessage(
        "logical error starting and exiting thread\n");
    return false;
  }

  // make a new client - and show it where to put data and how to talk to a
  // client
  bool bAsync =
      m_AsynchronousClientSet.find(sName) != m_AsynchronousClientSet.end();
  ...
  SharedClientThread pNewClientThread = new ClientThread(
      sName, NewClientSocket, m_SharedDataListFromClient, bAsync,
      dfConsolidationTime, m_dfClientTimeout, m_bBoostIOThreads);
...
/**
 * This is the main loop - it looks for complete Pkt being placed in the
 * incoming list and invokes a handler
 * @return true on exit
 */
bool ThreadedCommServer::ServerLoop() {
  ...
  while (!m_ServerThread.IsQuitRequested()) {
    ...
    if (m_bPrintHeartBeat &&
        MOOS::Time() - last_heart_beat > kHeartBeatPrintPeriod) {
...
bool ThreadedCommServer::ProcessClient(ClientThreadSharedData &SDFromClient,
  ...
  try {
    ...
    if (m_pfnRxCallBack != NULL) {
      ...
      Auditor.AddStatistic(sWho, SDFromClient._pPkt->GetStreamLength(),
                           MsgLstRx.size(), dfTNow, true);
      ...
      for (MOOSMSG_LIST::iterator q = MsgLstRx.begin(); q != MsgLstRx.end();
           ++q) {
        if (q->IsType(MOOS_NOTIFY)) {
          if (dfTNow - q->GetTime() > dfLargeDelay) {
            std::cout << "WARNING : Message " << q->GetKey() << " from "
                      << q->GetSource() << " is "
                      << (dfTNow - q->GetTime()) * 1000 << " ms delayed\n";
          }
      ...
      if (MsgLstRx.front().IsType(MOOS_TIMING)) {
        ...
        Auditor.AddTimingStatistic(sWho, TimingMsg.GetTime(),
                                   TimingMsg.GetDouble());
      ...
      if (!(*m_pfnRxCallBack)(sWho, MsgLstRx, MsgLstTx, m_pRxCallBackParam)) {
        ...
        // std::cerr<<"picked up "<<MsgLstTx.size()<<" messages for
        // "<<sWho<<"\n";
      }
      ...
      ClientThreadSharedData SDDownStream(sWho,
      ...
      if (!MsgLstTx.empty()) {
        ...
        Auditor.AddStatistic(sWho, SDDownStream._pPkt->GetStreamLength(),
                             nMessages, MOOS::Time(), false);
      ...
      for (q = m_ClientThreads.begin(); q != m_ClientThreads.end(); ++q) {
        ...
        if (m_pfnFetchAllMailCallBack != NULL && pClient->IsAsynchronous()) {
          ...
          MsgLstTx.clear();
          if ((*m_pfnFetchAllMailCallBack)(q->first, MsgLstTx,
                                           m_pFetchAllMailCallBackParam)) {
            // any pending mail?
            if (MsgLstTx.size() == 0)
              continue;

            ClientThreadSharedData SDAdditionalDownStream(
                sWho, ClientThreadSharedData::PKT_WRITE);

            // stuff all notifications into a packet
            unsigned int nMessages = MsgLstTx.size();
            SDAdditionalDownStream._pPkt->Serialize(MsgLstTx, true);

            Auditor.AddStatistic(
                q->first, SDAdditionalDownStream._pPkt->GetStreamLength(),
                nMessages, MOOS::Time(), false);
...
bool ThreadedCommServer::ProcessClient() { return BASE::ProcessClient(); }
...
bool ThreadedCommServer::OnClientDisconnect(ClientThreadSharedData &SD) {
  ...
  SOCKETLIST::iterator p = std::find(m_ClientSocketList.begin(),
                                     m_ClientSocketList.end(), m_pFocusSocket);
...
bool ThreadedCommServer::StopAndCleanUpClientThread(std::string sName) {
  ...
  if (q == m_ClientThreads.end())
    return MOOSFail(
        "runtime error ThreadedCommServer::StopAndCleanUpClientThread - cannot "
        "figure out worker thread");
...
bool ThreadedCommServer::WasteDisposalLoop() {
  ...
  while (!m_WasteDisposal.IsQuitRequested()) {
    ...
    if (m_OldClientThreadsToDestroy.WaitForPush(1000)) {
      ...
      std::string clue =
          MOOSFormat("dead thread collection of %s", thread_name.c_str());

      gPrinter.SimplyPrintTimeAndMessage(clue);

      gPrinter.SimplyPrintTimeAndMessage(MOOSFormat(
          "deleted client thread for \"%s\" safely ", thread_name.c_str()));
    }
  }
...
bool ThreadedCommServer::SupportsAsynchronousClients() { return true; }
...
ThreadedCommServer::ClientThread::ClientThread(
    const std::string &sName, XPCTcpSocket &ClientSocket,
    SHARED_PKT_LIST &SharedDataIncoming, bool bAsync,
    double dfConsolidationPeriodMS, double dfClientTimeout, bool bBoost)
    : m_sClientName(sName), m_ClientSocket(ClientSocket),
      m_SharedDataIncoming(SharedDataIncoming), m_bAsynchronous(bAsync),
      m_dfConsolidationPeriod(dfConsolidationPeriodMS / 1000.0),
      m_dfClientTimeout(dfClientTimeout), m_bBoostThread(bBoost) {
  ...
  setsockopt(m_ClientSocket.iGetSocketFd(), SOL_SOCKET, SO_SNDTIMEO,
             timeout_as_char_array, sizeof(timeout));
...
bool ThreadedCommServer::ClientThread::AsynchronousReadLoop() {
  ...
  while (!m_Reader.IsQuitRequested()) {
    ...
    // The select system call is set to timeout after 1 seconds with no data
    // existing on the socket. This reinitialisation has to be here, within the
    // loop as Linux actually writes over the timeout structure on completion of
    // select (now that was a hard bug to find)
    timeout.tv_sec = 1;
    timeout.tv_usec = 0;
    ...
    int iSelectRet =
        select(m_ClientSocket.iGetSocketFd() + 1, &fdset, NULL, NULL, &timeout);
    ...
    case 0:
      ...
      if (MOOSLocalTime(false) - dfLastGoodComms > m_dfClientTimeout) {
        ...
        std::cout << "Disconnecting \"" << m_sClientName << "\" after "
                  << m_dfClientTimeout << " seconds of silence\n";
    ...
    default:
      ...
      if (FD_ISSET(m_ClientSocket.iGetSocketFd(), &fdset) != 0) {
        if (!HandleClientWrite()) {
          // client disconnected!
          OnClientDisconnect();
          // MOOSTrace("socket thread for %s quits after
          // disconnect",_sClientName.c_str());
          return true;
        }
...
bool ThreadedCommServer::ClientThread::OnClientDisconnect() {
  ...
  ClientThreadSharedData SD(m_sClientName,
                            ClientThreadSharedData::CONNECTION_CLOSED);
...
bool ThreadedCommServer::ClientThread::Kill() {
  ClientThreadSharedData QuitInstruction("",
                                         ClientThreadSharedData::STOP_THREAD);
...
bool ThreadedCommServer::ClientThread::SendToClient(
    ClientThreadSharedData &OutGoing) {
...
bool ThreadedCommServer::ClientThread::AsynchronousWriteLoop() {
  ...
  try {
    ...
    gPrinter.SimplyPrintTimeAndMessage(
        "AsynchronousWriteLoop() catches exception and exits");
    return false;
  }
  return bResult;
}
    ...
    if (IsSynchronous()) {
      ...
      if (m_SharedDataOutgoing.Size() != 0) {
        std::cerr << "logical error "
                     "ThreadedCommServer::ClientThread::HandleClientWrite\n";
      }
    } else {
    }
