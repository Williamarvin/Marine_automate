...
CMOOSCommPkt::~CMOOSCommPkt() { delete[] m_pStream; }
...
bool CMOOSCommPkt::OnBytesWritten(unsigned char * /*PositionWrittento*/,
                                  int nData) {
...
int CMOOSCommPkt::GetStreamLength() { return m_nByteCount; }

unsigned char *CMOOSCommPkt::Stream() { return m_pStream; }

unsigned char *CMOOSCommPkt::NextWrite() { return m_pNextData; }

int CMOOSCommPkt::GetNumMessagesSerialised() { return m_nMsgsSerialised; }
...
bool CMOOSCommPkt::Serialize(MOOSMSG_LIST &List, bool bToStream, bool bNoNULL,
  ...
  if (bToStream) {
    ...
    m_pNextData = m_pStream;
    int nBC =
        IsLittleEndian() ? m_nByteCount : SwapByteOrder<int>(m_nByteCount);
