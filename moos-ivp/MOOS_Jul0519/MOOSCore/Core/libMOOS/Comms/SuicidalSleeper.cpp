...
int SuicidalSleeper::GetDefaultMulticastPort() { return 4000; }
...
std::string SuicidalSleeper::GetPassPhrase() { return pass_phrase_; }
std::string SuicidalSleeper::GetChannel() {
  return multicast_group_IP_address_;
}
int SuicidalSleeper::GetPort() { return multicast_port_; }
...
bool SuicidalSleeper::Work() {
  ...
  while (!thread_.IsQuitRequested()) {
    ...
    if (MCN.Read(msg, 500)) {
      ...
      std::string sHeader = MOOSFormat(
          "%-20s pid %-7d on %s", name_.c_str(), MOOS::ProcInfo::GetPid(),
      ...
      if (Action == "?") {
        ...
        //                std::string response = MOOSFormat("%s is committing
        //                suicide\n",sHeader.c_str());
        // Change by mikerb Apr0921
        std::string response =
            MOOSFormat("%s is self-exiting\n", sHeader.c_str());
        MCN.Write(response);
