...
std::string ActiveMailQueue::GetName() { return Name_; }
    ...
    case MOOS_NOTIFY: {
      ...
      if (pClassMemberFunctionCallback_) {
        if (!(*pClassMemberFunctionCallback_)(M)) {
          std::cerr
              << "ActiveMailQueue::DoWork() user callback returns false\n";
        }
      }
...
bool ActiveMailQueue::IsRunning() { return thread_.IsThreadRunning(); }

void ActiveMailQueue::SetCallback(bool (*pfn)(CMOOSMsg &M, void *pParam),
                                  void *pCallerParam) {
  pClassMemberFunctionCallback_ = NULL;
