...
public:
  ...
  bool Run(const std::string &destination_host, unsigned int port) {
    ...
    if (!quiet_) {
      std::cout << MOOS::ConsoleColours::Yellow()
                << "network performance data published on " << destination_host_
                << ":" << destination_port_ << "\n";
  ...
  bool Work() {
    ...
    while (!thread_.IsQuitRequested()) {
      ...
      lock_.Lock();
      {
        ...
        ss << std::endl
           << std::setw(12) << "client name" << std::setw(10) << "pkts in"
           << std::setw(10) << "pkts out" << std::setw(10) << "msgs in"
           << std::setw(10) << "msgs out" << std::setw(10) << "B/s in"
           << std::setw(10) << "B/s out\n";
      ...
      try {
        ...
        while (sent < s.size()) {
          ...
          sent += output.iSendMessageTo((void *)little_bit.data(),
                                        little_bit.size(), destination_port_,
                                        destination_host_);
        }
  ...
  bool GetTimingStatisticSummary(const std::string &sClient,
                                 std::string &sSummary) {
    std::map<std::string, ClientAudit>::iterator q = Audits_.find(sClient);
    if (q == Audits_.end())
      return false;
...
bool ServerAudit::SetQuiet(bool bQuiet) { return Impl_->SetQuiet(bQuiet); }

bool ServerAudit::AddStatistic(const std::string &sClient, unsigned int nBytes,
                               unsigned int nMessages, double dfTime,
                               bool bIncoming) {
