...
/*file scope function to redirect thread work to a particular instance of
 * CMOOSCommClient */
...
CMOOSCommClient::~CMOOSCommClient() { Close(); }

bool CMOOSCommClient::Run(const std::string &sServer, int Port,
                          const std::string &sMyName,
                          unsigned int nFundamentalFrequency) {
  if (IsRunning()) {
...
std::string CMOOSCommClient::GetMOOSName() { return m_sMyName; }
...
// void CMOOSCommClient::SetOnMailtCallBack(bool (__cdecl *pfn)( void *
// pMailtParam), void * pMailParam)
void CMOOSCommClient::SetOnMailCallBack(bool (*pfn)(void *pMailParam),
                                        void *pMailParam) {
  m_pfnMailCallBack = pfn;
  m_pMailCallBackParam = pMailParam;
}

bool CMOOSCommClient::HasMailCallBack() { return m_pfnMailCallBack != NULL; }

bool CMOOSCommClient::IsAsynchronous() { return false; }
...
std::string CMOOSCommClient::GetDBHostname() { return m_sDBHost; }

int CMOOSCommClient::GetDBHostPort() { return m_lPort; }

std::string CMOOSCommClient::GetClientName() { return m_sMyName; }

uint64_t CMOOSCommClient::GetNumBytesSent() { return m_nBytesSent; }
uint64_t CMOOSCommClient::GetNumBytesReceived() { return m_nBytesReceived; }

uint64_t CMOOSCommClient::GetNumPktsReceived() { return m_nPktsReceived; }

uint64_t CMOOSCommClient::GetNumMsgsReceived() { return m_nMsgsReceived; }

uint64_t CMOOSCommClient::GetNumMsgsSent() { return m_nMsgsSent; }

// void CMOOSCommClient::SetOnConnectCallBack(bool (__cdecl *pfn)( void *
// pConnectParam), void * pConnectParam)
void CMOOSCommClient::SetOnConnectCallBack(bool (*pfn)(void *pConnectParam),
                                           void *pConnectParam) {
  m_pfnConnectCallBack = pfn;
  m_pConnectCallBackParam = pConnectParam;
}

// void CMOOSCommClient::SetOnDisconnectCallBack(bool (__cdecl *pfn)( void *
// pConnectParam), void * pParam)
void CMOOSCommClient::SetOnDisconnectCallBack(bool (*pfn)(void *pConnectParam),
                                              void *pParam) {
  m_pfnDisconnectCallBack = pfn;
  m_pDisconnectCallBackParam = pParam;
}

bool CMOOSCommClient::IsRunning() { return m_ClientThread.IsThreadRunning(); }
...
void CMOOSCommClient::PrintMessageToActiveQueueRouting() {
  ...
  std::cerr << MOOS::ConsoleColours::Green()
            << "--- Message Routing for client \"" << GetMOOSName()
            << "\" ---\n";
...
bool CMOOSCommClient::RemoveActiveQueue(const std::string &sQueueName) {
  ...
  std::map<std::string, MOOS::ActiveMailQueue *>::iterator w =
      ActiveQueueMap_.find(sQueueName);
...
bool CMOOSCommClient::AddMessageRouteToActiveQueue(
    const std::string &sQueueName, const std::string &sMsgName,
    bool (*pfn)(CMOOSMsg &M, void *pYourParam), void *pYourParam) {
...
bool CMOOSCommClient::RemoveMessageRouteToActiveQueue(
    std::string const &sQueueName, std::string const &sMsgName) {
  if (!HasActiveQueue(sQueueName))
    return false;

  MOOS::ScopedLock L(ActiveQueuesLock_);

  std::map<std::string, std::set<std::string>>::iterator w =
      Msg2ActiveQueueName_.find(sMsgName);
...
// deprecated version
bool CMOOSCommClient::AddMessageCallBack(
    const std::string &sQueueName, const std::string &sMsgName,
    bool (*pfn)(CMOOSMsg &M, void *pYourParam), void *pYourParam) {
...
bool CMOOSCommClient::AddWildcardActiveQueue(
    const std::string &sQueueName, const std::string &sPattern,
    bool (*pfn)(CMOOSMsg &M, void *pYourParam), void *pYourParam) {
...
bool CMOOSCommClient::AddMessageRouteToActiveQueue(
  ...
  if (HasActiveQueue(sQueueName)) {
    ...
    std::cerr << "cannot add callback as queue " << sQueueName
              << " does not exist\n";
    return false;
  }
}
...
bool CMOOSCommClient::DispatchInBoxToActiveThreads() {
  ...
  while (t != m_InBox.end()) {
    ...
    if (q == Msg2ActiveQueueName_.end() || u == WildcardCheckSet_.end()) {
      ...
      // or maybe this is a new message whihc has not been seen by wildcard
      // queues
      ...
      for (w = WildcardQueuePatterns_.begin();
           w != WildcardQueuePatterns_.end(); ++w) {
        ...
        if (MOOSWildCmp(sPattern, t->GetKey())) {
          //				    std::cerr<<"found wildcard match adding queue
          //"<<w->first
          //				            <<" to routing for
          //"<<t->GetKey()<<"\n";
    ...
    for (r = q->second.begin(); r != q->second.end(); ++r) {
      ...
      //		    std::cerr<<"found queue that is relevent
      //"<<*r<<"\n";
...
bool CMOOSCommClient::ConnectToServer() {
  ...
  while (!m_bQuit) {
    if (!m_bQuiet)
      MOOSTrace("  contacting a MOOS server %s:%d -  try %.5d ",
                m_sDBHost.c_str(), m_lPort, ++nAttempt);
...
bool IsNullMsg(const CMOOSMsg &msg) { return msg.IsType(MOOS_NULL_MSG); }
...
bool CMOOSCommClient::HandShake() {
  try {
    ...
    if (WelcomeMsg.IsType(MOOS_POISON)) {
      ...
      MOOSValFromString(m_sDBHostAsSeenByDB, WelcomeMsg.m_sSrcAux, "hostname",
                        true);
...
bool CMOOSCommClient::IsConnected() { return m_bConnected; }
...
bool CMOOSCommClient::UnRegister(const string &sVarPattern,
                                 const string &sAppPattern) {
  if (!IsConnected())
    return false;
...
bool CMOOSCommClient::Register(const std::string &sVarPattern,
...
bool CMOOSCommClient::IsRegisteredFor(const std::string &sVariable) {
  return !m_Registered.empty() &&
         m_Registered.find(sVariable) != m_Registered.end();
}
...
bool CMOOSCommClient::Notify(const std::string &sVar, double dfVal,
                             const std::string &sSrcAux, double dfTime) {
  CMOOSMsg Msg(MOOS_NOTIFY, sVar.c_str(), dfVal, dfTime);
...
bool CMOOSCommClient::Notify(const string &sVar, const string &sVal,
                             double dfTime) {
...
bool CMOOSCommClient::Notify(const std::string &sVar, const std::string &sVal,
                             const std::string &sSrcAux, double dfTime) {
  CMOOSMsg Msg(MOOS_NOTIFY, sVar.c_str(), sVal.c_str(), dfTime);
...
bool CMOOSCommClient::Notify(const std::string &sVar, const char *sVal,
                             double dfTime) {
...
bool CMOOSCommClient::Notify(const string &sVar, void *pData,
                             unsigned int nSize, double dfTime) {
...
bool CMOOSCommClient::Notify(const string &sVar, void *pData,
                             unsigned int nSize, const std::string &sSrcAux,
                             double dfTime) {
...
bool CMOOSCommClient::ServerRequest(const string &sWhat, MOOSMSG_LIST &MsgList,
                                    double dfTimeOut, bool bClear) {
  if (!IsConnected())
    return false;
...
bool CMOOSCommClient::Peek(MOOSMSG_LIST &MsgList, int nIDRequired,
                           bool bClear) {
  MsgList.clear();
...
bool CMOOSCommClient::PeekMail(MOOSMSG_LIST &Mail, const string &sKey,
                               CMOOSMsg &Msg, bool bRemove,
                               bool bFindYoungest) {
  MOOSMSG_LIST::iterator p;
...
bool CMOOSCommClient::PeekAndCheckMail(MOOSMSG_LIST &Mail,
                                       const std::string &sKey, CMOOSMsg &Msg,
                                       bool bErase, bool bFindYoungest) {
...
bool CMOOSCommClient::Flush() { return DoClientWork(); }

// std::auto_ptr<std::ofstream> SkewLog(NULL);

bool CMOOSCommClient::UpdateMOOSSkew(double dfRqTime, double dfTxTime,
                                     double dfRxTime) {
...
//#define MOOS_DETECT_CLOCK_DRIFT
  ...
  // This is an experimental and unfinished feature.  It tracks the drift
  // between clocks on client and server, in order to provide highly accurate
  // time stamps at the client, which are within about 0.1ms of the server time.
  //
  // In its current implementation, the filter begins to suffer from numerical
  // issues after around 3 or 4 hours of use.  This is known and expected
  // behaviour and will be fixed soon.
  //
  // arh 30/03/2009

  if (!m_pSkewFilter.get()) {
    // Make a fresh skew filter
    // m_pSkewFilter = std::auto_ptr<MOOS::CMOOSSkewFilter>(new
    // MOOS::CMOOSConditionedSkewFilter);
    m_pSkewFilter =
        MOOS::ScopedPtr<MOOS::CMOOSSkewFilter>(new MOOS::CMOOSSkewFilter);
    if (!m_pSkewFilter.get())
      return false;
  }
  ...
  double dfNewSkew =
      m_pSkewFilter->Update(dfRqTime, dfTxTime, dfRxTime, &skewinfo);
  ...
  //	MOOSTrace("\n%s\nTx Time = %.4f \nDB time = %.4f\nreply = %.4f\nskew =
  //%.5f\n", 			m_sMyName.c_str(), 			dfRqTime, 			dfTxTime, 			dfRxTime, 			dfNewSkew);
  //
  //	MOOSTrace("local = %.4f\nMOOS = %.4f\n ", MOOSLocalTime(false),
  //MOOS::Time());
  //
...
bool CMOOSCommClient::ControlClientCommsStatusMonitoring(bool bEnable) {
  if (bEnable) {
    ...
    if (!AddActiveQueue("_ClientSummaries", this,
                        &CMOOSCommClient::ProcessClientCommsStatusSummary))
      return false;
...
bool CMOOSCommClient::GetClientCommsStatus(const std::string &sClient,
                                           MOOS::ClientCommsStatus &TheStatus) {
  MOOS::ScopedLock L(m_ClientStatusLock);
  std::map<std::string, MOOS::ClientCommsStatus>::iterator q =
      m_ClientStatuses.find(sClient);
...
void CMOOSCommClient::GetClientCommsStatuses(
...
bool CMOOSCommClient::ProcessClientCommsStatusSummary(CMOOSMsg &M) {
  ...
  if (M.GetName() == "DB_QOS") {
    while (!M.m_sVal.empty()) {
      ...
      if (sC.empty())
        return MOOSFail(
            "CMOOSCommClient::ProcessClientSummary empty client name");
...
bool CMOOSCommClient::ApplyRecurrentSubscriptions() {
  ...
  for (q = m_RecurrentSubscriptions.begin();
       q != m_RecurrentSubscriptions.end(); ++q) {
...
bool CMOOSCommClient::AddRecurrentSubscription(const std::string &sVar,
                                               double dfPeriod) {
  if (sVar.empty())
    return false;
