...
CMOOSCommServer::~CMOOSCommServer() { Stop(); }
...
bool CMOOSCommServer::Run(long lPort, const string &sCommunityName,
                          bool bDisableNameLookUp, unsigned int nAuditPort) {
  ...
  if (m_CommandLineParser.IsAvailable()) {
    ...
    for (std::vector<std::string>::iterator q = sLL.begin(); q != sLL.end();
         ++q) {
      try {
        std::string sNum = *q;
        std::string sClient = "*";
        ...
        if (!MOOSIsNumeric(sNum))
          throw std::runtime_error("error processing response " + *q +
                                   " expected form [client_name:]time_ms\n");
...
bool CMOOSCommServer::ListenLoop() {
  ...
  try {
    ...
    if (m_bDisableNagle) {
      if (!m_bQuiet) {
        gMOOSCommsServerTheadPrinter.SimplyPrintTimeAndMessage(
            "disabling nagle");
      }
    ...
    MOOSTrace("Error binding to listen socket - Is there another CommServer "
              "Running?\n");
  ...
  while (!m_ListenThread.IsQuitRequested()) {
    ...
    try {
      ...
      for (;;) {
        ...
        if (select(m_pListenSocket->iGetSocketFd() + 1, &fdset, NULL, NULL,
                   &timeout) != 0) {
...
bool CMOOSCommServer::ServerLoop() {
  ...
  while (!m_ServerThread.IsQuitRequested()) {
    ...
    // existing on the socket. This has to be here, within the loop as Linux
    // actually writes over the timeout structure on completion of select (no
    ...
    int iSelectRet = select(m_nMaxSocketFD + 1, &fdset, NULL, NULL, &timeout);
...
bool CMOOSCommServer::OnNewClient(XPCTcpSocket *pNewClient, char *sName) {
  ...
  if (!m_bQuiet)
    std::cout << "\n------------" << MOOS::ConsoleColours::Green() << "CONNECT"
  ...
  if (HandShake(pNewClient)) {
    ...
    if (!m_bQuiet) {
      std::cout << "  Handshaking   :  " << MOOS::ConsoleColours::green()
                << "OK\n"
                << MOOS::ConsoleColours::reset();
      ...
      if (!sName.empty()) {
        std::cout << "  Client's name :  " << MOOS::ConsoleColours::green()
                  << sName << MOOS::ConsoleColours::reset() << "\n";
      }
      if (m_AsynchronousClientSet.find(sName) !=
          m_AsynchronousClientSet.end()) {
        std::cout << "  Type          :  " << MOOS::ConsoleColours::Yellow()
                  << "Asynchronous" << MOOS::ConsoleColours::reset() << "\n";
      } else {
        std::cout << "  Type          :  " << MOOS::ConsoleColours::green()
                  << "Synchronous" << MOOS::ConsoleColours::reset() << "\n";
      }

      if (m_bBoostIOThreads) {
        std::cout << "  Priority      :  " << MOOS::ConsoleColours::Yellow()
                  << "raised" << MOOS::ConsoleColours::reset() << "\n";
      } else {
        std::cout << "  Priority      :  " << MOOS::ConsoleColours::green()
                  << "normal" << MOOS::ConsoleColours::reset() << "\n";
      }
    }
  } else {
    if (!m_bQuiet) {
      std::cerr << "  Handshaking   :  " << MOOS::ConsoleColours::Red()
                << "FAIL\n"
                << MOOS::ConsoleColours::reset() << "\n";
      std::cerr << MOOS::ConsoleColours::Red()
                << "Handshaking failed - client is spurned\n"
                << MOOS::ConsoleColours::reset();
    }
  ...
  if (!m_bQuiet)
    std::cout << "  Total Clients :  " << MOOS::ConsoleColours::green()
              << m_Socket2ClientMap.size() << MOOS::ConsoleColours::reset()
              << "\n";
...
bool CMOOSCommServer::OnClientDisconnect() {
  ...
  if (!m_bQuiet)
    std::cout << "\n----------" << MOOS::ConsoleColours::Yellow()
              << "DISCONNECT" << MOOS::ConsoleColours::reset()
              << "------------\n";
...
// void CMOOSCommServer::SetOnRxCallBack(bool (__cdecl *pfn)(MOOSMSG_LIST &
// MsgListRx,MOOSMSG_LIST & MsgListTx, void * pParam), void * pParam)
void CMOOSCommServer::SetOnRxCallBack(bool (*pfn)(const std::string &,
...
// void CMOOSCommServer::SetOnDisconnectCallBack(bool (__cdecl *pfn)(string &
// MsgListRx, void * pParam), void * pParam)
void CMOOSCommServer::SetOnDisconnectCallBack(bool (*pfn)(string &,
                                                          void *pParam),
                                              void *pParam) {
  // address of function to invoke (static)
...
void CMOOSCommServer::SetOnConnectCallBack(bool (*pfn)(string &, void *pParam),
                                           void *pParam) {
  // address of function to invoke (static)
...
void CMOOSCommServer::SetOnFetchAllMailCallBack(
    bool (*pfn)(const std::string &sClient, MOOSMSG_LIST &MsgListTx,
                void *pParam),
    void *pParam) {
  // address of function to invoke (static)
...
// chances of a comm packet spelling out a protocol name are pretty damn
// slim.....
bool CheckProtocol(XPCTcpSocket *pNewClient) {
  char sProtocol[MOOS_PROTOCOL_STRING_BUFFER_SIZE + 1] = {};
  int nRead =
      pNewClient->iRecieveMessage(sProtocol, MOOS_PROTOCOL_STRING_BUFFER_SIZE);
  if (nRead <= 0) {
  ...
  if (!MOOSStrCmp(sProtocol, MOOS_PROTOCOL_STRING)) {
    // this is bad - wrong flavour of comms - perhaps client needs to be
    // recompiled...
    return MOOSFail(
        "Incompatible wire protocol between DB and Client:\n  "
  ...
  try {
    ...
    if (ReadMsg(pNewClient, Msg, 5)) {
      ...
      if (IsUniqueName(Msg.m_sVal)) {
        ...
        // std::cerr<<"CMOOSCommServer::HandShake added "<<Msg.m_sVal<<" to
        // m_Socket2ClientMap \n";
...
bool CMOOSCommServer::SupportsAsynchronousClients() { return false; }
...
void CMOOSCommServer::DoBanner() {
  ...
  std::cout << MOOS::ConsoleColours::green() << m_lListenPort
            << MOOS::ConsoleColours::reset() << "\n";
