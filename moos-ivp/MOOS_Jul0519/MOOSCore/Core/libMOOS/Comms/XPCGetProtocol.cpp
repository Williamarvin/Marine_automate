...
XPCGetProtocol::~XPCGetProtocol() { MOOS::ScopedLock L(_ProtocolLock); }
...
char XPCGetProtocol::cGetNextProtocol() {
  ...
  if (_index + 1 >= static_cast<int>(_protocols.size()))
    return 0;
  ++_index;
  return 1;
}
...
XPCGetProtocol::ProtoEnt::ProtoEnt(struct protoent const *ent)
    : _name(ent ? ent->p_name : ""), _number(ent ? ent->p_proto : 0) {
  if (ent == 0)
    return;
...
std::string const &XPCGetProtocol::ProtoEnt::name() const { return _name; }

int XPCGetProtocol::ProtoEnt::number() const { return _number; }
