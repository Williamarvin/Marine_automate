...
public:
  ...
  CMOOSVariable();
  CMOOSVariable(std::string sName, std::string sSubscribe, std::string sPublish,
...
protected:
  ...
  double m_dfCommsTime; // time used when registering (how often should we
                        // receive updates?
...
operator<<(std::basic_ostream<_CharT, _Traits> &OutputStream,
           const CMOOSVariable &MOOSVar) {
