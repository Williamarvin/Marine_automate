...
public:
  ...
  XPCGetProtocol() { vOpenProtocolDb(); }
#endif
  ...
  char const *sGetProtocolName() {
    return _index >= static_cast<int>(_protocols.size())
               ? ""
               : _protocols[_index].name().c_str();
  }

  // Returns the protcol number
  int iGetProtocolNumber() {
    return _index >= static_cast<int>(_protocols.size())
               ? 0
               : _protocols[_index].number();
  }
};
#endif
