...
/** \internal
        @brief provides a queue (serviced by a thread and callback) of
   CMOOSMsg's
...
public:
  ...
  void SetCallback(bool (*pfn)(CMOOSMsg &M, void *pParamCaller),
                   void *pCallerParam);

  // install a callback onto the queue but supply a instance of a class and a
  // member function which takes single message as a parameter
  template <class T>
