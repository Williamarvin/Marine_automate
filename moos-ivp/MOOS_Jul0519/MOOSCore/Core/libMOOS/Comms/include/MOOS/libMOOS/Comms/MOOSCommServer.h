...
#if !defined(                                                                  \
    AFX_MOOSCOMMSERVER_H__2FDF870F_F998_4D3C_AD18_FCC2C5C12DDA__INCLUDED_)
...
/** This class is the MOOS Comms Server. It lies at the heart of the
communications architecture and typically is of no interest to the component
developer. It maintains a list of all the connected clients and their names. It
simultaneously listens on all sockets for calling clients and then calls a user
supplied call back to handle the request. This class is only used by the CMOOSDB
application*/
...
public:
  ...
  /** Set the recieve message call back handler. The callback will be called
  whenever a client sends one or more messages to the server. The supplied call
  back must be of the form static bool MyCallBack(MOOSMSG_LIST &
  RxLst,MOOSMSG_LIST & TxLst, void * pParam).
  @param sClient    Name of client at the end of the socket sending this Pkt
  @param RxLst    contains the incoming messages.
  @param TxLst    passed to the handler as a recepticle for all the message that
  should be sent back to the client in response to the incoming messages.
  @param pParam      user suplied parameter to be passed to callback function
  */
  void SetOnRxCallBack(bool (*pfn)(const std::string &sClient,
                                   MOOSMSG_LIST &MsgListRx,
                                   MOOSMSG_LIST &MsgListTx, void *pParam),
                       void *pParam);

  /** Set the disconnect message call back handler.  The supplied call back must
  be of the form static bool MyCallBack(std::string  & sClient,, void * pParam).
  @param sClient   Name of client at the end of the socket sending this Pkt
  @param pParam      user suplied parameter to be passed to callback function
  */
  void SetOnDisconnectCallBack(bool (*pfn)(std::string &sClient, void *pParam),
                               void *pParam);

  /** Set the connect  call back handler.  The supplied call back must be of the
  form static bool MyCallBack(std::string  & sClient,, void * pParam).
  @param sClient   Name of client just connected
  @param pParam      user suplied parameter to be passed to callback function
  */
  void SetOnConnectCallBack(bool (*pfn)(std::string &sClient, void *pParam),
                            void *pParam);
   ...
   * @param pParam
   */
  void SetOnFetchAllMailCallBack(bool (*pfn)(const std::string &sClient,
                                             MOOSMSG_LIST &MsgListTx,
                                             void *pParam),
                                 void *pParam);

  /** This function is the listen loop called from one of the two server
  threads. It is responsible for accepting a coonection and creating a new
  client socket.    */
  virtual bool ListenLoop();

  /** This function is the server loop called from one of the two server
  threads. It listens to all presently connected sockets and when a call is
  received invokes thse user supplied callback */
  ...
  /** specify a threshold above which the DB will print a warning if the time
   * between the time stamp in a message and it being printed is exceeded
   * @param dfPeriod
   * @return
   */
  void SetWarningLatencyMS(double dfPeriod);

  /** Initialise the server. This is a non blocking call and launches the MOOS
  Comms server threads.
  @param lPort port number to listen on
  */
  virtual bool Run(long lPort, const std::string &sCommunityName,
  ...
  /** used to control how verbose the server is. Setting to true turns off all
   * Tracing */
...
protected:
  ...
  /** figures out what the largest socket FD of all connected sockets. (needed
   * by select)*/
  int GetMaxSocketFD();
  ...
  /** Send a Poisoned mesasge to the client on the end of pSocket. This may
  cause the client comms thrad to die */
  ...
  */
  ...
  @see SetOnRxCallBack */
  bool (*m_pfnRxCallBack)(const std::string &sClient, MOOSMSG_LIST &MsgListRx,
                          MOOSMSG_LIST &MsgListTx, void *pCaller);

  /** place holder for the address of the object passed back to the user during
  an Rx callback
  @see SetOnRxCallBack */
  ...
  /** user supplied FetchAllMail callback which is called after a message has
     caused a call back
      @see SetOnFetchAllMailCallBack */
  bool (*m_pfnFetchAllMailCallBack)(const std::string &sClient,
                                    MOOSMSG_LIST &MsgListTx, void *pCaller);

  /** place holder for the address of the object passed back to the user during
  an FetchAllMail callback
  @see SetOnFetchAllMailCallBack */
  ...
  /** map of socket file descriptors to the std::string  name of the client
   * process at the other end*/
  SOCKETFD_2_CLIENT_NAME_MAP m_Socket2ClientMap;

  /** map of socket file descriptors to the std::string  name of the client if
   * that client supports asynchronous reception of data*/
  ...
  /** called from Server loop this function handles all the processing for the
  current client call. It inturn invokes the user supplied callback function */
