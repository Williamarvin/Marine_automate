...
#if !defined(                                                                  \
    AFX_MOOSCOMMOBJECT_H__88A30007_9205_4FDA_B938_915FBE43027D__INCLUDED_)
...
class provides basic Receive and Transmit capabilities of CMOOSMsg's and
CMOOSCommPkts. Where messages are passed as parameters then there are
transparently packed into packets.*/
...
protected:
  ...
  bool ReadPkt(XPCTcpSocket *pSocket, CMOOSCommPkt &PktRx,
...
public:
  /**
   * called to initialise system socket services. Only does something useful in
   * Win32 land
   */
  static bool SocketsInit();

  /**
   * Configure the simulation of sick communications - only useful in stress
   * testing - not usually used.
   * @param dfDodgeyCommsProbability prob a delay in communications
   * @param dfDodgeyCommsDelay length of delay
   * @param dfTerminateProbability probability of application termination during
   * comms
   * @return
   */
  bool ConfigureCommsTesting(double dfDodgeyCommsProbability,
                             double dfDodgeyCommsDelay,
                             double dfTerminateProbability = 0.0);
