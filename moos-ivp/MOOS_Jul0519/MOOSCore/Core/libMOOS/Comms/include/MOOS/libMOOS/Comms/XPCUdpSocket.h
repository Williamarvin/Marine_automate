...
public:
  ...
  // Sends a message to a specified host on a specified port. The number of
  // bytes sent is returned
  int iSendMessageTo(void *_vMessage, int _iMessageSize, long int nPort,
  ...
  // int iReadMessageWithTimeOut(void *_vMessage, int _iMessageSize, double
  // dfTimeOut,int _iOption=0);

protected:
  bool GetAddress(long int nPort, const std::string &sHost,
                  sockaddr_in &Address);
