...
// 5 seconds time difference between client clock and MOOSDB clock will be
// allowed
...
public:
  ...
  CMOOSMsg(char cMsgType, const std::string &sKey, double dfVal,
           double dfTime = -1);

  /** specialised construction*/
  CMOOSMsg(char cMsgType, const std::string &sKey, const std::string &sVal,
           double dfTime = -1);

  /** specialised construction for binary data*/
  CMOOSMsg(char cMsgType, const std::string &sKey, unsigned int nDataSize,
  ...
  bool IsString() const {
    return IsDataType(MOOS_STRING) || IsDataType(MOOS_BINARY_STRING);
  }
  ...
  /**return true if mesage is substantially (SKEW_TOLERANCE) older than
     dfTimeNow if pdfSkew is not NULL, the time skew is returned in *pdfSkew*/
