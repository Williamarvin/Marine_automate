...
public:
  ...
  /** Wildcard unregister */
  bool UnRegister(const std::string &sVarPattern,
  ...
  @param MsgList a list of messages into which the newly received message will
  be placed
  @return true if there is new mail */
  bool Fetch(MOOSMSG_LIST &MsgList);

  /** place a single message in the out box and return immediately. Completion
  of this method does not imply transmission. However transmission will occur at
  the next available oppurtunity. In practice the apparent speed of message
  transmission will be very fast indeed. This model however prevents wayward
  user software bring down the MOOSComms by way of denial of service. (ie
  hogging the network)
  @param Msg reference to CMOOSMsg which user wishes to send*/
  virtual bool Post(CMOOSMsg &Msg, bool bKeepMsgSourceName = false);

  /** internal method which runs in a seperate thread and manages the input and
  output of messages from the server. DO NOT CALL THIS METHOD.*/
  virtual bool ClientLoop();

  /** called by the above to do the client mail box shuffling **/
  virtual bool DoClientWork();

  /** Run the MOOSCommClient Object. This call is non blocking and begins
  managing process IO with the MOOSComms protocol
  @param sServer Name of machine on which server resides eg LOCALHOST or
  guru.mit.edu
  @param lPort   number of port on which server is listening for new connections
  eg 9000
  @param sMyName std::string name by which this MOOS process will be known - eg
  "MotionController" or "DepthSensor"
  @param nFundamentalFrequency the basic tick frequency of the comms loop.
  Default value of 5 implies mail will be retrieved and sent from the server at
  5Hz
  */
  // bool Run(const char * sServer,long lPort, const char * sMyName, unsigned
  // int nFundamentalFreq=5);
  bool Run(const std::string &sServer, int Port, const std::string &sMyName,
           unsigned int nFundamentalFrequency = 5);

  /** set the user supplied OnConnect call back. This callback , when set, will
  be invoked when a connection to the server is made. It is a good plan to
  register for notification of variables in this callback
  ...
  /** set the user supplied OnConnect call back. This callback , when set, will
  be invoked when a connection to the server is lost.
  @param pfn pointer to static function of type bool Fn(void * pParam)
  @param pCallerParam parameter passed to callback function when invoked*/
  void SetOnDisconnectCallBack(bool (*pfn)(void *pParamCaller),
                               void *pCallerParam);

  /** return true if a mail callback is installed */
  bool HasMailCallBack();

  /** Directly and asynchronously make a request to the server (use this very
  rarely if ever. It's not meant for public consumption)
  @param sWhat string specifying what request to make - ALL, DB_CLEAR,
  PROCESS_SUMMARY or VAR_SUMMARY
  @param MsgList List of messages returned by server
  @param dfTimeOut TimeOut
  @param bContinuouslyClearBox true if all other message returned with query are
  to be discarded.*/
  bool ServerRequest(const std::string &sWhat, MOOSMSG_LIST &MsgList,
                     double dfTimeOut = 2.0, bool bContinuouslyClearBox = true);

  /**a static helper function that lets a user browse a mail list
  the message is removed if bremove is true*/
  static bool PeekMail(MOOSMSG_LIST &Mail, const std::string &sKey,
                       CMOOSMsg &Msg, bool bErase = false,
                       bool bFindYoungest = false);

  /**a static helper function that lets a user browse a mail list as for
   PeekMail. true is returned if mail is found and it is not more that 5 seconds
   old*/
  static bool PeekAndCheckMail(MOOSMSG_LIST &Mail, const std::string &sKey,
                               CMOOSMsg &Msg, bool bErase = false,
                               bool bFindYoungest = false);

  /** Have a peek at mail box and remove a particular message, by default all
  other messages are removed. Note this is quite different from ::PeekMail*/
  bool Peek(MOOSMSG_LIST &List, int nIDRequired, bool bClearBox = true);

  /** describe this client in a string */
  std::string GetDescription();

  /** call with "true" if you want this client to fake its own outgoing name.
   * This is rarely used but very useful when it is */
  ...
  /** return true if this client is Asynchronous (so can have data pushed to it
   * at any time)*/
  ...
  /** used to control whether local clock skew (used by MOOSTime())  is se via
   the server at the other end of this connection */
  ...
  /** Set the number of ms between loops of the Comms thread (not relevent if
   * teh cleint is Asynchoronous)*/
  ...
  /** set the user supplied OnMail callback. This is an internal function - do
   * not call it yourself if you want rapid response use V10 */
  ...
  template <class T>
  bool AddWildcardActiveQueue(const std::string &sQueueName,
                              const std::string &sPattern, T *Instance,
                              bool (T::*memfunc)(CMOOSMsg &));
  ...
  bool GetClientCommsStatus(const std::string &sClient,
...
protected:
  ...
  /** true if we want to be able to fake sources of messages (used by
   * playback)*/
  bool m_bFakeSource;
  ...
  /** performs a handshake with the server when a new connection is made. Within
  this function this class tells the server its name*/
  bool HandShake();
  ...
  */
  ...
  /** a set of strings of the resources (messages names/keys) that have been
   * registered for */
  ...
  /** true if after handshaking DB announces its ability to support aysnc
   * comms*/
  bool m_bDBIsAsynchronous;

  /** true if we expect Comms to overflow and want older (unsent) messages to be
