...
XPCTcpSocket *XPCTcpSocket::Accept(char *_sHost) {
  ...
  if ((iNewSocket = (short)accept(iSocket, (struct sockaddr *)&clientAddress,
                                  (socklen_t *)&iClientAddressLen)) == -1) {
    char sMsg[512];
...
#else
  if ((iNumBytes = send(iSocket, _vMessage, _iMessageSize, 0)) == -1)
#endif
  {
    char sMsg[512];
...
int XPCTcpSocket::iRecieveMessageAll(void *_vMessage, int _iMessageSize) {
  ...
  // While the number of bytes received is less than the number
  // requestedcontinue to retrieve more data
  while (iNumBytes < iCurrentSize) {
    ...
    if (iNumBytes == -1) {
      // If the reason for failure is a client disconnect, an exception isnot
      // thrown. The number of bytes returned is 0
    ...
    // If the total number of bytes requested are not returned, theoffset is
    // adjusted and the number of bytes left to receive is also adjusted
...
int XPCTcpSocket::iRecieveMessage(void *_vMessage, int _iMessageSize,
                                  int _iOption) {
  int iNumBytes; // The number of bytes recieved

#ifdef WINDOWS_NT
  if (_iOption == MSG_WAITALL)
    // If the option is MSG_WAITALL and this is a WINDOW_NT machinecall
    // iReceiveMessageAll to process it
  ...
  if (iNumBytes == -1) {
    // If the reason for failure is a client disconnect, anexception is not
    // thrown. The number of bytes returned is 0
#ifdef UNIX
    if (errno == ECONNRESET)
      return 0;
...
int XPCTcpSocket::iReadMessageWithTimeOut(void *_vMessage, int _iMessageSize,
                                          double dfTimeOut, int _iOption) {
  int iNumBytes = 0;
  ...
  // actually writes over the timeout structure on completion of select (now
  ...
  int iSelectRet = select(iGetSocketFd() + 1, &fdset, NULL, NULL, &timeout);
...
void XPCTcpSocket::vSetNoDelay(int _iToggle) {
  if (setsockopt(iSocket, IPPROTO_TCP, TCP_NODELAY, (char *)&_iToggle,
                 sizeof(_iToggle)) == -1) {
    char sMsg[512];
