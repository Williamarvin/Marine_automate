...
XPCSocket::XPCSocket(const char *_sProtocol, int _iPort) {
  ...
  try {
    ...
    if (strcmp(socketProtocol.sGetProtocolName(), "udp") == 0) {
      if ((iSocket = socket(AF_INET, SOCK_DGRAM,
                            socketProtocol.iGetProtocolNumber())) == -1) {
        char sMsg[512];
    ...
    // If the protocol is TCP a TCP socket is created
      ...
      if ((iSocket = socket(AF_INET, SOCK_STREAM,
                            socketProtocol.iGetProtocolNumber())) == -1) {
        char sMsg[512];
    ...
    sprintf(sMsg, "Protocol Error Definition: %s",
            exceptObject.sGetException());
    XPCException socketExcept(sMsg);
    throw socketExcept;
  }
...
void XPCSocket::vSetDebug(int _iToggle) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_DEBUG, (char *)&_iToggle,
                 sizeof(_iToggle)) == -1) {
    char sMsg[512];
...
void XPCSocket::vSetBroadcast(int _iToggle) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_BROADCAST, (char *)&_iToggle,
                 sizeof(_iToggle)) == -1) {
    char sMsg[512];
...
void XPCSocket::vSetReuseAddr(int _iToggle) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&_iToggle,
                 sizeof(_iToggle)) == -1) {
    char sMsg[512];
...
void XPCSocket::vSetKeepAlive(int _iToggle) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_KEEPALIVE, (char *)&_iToggle,
                 sizeof(_iToggle)) == -1) {
    char sMsg[512];
...
void XPCSocket::vSetLinger(struct linger _lingerOption) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_LINGER, (char *)&_lingerOption,
...
void XPCSocket::vSetSendBuf(int _iSendBufSize) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_SNDBUF, (char *)&_iSendBufSize,
                 sizeof(_iSendBufSize)) == -1) {
    char sMsg[512];
...
void XPCSocket::vSetRecieveBuf(int _iRecieveBufSize) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_SNDBUF, (char *)&_iRecieveBufSize,
                 sizeof(_iRecieveBufSize)) == -1) {
    char sMsg[512];
...
#endif
      {
        sprintf(sMsg, "Error Turning ON Socket Blocking Status: %s",
                sGetError());
        XPCException sockOptExcept(sMsg);
        throw sockOptExcept;
      }
...
#endif
      {
        sprintf(sMsg, "Error Turning OFF Socket Blocking Status: %s",
                sGetError());
        XPCException sockOptExcept(sMsg);
        throw sockOptExcept;
      }
...
int XPCSocket::iGetDebug() {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_DEBUG, (char *)&iGetOption,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
int XPCSocket::iGetBroadcast() {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_BROADCAST, (char *)&iGetOption,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
int XPCSocket::iGetReuseAddr() {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&iGetOption,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
int XPCSocket::iGetKeepAlive() {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_KEEPALIVE, (char *)&iGetOption,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
void XPCSocket::vGetLinger(struct linger &_lingerOption) {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_LINGER, (char *)&_lingerOption,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
int XPCSocket::iGetSendBuf() {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_SNDBUF, (char *)&iSendBuf,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
int XPCSocket::iGetRecieveBuf() {
  ...
  if (getsockopt(iSocket, SOL_SOCKET, SO_RCVBUF, (char *)&iRcvBuf,
                 (socklen_t *)&iOptionLen) == -1) {
    char sMsg[512];
...
void XPCSocket::vSetRecieveTimeOut(int nTimeOut) {
  if (setsockopt(iSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&nTimeOut,
                 sizeof(nTimeOut)) == -1) {
    char sMsg[512];
