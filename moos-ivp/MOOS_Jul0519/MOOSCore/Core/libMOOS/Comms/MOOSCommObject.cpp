...
void CMOOSCommObject::SimulateCommsError() {
  if (MOOSUniformRandom(0.0, 1.0) < m_dfDodgeyCommsProbability) {
    ...
    std::cout << "faking slow connection..." << m_dfDodgeyCommsDelay
              << "s sleep\n";
...
bool CMOOSCommObject::ReadPkt(XPCTcpSocket *pSocket, CMOOSCommPkt &PktRx,
                              int nSecondsTimeout) {
#define CHUNK_READ 8192
  ...
  while ((nRqd = PktRx.GetBytesRequired()) != 0) {
    ...
    try {
      if (nRqd < CHUNK_READ) {
        ...
        if (nSecondsTimeout < 0) {
          ...
          nRxd = pSocket->iReadMessageWithTimeOut(PktRx.NextWrite(), nRqd,
                                                  (double)nSecondsTimeout);
        }
      } else {
        if (nSecondsTimeout < 0) {
          nRxd = pSocket->iRecieveMessage(PktRx.NextWrite(), CHUNK_READ);
        } else {
          nRxd = pSocket->iReadMessageWithTimeOut(PktRx.NextWrite(), CHUNK_READ,
                                                  (double)nSecondsTimeout);
        }
      }
    ...
    case 0:
      if (nSecondsTimeout > 0)
        throw CMOOSException(MOOSFormat(
            "remote side closed or lazy client ( waited more than %ds )",
            nSecondsTimeout));
      else
    ...
    default:
      if (!PktRx.OnBytesWritten(PktRx.NextWrite(), nRxd))
        throw CMOOSException(
            "CMOOSCommObject::ReadPkt() Failed Rx - Packet rejects filling");
      break;
    }
  }
...
// bool CMOOSCommObject::ReadPktV2(XPCTcpSocket *pSocket, CMOOSCommPkt &PktRx,
// int nSecondsTimeout)
//{
//     #define CHUNK_READ 8192
...
//                {
//                    nRxd  =
//                    pSocket->iReadMessageWithTimeOut(pBuffer,nRqd,(double)nSecondsTimeout);
//                }
//            }
//            else
//            {
...
//                {
//                    nRxd  =
//                    pSocket->iReadMessageWithTimeOut(pBuffer,CHUNK_READ,(double)nSecondsTimeout);
//                }
//            }
//        }
//        catch(XPCException e)
...
//        case 0:
//            if(nSecondsTimeout>0)
//                throw CMOOSException(MOOSFormat("remote side closed or lazy
//                client ( waited more than %ds )",nSecondsTimeout));
//            else
//                throw CMOOSException("remote side closed....");
//            break;
//        default:
...
bool CMOOSCommObject::SendPkt(XPCTcpSocket *pSocket, CMOOSCommPkt &PktTx) {
  ...
  try {
    ...
    if (m_bFakeDodgyComms) {
      ...
      SimulateCommsError();
      nSent += pSocket->iSendMessage(PktTx.Stream() + sizeof(int),
                                     PktTx.GetStreamLength() - sizeof(int));
    } else {
...
bool CMOOSCommObject::ReadMsg(XPCTcpSocket *pSocket, CMOOSMsg &Msg,
                              int nSecondsTimeout) {
  MOOSMSG_LIST MsgList;
