...
/// This class is used in the new massively improved timing protocol added in
/// the summer of 2008 by Alastair Harrison of MRG arh@robots.ox.ac.uk
...
double CMOOSSkewFilter::SmoothingFilter(double dfDT, double dfOldFilterVal,
                                        double dfNewMeas,
                                        double dfGradient) const {
...
void CMOOSSkewFilter::UpdateEnvelopes(double dfTXtime, double dfSkewLB,
                                      double dfSkewUB) {
...
double CMOOSSkewFilter::Update(double dfRQtime, double dfTXtime,
                               double dfRXtime, tSkewInfo *skewinfo) {
  ...
  if (m_nMeas < 10)
    dfSkew = dfSkewLB; // UB often in error early on
...
bool CConvexEnvelope::GetLongestSeg(tSeg &seg) const {
  if (GetNumSegs() < 1)
    return false;
  if (m_uiLongestSegID >= GetNumSegs())
    return false;
...
bool CConvexEnvelope::GetLineParams(const tPt &p1, const tPt &p2, double &M,
                                    double &C) const {
  if (p1.x == p2.x)
    return false;
