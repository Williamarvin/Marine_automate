...
bool MulticastNode::ReadLoop() {
  ...
  try {
    ...
    int reuse = 1;
    if (setsockopt(socket_rx_, SOL_SOCKET, SO_REUSEADDR /* SO_REUSEPORT*/,
    ...
    if (setsockopt(socket_rx_, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                   (const char *)&mreq, sizeof(mreq)) == -1) {
      throw std::runtime_error(
          "MulticastNode::ReadLoop()::setsockopt::ADD_MEMBERSHIP");
    }
    ...
    while (!read_thread_.IsQuitRequested()) {
      if (MOOS::WaitForSocket(socket_rx_, 1)) {
        ...
        int n = recvfrom(socket_rx_, (char *)t, sizeof(t), 0,
    ...
    std::cerr << "MulticastNode::ReadLoop issue with socket creation :"
              << e.what() << "\n";
    return false;
  }
...
bool MulticastNode::WriteLoop() {
  ...
  try {
    ...
    // we want to be able to resuse it (multiple folk are interested)
    int reuse = 1;
    if (setsockopt(socket_tx, SOL_SOCKET, SO_REUSEADDR /* SO_REUSEPORT*/,
                   (const char *)&reuse, sizeof(reuse)) == -1) {
      throw std::runtime_error("MulticastNode::WriteLoop()::setsockopt::reuse");
    }

    if (setsockopt(socket_tx, IPPROTO_IP, IP_MULTICAST_TTL,
                   (const char *)&hops_, sizeof(hops_)) == -1) {
      throw std::runtime_error("MulticastNode::WriteLoop()::setsockopt::reuse");
    }
    ...
    while (!write_thread_.IsQuitRequested()) {
      ...
      if (!outbox_.IsEmpty() || outbox_.WaitForPush(100)) {
        outbox_.Pull(v);
        int nSent = sendto(socket_tx, (char *)v.data(), v.size(), 0,
                           (struct sockaddr *)&mc_addr, sizeof(mc_addr));
        if (nSent != (int)v.size()) {
          std::cerr << "MulticastNode::WriteLoop() failed to send complete "
                       "telegram\n";
