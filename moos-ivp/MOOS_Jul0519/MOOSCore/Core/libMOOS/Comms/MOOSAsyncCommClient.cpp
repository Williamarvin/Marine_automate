...
/// default destructor
MOOSAsyncCommClient::~MOOSAsyncCommClient() { Close(); }

std::string MOOSAsyncCommClient::HandShakeKey() { return "asynchronous"; }

void MOOSAsyncCommClient::DoBanner() { return; }
...
bool MOOSAsyncCommClient::Flush() { return true; }
...
bool MOOSAsyncCommClient::IsAsynchronous() { return true; }
...
bool MOOSAsyncCommClient::DoWriting() {
  ...
  try {
    ...
    for (MOOSMSG_LIST::iterator q = StuffToSend.begin(); q != StuffToSend.end();
         ++q) {
      if (q->IsType(MOOS_TERMINATE_CONNECTION)) {
        // std::cerr<<"writing thread receives terminate connection request from
    ...
    if ((MOOSLocalTime(false) - m_dfLastTimingMessage) >
        TIMING_MESSAGE_PERIOD) {
...
bool MOOSAsyncCommClient::MonitorAndLimitWriteSpeed() {
  ...
  unsigned int sleep_ms = static_cast<unsigned int>(TotalDelay);
  if (sleep_ms > 0) {
    // std::cerr << "I'm sleeping for " << TotalDelay << " ms
    // ("<<TotalDelay/GetMOOSTimeWarp()<<" real ms)\n";
    ...
    m_InLock.Lock();
    {
      ...
      case MOOS_TIMING: {
        ...
        if (m_bDoLocalTimeCorrection && GetNumPktsReceived() > 1) {
          ...
          UpdateMOOSSkew(q->GetTime(), q->GetDouble(), dfLocalRxTime);
        }
      ...
      case MOOS_NULL_MSG: {
        ...
        if (m_bDoLocalTimeCorrection) {
          UpdateMOOSSkew(dfTimeSentToDBApprox, dfTimeSentFromDB, dfLocalRxTime);
        }
