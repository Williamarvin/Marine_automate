...
CMOOSDBVar::CMOOSDBVar()
    : m_cDataType(MOOS_NOT_SET), m_sName(), m_dfTime(-1.0), m_dfVal(-1.0),
      m_dfWriteFreq(0.0), m_dfWrittenTime(-1.0), m_sVal(), m_sWhoChangedMe(),
      m_sSrcAux(), m_sOriginatingCommunity(), m_Stats(), m_nWrittenTo(0),
      m_Subscribers(), m_Writers() {}

CMOOSDBVar::CMOOSDBVar(const string &sName)
    : m_cDataType(MOOS_NOT_SET), m_sName(sName), m_dfTime(-1.0), m_dfVal(-1.0),
      m_dfWriteFreq(0.0), m_dfWrittenTime(-1.0), m_sVal(), m_sWhoChangedMe(),
      m_sSrcAux(), m_sOriginatingCommunity(), m_Stats(), m_nWrittenTo(0),
      m_Subscribers(), m_Writers() {}
...
bool CMOOSDBVar::AddSubscriber(const string &sClient, double dfPeriod) {
  ...
  if (sClient.empty()) {
    MOOSTrace("[X] Failed to add subscription to \"%s\" for a client with "
              "empty name \n",
              m_sName.c_str());
    return false;
  }
...
void CMOOSDBVar::RemoveSubscriber(string &sWho) {
  ...
  if (p != m_Subscribers.end()) {
    // MOOSTrace("MOOSDB: Removing \"%s\"'s subscription to
