...
void CMOOSDBHTTPServer::Initialise(long lDBPort, long lWebServerPort) {
  ...
  m_pMOOSComms->Run("localhost", lDBPort, "DBWebServer",
                    WEBSERVER_COMMCLIENT_TICK);

  // we'll be accessing the same DB interface from lots of threads (one for each
  // connected browser so we had better be safe
...
bool CMOOSDBHTTPServer::Listen() {
  ...
  try {
    ...
    UNUSED_PARAMETER(e);
    MOOSTrace("Error binding to HTTP listen socket - Is there another "
              "HTTPDBServer Running?\n");
  ...
  while (1) {
    ...
    try {
      ...
      CHTTPConnection *pNewConnection =
          new CHTTPConnection(pNewSocket, m_pMOOSComms, m_pMOOSCommsLock);
