  ...
  if (rVar.m_cDataType == Msg.m_cDataType) {
    ...
    if (dfDT > 0.5) {
      ...
      if (dfDT > 10.0) {
        ...
        rVar.m_dfWriteFreq =
            dfAlpha * rVar.m_dfWriteFreq + (1.0 - dfAlpha) / (df);
      }
    ...
    MOOSTrace("Attempting to update var \"%s\" which is type %c with data type "
              "%c by client %s (=%s)\n",
...
next call in they shall be informed of the change by stuffing this msg into a
return packet */
