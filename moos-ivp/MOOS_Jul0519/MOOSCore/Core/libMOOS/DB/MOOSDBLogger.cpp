...
struct DBLogEvent {
  DBLogEvent(){};
  DBLogEvent(const std::string &event_name, const std::string &event_client,
             const std::string &event_details, const std::string &event_time)
      : event_name_(event_name), event_client_(event_client),
        event_details_(event_details), event_time_(event_time) {
...
public:
  ...
  std::string TimeStamp() {
    ...
    return MOOSFormat("%.2d:%.2d:%.2d %d/%d/%d ", Now->tm_hour, Now->tm_min,
  ...
  bool AddEvent(const std::string &sEvent, const std::string &sClient,
    ...
    if (thread_.IsThreadRunning()) {
      return event_list_.Push(
          DBLogEvent(sEvent, sClient, sDetails, TimeStamp()));
    } else
      return false;
  }
  ...
  bool Work() {
    try {
      ...
      output_stream.open(output_stream_name_.c_str(),
