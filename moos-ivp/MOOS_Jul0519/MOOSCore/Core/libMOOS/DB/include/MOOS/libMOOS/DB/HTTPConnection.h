...
public:
  ...
  /** simple constructor*/
  CHTTPConnection(XPCTcpSocket *pNewSocket, CMOOSCommClient *pMOOSComms,
...
protected:
  ...
  bool BuildSingleVariableWebPageContents(std::ostringstream &wp,
                                          MOOSMSG_LIST &MsgList);
  /** build we page of whole DB*/
  bool BuildFullDBWebPageContents(std::ostringstream &wp,
                                  MOOSMSG_LIST &MsgList);
  ...
  /** pointer to a COmms Client (The DB Connection is shared between all
   * HTTPConnections)*/
  CMOOSCommClient *m_pMOOSComms;

  /** pointer to a lock which provides protected access to the MOOS comms client
   * object*/
