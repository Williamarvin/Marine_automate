...
/** The CMOOSDB class is the core of the MOOS comms protocol. It is only of
interest to the developer modifying the MOOSDB application server*/
class CMOOSDB {
public:
  bool OnDisconnect(std::string &sClient);
  bool OnConnect(std::string &sClient);

  /** callback function passed to CMOOSCommServer member object. This STATIC
  function allows entry back into this object by invoking OnRxPkt()*/
  static bool OnRxPktCallBack(const std::string &sClient,
                              MOOSMSG_LIST &MsgLstRx, MOOSMSG_LIST &MsgLstTx,
                              void *pParam);
  ...
  static bool OnFetchAllMailCallBack(const std::string &sWho,
                                     MOOSMSG_LIST &MsgListTx, void *pParam);

  /** called internally when a MOOSPkt (a collection of MOOSMsg's ) is
  received by the server */
  bool OnRxPkt(const std::string &sClient, MOOSMSG_LIST &MsgLstRx,
  ...
  /** called by the owning application to start the DB running. It launches
  threads and returns */
...
protected:
  ...
  Very occasionally a client with a singularly unusual role may want to ask
  questions directly to the DB. The paired function is in
  MOOCCommClient::ServerRequest which unusually, is blocking (with timeout) -
  hence my edgey feel about these utilities*/
...
private:
  ...
  // pointer to the comms server (could be a threaded one but base class is
  // CMOOSCommServer
