...
std::string MsgFilter::app_filter() const { return filters_.first; }
std::string MsgFilter::var_filter() const { return filters_.second; }

std::string MsgFilter::as_string() const {
  return var_filter() + ":" + app_filter();
}

double MsgFilter::period() const { return period_; }
MsgFilter::MsgFilter() {
  period_ = 0.0;
