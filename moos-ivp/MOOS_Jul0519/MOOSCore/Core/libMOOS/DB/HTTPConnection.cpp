...
public:
  CHTMLTag(std::ostringstream &sStream, const std::string &sTag,
           const std::string &sOptions = "", const std::string &sContents = "",
           bool bNewLine = true)
      : m_Stream(sStream), m_sTag(sTag) {
    Open(sStream, m_sTag, sOptions, sContents, bNewLine);
  }

  ~CHTMLTag() { Close(m_Stream, m_sTag); }

  static void Open(std::ostringstream &sStream, const std::string &sTag,
                   const std::string &sOptions = "",
                   const std::string &sContents = "", bool bNewLine = false) {
...
CHTTPConnection::CHTTPConnection(XPCTcpSocket *pSocket,
    ...
    : m_pSocket(pSocket), m_pMOOSComms(pMOOSComms),
      m_pMOOSCommsLock(pMOOSCommsLock) {}
...
bool CHTTPConnection::Serve() {
  ...
  // closing socket too quickly (and I mean with a few uS) prevents final read
  // of page even though XPCSocket reports all data is sent
  MOOSPause(100);
    ...
    {
      {
        CHTMLTag HTML(wp, "HTML", "style=\"font-family:arial\"");
        {
          ...
          wp << "<meta http-equiv=\"Content-Type\" content=\"text/html; "
                "charset=utf-8\">\n";
          // wp<<"<meta http-equiv=\"refresh\"
          // content=\"0.5;URL=http://localhost:9080\">\n";
...
bool CHTTPConnection::BuildSingleVariableWebPageContents(
    std::ostringstream &wp, MOOSMSG_LIST &MsgList) {
  ...
  if (m_pMOOSComms->PeekMail(MsgList, m_sFocusVariable, Msg)) {

    {
      ...
      // data row
      {
        CHTMLTag Row(wp, "TR");
        {
          ...
          std::string sControl = MOOSFormat(
              "<input name=NewValue value=\"%s\">", Msg.GetAsString().c_str());
    ...
    wp << CHTMLTag::Print(
        "p", "", "edit value box and press return to poke (set) this variable");

  } else {
    wp << CHTMLTag::Print(
        "H2", "ALIGN=middle,TEXT=red",
        MOOSFormat("The variable %s does not exist in this DB!",
                   m_sFocusVariable.c_str()));
  }
...
bool CHTTPConnection::BuildFullDBWebPageContents(std::ostringstream &wp,
...

  {
    ...
    for (q = MsgList.begin(); q != MsgList.end(); ++q) {
      ...
      wp << CHTMLTag::Print(
          "TD", "valign=\"left\"",
          CHTMLTag::Print("A", "href = /" + rMsg.GetName(), rMsg.GetName()));
      wp << CHTMLTag::Print("TD", "valign=\"left\"",
                            MOOSFormat("%.3f", rMsg.GetTime()));
      wp << CHTMLTag::Print("TD", "valign=\"left\"",
                            MOOSFormat("%s", sT.c_str()));
      wp << CHTMLTag::Print("TD", "valign=\"left\"",
                            MOOSFormat("%.3f", rMsg.m_dfVal2));
      wp << CHTMLTag::Print("TD", "valign=\"left\"", rMsg.GetSource());
      wp << CHTMLTag::Print("TD", "valign=\"left\"", rMsg.GetCommunity());
      wp << CHTMLTag::Print("TD",
                            "valign=\"left\" style=\"WORD-BREAK:BREAK-ALL\"",
                            rMsg.GetAsString().c_str());
    }
  }

  wp << CHTMLTag::Print("p", "",
                        "Click on a name column for individual variable pages. "
                        "<A href = /> refresh </A>");
    ...
    CHTMLTag Form(wp, "FORM", "action='/'");
    {
      CHTMLTag Table(wp, "TABLE");
      {
...

        {
          ...
          wp << CHTMLTag::Print("TD", "",
                                "<input type=submit name=DoPoke value=Poke>");
        }
      }
    }

    wp << CHTMLTag::Print("p", "",
                          "Add a new MOOS variable or change an existing one");
  }
