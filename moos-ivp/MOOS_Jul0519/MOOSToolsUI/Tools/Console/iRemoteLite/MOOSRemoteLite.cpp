...
bool CMOOSRemoteLite::StartThreads() {
  ...
  m_hWDThread = ::CreateThread(NULL, 0, WDLoopProc, this, CREATE_SUSPENDED,
                               &m_nWDThreadID);
  ResumeThread(m_hWDThread);

  // this is the main WD thread
  m_hMailThread = ::CreateThread(NULL, 0, MailLoopProc, this, CREATE_SUSPENDED,
                                 &m_nMailThreadID);
  ...
  int Status =
      pthread_create((pthread_t *)&m_nWDThreadID, NULL, WDLoopProc, this);
  ...
  Status =
      pthread_create((pthread_t *)&m_nMailThreadID, NULL, MailLoopProc, this);
...
bool CMOOSRemoteLite::Run(const char *sName, const char *sMissionFile) {
...
void CMOOSRemoteLite::PrintHelp() {
  ...
  for (p = m_CustomKeys.begin(); p != m_CustomKeys.end(); p++) {
    ...
    MOOSTrace("%c     %s with %s\n", rKey.m_cChar, rKey.m_sKey.c_str(),
              rKey.m_sVal.c_str());
  }
}
      ...
      for (p = MailIn.begin(); p != MailIn.end(); p++) {
        ...
        if (MOOSStrCmp(p->m_sKey, "NAV_SUMMARY")) {
          ...
          MOOSTrace(">%-10s @ %7.2f \"%s\"\n", p->m_sSrc.c_str(),
                    p->m_dfTime - GetAppStartTime(), sMsg.c_str());
        }
      ...
      if (MOOSStrCmp(sTok, "CUSTOMKEY")) {
        ...
        if (!sChar.empty()) {
          ...
          if (isdigit(NewKey.m_cChar)) {
            ...
            MOOSTrace("CMOOSRemote: can only bind custom keys to numeric "
                      "characters!\n");
          }
        }
      }
    ...
    if (confirmed) {
      if (rKey.bIsNumeric) {
        ...
        MOOSTrace("CustomKey[%c] : %s {%f}\n", rKey.m_cChar,
                  rKey.m_sKey.c_str(), atof(rKey.m_sVal.c_str()));
      } else {
        m_Comms.Notify(rKey.m_sKey, rKey.m_sVal);
        MOOSTrace("CustomKey[%c] : %s {%s}\n", rKey.m_cChar,
                  rKey.m_sKey.c_str(), rKey.m_sVal.c_str());
      }
    } else {
...
bool CMOOSRemoteLite::MakeCustomJournals() {
  ...
  if (m_MissionReader.GetConfiguration(GetAppName(), sParams)) {
    ...
    for (p = sParams.begin(); p != sParams.end(); p++) {
      ...
      // CUSTOMJOURNAL = Name = <MOOSNAME>, Key = <key>, History  = Size ,
      // Period = <Time>
      if (MOOSStrCmp(sTok, "CUSTOMJOURNAL")) {
        ...
        MOOSTrace("  Journal Started for %s:\n    bound to \"%c\"\n    history "
                  "of %d\n    collecting %s \n",
                  sName.c_str(), cKey, nSize,
                  dfT > 0 ? MOOSFormat("at %f Hz", 1.0 / dfT).c_str()
                          : "every notification");
