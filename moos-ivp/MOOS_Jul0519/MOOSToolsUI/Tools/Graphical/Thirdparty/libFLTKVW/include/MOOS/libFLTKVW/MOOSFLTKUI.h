...
public:
  ...
  struct ControlCallBackInfo {
    ControlCallBackInfo(Fl_Widget *pW, void *p, int i) {
      pWidget = pW;
      pThis = p;
      ID = i;
    }
  ...
  void SetID(Fl_Widget *pWidget, int ID) {
    ControlCallBackInfo *pCCBO =
        new ControlCallBackInfo(pWidget, (void *)this, ID);
  ...
  void StopTimer() { Fl::remove_timeout(TimerCallBack, this); }
