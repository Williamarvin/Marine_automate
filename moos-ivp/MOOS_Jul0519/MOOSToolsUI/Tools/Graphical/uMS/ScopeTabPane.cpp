...
//////////////////////////////////////////////////////////////////////
CScopeTabPane::~CScopeTabPane() { StopTimer(); }

CScopeTabPane::CScopeTabPane(int X, int Y, int W, int H, char *l)
    : BASE(X, Y, W, H, l) {
...

  {
    ...
    // the process list
    m_pProcessList = new Fl_Check_Browser(LHS, BOTTOM_GRID + 10, PROC_W, PROC_H,
                                          "Processes");
    SetID(m_pProcessList, ID_PROCESS);
    m_pProcessList->tooltip(
        "Click a process name to examine subscriptions and publications");
    m_pProcessList->textsize(FONT_SIZE);
    m_pProcessList->when(FL_WHEN_RELEASE);

    // the subscriber list
    m_pSubscribeList =
        new Fl_Browser(m_pProcessList->x() + PROC_W + 30, BOTTOM_GRID + 10,
                       PROC_W, PROC_H, "Subscribes");
    m_pSubscribeList->textsize(FONT_SIZE);

    // the publish list
    m_pPublishList =
        new Fl_Browser(m_pSubscribeList->x() + PROC_W + 5, BOTTOM_GRID + 10,
                       PROC_W, PROC_H, "Publishes");
    m_pPublishList->textsize(FONT_SIZE);

    Fl_Button *pLB = new Fl_Button(m_pPublishList->x() + PROC_W + 5,
                                   m_pPublishList->y(), 15, 20, "?");
    ...
    Fl_Group *pMOOSParams = new Fl_Group(RHS - 180, BOTTOM_GRID + 10, 180, 100);
    {
      ...
      m_pDBHostInput =
          new Fl_Input(RHS - 180, BOTTOM_GRID + 10, 100, 25, "HostName");
      ...
      m_pDBPortInput = new Fl_Int_Input(
          m_pDBHostInput->x(), m_pDBHostInput->y() + m_pDBHostInput->h() + 5,
          100, 25, "Port");
      ...
      m_pConnectButton = new Fl_Button(
          m_pDBPortInput->x(), m_pDBPortInput->y() + m_pDBPortInput->h() + 5,
          160, 25, "Connect");
...
void CScopeTabPane::OnControlWidget(Fl_Widget *pWidget, int ID) {
  ...
  case ID_PROCESS: {
    if (Fl::event_shift()) {
      ...
      m_pProcessList->checked(m_pProcessList->value(),
                              m_ProcessOptions[GetFocusProcess()].m_bShow);
...
void CScopeTabPane::OnTimer() {
  ...
  if (pActive == this) {
    ...
    if (m_nCounts % 5 == 0) {
      ...
      m_pScopeGrid->SetTitle(MOOSFormat("%d Processes %d Variables",
                                        sProcs.size(),
                                        m_DBImage.GetNumVariables())
                                 .c_str());
    }
  }
}
