...
public:
  ...
  void value(const std::string &sStr) {
    ...
    int W = w(), H = h();
    fl_measure(m_sText, W, H,
               Fl_Align(FL_ALIGN_LEFT | FL_ALIGN_WRAP | FL_ALIGN_TOP));
    size(W + 10, H + 10);
    redraw();
  }
};
...
public:
  ...
  static void GridCallback(CScopeGrid *pMe, void *) { pMe->OnGridCallBack(); }
  CScopeGrid(int X, int Y, int W, int H, const char *l = 0);
  virtual void draw_cell(int Offset, int &X, int &Y, int &W, int &H, int R,
                         int C);
