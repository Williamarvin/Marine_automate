...
Fl_Group *MakeCommunityPane(int x, int y, int w, int h,
...
void OnRemoveCommunity(Fl_Widget *pWidget, void *pParam) {
  ...
  if (pTab->children() > 1) {
    if (fl_choice("Really delete Scope of \"%s\" ?", "No", "Yes", NULL,
                  pTab->value()->label())) {
...
int main(int argc, char *argv[]) {
  ...
  {
    // add
    Fl_Button *pNewCommunityButton =
        new Fl_Button(10, 420, 160, 30, "Add Community");
    pNewCommunityButton->callback(OnAddCommunity, pTab);

    // delete
    Fl_Button *pDelCommunityButton =
        new Fl_Button(180, 420, 160, 30, "Remove Community");
