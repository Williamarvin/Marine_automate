...
CScopeGrid::CScopeGrid(int X, int Y, int W, int H, const char *l)
    : Flv_Table(X, Y, W, H, l) {
  m_pDBImage = NULL;
  m_nCount = 0;
  m_pComms = NULL;
...
void CScopeGrid::draw_cell(int Offset, int &X, int &Y, int &W, int &H, int R,
                           int C) {
  Flv_Style s;
  get_style(s, R, C);
  ...
  if (R == -1) {
    // this is the header:
    const char *ColumnNames[] = {"Name",   "Time",      "Type", "Freq",
                                 "Source", "Community", "Value"};
...
void CScopeGrid::SetDBImage(CDBImage *pDBImage) { m_pDBImage = pDBImage; }
