...
bool CMOOSPlayBack::IsOpen() { return m_InputFile.is_open(); }
...
int CMOOSPlayBack::GetSize() { return m_PlayBackList.size(); }

double CMOOSPlayBack::GetTimeNow() {
  return m_dfLastMessageTime - m_dfLogStart;
}

int CMOOSPlayBack::GetCurrentLine() { return m_nLastLine; }
...
bool CMOOSPlayBack::IsEOF() { return m_bEOF; }
...
double CMOOSPlayBack::GetLastMessageTime() { return m_dfLastMessageTime; }
...
bool CMOOSPlayBack::IsWaitingForClient() { return m_bWaitingForClientCatchup; }
...
bool CMOOSPlayBack::GotoTime(double dfT) {
  ...
  for (q = m_PlayBackList.begin(); q != m_PlayBackList.end(); q++) {
    nTmp++;
    if (q->m_dfTime > dfT) {
      m_pListIterator = q;
      m_nLastLine = nTmp;
