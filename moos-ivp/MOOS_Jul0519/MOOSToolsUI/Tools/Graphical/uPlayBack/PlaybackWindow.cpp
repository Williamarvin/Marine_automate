...
CPlaybackWindow::CPlaybackWindow(int X, int Y, int W, int H, const char *l)
    : BASE(X, Y, W, H, l) {
...
bool CPlaybackWindow::SetTitle(bool bOnline) {
  ...
  sprintf(sTitle, "uPlayback : %s:%d %s", m_sServerHost.c_str(),
          (int)m_lServerPort,
          m_Comms.IsConnected() || bOnline ? "Online" : "Offline");
  p->label(sTitle);
  p->labelcolor(FL_RED);
  return true;
}
...
bool CPlaybackWindow::OnFile() {
  ...
  const char *pFile =
      fl_file_chooser("Select an alog file", "*.alog", LastFile);
  if (pFile) {
    // CWaitCursor Cursor;
    ...
    if (m_PlayBack.Initialise(pFile)) {
      ...
      sprintf(sFileSize, "%.2f KB loaded",
              (double)m_PlayBack.m_ALog.GetSize() / 1024);
...
bool CPlaybackWindow::OnMOOS() {
  ...
  return m_Comms.Run(m_sServerHost.c_str(), m_lServerPort, "uPlayback",
                     DEFAULT_COMMS_TICK);
}
...
bool CPlaybackWindow::ShowProgress() {
  ...
  double dfDuration = m_PlayBack.GetFinishTime() - m_PlayBack.GetStartTime();
  double dfElapsed =
      m_PlayBack.GetLastMessageTime() - m_PlayBack.GetStartTime();
  ...
  sprintf(Time, "%.2d:%.2d.%.2d", pClock->hour(), pClock->minute(),
          pClock->second());
  pClock->label(Time);
