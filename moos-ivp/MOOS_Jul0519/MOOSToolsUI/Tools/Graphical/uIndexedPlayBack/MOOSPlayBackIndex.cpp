...
bool CMOOSPlayBackIndex::IsOpen() { return m_ALog.IsOpen(); }

int CMOOSPlayBackIndex::GetSize() { return m_ALog.GetLineCount(); }

double CMOOSPlayBackIndex::GetTimeNow() {
  return m_dfLastMessageTime - m_dfLogStart;
}

int CMOOSPlayBackIndex::GetCurrentLine() { return m_nLastLine; }
...
bool CMOOSPlayBackIndex::MessageFromLine(const std::string &sLine,
  ...
  if (MOOSIsNumeric(sData)) {
    ...
    if (sData.find("<MOOS_BINARY>") != std::string::npos) {
      ...
      // corner case of binary file changing half way through a log....(Why this
      // might happen I don't know but catch it anyway....
      ...
      if (!m_BinaryFile.is_open()) {
        ...
        // here we need to point the file towards the full path - the blog file
        // should be sitting next to teh alog being played

        /** splits a fully qualified path into parts -path, filestem and
         * extension */
        std::string sPath, sFileName, sExtension;

        if (!MOOSFileParts(m_ALog.GetFileName(), sPath, sFileName, sExtension))
          return MOOSFail("failed to parse alog file into file parts %s",
                          MOOSHERE);
...
double CMOOSPlayBackIndex::GetLastMessageTime() { return m_dfLastMessageTime; }
