...
bool CMOOSCommunity::InitialiseMOOSClient(const string &sHostName, long nPort,
                                          const string &sMOOSName, int nFreq) {
  ...
  m_bMOOSClientRunning = m_CommClient.Run(
      sHostName.c_str(), nPort, sMOOSName.c_str(), DEFAULT_BRIDGE_COMMS_TICK);
...
bool CMOOSCommunity::IsMOOSClientRunning() { return m_bMOOSClientRunning; }

std::string CMOOSCommunity::GetCommunityName() { return m_sCommunityName; }
...
bool CMOOSCommunity::HasUDPConfigured() const { return m_nUDPPort != -1; }

int CMOOSCommunity::GetUDPPort() { return m_nUDPPort; }

std::string CMOOSCommunity::GetUDPHost() { return m_sUDPHost; }

bool CMOOSCommunity::Fetch(MOOSMSG_LIST &Mail) {
  return m_CommClient.Fetch(Mail);
}

bool CMOOSCommunity::Post(CMOOSMsg &M) { return m_CommClient.Post(M); }
...
string CMOOSCommunity::GetFormattedName() {
  return MOOSFormat("%s@%s", m_sCommunityName.c_str(),
                    m_CommClient.GetDescription().c_str());
}

string CMOOSCommunity::GetCommsName() { return m_CommClient.GetDescription(); }
