...
bool CMOOSUDPLink::Post(CMOOSMsg &M, const std::string &sHost, long int nPort) {
  ...
  if (m_pUDPSocket == NULL)
    return MOOSFail(
        "CMOOSUDPLink::Post No functioning UDP socket - has run been called\n");
  ...
  try {
    if (P.GetStreamLength() > MAX_UDP_PKT_SIZE) {
      // too big
      throw XPCException(
          "serialised packet is too big to send ....ask for an upgrade");
    }

    int nSent = m_pUDPSocket->iSendMessageTo(P.Stream(), P.GetStreamLength(),
                                             nPort, sHost);
...
bool CMOOSUDPLink::ListenLoop() {
  ...
  while (!m_ListenThread.IsQuitRequested()) {
    int nRead =
        m_pUDPSocket->iRecieveMessage((unsigned char *)Buff, sizeof(Buff));
    CMOOSCommPkt P;
...
bool CMOOSUDPLink::ReadPktFromArray(unsigned char *pBuffer, int nBytes,
                                    CMOOSCommPkt &PktRx) {
