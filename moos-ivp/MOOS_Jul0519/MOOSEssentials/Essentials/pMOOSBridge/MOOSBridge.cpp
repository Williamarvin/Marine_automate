...
bool CMOOSBridge::Run(const string &sMissionFile, const string &sMOOSName) {
  ...
  if (!Configure()) {
    return MOOSFail("MOOSBridge failed to configure itself - probably a "
            ...
            if (pDestCommunity->WantsToSink(Index)) {
              ...
              if (IsUDPShare(Index)) {
                if (pDestCommunity->HasUDPConfigured()) {
                  ...
                  MOOSTrace("UDP posting %s to community %s@%s:%d as %s "
                            "(source community is %s)\n",
                            w->GetKey().c_str(),
                            pDestCommunity->GetCommunityName().c_str(),
                            pDestCommunity->GetUDPHost().c_str(),
                            pDestCommunity->GetUDPPort(),
                            MsgCopy.m_sKey.c_str(), w->GetCommunity().c_str());
#endif

                  // Send via UDP (directed to  single machine and port) - fire
                  // and forget...
                  m_UDPLink.Post(MsgCopy, pDestCommunity->GetUDPHost(),
                                 pDestCommunity->GetUDPPort());
                } else {
                  MOOSTrace("cannot send %s via UDP to %s - destination "
                            "community has no UDP port\n",
    ...
    if (qq != m_Communities.end()) {
      ...
      for (p = UDPMail.begin(); p != UDPMail.end(); p++) {
        ...
        MOOSTrace("Received %s from %s on UDP  - inserting into %s\n",
                  p->GetKey().c_str(), p->GetCommunity().c_str(),
                  pLocalCommunity->GetFormattedName().c_str());
#endif

        // now be careful we aren't looking to subscribing for this mail....now
        // that would be some horrible positive feedback loop! Can alos check as
        // mikerb suggess on source community - it must not be us!
...
bool CMOOSBridge::Configure() {
  ...
  // if user set LOOPBACK = TRUE then both src and destination communities can
  // be identical default is FALSE this means if src=dest then the bridging will
  // be ignored
  ...
  if (!m_MissionReader.GetValue("COMMUNITY", m_sLocalCommunity)) {
    MOOSTrace(
        "WARNING : Cannot read ::MOOS-scope variable COMMUNITY - assuming %s\n",
        m_sLocalCommunity.c_str());
  }

  if (!m_MissionReader.GetValue("SERVERPORT", sLocalPort)) {
    MOOSTrace(
        "WARNING :Cannot read ::MOOS-scope variable SERVERPORT - assuming %s\n",
        sLocalPort.c_str());
  }

  if (!m_MissionReader.GetValue("SERVERHOST", sLocalHost)) {
    MOOSTrace(
        "WARNING :Cannot read ::MOOS-scope variable SERVERHOST - assuming %s\n",
        sLocalHost.c_str());
  }

  // how fast should the bridge operate in Hz (setting this to zero is a special
  // case and makes all registrations with dfPeriod = 0)
  m_nBridgeFrequency = DEFAULT_BRIDGE_FREQUENCY;
  ...
  for (q = sParams.begin(); q != sParams.end(); q++) {
    ...
    // COMMUNITYNAME@HOSTNAME:PORT [VarAlias1,....]
    //  or using mission file defaults i.e file scope constants
    // SHARE = [VAR1,VAR2,VAR3,....] -> COMMUNITYNAME@HOSTNAME:PORT
    // [VarAlias1,....]
    ...
    if (MOOSStrCmp(sCmd, "SHARE") || MOOSStrCmp(sCmd, "UDPSHARE")) {
      ...
      if (sSrc[0] == '[') {
        ...
        MOOSTrace("Using abbreviated configuration protocol Source: %s@%s:%s\n",
                  sSrcCommunity.c_str(), sSrcCommunityPort.c_str(),
      ...
      long lSrcPort = 0;
      if (sSrcCommunity.empty() || sSrcCommunityHost.empty() ||
          sSrcCommunityPort.empty()) {
        MOOSTrace("error on SHARED configuration %s\n correct line format is "
                  "\nSHARE = COMMUNITYNAME@HOSTNAME:PORT [VAR1,VAR2,VAR3,....] "
                  "-> COMMUNITYNAME@HOSTNAME:PORT\n",
                  q->c_str());
        continue;
      } else {
      ...
      long lDestPort = 0;
      if (sDestCommunity.empty() || sDestCommunityHost.empty() ||
          sDestCommunityPort.empty()) {
        MOOSTrace("error on SHARED configuration %s\n correct line format is "
                  "\nSHARE = COMMUNITYNAME@HOSTNAME:PORT [VAR1,VAR2,VAR3,....] "
                  "-> COMMUNITYNAME@HOSTNAME:PORT\n",
                  q->c_str());
      ...
      // we will force all broadcast address directives to be the same
      // "community" called "ALL"
      if (MOOSStrCmp(sDestCommunityHost, "BROADCAST") ||
          MOOSStrCmp(sDestCommunity, "ALL")) {
        // this is trixksy - need to qualify this generic address with the a
        // port so each Bridge can UDP broadcast to multiple addresses
        sDestCommunity = "ALL";
      ...
      if (!bUDP) {
        ...
        std::string sFullyQualifiedMOOSName =
            m_MissionReader.GetAppName() + "@" + m_sLocalCommunity;

        // for (connecting to) the source community (where messages come from)
        if (!pSrcCommunity->IsMOOSClientRunning()) {
          pSrcCommunity->InitialiseMOOSClient(sSrcCommunityHost, lSrcPort,
        ...
        if (!pDestCommunity->IsMOOSClientRunning()) {
          pDestCommunity->InitialiseMOOSClient(sDestCommunityHost, lDestPort,
                                               sFullyQualifiedMOOSName,
                                               m_nBridgeFrequency);
        }

      } else {
        // MOOSTrace("Setting UDP port for community %s as
        // %s:%d\n",pDestCommunity->GetCommunityName().c_str(),sDestCommunityHost.c_str(),lDestPort);
        if (sDestCommunityHost.find("BROADCAST-") != std::string::npos &&
            MOOSStrCmp(sDestCommunity, "ALL")) {
          // this is special
      ...
      if (pSrcCommunity && pDestCommunity) {
        ...
        while (!sVar.empty()) {
          ...
          if (bUDP) {
            // we need to store in the Bridge class what variables appearing in
            // our local community we are asked to forward on via UDP to some
            // other commnity
  ...
  /// think about setting up UDP connections....there is one UDP Link per
  /// instance of a
  // MOOSBridge. If poepl wnat UDP bridging they need on pMOOSBridge per
  // community (ie the toplogy of N communities and just 1 bridge is not allowed
  ...
  if (m_MissionReader.GetConfigurationParam("UDPListen", nLocalUDPPort)) {
    ...
    MOOSTrace("warning no UDPListen port specified for local community - "
              "outgoing UDP comms only\n");

    // passing run with a -1 port means build the socket but don't bind or start
    // a listen thread
    m_UDPLink.Run(-1);
  }
  // ensure we have at least the local MOOS-enabled community in existence -
  // maybe all we want to do is map LocalMOOS->UDP_Out
  ...
  if (pLocalCommunity != NULL && !pLocalCommunity->IsMOOSClientRunning()) {
    ...
    pLocalCommunity->InitialiseMOOSClient(sLocalHost, atoi(sLocalPort.c_str()),
