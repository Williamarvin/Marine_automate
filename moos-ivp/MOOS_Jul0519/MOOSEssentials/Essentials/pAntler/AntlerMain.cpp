...
int main(int argc, char *argv[]) {
    ...
    if (t.find("--") == 0) {
      ...
      if (t.empty()) {
        if (MOOSStrCmp(sVar, "quiet")) {
          ...
          MOOSTrace("error incomplete overloading of parameter  --%s=value "
                    "(are there extraneous whitespaces?)\n",
                    sVar.c_str());
          return -1;
        }
      } else {
      ...
      // nope this is a regular parameter so it gets passed onto what is yet to
      // come
      vArgv.push_back(t);
    }
  }
  ...
  case 3: {
    ...
    // this rather fancy looking bit f stl simply iterates over a list of
    // strings
    std::copy(istream_iterator<std::string>(S), istream_iterator<string>(),
