...
public:
  XPCProcessAttrib(LPTSTR appName = NULL, LPSTR commandLine = NULL,
                   BOOL processInheritable = FALSE,
                   BOOL threadInheritable = FALSE, BOOL inheritHandles = FALSE,
                   DWORD creationFlags = 0, LPVOID environment = NULL,
  ...
  LPVOID GetEnvironmentBlock() { return pEnvironment; }
  ...
  void vSetInheritHandles(BOOL bInherit) { bInheritHandles = bInherit; }

  void vSetCreationFlag(DWORD dwBitmap) { dwCreationFlags |= dwBitmap; }
  ...
  LPPROCESS_INFORMATION pGetProcessInformation() { return pProcessInfo; }
