...
XPCProcess::XPCProcess(XPCProcessAttrib &attrib) : procAttrib(attrib) {

  if (!CreateProcess(attrib.pApplicationName, attrib.pCommandLine,
                     attrib.pProcessSA, attrib.pThreadSA,
                     attrib.bInheritHandles, attrib.dwCreationFlags,
                     attrib.pEnvironment, attrib.pCurrentDirectory, attrib.pSI,
                     attrib.pProcessInfo)) {
...
DWORD XPCProcess::dwGetExitCode() {
  ...
  if (!GetExitCodeProcess(procAttrib.pGetProcessInformation()->hProcess,
                          &dwRet)) {
...
void XPCProcess::vResume() {
  if (ResumeThread(procAttrib.pGetProcessInformation()->hThread) ==
      0xFFFFFFFF) {
