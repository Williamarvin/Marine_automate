...
XPCProcessAttrib::XPCProcessAttrib(LPTSTR appName, LPSTR commandLine,
                                   BOOL processInheritable,
                                   BOOL threadInheritable, BOOL inheritHandles,
                                   DWORD creationFlags, LPVOID environment,
                                   LPTSTR currentDirectory) {
  pSI = (LPSTARTUPINFO)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                                 sizeof(STARTUPINFO));

  pProcessSA = (LPSECURITY_ATTRIBUTES)HeapAlloc(
      GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(SECURITY_ATTRIBUTES));

  pThreadSA = (LPSECURITY_ATTRIBUTES)HeapAlloc(
      GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(SECURITY_ATTRIBUTES));

  pProcessInfo = (LPPROCESS_INFORMATION)HeapAlloc(
      GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(PROCESS_INFORMATION));
...
void XPCProcessAttrib::AddEnvironmentVariable(LPCTSTR param, LPCTSTR val) {
  ...
  if (pEnvironment) {
    pEnvironment = HeapReAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, pEnvironment,
                               dwEnvSize += strlen(param) + strlen(val) + 2);
  } else {
    pEnvironment = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
...
void XPCProcessAttrib::vSetCommandLine(LPTSTR commandLine) {
  ...
  if (commandLine) {
    pCommandLine =
        (LPTSTR)HeapAlloc(GetProcessHeap(), 0, strlen(commandLine) + 1);
    if (!pCommandLine) {
      XPCException ex(ErrorString("HeapAlloc failed"));
      throw ex;
      return;
...
void XPCProcessAttrib::vSetApplicationName(LPTSTR appName) {
  ...
  if (appName) {
    pApplicationName =
        (LPTSTR)HeapAlloc(GetProcessHeap(), 0, strlen(appName) + 1);
    if (!pApplicationName) {
      XPCException ex(ErrorString("HeapAlloc failed"));
      throw ex;
      return;
...
void XPCProcessAttrib::vSetCurrentDirectory(LPTSTR currentDirectory) {
  ...
  if (currentDirectory) {
    pCurrentDirectory =
        (LPTSTR)HeapAlloc(GetProcessHeap(), 0, strlen(currentDirectory) + 1);
    if (!pCurrentDirectory) {
      XPCException ex(ErrorString("HeapAlloc failed"));
      throw ex;
      return;
