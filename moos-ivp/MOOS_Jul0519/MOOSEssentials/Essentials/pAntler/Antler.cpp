...
bool CAntler::Run(const std::string &sHost, int nPort,
    ...
    if (m_pMOOSComms->Fetch(NewMail)) {
      ...
      if (m_bHeadless) {
        ...
        if (m_pMOOSComms->PeekMail(NewMail, "MISSION_FILE", Msg)) {
          ...
          m_sReceivedMissionFile =
              MOOSFormat("runtime_%s.moos", MOOSGetTimeStampString().c_str());
...
bool CAntler::ConfigureMOOSComms() {
  ...
  if (!m_pMOOSComms->Run(m_sDBHost.c_str(), (long int)m_nDBPort, sMe.c_str(),
                         1))
...
bool CAntler::SendMissionFile() {
  ...
  // copy the filters in
  ss << "ANTLERFILTER:";
  std::copy(m_Filter.begin(), m_Filter.end(),
...
#else
  // MOOSTrace("Warning - gentle killing of win32 processes is not
  // implemented\n");
...
bool CAntler::Spawn(const std::string &sMissionFile, bool bHeadless) {
  ...
  m_MissionReader.SetAppName(
      "ANTLER"); // NB no point in running under another name...(I guess Anter1
                 // could launch Antler2 though...)
  ...
  m_MissionReader.GetConfigurationParam("MSBetweenLaunches",
                                        nTimeMSBetweenSpawn);

  // here we'll figure out a what paths to use when looking for  executables
  m_sDefaultExecutablePath = "SYSTEMPATH";
  m_MissionReader.GetConfigurationParam("ExecutablePath",
  ...
  // no cycle through each line in the configuration block. If it begins with
  // run then it means launch
  ...
  if (bHeadless == false) {
    ...
    m_MissionReader.GetConfigurationParam("KillOnDBDisconnect",
                                          m_bKillOnDBDisconnect);
  }
...
bool CAntler::MakeExtraExecutableParameters(
    std::string sParam, STRING_LIST &ExtraCommandLineParameters,
...
bool CAntler::MakeConsoleLaunchParams(std::string sParam,
  ...
  if (!MOOSValFromString(sLaunchConfigurationName, sParam, sLaunchKey)) {
    // some applications are v.important in MOOS if not told otherwise they get
    // special colours
    ...
    if (bNIX) {
      sLaunchConfiguration =
          "-geometry," + MOOSFormat("80x12+2+%d", (m_nCurrentLaunch++) * 50) +
          ",+sb," + ",-fg," + sFgColor + ",-bg," + sBgColor + ",-T," +
          MOOSFormat("%s %s", sProcName.c_str(),
                     sMOOSName.empty()
                         ? ""
                         : MOOSFormat("as MOOSName \"%s\"", sMOOSName.c_str())
                               .c_str());
    } else {
    ...
    if (!m_MissionReader.GetConfigurationParam(sLaunchConfigurationName,
                                               sLaunchConfiguration))
      return MOOSFail(
          "   warning: could not find resource string called \"%s\"",
...
CAntler::MOOSProc *CAntler::CreateMOOSProcess(string sConfiguration) {
  ...
  // do we want to inhibit the passing of MOOS parameters (mission file and
  // MOOSName)
  ...
  // "ExectutablePath=<Path>" in the mission file configuration block. user has
  // the option of specifying paths individually process by process.
  // alternativelt they can specify that a particular process shouod be located
  // by  system wide and not in the default executable path
  ...
  // here we figure out what MOOS name is implied if none is given (thanks to
  // N.P and F.A)
  if (sMOOSName.empty()) {
    ...
    if (sTmp.empty()) {
      // ended with a / ?
      MOOSTrace(
          "Error in configuration  -MOOS Name cannot end in \" / \" : %s\n",
          sProcName.c_str());
      return NULL;
    }
    sMOOSName = sTmp;
  ...
  // it is pssible to specifiy complicated parameters to the process being
  // launched. (For example xterm being passed a whole load of configurations
  // and then the name of the MOOS process it should itself launch. This next
  // call fills in a string list of such parameters
  STRING_LIST sLaunchParams;
  if (bNewConsole)
    MakeConsoleLaunchParams(sOption, sLaunchParams, sProcName, sMOOSName);

  // here we figure what extra command line parameters should be given to the
  // MOOS Process being launched.
  STRING_LIST ExtraCommandLineParameters;
  MakeExtraExecutableParameters(sOption, ExtraCommandLineParameters, sProcName,
                                sMOOSName);
...
bool CAntler::DoWin32Launch(CAntler::MOOSProc *pNewProc) {
  ...
  try {
    ...
    // continuing this task, here we pass extra parameters to the MOOS process
    // if required
    for (STRING_LIST::iterator p =
             pNewProc->m_ExtraCommandLineParameters.begin();
         p != pNewProc->m_ExtraCommandLineParameters.end(); p++) {
      sCmd += *p + " ";
    }
    ...
    // set up white text
    StartUp.dwFillAttribute = FOREGROUND_INTENSITY | FOREGROUND_BLUE |
    ...
    for (STRING_LIST::iterator q = pNewProc->m_ConsoleLaunchParameters.begin();
         q != pNewProc->m_ConsoleLaunchParameters.end(); q++) {
...
bool CAntler::DoNixOSLaunch(CAntler::MOOSProc *pNewProc) {
  ...
  if ((pNewProc->m_ChildPID = fork()) < 0) {
    ...
    unsigned int nExecParams = pNewProc->m_ConsoleLaunchParameters.size() +
                               pNewProc->m_ExtraCommandLineParameters.size() +
                               6;
  ...
  // does get sent to all the launched xterms. Instead we will catch ctrl-C
  // ourself and find the MOOS process running within the sub process, signal it
  // to die so subprocesses can clean up nicely
  if (m_bSupportGentleKill)
