...
bool CMOOSScheduler::StartThreads() {
  ...
  m_hScheduleThread = ::CreateThread(NULL, 0, ScheduleLoopProc, this,
                                     CREATE_SUSPENDED, &m_nScheduleThreadID);
  ResumeThread(m_hScheduleThread);

#else

  int Status = pthread_create((pthread_t *)&m_nScheduleThreadID, NULL,
                              ScheduleLoopProc, this);
...
bool CMOOSScheduler::Iterate() { return true; }
...
bool CMOOSScheduler::CEvent::GetOutput(double dfTimeNow, MOOSMSG_LIST &Out) {
  if (dfTimeNow > m_dfFireTime && m_bEnabled) {
    ...
    CMOOSMsg Msg(MOOS_NOTIFY, m_sName.c_str(), m_sVal.c_str(), dfTimeNow);
...
bool CMOOSScheduler::CEvent::Initialise(string sName, string sVal,
                                        double dfPeriod, double dfOffset) {
  m_sName = sName;
  m_sVal = sVal;
  m_dfPeriod = dfPeriod;
  m_dfOffset = dfOffset;
...
bool CMOOSScheduler::CEvent::OnNewMail(MOOSMSG_LIST &NewMail) {
  CMOOSMsg Msg;
  if (!m_sStartFlag.empty() &&
      CMOOSCommClient::PeekMail(NewMail, m_sStartFlag, Msg)) {
    if (!Msg.IsSkewed(MOOSTime())) {
      Enable(true, MOOSTime());
    }
  }
  if (!m_sStopFlag.empty() &&
      CMOOSCommClient::PeekMail(NewMail, m_sStopFlag, Msg)) {
    if (!Msg.IsSkewed(MOOSTime())) {
  ...
  if (!m_sVarName.empty() &&
      CMOOSCommClient::PeekMail(NewMail, m_sVarName, Msg)) {
    if (!Msg.IsSkewed(MOOSTime())) {
