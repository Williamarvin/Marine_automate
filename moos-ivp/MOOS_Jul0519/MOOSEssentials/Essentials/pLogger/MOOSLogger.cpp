...
#define DEFAULT_WILDCARD_TIME                                                  \
  1.0 // how often to call into the DB to get a list of all variables if wild
      // card loggin is turned on
#define DEFAULT_DOUBLE_PRECISION                                               \
  5 // how many DP to use when logging double time stamps
...
CMOOSLogger::CMOOSLogger() {
  ...
  // by default to not mark (with a @) variables being sent from external
  // communities
...
CMOOSLogger::~CMOOSLogger() { ShutDown(); }
...
bool CMOOSLogger::OnStartUp() {
  ...
  // are we required to run an exclusion log (which is where wildcard rejections
  // can be sent for paranoid people
  m_MissionReader.GetConfigurationParam("WildcardExclusionLog",
                                        m_bUseExcludedLog);

  // what sort of file name are we using
  m_MissionReader.GetConfigurationParam("FILETIMESTAMP",
                                        m_bAppendFileTimeStamp);
  ...
  m_MissionReader.GetConfigurationParam("MarkExternalCommunityMessages",
                                        m_bMarkExternalCommunityMessages);
  ...
  if (!m_MissionReader.GetValue("GLOBALLOGPATH", m_sPath)) {
    ...
    if (!m_MissionReader.GetConfigurationParam("PATH", m_sPath)) {
      MOOSTrace("Warning:\n\tneither \"::GlobalLogPath\" or \"Path\" are "
                "specified\n");
...
#ifndef ZLIB_FOUND
    m_bCompressAlog = false;
    MOOSTrace("warning:\n\talogs will not be compressed because zlib was not "
              "found at build time");
#endif
  }
...
bool CMOOSLogger::ConfigureLogging() {
  ...
  if (m_MissionReader.GetConfiguration(m_sAppName, Params)) {
    ...
    MOOSTrace("Warning:\n\tNo Configuration block was read - unusual but not "
              "terminal\n");
  }
  ...
  m_MissionReader.GetConfigurationParam("DynamicSyncLogColumns",
  ...
  if (bHasMissionFile) {
    m_MissionReader.GetConfigurationParam("WildcardLogging",
                                          m_bWildCardLogging);
  } else {
  ...
  if (m_bWildCardLogging) {
    ...
    // we never want to log mission files sent between communities - this is
    // done elsewhere
    m_sWildCardOmitted.push_back("MISSION_FILE");

    // there was a request to allow multiple statements of the these
    // patterns...hence the more  complicated parsing here
    STRING_LIST sList;
  ...
  if (!RegisterMOOSVariables())
    MOOSDebugWrite(
        "Variable subscription is still pending - not terminal, but unusual");

  return true;
}

bool CMOOSLogger::HandleLogRequest(std::string sParam, std::string &sVar,
                                   bool bDynamic) {
        ...
        if (GetMOOSVar(sVar) == NULL) {
          ...
          if (m_bUseExcludedLog) {
            ...
            if (bWouldNormallyAccept && !bWouldNormallyReject) {
              ...
              // MOOSTrace("  denied added wildcard logging of %-20s   (fits
              // Omit pattern as well as Accept pattern)\n",sVar.c_str());
              bWanted = false;
            }
          }
...
struct StringMatcher {
  ...
  StringMatcher(const std::string &sStr) { m_sString = sStr; }
...
bool CMOOSLogger::IsWildCardRejected(const std::string &sVariableName) const {
  return std::find_if(m_sWildCardOmitted.begin(), m_sWildCardOmitted.end(),
...
bool CMOOSLogger::IsWildCardAccepted(const std::string &sVariableName) const {
  ...
  return std::find_if(m_sWildCardAccepted.begin(), m_sWildCardAccepted.end(),
                      StringMatcher(sVariableName)) !=
...
std::string CMOOSLogger::MakeLogName(string sStem) {
  ...
  if (m_bAppendFileTimeStamp) {
    ...
    sTmp = MOOSFormat("%s_%d_%d_%d_____%.2d_%.2d_%.2d", sStem.c_str(),
...
bool CMOOSLogger::DoSyncLog(double dfTimeNow) {
  ...
  // begin with time...
  m_SyncLogFile << setw(COLUMN_WIDTH) << setprecision(7)
                << dfTimeNow - GetAppStartTime() << ' ';
...
bool CMOOSLogger::OpenSyncFile() {
  ...
  STRING_LIST::iterator q;
  for (q = m_UnusedDynamicVariables.begin();
       q != m_UnusedDynamicVariables.end(); q++) {
    m_SyncLogFile << "%%   (" << nCount++ << ") ";

    // remember where the next string will be..
    m_DynamicNameIndex[*q] = m_SyncLogFile.tellp();

    // write the as yet unclaimed name - note we are also writing a tonne of
    // spare space so we can later acccess the file here and write a new MOOS
    // variabl name..
...
bool CMOOSLogger::LogSystemMessages(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (IsSystemMessage(rMsg.m_sKey) && !rMsg.IsSkewed(dfTimeNow)) {
      ...
      m_SystemLogFile << setw(10) << setprecision(7)
                      << rMsg.m_dfTime - GetAppStartTime() << ' ';
...
bool CMOOSLogger::IsSystemMessage(string &sKey) {
  if (MOOSStrCmp(sKey, "MOOS_DEBUG"))
    return true;
  if (MOOSStrCmp(sKey, "MOOS_SYSTEM"))
    return true;
...
bool CMOOSLogger::DoLogBanner(ostream &os, string &sFileName) {
  ...
  os << "%% LOGSTART        " << setw(20) << setprecision(16)
     << GetAppStartTime() << endl; // mikerb 12->16
  if (m_bMarkDataType)
...
bool CMOOSLogger::AddSyncLineOfTimes(double dfTimeNow) {
  ...
  int nLogVarsSize =
      m_SynchronousLogVars.size() + m_UnusedDynamicVariables.size();
  for (int nVar = 0; nVar < nLogVarsSize; nVar++) {
...
bool CMOOSLogger::OnNewSession() {
  ...
  if (!CMOOSLogger::CreateDirectory(sLogDirectory)) {
    ...
    if (!CMOOSLogger::CreateDirectory(sLogDirectory)) {
      return MOOSFail("Failed to create a logging directory either in "
                      "specified path or locally\n");
    } else {
      MOOSTrace("OK\n");
    }
  }
...
#else
    m_bCompressAlog = false;
    MOOSTrace("WARNING: alogs will not be compressed because zlib was not "
              "found at build time");
#endif
  }
...
bool CMOOSLogger::OnLoggerRestart() {
  string sTxt =
      MOOSFormat("Closing : %s", GetDirectoryName(m_sLogDirectoryName).c_str());
  MOOSDebugWrite(sTxt);
  ...
  MOOSDebugWrite(MOOSFormat("Now Logging to : %s",
                            GetDirectoryName(m_sLogDirectoryName).c_str()));
  ...
  if (m_bAsynchronousLog) {
    ...
    for (q = NewMail.begin(); q != NewMail.end(); q++) {
      ...
      if (m_MOOSVars.find(rMsg.m_sKey) != m_MOOSVars.end()) {
        ...
        sEntry << setw(15) << setprecision(5)
               << rMsg.GetTime() - GetAppStartTime()
               << ' '; // mikerb change from 3-5
        ...
        if (rMsg.IsDataType(MOOS_STRING) || rMsg.IsDataType(MOOS_DOUBLE)) {
          ...
          // here we append to the binary log and begin each line with a
          // summary....
          m_BinaryLogFile << sEntry.str();

          // write in coordinates in the alog
          sEntry << "<MOOS_BINARY>File=" << (m_sLogRootName + ".blog")
                 << ",Offset=" << m_BinaryLogFile.tellp()
                 << ",Bytes=" << rMsg.m_sVal.size() << "</MOOS_BINARY>";

          // write the binary data to file
          m_BinaryLogFile.write(rMsg.m_sVal.data(), rMsg.m_sVal.size());

          // add a new line so even the binary log file is broadly human
          // readable
...
bool CMOOSLogger::CopyMissionFile() {
  ...
  if (!MissionFile.is_open())
    return MOOSFail("\nWarning:\n\tfailed to open copy of mission file - it "
                    "can't be backed up\n");
  ;
...
bool CMOOSLogger::HandleCopyFileRequest(std::string sFileToCopy) {
  ...
  std::string sBackUpName =
      m_sLogDirectoryName + "/" + sFile + "." + sExtension;
...
bool CMOOSLogger::HandleDynamicLogRequest(std::string sRequest) {
  ...
  if (HandleLogRequest(sRequest, sNewVar, true)) {
    ...
    // sync logs - there maynot be space - but that the price you pay for not
    // talking to each other users can set DynamicSyncLogColumns = N to reserve
    // space
    if (!m_UnusedDynamicVariables.empty()) {
      ...
      if (sNewVar.size() > DYNAMIC_NAME_SPACE - 1) {
        ...
        MOOSTrace("warning:\n\t name  \"%s\" will be truncated to %d "
                  "characters in the slog\n",
                  sNewVar.c_str(), DYNAMIC_NAME_SPACE - 1);
      }
    ...
    MOOSTrace("Processed dynamic log request on \"%s\". \nSpace for %d more "
              "such requests before slog becomes full\n",
              sNewVar.c_str(), m_UnusedDynamicVariables.size());
  } else {
    MOOSDebugWrite(
        "failed to process dynamic log request  - see pLogger output\n");
    return false;
  }
