...
public:
  ...
  /** virtual overide of base class CMOOSApp member. Here we do all the
   * processing and IO*/
  bool Iterate();

  /** virtual overide of base class CMOOSApp member. Here we register for data
  we wish be informed about*/
  ...
  /** called when  command mesage is recieved  - use this channle for dynamic
   * logging*/
...
protected:
  /** called to set up machinery to log a variable geiven a command string Log =
   * Var @ etc*/
  bool HandleLogRequest(std::string sParam, std::string &sNewVariable,
                        bool bDynamic = false);
  ...
  enum LogType { XLOG, YLOG, SLOG, ALOG, UNKNOWN };
...
private:
  ...
  // collection of strings which specify names (can use wild card * and ? )
  // which should be ommited from dynamic logging
  std::vector<std::string> m_sWildCardOmitted;

  // collection of strings which specify names (can use wild card * and ? )
  // which should be allowed in dynamic logging. If empty all strings are
  // assumed to be wanted unless they are in m_sDynamicMasked
