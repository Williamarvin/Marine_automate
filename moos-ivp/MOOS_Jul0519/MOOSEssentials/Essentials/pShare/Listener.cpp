...
Listener::Listener(SafeList<CMOOSMsg> &queue, const MOOS::IPV4Address &address,
                   bool multicast)
    : queue_(queue), address_(address), multicast_(multicast) {
...
bool Listener::ListenLoop() {
  try {
    ...
    /*		if (setsockopt(socket_fd, SOL_SOCKET, SO_REUSEPORT,
                            &reuse, sizeof(reuse)) == -1)
                    throw std::runtime_error("Listener::ListenLoop()::failed to
       set resuse port option");*/
    ...
    if (setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUF, &rx_buffer_size,
    ...
    while (!thread_.IsQuitRequested()) {
      ...
      int num_bytes_read =
          read(socket_fd, incoming_buffer.data(), incoming_buffer.size());
    ...
    std::cerr << "    input route abandoned (" << address_.to_string() << ")"
              << std::endl;
