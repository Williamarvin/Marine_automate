...
protected:
  ...
  bool ProcessIOConfigurationString(std::string configuration_string,
                                    bool is_output);

  bool ProcessShortHandIOConfigurationString(std::string configuration_string,
                                             bool is_output);

  bool AddRoute(const std::string &src_name, const std::string &dest_name,
                MOOS::IPV4Address address, bool multicast, double frequency);

  bool AddMulticastAliasRoute(const std::string &src_name,
                              const std::string &dest_name,
                              unsigned int channel_num, double frequency);

  MOOS::IPV4Address
  GetAddressFromChannelAlias(unsigned int channel_number) const;

  std::string
  GetChannelAliasFromMutlicastAddress(const MOOS::IPV4Address &address) const;
...
private:
  ...
  typedef std::map<std::pair<std::string, std::string>, std::list<Route>>
      WildcardRouteMap;
...
bool Share::Impl::AddInputRoute(MOOS::IPV4Address address, bool multicast) {
  ...
  listeners_[address] = new Listener(incoming_queue_, address, multicast);
...
Share::Impl::GetRepeatedConfigurations(const std::string &token) {
  STRING_LIST params;
...
bool Share::Impl::OnStartUp() {
  ...
  try {
    /*
                    AddRoute("  X
       ","X",MOOS::IPV4Address("127.0.0.1",9010),false);
                    ...
                    ProcessIOConfigurationString("src_name
       =X,dest_name=Z,route=multicast_8",true);
                    ProcessIOConfigurationString("src_name
       =X,dest_name=Z,route=161.4.5.6:9000&multicast_21&localhost:9832",true);
    ...
    for (std::vector<std::string>::iterator q = outputs.begin();
         q != outputs.end(); q++) {
      if (q->find("route") != std::string::npos ||
          q->find("Route") != std::string::npos) {
    ...
    std::cerr << RED << "OnStartUp::exception " << e.what() << NORMAL
              << std::endl;
  }
...
bool Share::Impl::ProcessShortHandIOConfigurationString(
    std::string configuration_string, bool is_output) {
  ...
  if (is_output) {
    ...
    while (!configuration_string.empty()) {
      std::string whole_route_description =
          trim(MOOS::Chomp(configuration_string, "&"));
      ...
      if (route_description.find(":") == std::string ::npos &&
          (route_description.find("multicast_") == std::string::npos)) {
      ...
      if (parts.back().find("multicast_") != 0) {
        ...
        if (parts.size() == 3) {
          dest_name = parts.front();
          parts.pop_front();
        }
        std::string host_name = parts.front();
        parts.pop_front();
        std::string host_port = parts.front();
        parts.pop_front();
...
bool Share::Impl::ProcessIOConfigurationString(std::string configuration_string,
                                               bool is_output) {
  //	std::cout<<"ProcessShortHandIOConfigurationString...\n";
    ...
    if (incoming_queue_.Pull(new_msg)) {
      ...
      if (!m_Comms.IsRegisteredFor(new_msg.GetKey())) {
        ...
        if (verbose_) {
          ...
          std::cout << MOOS::ConsoleColours::green() << std::setw(10)
                    << std::fixed
                    << (MOOS::Time() - CMOOSApp::GetAppStartTime()) << ": "
                    << MOOS::ConsoleColours::reset();
          std::cout << "forwarding share of \"" << new_msg.GetName()
                    << "\" from " << new_msg.m_sSrc << std::endl;
        }
      }
    }
  }
...
bool Share::Impl::PublishSharingStatus() {
  ...
  for (q = routing_table_.begin(); q != routing_table_.end(); q++) {
    ...
    for (p = routes.begin(); p != routes.end(); p++) {
      ...
      if (route.multicast) {
        sso << ":multicast_"
            << route.dest_address.port() - base_address_.port();
      } else {
        // sso<<":udp";
      }
    }
...
bool Share::Impl::OnNewMail(MOOSMSG_LIST &new_mail) {
  ...
  for (q = new_mail.begin(); q != new_mail.end(); q++) {
    ...
    try {
      ...
      std::cerr << RED << "Exception thrown: " << e.what() << NORMAL
                << std::endl;
    }
  }
...
bool Share::Impl::AddRoute(const std::string &src_name,
                           const std::string &dest_name,
                           MOOS::IPV4Address address, bool multicast,
                           double frequency) {
  ...
  if (trimed_src_name.find_last_of("*?:") == std::string::npos) {
    ...
    std::list<Route> &rlist =
        wildcard_routing_table_[std::make_pair(var_pattern, app_pattern)];
...
bool Share::Impl::DoRegistrations() {
  ...
  for (g = wildcard_routing_table_.begin(); g != wildcard_routing_table_.end();
       g++) {
...
bool Share::Impl::OnConnectToServer() { return DoRegistrations(); }
...
bool Share::Impl::OnCommandMsg(CMOOSMsg Msg) {
  ...
  try {
    ...
    if (MOOSStrCmp("output", cmd)) {
      ...
      throw std::runtime_error(
          "cmd=X - X was neither \"output\" or \"input\"\n");
    }
  } catch (const std::exception &e) {
    std::cerr << RED
              << "oops failed to parse dynamic request:\n"
                 "    \""
              << e.what() << "\"\n"
              << NORMAL;
  }
...
bool Share::Impl::PrintSocketMap() {
  ...
  for (q = socket_map_.begin(); q != socket_map_.end(); q++) {
    std::cout << " " << q->first.to_string() << " -> " << q->second.socket_fd
              << std::endl;
  }
  return true;
}
...
void Share::Impl::PrintRoutes() {
  std::cout << "---------------------- " << YELLOW << "ROUTING INFORMATION"
            << NORMAL << " ------------------------------------" << std::endl;
  ...
  for (q = routing_table_.begin(); q != routing_table_.end(); q++) {
    ...
    for (p = routes.begin(); p != routes.end(); p++) {
      ...
      if (route.multicast)
        std::cout << " ["
                  << GetChannelAliasFromMutlicastAddress(route.dest_address)
                  << "]";
      else
        std::cout << " [udp]";
  ...
  for (w = wildcard_routing_table_.begin(); w != wildcard_routing_table_.end();
       w++) {
    std::list<Route> &routes = w->second;
    std::string var_pattern = w->first.first;
    std::string app_pattern = w->first.second;

    std::cout << "  routing for variables matching \"" << var_pattern
              << "\" from sources matching \"" << app_pattern
              << "\": " << std::endl;
...
bool WildcardMatch(const std::string &var_pattern,
                   const std::string &src_app_pattern, const std::string &var,
...
bool Share::Impl::ApplyWildcardRoutes(CMOOSMsg &msg) {
  ...
  for (g = wildcard_routing_table_.begin(); g != wildcard_routing_table_.end();
    ...
    for (h = routes.begin(); h != routes.end(); h++) {
      Route &route = *h;
      if (WildcardMatch(var_pattern, app_pattern, msg.GetKey(),
                        msg.GetSource())) {
        ...
        std::cout << "dynamically creating outgoing route : " << msg.GetKey()
                  << "->" << new_route.dest_name << " on ";
        if (new_route.multicast) {
          std::cout << GetChannelAliasFromMutlicastAddress(
                           new_route.dest_address)
                    << "\n";
        } else {
...
bool Share::Impl::ApplyRoutes(CMOOSMsg &msg) {
  ...
  for (q = route_list.begin(); q != route_list.end(); q++) {
    ...
    if (route.frequency > 0.0 &&
        now - route.last_time_sent < (1.0 / route.frequency))
      continue;

    SocketMap::iterator mcg = socket_map_.find(route.dest_address);
    if (mcg == socket_map_.end()) {
      std::stringstream ss;
      ss << "no output socket for " << route.dest_address.to_string()
         << std::endl;
      PrintSocketMap();
    ...
    if (verbose_) {
      std::cout << std::setprecision(1);
      std::cout << MOOS::ConsoleColours::green() << std::setw(10) << std::fixed
                << (MOOS::Time() - CMOOSApp::GetAppStartTime()) << ": "
                << MOOS::ConsoleColours::reset();

      std::cout << "sending \"" << msg.m_sKey << "\" as \"" << route.dest_name
                << "\" to " << route.dest_address.to_string() << "\n";
    }
    ...
    if (msg_buffer_size > MAX_UDP_SIZE) {
      std::cerr << "Message size exceeded payload size of "
                << MAX_UDP_SIZE / 1024 << " kB - not forwarding\n";
      return false;
    }
...
MOOS::IPV4Address
Share::Impl::GetAddressFromChannelAlias(unsigned int channel_number) const {
  MOOS::IPV4Address address = base_address_;
  address.set_port(channel_number + address.port());
  return address;
}

std::string Share::Impl::GetChannelAliasFromMutlicastAddress(
    const MOOS::IPV4Address &address) const {
  std::stringstream ss;

  int channel = address.port() - base_address_.port();
  if (channel < 0)
    throw std::runtime_error(
        "multicast address has port number below base port\n");
...
bool Share::Impl::AddOutputRoute(MOOS::IPV4Address address, bool multicast) {
  ...
  int reuse = 1;
  if (setsockopt(new_socket.socket_fd, SOL_SOCKET, SO_REUSEADDR, &reuse,
                 sizeof(reuse)) == -1)
  ...
  */
  ...
  if (setsockopt(new_socket.socket_fd, SOL_SOCKET, SO_SNDBUF, &send_buffer_size,
  ...
  std::string dotted_ip =
      MOOS::IPV4Address::GetNumericAddress(new_socket.address.host());

  if (inet_aton(dotted_ip.c_str(), &new_socket.sock_addr.sin_addr) == 0) {
    throw std::runtime_error("failed to intepret " + dotted_ip +
                             " as an ip address");
  }

  // new_socket.sock_addr.sin_addr.s_addr =
  // inet_addr(new_socket.address.ip_num.c_str());
