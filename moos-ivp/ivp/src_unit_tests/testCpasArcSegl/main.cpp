...
int main(int argc, char **argv) {
  ...
  if (!thresh_set)
    return (cmdLineErr("thresh is not set. Exiting."));

  if (!segl_set)
    return (cmdLineErr("seglist is not set. Exiting."));
  ...
  vector<double> vdcpa;
  unsigned int pts =
      cpasArcSegl(cx, cy, cr, lang, rang, segl, thresh, true, vcpa, vdcpa);
