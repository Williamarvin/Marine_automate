...
bool HelmReporter::handle(const string &alogfile) {
  ...
  while (!done) {
    ...
    if (m_report_life_events && !m_report_mode_changes &&
        !m_report_bhv_changes) {
...
void HelmReporter::handleNewHelmSummary(string summary, string tstamp) {
  ...
  if (m_watch_behavior != "") {
    if (strContains(bhvs_active, m_watch_behavior) &&
        !strContains(m_prev_report.getActiveBehaviors(full),
                     m_watch_behavior)) {
      color_active = "blue";
    }
    if (strContains(bhvs_running, m_watch_behavior) &&
        !strContains(m_prev_report.getRunningBehaviors(full),
                     m_watch_behavior)) {
      color_running = "blue";
    }
    ...
    if (strContains(bhvs_complete, m_watch_behavior) &&
        !strContains(m_prev_report.getCompletedBehaviors(full),
                     m_watch_behavior)) {
      color_complete = "blue";
    }
  }
