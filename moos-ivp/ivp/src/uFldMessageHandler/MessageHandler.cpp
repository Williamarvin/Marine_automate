...
bool MessageHandler::handleMailNodeMessage(const string &msg) {
  ...
  if ((dest_node != "") && (dest_node != "all") &&
      (dest_node != m_host_community))
    reject = true;
...
bool MessageHandler::buildReport() {
  ...
  if (m_last_invalid_msgs.size() == 0)
    ...
    for (p2 = m_last_invalid_msgs.begin(); p2 != m_last_invalid_msgs.end();
         p2++) {
      string msg = *p2;
  ...
  if (m_last_rejected_msgs.size() == 0)
    ...
    for (p2 = m_last_rejected_msgs.begin(); p2 != m_last_rejected_msgs.end();
         p2++) {
      string msg = *p2;
      m_msgs << "  " << truncString(msg, 70) << endl;
    }
