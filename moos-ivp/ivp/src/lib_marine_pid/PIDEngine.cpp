...
bool PIDEngine::handleYawSettings() {
  ...
  if (!yaw_pid_ilim_found)
    m_config_errors.push_back(
        "YAW_PID_INTEGRAL_LIMIT not found in Mission File");
  if (!max_rudder_found)
...
bool PIDEngine::handleSpeedSettings() {
  ...
  m_config_info.push_back("SPEED_PID_KI_LIM = " +
                          doubleToStringX(spd_pid_ilim, 3));
  m_config_info.push_back("MAXTHRUST      = " +
                          doubleToStringX(m_max_thrust, 3));
...
bool PIDEngine::handleDepthSettings() {
  ...
  if (!z_top_pid_ilim_found)
    m_config_errors.push_back(
        "Z_TO_PITCH_PID_INTEGRAL_LIMIT not in Mission File");
  if (!max_pitch_found)
  ...
  m_config_info.push_back("Z_TO_PITCH_PID_KP   = " +
                          doubleToStringX(z_top_pid_kp, 3));
  m_config_info.push_back("Z_TO_PITCH_PID_KD   = " +
                          doubleToStringX(z_top_pid_kd, 3));
  m_config_info.push_back("Z_TO_PITCH_PID_KI   = " +
                          doubleToStringX(z_top_pid_ki, 3));
  m_config_info.push_back("Z_TO_PITCH_PID_KI_LIM = " +
                          doubleToStringX(z_top_pid_ilim, 3));
  m_config_info.push_back("MAXPITCH            = " +
                          doubleToStringX(m_max_pitch, 3));
  ...
  m_config_info.push_back("PITCH_PID_KP   = " +
                          doubleToStringX(pitch_pid_kp, 3));
  m_config_info.push_back("PITCH_PID_KD   = " +
                          doubleToStringX(pitch_pid_kd, 3));
  m_config_info.push_back("PITCH_PID_KI   = " +
                          doubleToStringX(pitch_pid_ki, 3));
  m_config_info.push_back("PITCH_PID_KI_LIM = " +
                          doubleToStringX(pitch_pid_ilim, 3));
  m_config_info.push_back("MAXELEVATOR    = " +
                          doubleToStringX(m_max_elevator, 3));
