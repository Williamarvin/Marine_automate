...
void UMV_MOOSApp::setPendingEventsPipe(
...
void UMV_MOOSApp::handleNewMail(const MOOS_event &e) {
  ...
  for (size_t i = 0; i < e.mail.size(); ++i) {
    ...
    if (key == "APPCAST") {
      ...
      bool handled =
          m_realm_repo->addRealmSummary(summary, changed_node_or_proc);
...
void UMV_MOOSApp::postAppCastRequest(string channel_node, string channel_proc,
