  ...
  if (source == "mail") {
    if (m_given_max_duration > 0) {
      ...
      if (duration > m_given_max_duration) {
        string msg =
            "Incoming GIVEN_OBSTACLE has duration exceeding max duration";
        reportRunWarning(msg);
        return (false);
      }
    }
...
XYPolygon ObstacleManager::placeholderConvexHull(string key) {
  ...
  for (unsigned int i = 0; i < psize; i++) {
    double this_dist =
        distPointToPoint(ctr_x, ctr_y, points[i].x(), points[i].y());
  ...
  stringstream ss;
  ss << "format=radial, x=" << ctr_x << ",y=" << ctr_y << ",radius=" << max_dist
     << ",pts=8";
