...
vector<string> parseStringQ(const string &string_str, char separator) {
  ...
  while (str[0] != '\0') {
    int i = 0;
    while (((str[i] != separator) || in_quotes || (brace_count > 0)) &&
           (str[i] != '\0')) {
      if (str[i] == '"')
        in_quotes = !in_quotes;
      else if (str[i] == '{')
...
vector<string> parseStringZ(const string &string_str, char separator,
  ...
  while (str[0] != '\0') {
    ...
    while (((str[i] != separator) || (quote_count > 0) || (brace_count > 0) ||
...
vector<string> parseString(const string &string_str, const string &separator) {
...
vector<string> mergeVectors(vector<string> vector1, vector<string> vector2) {
...
bool listContains(const list<string> &str_list, const string &str,
                  bool case_sensitive) {
...
string padString(const string &str, std::string::size_type target_size,
                 bool front) {
  string rstr = str;
  string::size_type str_size = str.size();
...
bool tokParse(const string &str, const string &left, char gsep, char lsep,
              string &rstr) {
  rstr = "error";
...
string tokStringParse(const string &str, const string &left, char gsep,
                      char lsep) {
  vector<string> svector1 = parseStringQ(str, gsep);
  for (vector<string>::size_type i = 0; i < svector1.size(); i++) {
    vector<string> svector2 = parseString(svector1[i], lsep);
...
double tokDoubleParse(const string &str, const string &left, char gsep,
                      char lsep) {
...
int getArg(int argc, char **argv, int pos, const char *str1, const char *str2) {
...
bool scanArgs(int argc, char **argv, const char *str1, const char *str2,
              const char *str3) {
  for (int i = 0; i < argc; i++) {
    bool match1 = !strncmp(str1, argv[i], strlen(argv[i]));
    bool match2 = !strncmp(str1, argv[i], strlen(str1));
...
bool setDoubleRngOnString(double &dval, string str, double minv, double maxv) {
  if (!isNumber(str))
    return (false);
  if (minv > maxv)
    return (false);
...
bool setDoubleClipRngOnString(double &dval, string str, double minv,
                              double maxv) {
  if (!isNumber(str))
    return (false);
  if (minv > maxv)
...
bool setDoubleStrictRngOnString(double &dval, string str, double minv,
                                double maxv) {
  if (!isNumber(str))
    return (false);
  if (minv > maxv)
...
bool setMinPartOfPairOnString(double &minval, double &maxval, string str,
                              bool negok) {
  if (!isNumber(str))
    return (false);
  double dval = atof(str.c_str());
...
bool setMaxPartOfPairOnString(double &minval, double &maxval, string str,
                              bool negok) {
  if (!isNumber(str))
    return (false);
  double dval = atof(str.c_str());
...
string parseAppName(const string &name) {
  ...
  if (appFilename.substr(appFilename.size() - suffix.size(), suffix.size()) ==
      suffix) {
    // Remove the suffix
