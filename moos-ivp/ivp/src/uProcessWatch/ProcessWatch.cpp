...
bool ProcessWatch::Iterate() {
  ...
  if (m_proc_watch_summary_changed || post_based_on_time) {
    AppCastingMOOSApp::Notify(postVar("PROC_WATCH_SUMMARY"),
                              m_proc_watch_summary);
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    if (param == "watch")
      ...
      if (tolower(value) == "true")
        ...
        reportConfigWarning(
            "Param ALLOW_RETRACTIONS must be Boolean or numerial");
...
bool ProcessWatch::buildReport() {
  ...
  if (m_watch_all_antler == false)
    ...
    string alist;
    for (p = m_set_antler_clients.begin(); p != m_set_antler_clients.end();
         p++) {
      string client = *p;
      if (alist != "")
        alist += ",";
...
void ProcessWatch::handleMailStatusUpdate(string status) {
  ...
  if (cpuload == "") {
    reportRunWarning("Unhandled STATUS update: Missing cpuload for " +
                     procname);
    return;
  }
...
void ProcessWatch::populateAntlerList() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    if (param == "run") {
      ...
      if ((app != "") && (app != "uProcessWatch") &&
          !strBegins(app, "MOOSDB")) {
...
bool ProcessWatch::processExcluded(const string &procname) {
  ...
  for (i = 0; i < vsize; i++) {
    if (procname == m_exclude_list[i])
      excluded = true;
    else if (m_exclude_list_prefix[i] && strBegins(procname, m_exclude_list[i]))
      excluded = true;
  }
  return (excluded);
}
...
bool ProcessWatch::processIncluded(const string &procname) {
  ...
  for (i = 0; i < vsize; i++) {
    if (procname == m_include_list[i])
      included = true;
    else if (m_include_list_prefix[i] && strBegins(procname, m_include_list[i]))
      included = true;
  }
  return (included);
}
