...
void SimEngine::propagate(NodeRecord &record, double delta_time,
  ...
  double s =
      sin(degToRadians(prior_heading)) + sin(degToRadians(record.getHeading()));

  double c =
      cos(degToRadians(prior_heading)) + cos(degToRadians(record.getHeading()));
...
void SimEngine::propagateDepth(NodeRecord &record, double delta_time,
  ...
  if (speed <= 0) {
    ...
    double total_depth_rate =
        (-buoyancy_rate) + pitch_depth_rate + actuator_depth_rate;
...
void SimEngine::propagateSpeed(NodeRecord &record, const ThrustMap &tmap,
                               double delta_time, double thrust, double rudder,
...
void SimEngine::propagateSpeedSailing(NodeRecord &record, const ThrustMap &tmap,
                                      double delta_time, double thrust,
                                      double rudder, double max_accel,
                                      double max_decel,
                                      const ThrustMap &tmap_fan,
                                      double thrust_fan, double max_sail_spd) {
  if (delta_time <= 0)
    return;
...
void SimEngine::propagateHeading(NodeRecord &record, double delta_time,
                                 double rudder, double thrust, double turn_rate,
                                 double rotate_speed) {
...
void SimEngine::propagateSpeedDiffMode(NodeRecord &record,
                                       const ThrustMap &tmap, double delta_time,
                                       double thrust_lft, double thrust_rgt,
                                       double max_accel, double max_decel) {
  // Sanity check
  if (delta_time <= 0)
    return;
  ...
  propagateSpeed(record, tmap, delta_time, thrust, rudder, max_accel,
                 max_decel);
}
...
void SimEngine::propagateHeadingDiffMode(NodeRecord &record, double delta_time,
                                         double thrust_lft, double thrust_rgt,
                                         double turn_rate,
                                         double rotate_speed) {
  // Sanity check
