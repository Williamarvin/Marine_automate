...
bool OpAreaSpec::addVertex(const std::string &str, CMOOSGeodesy &geodesy) {
  ...
  for (unsigned int i = 0; i < vsize; i++) {
    ...
    if (left == "xpos")
      ...
      dashed = tolower(right);
    else if (left == "looped")
      looped = tolower(right);
  }
  ...
  addVertex(xpos_d, ypos_d, lwidth_d, group, label, lcolor, vcolor, looped_b,
            dashed_b);
