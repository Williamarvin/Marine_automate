...
public:
  ...
  double getStaleReportThresh() {
    return (m_vehi_settings.getStaleReportThresh());
  }
  double getStaleRemoveThresh() {
    return (m_vehi_settings.getStaleRemoveThresh());
  }
...
protected:
  ...
  void drawCommonVehicle(const NodeRecord &, const BearingLine &,
                         const ColorPack &body_color,
                         const ColorPack &vname_color, bool vname_draw,
                         unsigned int line = 0, double transparency = 1);

  void drawMarkers(const std::map<std::string, XYMarker> &, double tstamp = 0);
  void drawMarker(const XYMarker &, double tstamp = 0);

  void drawTextBoxes(const std::map<std::string, XYTextBox> &,
                     double tstamp = 0);
  ...
  void drawSegLists(const std::map<std::string, XYSegList> &,
                    double timestamp = 0);
  ...
  void drawCircles(const std::map<std::string, XYCircle> &,
                   double timestamp = 0);
  ...
  void drawDropPoints();
  void drawText(double px, double py, const std::string &, const ColorPack &,
                double font_size);
  void drawTextX(double px, double py, const std::string &, const ColorPack &,
                 double font_size);

  void drawHexagons();

  void drawGLPoly(double *points, unsigned int numPoints, ColorPack fill_color,
                  double thickness = 0, double scale = 1, double alpha = 100);
