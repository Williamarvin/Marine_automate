...
bool VehicleSet::getDoubleInfo(const string &g_vname, const string &info_type,
                               double &result) const {
  ...
  if ((info_type == "xpos") || (info_type == "meters_x") ||
    ...
    extrapolator.setPosition(record.getX(), record.getY(), record.getSpeed(),
                             record.getHeading(), record.getTimeStamp());
...
bool VehicleSet::getStringInfo(const string &g_vname, const string &info_type,
                               string &result) const {
  string vname = g_vname;
  if (g_vname == "active")
    vname = m_vehicles_active_name;
...
bool VehicleSet::handleNodeReport(double local_time, string node_report_str,
                                  string &whynot) {
