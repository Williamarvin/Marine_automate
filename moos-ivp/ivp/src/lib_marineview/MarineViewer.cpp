...
MarineViewer::~MarineViewer() { delete[] m_textures; }
...
bool MarineViewer::applyTiffFiles() {
  ...
  for (unsigned int ix = 0; ix < cnt; ix++) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img_width, img_height, 0, GL_RGBA,
                 GL_UNSIGNED_INT_8_8_8_8_REV, (unsigned char *)img_data);
...
double MarineViewer::img2meters(char xy, double img_val) const {
  ...
  if (xy == 'x') {
    double range =
        (m_back_img.get_x_at_img_right() - m_back_img.get_x_at_img_left());
    meters_val = (img_val - m_back_img.get_img_centx()) * range;
  }

  if (xy == 'y') {
    double range =
        (m_back_img.get_y_at_img_top() - m_back_img.get_y_at_img_bottom());
...
void MarineViewer::drawTiff() {
  ...
  glVertex2f(lower_x, lower_y);
  glTexCoord2f(0.0f, 1.0f);
  glVertex2f(lower_x, upper_y);
  glTexCoord2f(1.0f, 1.0f);
  glVertex2f(upper_x, upper_y);
  glTexCoord2f(1.0f, 0.0f);
  glVertex2f(upper_x, lower_y);
...
void MarineViewer::drawGLPoly(double *points, unsigned int numPoints,
                              ColorPack cpack, double thickness, double scale,
                              double alpha) {
  if (thickness <= 0) {
    glEnable(GL_BLEND);
...
bool MarineViewer::coordInView(double x, double y) {
  ...
  // cout << " PositionF: " << doubleToString(bx,0) << "," <<
  // doubleToString(by,0) << endl;
...
void MarineViewer::drawCommonVehicle(const NodeRecord &record_mikerb,
                                     ...
                                     bool vname_draw, unsigned int outer_line,
                                     double transparency) {
  ...
  if (vehibody == "kayak") {
    ...
    drawGLPoly(g_heronBody, g_heronBodySize, dk_gray, 0, factor_x,
               transparency);
    glTranslatef(2 * cx, 0, 0);
    drawGLPoly(g_heronBody, g_heronBodySize, dk_gray, 0, factor_x,
               transparency);
    glTranslatef(-cx, 0, 0);
    drawGLPoly(g_heronBack, g_heronBackSize, body_color, 0, factor_x,
               transparency);
    drawGLPoly(g_heronFront, g_heronFrontSize, dk_gray, 0, factor_x,
               transparency);
    ...
    drawGLPoly(g_crayBody, g_crayBodySize, body_color, 0, factor_x,
               transparency);
    if (draw_base_fin)
      drawGLPoly(g_crayBaseFinR, g_crayBaseFinRSize, fincolor, 0, factor_x,
                 transparency);
    drawGLPoly(g_crayFinR1, g_crayFinR1Size, fincolor, 0, factor_x,
               transparency);
    drawGLPoly(g_crayFinR2, g_crayFinR2Size, fincolor, 0, factor_x,
               transparency);
    drawGLPoly(g_crayFinR3, g_crayFinR3Size, fincolor, 0, factor_x,
               transparency);

    if (draw_base_fin)
      drawGLPoly(g_crayBaseFinL, g_crayBaseFinLSize, fincolor, 0, factor_x,
                 transparency);
    drawGLPoly(g_crayFinL1, g_crayFinL1Size, fincolor, 0, factor_x,
               transparency);
    drawGLPoly(g_crayFinL2, g_crayFinL2Size, fincolor, 0, factor_x,
               transparency);
    drawGLPoly(g_crayFinL3, g_crayFinL3Size, fincolor, 0, factor_x,
               transparency);

    if (outer_line)
      drawGLPoly(g_crayBody, g_crayBodySize, black, outer_line, factor_x,
                 transparency);
    glTranslatef(cx, cy, 0);
    ...
    glTranslatef(-cx, -cy, 0);
    drawGLPoly(g_auvBody, g_auvBodySize, body_color, 0, factor_x, transparency);
    if (outer_line > 0)
      drawGLPoly(g_auvBody, g_auvBodySize, black, outer_line, factor_x,
                 transparency);
    drawGLPoly(g_propUnit, g_propUnitSize, blue, 0, factor_x, transparency);
    glTranslatef(cx, cy, 0);
    ...
    glTranslatef(-cx, -cy, 0);
    drawGLPoly(g_triangleBody, g_triangleBodySize, body_color, 0, factor_x,
               transparency);
    if (outer_line > 0)
    ...
    glTranslatef(-cx, -cy, 0);
    drawGLPoly(g_circleBody, g_circleBodySize, body_color, 0, factor_x,
               transparency);
    if (outer_line > 0)
      drawGLPoly(g_circleBody, g_circleBodySize, black, outer_line, factor_x,
                 transparency);
    glTranslatef(cx, cy, 0);
    ...
    glTranslatef(-cx, -cy, 0);
    drawGLPoly(g_longShipBody, g_longShipBodySize, body_color, 0, factor_x,
               transparency);
    if (outer_line > 0)
...
void MarineViewer::drawRangePulse(const XYRangePulse &pulse, double timestamp) {
  ColorPack edge_c("blue");
...
void MarineViewer::drawCommsPulse(const XYCommsPulse &pulse, double timestamp) {
  ColorPack fill_c("light_blue");
  if (pulse.color_set("fill")) // fill_color
    fill_c = pulse.get_color("fill");
