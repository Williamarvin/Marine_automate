...
bool BackImg::locateTiffAndInfoFiles(string tiff_file) {
  ...
  // =========================================================
  cout << "[1] Looking for " << tiff_file << " and " << info_file
       << " in:" << endl;
  ...
  if (dirs) {
    ...
    for (unsigned int i = 0; i < svector.size(); i++) {
      cout << "[" << i + 2 << "] ";
      cout << "Looking for " << tiff_file << " and " << info_file
           << " in:" << endl;
...
bool BackImg::processConfiguration() {
  if (m_boundary_set) {
    ...
    m_img_centx = (m_datum_lon - m_lon_west) / (m_lon_east - m_lon_west);
    m_img_centy = (m_datum_lat - m_lat_south) / (m_lat_north - m_lat_south);
  }
  ...
  if (m_verbose) {
    ...
    cout << "Geodesy: m_img_meters_x: " << doubleToStringX(m_img_meters_x)
         << endl;
    cout << "Geodesy: m_img_meters_y: " << doubleToStringX(m_img_meters_y)
         << endl;
    ...
    cout << "Geodesy:       m_img_mtr_width: "
         << doubleToStringX(m_img_mtr_width, 2) << endl;
    cout << "Geodesy:       m_img_mtr_height: "
         << doubleToStringX(m_img_mtr_height, 2) << endl;
  }
...
void BackImg::setTexture() {
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_img_pix_width, m_img_pix_height, 0,
