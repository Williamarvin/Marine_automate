...
void TurnGenWilliamson::generate() {
  ...
  m_lane_gap =
      distPointToLine(m_endx, m_endy, m_start_osx, m_start_osy, px, py);
  ...
  if (fore) {
    ...
    projectPoint(star_angle, 20, m_start_osx, m_start_osy, px, py);
    m_natural_extent =
        -1 * distPointToLine(m_endx, m_endy, m_start_osx, m_start_osy, px, py);
  }
...
void TurnGenWilliamson::generateLaneSwitch1() {
  ...
  double cx3 = 0, cy3 = 0;
  if (m_port_turn) {
    projectPoint(port_angle, m_lane_gap / 2 - m_bias, m_start_osx, m_start_osy,
                 cx0, cy0);
    projectPoint(star_angle, m_turn_radius, m_start_osx, m_start_osy, cx1, cy1);
    projectPoint(port_angle, m_turn_radius + d, m_start_osx, m_start_osy, cx2,
                 cy2);
    projectPoint(m_start_osh, be, cx0, cy0, cx3, cy3);
  } else {
    projectPoint(star_angle, m_lane_gap / 2 - m_bias, m_start_osx, m_start_osy,
                 cx0, cy0);
    projectPoint(star_angle, m_turn_radius + d, m_start_osx, m_start_osy, cx1,
                 cy1);
...
void TurnGenWilliamson::generateLaneSwitch2() {
  ...
  double cx3 = 0, cy3 = 0;
  if (m_port_turn)
    projectPoint(port_angle, m_lane_gap / 2, m_start_osx, m_start_osy, cx0,
                 cy0);
  else
    projectPoint(star_angle, m_lane_gap / 2, m_start_osx, m_start_osy, cx0,
                 cy0);
