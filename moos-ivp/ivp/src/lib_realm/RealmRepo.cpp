...
string RealmRepo::report() const { return (m_realm_tree.report()); }
...
bool RealmRepo::addRealmSummary(RealmSummary summary, bool &changed) {
  ...
  bool ok =
      m_realm_tree.addRealmSummary(summary, m_onstart_node, m_onstart_proc);
  ...
  if ((new_curr_node != prev_curr_node) || (new_curr_proc != prev_curr_proc))
    changed = true;
