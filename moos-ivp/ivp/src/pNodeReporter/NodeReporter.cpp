...
bool NodeReporter::OnNewMail(MOOSMSG_LIST &NewMail) {
  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    // rmod Aug1821: Group info may be dynamically altered
      ...
      if ((helm_status == "DRIVE") || (helm_status == "PARK") ||
          (helm_status == "MALCONFIG") || (helm_status == "DISABLED"))
        m_helm_status_primary = helm_status;
      else if ((helm_status == "DRIVE+") || (helm_status == "PARK+") ||
               (helm_status == "MALCONFIG+") || (helm_status == "STANDBY"))
...
bool NodeReporter::OnStartUp() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    } else if ((param == "platform_group") || (param == "group")) {
      m_group_name = value;
      handled = true;
    ...
    } else if ((param == "cross_fill_policy") || (param == "crossfill_policy"))
...
bool NodeReporter::Iterate() {
  ...
  if (m_extrap_enabled && (delta_time > 0)) {
    if (delta_time < m_extrap_max_gap) {
      NodeRecord extrap_record =
          extrapolateRecord(m_record_last_posted, m_curr_time, 5);
...
bool NodeReporter::setCrossFillPolicy(string policy) {
  ...
  if ((policy == "literal") || (policy == "fill-empty") ||
      (policy == "global") || (policy == "use-latest") || (policy == "local")) {
...
void NodeReporter::crossFillCoords(NodeRecord &record, double nav_xy_updated,
  ...
  if (m_crossfill_policy == "fill-empty") {
    if (!record.valid("x") && !record.valid("y") && record.valid("lat") &&
        record.valid("lon"))
      crossFillGlobalToLocal(record);
    if (record.valid("x") && record.valid("y") && !record.valid("lat") &&
        !record.valid("lon")) {
...
bool NodeReporter::handleMailRiderVars(string var, string sval, double dval) {
