...
void VCheckSet::handleMailKey(string key, string sval, double dval,
                              double mtime) {
...
void VCheckSet::handleMailAll(string key, string sval, double dval,
                              double mtime) {
...
list<string> VCheckSet::getReport() const {
  ...
  for (unsigned int i = 0; i < m_vchecks.size(); i++) {
    ...
    string real_time =
        doubleToStringX(m_vchecks[i].getActualTime(), 2); // col 5

    string targ_delta =
        doubleToStringX(m_vchecks[i].getMaxTimeDelta(), 2); // col 6
    if (vthresh)
      targ_delta = doubleToStringX(m_vchecks[i].getMaxValDelta(), 2);

    string real_delta =
        doubleToStringX(m_vchecks[i].getActualDelta(), 2); // col 7
  ...
  if (hidden > 0)
    rep.push_back("  --- " + intToString(hidden) +
                  " additional checks PASSED, not shown ---");

  if (m_vchecks.size() > (hidden + amt_out)) {
    int remaining = m_vchecks.size() - (hidden + amt_out);
    rep.push_back("  --- " + intToString(remaining) +
                  " additional checks pending, not shown ---");
  }
