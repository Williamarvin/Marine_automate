...
bool GenericSensor_MOOSApp::handleMailNodeReport(
    const string &node_report_str) {
...
bool GenericSensor_MOOSApp::handleMailSensorRequest(const string &request) {
  ...
  if (m_map_node_records.count(vname) == 0) {
    reportRunWarning("Cant handle sensor request: no node reports rec'd from " +
                     vname);
    return (false);
  }
  ...
  if (m_map_vehicle_sensor_range.count(vname) == 0) {
    ...
    if (ok)
      reportRunWarning("Warning: no sensor setting for " + vname +
                       ". Default chosen");
    if (!ok) {
...
void GenericSensor_MOOSApp::postSensorReport(double ptx, double pty,
                                             string vname) {
...
bool GenericSensor_MOOSApp::buildReport() {
  ...
  m_msgs << endl;
  m_msgs << "Sensor Settings for known vehicles: (" << curr_loc_time << ")"
         << endl;
  m_msgs << "=================================================" << endl;
