...
bool PickPos::setHdgConfig(string str) {
  ...
  if (strContains(str, ',')) {
    ...
    if (svector.size() != 3)
      return (false);
    if (!isNumber(svector[0]) || !isNumber(svector[1]) || !isNumber(svector[2]))
      return (false);
...
void PickPos::pickPosByFile() {
  ...
  for (unsigned int i = 0; i < m_pick_amt; i++) {
    ...
    bool found = false;
    while (!found) {
      for (unsigned int i = (unsigned int)(rval); ((i < choices) && !found);
           i++) {
...
void PickPos::pickPosByPoly() {
  ...
  if (points.size() != m_pick_amt) {
    cout << "Unable to squeeze " << m_pick_amt << " pts in given region."
         << endl;
...
bool PickPos::pickPosByCircle(double min_sep) {
  ...
  if (min_sep < 0) {
    ...
    for (unsigned int i = 0; i < m_pick_amt; i++) {
      ...
      string s =
          "x=" + doubleToStringX(newx, 2) + ",y=" + doubleToStringX(newy, 2);
