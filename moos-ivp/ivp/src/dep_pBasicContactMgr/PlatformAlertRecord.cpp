...
void PlatformAlertRecord::setAlertedValue(string vehicle, string alertid,
  ...
  if (alertid != "all_alerts")
    ...
    for (p = m_par_alerted[vehicle].begin(); p != m_par_alerted[vehicle].end();
         p++)
      p->second = bval;
  }
}
...
bool PlatformAlertRecord::getAlertedValue(string vehicle,
                                          string alertid) const {
  vehicle = tolower(vehicle);

  map<string, map<string, bool>>::const_iterator p =
      m_par_alerted.find(vehicle);
