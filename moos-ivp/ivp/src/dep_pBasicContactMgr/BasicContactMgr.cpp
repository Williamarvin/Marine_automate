...
void BasicContactMgr::updateRanges() {
  ...
  for (p = m_map_node_records.begin(); p != m_map_node_records.end(); p++) {
    ...
    double range_cpa =
        engine.evalCPA(m_nav_hdg, m_nav_spd, alert_range_cpa_time);
  ...
  for (p = m_map_alerts.begin(); p != m_map_alerts.end(); p++) {
    ...
    if (alert_range_cpa > alert_range) {
      ...
      if (!active || (alert_range_cpa_color == "invisible") ||
          (alert_range_cpa_color == "empty") || (alert_range_cpa_color == ""))
        active = false;
...
bool BasicContactMgr::buildReport() {
  ...
  m_msgs << "DisplayRadii:              " << boolToString(m_display_radii)
         << endl;
  m_msgs << "Deriving X/Y from Lat/Lon: " << boolToString(m_use_geodesy)
         << endl;
  ...
  for (p = m_map_alerts.begin(); p != m_map_alerts.end(); p++) {
    ...
    m_msgs << "  CPA_RANGE = " << alert_rng_cpa << ", " << alert_rng_cpa_color
           << endl;
