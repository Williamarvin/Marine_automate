...
ObShipModelV24::ObShipModelV24(double osx, double osy, double osh, double osv) {
...
bool ObShipModelV24::setPose(double osx, double osy, double osh, double osv) {
  setPoseOSX(osx);
  setPoseOSY(osy);
  setPoseOSH(osh);
  setPoseOSV(osv);
...
bool ObShipModelV24::paramIsSet(string param) const {
  if ((param == "osx") || (param == "osy") || (param == "osh") ||
      (param == "osv"))
...
double ObShipModelV24::rayCPA(double hdg, double &rx, double &ry) const {
  double osx = getOSX();
  double osy = getOSY();
...
double ObShipModelV24::seglrCPA(double hdg, double &rx, double &ry,
  ...
  double cpa_dist =
      distSeglrToPoly(seglr, m_gut_poly, rx, ry, stemdist, verbose);
  return (cpa_dist);
}
...
double ObShipModelV24::evalHdgSpd(double hdg, double spd, bool verbose) const {
...
bool ObShipModelV24::setBngMinMaxToGutPoly() {
  ...
  for (unsigned int ang = 0; ang < 360; ang++) {
    ...
    double cpa_dist = distSeglrToPoly(seglr, m_gut_poly, ix, iy, stem_dist);
...
bool ObShipModelV24::setBngMinMaxToRimPoly() {
  ...
  for (unsigned int ang = 0; ang < 360; ang++) {
    XYSeglr seglr = m_plat_model.getTurnSeglr((double)(ang));
    double cpa_dist = distSeglrToPoly(seglr, m_rim_poly, ix, iy, stem_dist);
