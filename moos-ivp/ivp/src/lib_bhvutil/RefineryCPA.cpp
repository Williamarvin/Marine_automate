...
bool RefineryCPA::init(double osx, double osy, double cnx, double cny,
                       double cnh, double cnv, double ostol, double min_ucd,
                       double max_ucd, IvPDomain domain,
...
vector<IvPBox> RefineryCPA::getRefineRegionsAft() {
  ...
  if (m_range_gamma < m_max_util_cpa_dist) {
    if (m_os_star_of_cn) {
      ...
      if (m_verbose)
        cout << "pass_hdg (pre_star):  " << pass_hdg << endl;
      pass_hdg = getMinLastSatHdgAtSpd(max_spd, pass_hdg);
      if (m_verbose)
        cout << "pass_hdg (post_star):  " << pass_hdg << endl;
    } else {
      pass_hdg = angle360(m_os_cn_abs_bng - m_theta_delta);
      if (m_verbose)
        cout << "pass_hdg (pre_port):  " << pass_hdg << endl;
      pass_hdg = getMaxLastSatHdgAtSpd(max_spd, pass_hdg);
      if (m_verbose)
        cout << "pass_hdg (post_port):  " << pass_hdg << endl;
    }
  }
  ...
  double inner_max_spd_next =
      m_domain.getNextHigherVal(m_spd_ix, inner_max_spd, 1, 0);
  ...
  vector<IvPBox> boxes =
      buildBoxesHdgSpd(m_domain, min_hdg, max_hdg, inner_max_spd_next, max_spd);
...
vector<IvPBox> RefineryCPA::getRefineRegionsForeOutRGam() {
  ...
  if (m_verbose) {
    cout << "***********************************************************"
         << endl;
    cout << "In getRefineRegionsForeOutRGam()                           "
         << endl;
    cout << "***********************************************************"
         << endl;
  }
  ...
  if (m_os_star_of_cn) {
    ...
    if (m_verbose)
      cout << " ** NEW pass_hdg:" << pass_hdg << endl;
    min_hdg = cross_hdg;
    max_hdg = pass_hdg;
  }
  ...
  if (do_shadows) {
    if (m_os_star_of_cn) {
      ...
      vector<IvPBox> boxes1 =
          buildBoxesHdgSpd(m_domain, mid_hdg1, min_hdg, mid_spd1, max_spd);
      for (unsigned int i = 0; i < boxes1.size(); i++)
        regions.push_back(boxes1[i]);

      double mid_hdg2 = getMinLastSatHdgAtSpd(mid_spd2, min_hdg);
      if (m_verbose)
        cout << "mid_hdg2:" << mid_hdg2 << endl;
      vector<IvPBox> boxes2 =
          buildBoxesHdgSpd(m_domain, mid_hdg2, min_hdg, mid_spd2, mid_spdx);
      for (unsigned int i = 0; i < boxes2.size(); i++)
        regions.push_back(boxes2[i]);
    } else {
      double mid_hdg1 = getMaxLastSatHdgAtSpd(mid_spd1, max_hdg);
      vector<IvPBox> boxes1 =
          buildBoxesHdgSpd(m_domain, max_hdg, mid_hdg1, mid_spd1, max_spd);
      for (unsigned int i = 0; i < boxes1.size(); i++)
        regions.push_back(boxes1[i]);

      double mid_hdg2 = getMaxLastSatHdgAtSpd(mid_spd2, max_hdg);
      vector<IvPBox> boxes2 =
          buildBoxesHdgSpd(m_domain, max_hdg, mid_hdg2, mid_spd2, mid_spdx);
      for (unsigned int i = 0; i < boxes2.size(); i++)
        regions.push_back(boxes2[i]);
    }
  }
  ...
  if (do_fore) {
    if (m_os_star_of_cn) {
      ...
      vector<IvPBox> boxes =
          buildBoxesHdgSpd(m_domain, max_hdg, mid_hdg1, fore_spdx, fore_spd1);
      for (unsigned int i = 0; i < boxes.size(); i++) {
        if (m_verbose)
          boxes[i].print();
        regions.push_back(boxes[i]);
      ...
      if (m_verbose)
        cout << "port:mid_hdg1: " << mid_hdg1 << endl;
      mid_hdg1 = m_domain.getSnappedVal(m_crs_ix, mid_hdg1, 1);
      if (m_verbose)
        cout << "port:mid_hdg1: " << mid_hdg1 << endl;
      vector<IvPBox> boxes =
          buildBoxesHdgSpd(m_domain, mid_hdg1, min_hdg, fore_spdx, fore_spd1);
      for (unsigned int i = 0; i < boxes.size(); i++)
        regions.push_back(boxes[i]);
    }
  }
...
vector<IvPBox> RefineryCPA::getRefineRegionsForeInRGam() {
  ...
  if (m_verbose) {
    cout << "***********************************************************"
         << endl;
    cout << "In getRefineRegionsForeInRGam()                            "
         << endl;
    cout << "***********************************************************"
         << endl;
  }
  ...
  if (sit_cpa >= m_max_util_cpa_dist) {
    if (m_ostol != 0) {
      ...
      if (m_verbose) {
        ...
        cout << "(m_contact_range / m_ostol): " << m_contact_range / m_ostol
             << endl;
      }
    }
  ...
  if (m_verbose)
    cout << "inner_max_spd(1):" << inner_max_spd << endl;
  double port_bnd_hdg =
      getMinFirstSatHdgAtSpd(inner_max_spd, cn_flee_hdg_step_up);

  if (m_verbose)
    cout << "cn_flee_hdg_step_down:" << cn_flee_hdg_step_down << endl;
  double star_bnd_hdg =
      getMaxFirstSatHdgAtSpd(inner_max_spd, cn_flee_hdg_step_down);
  ...
  if ((port_bnd_hdg >= 0) && (star_bnd_hdg >= 0)) {
    vector<IvPBox> boxes;
    boxes = buildBoxesHdgSpd(m_domain, port_bnd_hdg, star_bnd_hdg, min_spd,
                             max_spd);
    for (unsigned int i = 0; i < boxes.size(); i++)
      regions.push_back(boxes[i]);
  }
  ...
  // =======================================================
  double inner_max_spd_m1 =
      m_domain.getNextLowerVal(m_spd_ix, inner_max_spd, 1);
...
#if 1
    double port_min_hdg =
        getMinFirstSatHdgAtSpd(port_inner_spd, cn_flee_hdg_step_up);
...
#if 1
    double star_max_hdg =
        getMaxFirstSatHdgAtSpd(star_inner_spd, cn_flee_hdg_step_down);
...
double RefineryCPA::getMinFirstSatHdgAtSpd(double spd, double hmin) {
  if (m_verbose)
    cout << "In getMinFirstSatHdgAtSpd(): spd,hdt: " << spd << "," << hmin
         << endl;
  // Sanity checks
  if (!m_initialized)
    return (-1);
...
double RefineryCPA::getMaxFirstSatHdgAtSpd(double spd, double hmax) {
  if (m_verbose)
    cout << "In getMaxFirstSatHdgAtSpd(): spd,hdg" << spd << "," << hmax
         << endl;
  // Sanity checks
  if (!m_initialized)
    return (-1);
...
double RefineryCPA::getMinSpdClockwise(double hmin, double hmax,
  ...
  for (unsigned int i = 0; i < range_crs_pts; i++) {
    ...
    if (m_verbose)
      cout << "getMinSpdClockwise: spd=" << curr_spd << ", hdg=" << hdg
           << ", cpa=" << cpa << endl;
    if (cpa < m_max_util_cpa_dist) {
      bool done = false;
      while (!done) {
...
double RefineryCPA::getPassHdgCounterClock(double hdg, double start_spd,
  ...
  if (m_verbose)
    cout << "getPassHdgCounterClock: max_util_cpa=" << m_max_util_cpa_dist
         << endl;
  for (unsigned int i = 0; i < total_crs_pts; i++) {
    unsigned int index = first_ix + i;
    if (index >= total_crs_pts)
    ...
    double cpa = evalCPA(hdg, curr_spd, m_ostol);
    if (m_verbose)
      cout << "getPassHdgCounterClock: spd=" << curr_spd << ", hdg=" << hdg
           << ", cpa=" << cpa << endl;
