...
void WaypointEngineX::setSegList(const XYSegList &g_seglist, bool retain) {
  m_state = "transit";
  m_state_prev = "transit";
...
void WaypointEngineX::setSegList(const vector<XYPoint> &pts, bool retain) {
  XYSegList segl;
...
void WaypointEngineX::setReverseToggle() { setReverse(!m_reverse); }
...
string WaypointEngineX::setNextWptAux(double osx, double osy) {
  ...
  if (dist < m_capture_radius) {
    ...
    if ((m_slip_radius > m_capture_radius) && (dist > m_current_cpa) &&
...
bool WaypointEngineX::hasCompleted() const { return (m_state == "completed"); }

//-----------------------------------------------------------
// Procedure: hasAdvanced()

bool WaypointEngineX::hasAdvanced() const {
  if ((m_state == "completed") || (m_state == "cycled") ||
      (m_state == "advanced"))
    return (true);
...
bool WaypointEngineX::isUnderWay() const {
  if ((m_state == "cycled") || (m_state == "advanced") ||
      (m_state == "transiting"))
    return (true);
