...
void ObstacleSim::updateVehiDists() {
  ...
  for (p = m_map_vrecords.begin(); p != m_map_vrecords.end(); p++) {
    string vname = p->first;
    for (unsigned int i = 0; i < num_obstacles; i++) {
      if (m_map_vdists.count(vname) && m_map_vdists_new.count(vname) &&
          m_map_vdists_min.count(vname) && (i < m_map_vdists[vname].size()) &&
...
void ObstacleSim::postFlags(const vector<VarDataPair> &flags, double dist,
                            string vname, unsigned int ix) {
