...
bool HostInfo::OnStartUp() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    else if ((param == "default_hostip") || (param == "default_host_ip")) {
...
int HostInfo::generateIPInfoFiles() {
  ...
  sys_call +=
      m_tmp_file_dir + "ipinfo_osx_usb_10_100_1000_lan_" + name + ".txt" + bgd;
  result = system(sys_call.c_str());
  ...
  sys_call =
      "ifconfig bridge100 | grep 'broadcast 192.168.2.255' | cut -d ' ' -f2 > ";
  ...
  // bgd; result = system(sys_call.c_str());
...
void HostInfo::gatherIPInfoFromFiles() {
  ...
  m_ip_osx_usb_1000 =
      readOSXInfoIP("ipinfo_osx_usb_10_100_1000_lan_" + name + ".txt");
  ...
  m_ip_osx_usb_ethernet =
      readOSXInfoIP("ipinfo_osx_usb_ethernet_" + name + ".txt");

  m_ip_linux_wifi = readLinuxInfoIP("ipinfo_linux_wifi_" + name + ".txt");
  m_ip_linux_ethernet0 =
      readLinuxInfoIP("ipinfo_linux_ethernet0_" + name + ".txt");
  m_ip_linux_ethernet1 =
      readLinuxInfoIP("ipinfo_linux_ethernet1_" + name + ".txt");
  ...
  m_ip_linux_hostname =
      readLinuxInfoIP("ipinfo_linux_hostname_" + name + ".txt");
...
bool HostInfo::handleConfigPreferInterface(string interface) {
  ...
  if ((interface != "wlan0") && (interface != "wifi") &&
      (interface != "eth0") && (interface != "eth1") && (interface != "usb0") &&
      (interface != "usb1") && (interface != "usb2")) {
    return (false);
  }
