...
bool BHV_AvdColregsV19::setParam(string param, string value) {
  ...
  if (param == "pwt_inner_dist") {
    return (
        setMinPartOfPairOnString(m_pwt_inner_dist, m_pwt_outer_dist, value));
  } else if (param == "pwt_outer_dist") {
    return (
        setMaxPartOfPairOnString(m_pwt_inner_dist, m_pwt_outer_dist, value));
  }

  else if (param == "min_util_cpa_dist") {
    return (setMinPartOfPairOnString(m_min_util_cpa_dist, m_max_util_cpa_dist,
                                     value));
  } else if (param == "max_util_cpa_dist") {
    return (setMaxPartOfPairOnString(m_min_util_cpa_dist, m_max_util_cpa_dist,
                                     value));
  }
...
void BHV_AvdColregsV19::updateAvoidMode() {
  ...
  if ((m_avoid_mode == "none") || (m_avoid_mode == "overtaking") ||
      (m_avoid_mode == "cpa"))
    checkModeOvertaking();

  if ((m_avoid_mode == "none") || (m_avoid_mode == "headon") ||
      (m_avoid_mode == "cpa"))
    checkModeHeadOn();

  if ((m_avoid_mode == "none") || (m_avoid_mode == "standon_ot") ||
      (m_avoid_mode == "cpa"))
    checkModeStandOnOT();

  if ((m_avoid_mode == "none") || (m_avoid_mode == "giveway") ||
      (m_avoid_mode == "cpa"))
    checkModeGiveWay();

  if ((m_avoid_mode == "none") || (m_avoid_mode == "standon") ||
      (m_avoid_mode == "cpa"))
    checkModeStandOn();
  ...
  if ((prev_avoid_mode != m_avoid_mode) ||
      (prev_avoid_submode != m_avoid_submode))
  ...
  string msg =
      "V19 CN:" + m_contact + "  Mode:" + m_avoid_mode + ":" + m_avoid_submode;
...
void BHV_AvdColregsV19::checkModeOvertaking() {
  ...
  if ((m_avoid_mode != "overtaking") && (m_avoid_mode != "none") &&
      (m_avoid_mode != "cpa"))
    return;
  ...
  if (m_avoid_mode == "overtaking") {
    ...
    if (m_avoid_submode == "port") { // possibly exit the port submode
      if ((m_cnos.os_cn_rel_bng() > 180) &&
          (m_cnos.os_cn_rel_bng() < (360 - thrash_buffer)))
  ...
  if ((m_cnos.cn_os_rel_bng() < corb_min) ||
      (m_cnos.cn_os_rel_bng() > corb_max)) {
    resetAvoidModes();
    return;
  }
...
void BHV_AvdColregsV19::checkModeHeadOn() {
  ...
  if ((m_avoid_mode != "headon") && (m_avoid_mode != "none") &&
      (m_avoid_mode != "cpa"))
    return;
  ...
  double os_cn_abs_rel_bearing =
      absRelBearing(m_osx, m_osy, m_osh, m_cnx, m_cny);
  double cn_os_abs_rel_bearing =
      absRelBearing(m_cnx, m_cny, m_cnh, m_osx, m_osy);
...
void BHV_AvdColregsV19::checkModeGiveWay() {
  ...
  if ((m_avoid_mode != "giveway") && (m_avoid_mode != "none") &&
      (m_avoid_mode != "cpa"))
    return;
  // Check if contact beyond outer priority weight distance
  if (m_contact_range >= m_pwt_outer_dist) {
  ...
  if (m_cnos.os_crosses_cn_bow()) {
    ...
    double acceptable_dist =
        (2 * m_max_util_cpa_dist + m_min_util_cpa_dist) / 3;
    ...
    turn_gap = turnGap(m_osx, m_osy, m_osh, m_turn_radius, m_cnx, m_cny, cnx2,
                       cny2, true);
...
void BHV_AvdColregsV19::checkModeStandOn() {
  ...
  //=====================================================================
  // Check if existing mode is either "cpa", "none", or "standon"
  if ((m_avoid_mode != "standon") && (m_avoid_mode != "none") &&
      (m_avoid_mode != "cpa"))
    return;
  // Check if contact beyond outer priority weight distance
  if (m_contact_range >= m_pwt_outer_dist) {
...
void BHV_AvdColregsV19::checkModeStandOnOT() {
  ...
  //=====================================================================
  // Check if existing mode is either "cpa", "none", or "standon"
  if ((m_avoid_mode != "standon_ot") && (m_avoid_mode != "none") &&
      (m_avoid_mode != "cpa"))
    return;
  // Check if contact beyond outer priority weight distance
  if (m_contact_range >= m_pwt_outer_dist) {
  ...
  if ((m_avoid_mode == "standon_ot") &&
      (m_contact_range <= m_max_util_cpa_dist) &&
      (m_cnos.os_curr_cpa_dist() <= m_min_util_cpa_dist)) {
    // assert 17.a.ii actions if:
    //    1) a stand on vessel
    //    2) range closer than pwt_inner (already taking max action)
...
//      Note: The context is an ownship with a contact on a trajectory to
//            cross ownship's stern. Slowing down should only help widen the
//            CPA.
