...
bool XMS::setContentMode(string str, bool toggle_on_match) {
  str = tolower(str);
  if ((str != "scoping") && (str != "procs") && (str != "history") &&
      (str != "help"))
    return (false);
...
void XMS::updateHistory(string entry, string source, string srcaux,
                        double htime) {
...
bool XMS::printHistoryReport() {
  ...
  if (m_refresh_mode == "streaming")
    do_the_report = true;
  if ((m_refresh_mode == "events") && (m_history_event || m_update_requested))
    do_the_report = true;
