...
BHV_Waypoint::BHV_Waypoint(IvPDomain gdomain) : IvPBehavior(gdomain) {
...
IvPFunction *BHV_Waypoint::onRunState() {
  ...
  if (next_point) {
    ...
    double dist =
        hypot((m_nextpt.x() - m_trackpt.x()), (m_nextpt.y() - m_trackpt.y()));
  ...
  if (m_var_dist_to_prev != "")
    postMessage(m_var_dist_to_prev,
                m_waypoint_engine.distToPrevWpt(m_osx, m_osy));
  if (m_var_dist_to_next != "")
    postMessage(m_var_dist_to_next,
                m_waypoint_engine.distToNextWpt(m_osx, m_osy));
...
bool BHV_Waypoint::setNextWaypoint() {
  ...
  if (m_lead_allowed && (m_lead_distance >= 0)) {
    ...
    if (track_anchor) {
      double nx, ny;
      perpSegIntPt(tx, ty, m_nextpt.x(), m_nextpt.y(), m_osx, m_osy, nx, ny);
...
IvPFunction *BHV_Waypoint::buildOF(string method) {
  ...
  if ((method == "roc") || (method == "rate_of_closure")) {
    ...
    double rel_ang_to_trk_pt =
        relAng(m_osx, m_osy, m_trackpt.x(), m_trackpt.y());
...
void BHV_Waypoint::markOdoLeg() {
  ...
  double dist_leg_linear =
      distPointToPoint(m_osx, m_osy, m_odo_setx, m_odo_sety);
...
string BHV_Waypoint::expandMacros(string sdata) {
  ...
  sdata =
      macroExpand(sdata, "IX", m_waypoint_engine.getCurrIndex()); // deprecated
  ...
  sdata =
      macroExpand(sdata, "WPTS_REM",
                  m_waypoint_engine.size() - m_waypoint_engine.getCurrIndex());
