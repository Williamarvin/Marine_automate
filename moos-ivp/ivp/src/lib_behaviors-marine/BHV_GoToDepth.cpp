...
BHV_GoToDepth::BHV_GoToDepth(IvPDomain gdomain) : IvPBehavior(gdomain) {
...
bool BHV_GoToDepth::setParam(string param, string val) {
  ...
  if (param == "depth") {
    ...
    m_repeat = ival;
    return (true);
  } else if ((param == "arrival_delta") || (param == "capture_delta")) {
    double dval = atof(val.c_str());
    if ((dval < 0) || (!isNumber(val)))
      return (false);
    m_arrival_delta = dval;
    return (true);
  } else if ((param == "arrival_flag") || (param == "capture_flag")) {
