...
IvPFunction *BHV_Trail::onRunState() {
  ...
  if (outside) {
    if (distance > m_nm_radius) { // Outside nm_radius
      ...
      double ahead_by =
          head_x * (m_osx - m_trail_pt_x) + head_y * (m_osy - m_trail_pt_y);
...
void BHV_Trail::onRunToIdleState() { postMessage("PURSUIT", 0); }
...
void BHV_Trail::calculateTrailPoint() {
  ...
  if (m_angle_relative) {
    ...
    projectPoint(m_trail_angle, m_trail_range, m_cnx, m_cny, m_trail_pt_x,
                 m_trail_pt_y);
}
