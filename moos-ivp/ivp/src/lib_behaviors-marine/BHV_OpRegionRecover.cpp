...
BHV_OpRegionRecover::BHV_OpRegionRecover(IvPDomain gdomain)
    : IvPBehavior(gdomain) {
...
IvPFunction *BHV_OpRegionRecover::onRunState() {
  ...
  double curr_hdg_dist_to_poly =
      m_polygon_buff.dist_to_poly(m_osx, m_osy, m_osh);
...
void BHV_OpRegionRecover::updateRangeCache() {
  ...
  for (unsigned int i = 0; i < hdg_pts; i++) {
    ...
    // Otherwise normalize
      ...
      m_range_cache_norm[i] =
          100 * (farthest_dist - m_range_cache[i]) / range_of_vals;
  }
