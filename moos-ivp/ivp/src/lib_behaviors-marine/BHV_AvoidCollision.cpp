...
bool BHV_AvoidCollision::setParam(string param, string param_val) {
  ...
  if (param == "pwt_inner_dist") {
    return (setMinPartOfPairOnString(m_pwt_inner_dist, m_pwt_outer_dist,
                                     param_val));
  } else if (param == "pwt_outer_dist") {
    return (setMaxPartOfPairOnString(m_pwt_inner_dist, m_pwt_outer_dist,
                                     param_val));
  }
  ...
  if (m_use_refinery) {
    ...
    if (m_check_plateaus) {
      ...
      if (m_verbose) {
        ...
        cout << "  BHV_AvoidCollision plat_thresh: " << m_pcheck_thresh << endl;
...
double BHV_AvoidCollision::getRelevance() {
  ...
  m_curr_closing_spd =
      closingSpeed(m_osx, m_osy, m_osv, m_osh, m_cnx, m_cny, m_cnv, m_cnh);

  postMessage("AVD_DEBUG",
              "In getRelevance: " + doubleToString(m_contact_range));
