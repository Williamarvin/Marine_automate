...
bool NodeBroker::handleConfigTryShoreHost(string original_line, bool dup_warn) {
  string pshare_route;
  ...
  if (strContains(pshare_route, ':') &&
      !strBegins(pshare_route, "localhost:")) {
...
bool NodeBroker::buildReport() {
  m_msgs << endl;
  m_msgs << "  Total OK  PHI_HOST_INFO    received: " << m_ok_phis_received
         << endl;
  m_msgs << "  Total BAD PHI_HOST_INFO    received: " << m_bad_phis_received
         << endl;
  m_msgs << "  Total HOST_INFO changes    received: " << m_host_info_changes
         << endl;
  m_msgs << "  Total          PSHARE_CMD    posted: " << m_pshare_cmd_posted
         << endl;
  m_msgs << "  Total BAD NODE_BROKER_ACK  received: " << m_bad_acks_received
         << endl;
  m_msgs << endl;
  m_msgs << "==========================================================="
         << endl;
  ...
  //  m_msgs << "  Comms Policy: " << commsPolicy()                           <<
  //  endl;
  ...
  m_msgs << " Phase 1: " << s1 << " Valid Host information retrieved (iroutes)."
         << endl;

  string s2 = (m_shore_routes.size() > 0) ? "(Y)" : "(N)";
  string num2 = uintToString(m_shore_routes.size());
  m_msgs << " Phase 2: " << s2 << " Valid TryHosts (" << num2 << ") configured."
         << endl;

  string s3 = (m_pings_posted > 0) ? "(Y)" : "(N)";
  m_msgs << " Phase 3: " << s3
         << " NODE_BROKER_PINGS are being sent to TryHosts." << endl;

  string s4 = (m_ok_acks_received > 0) ? "(Y)" : "(N)";
  m_msgs << " Phase 4: " << s4 << " A Valid NODE_BROKER_ACK has been received."
         << endl;
  ...
  m_msgs << " Phase 5: " << s5
         << " pShare requested to share user vars with shoreside." << endl;

  if (s1 == "(N)")
    m_msgs << "*Hint: Phase 1 problem. Check status/output of pHostInfo.       "
              "       "
           << endl;
  else if (s2 == "(N)")
    m_msgs << "*Hint: Phase 2 problem. Check your .moos file configuration.    "
              "       "
           << endl;
  else if (s3 == "(N)")
    m_msgs << "*Hint: Phase 3 problem. Check if our node's iroutes, timewarp "
              "are set. "
           << endl;
  else if (s4 == "(N)") {
    m_msgs << "*Hint: Phase 4 problem. Check that shoreside is running and     "
              "       "
           << endl;
    m_msgs << "      that shoreside has received this node's NODE_BROKER_PING. "
              "       "
           << endl;
    m_msgs << "      If the shoreside HAS NOT received the ping, check that "
              "the       "
           << endl;
    m_msgs << "      pShare input route matches one of the TryHosts in "
              "uFldNodeBroker."
           << endl;
    m_msgs << "      If the shoreside HAS received a ping, check that we have "
              "received"
           << endl;
    m_msgs << "      a NODE_BROKER_ACK in the local MOOSDB." << endl;
  } else if (s5 == "(N)") {
    m_msgs
        << "*Hint: Phase 5 problem. Check that the user has indeed configured "
        << endl;
    m_msgs << "      in the .moos config block for this app." << endl;
  } else
    m_msgs << "All Phases complete. Things should be working as configured. "
           << endl;
