      ...
      for (p = m_map_node_records.begin(); p != m_map_node_records.end(); p++) {
        ...
        if (node_record.valid()) {
          ...
          if (pingTrans(beacon_push_dist, vehicle_pull_dist, actual_range)) {
            ...
            reportEvent("Beacon[" + label + "]  ))-->  Range Broadcast to " +
                        VNAME);
            postBeaconRangeReport(bix, actual_range, vname);
          } else {
            if (m_verbose)
              reportEvent("Beacon[" + label +
                          "]  XX-->  Failed Range Brdcast to " + VNAME);
          }
        }
      }
    }
...
bool BRS_App::handleRangeRequest(const string &request) {
  ...
  if (!query_allowed) {
    reportRunWarning("Failed Range Request from " + VNAME +
                     ": exceeds query freq.");
    if (m_verbose) {
      reportEvent("Failed Range Request from " + VNAME +
                  ": exceeds query frequency.");
      reportEvent("  Elapsed time for " + VNAME + ": " +
                  doubleToString(elapsed_time));
    }
    m_map_node_xping_freq[vname]++;
    return (true);
  }

  reportEvent("Valid Range Request from " + VNAME +
              ". Checking beacon ranges...");
...
void BRS_App::postBeaconRangeReport(unsigned int beacon_ix, double actual_range,
                                    string vname) {
...
double BRS_App::getTrueBeaconNodeRange(unsigned int beacon_ix, string vname) {
  if (beacon_ix >= m_beacons.size())
    return (-1);
...
void BRS_App::postRangePulse(double x, double y, string color, string label,
                             double duration, double radius) {
  XYRangePulse pulse;
  pulse.set_x(x);
  pulse.set_y(y);
...
bool BRS_App::buildReport() {
  ...
  for (p = m_map_node_records.begin(); p != m_map_node_records.end(); p++) {
    ...
    actab << vname << spush << spull << pusol << pgend << prepl << toofr
          << toofa;
  }
  m_msgs << actab.getFormattedString();
