...
HelmEngine::HelmEngine(IvPDomain g_ivp_domain, InfoBuffer *g_info_buffer) {
...
HelmEngine::~HelmEngine() { delete (m_ivp_problem); }
...
bool HelmEngine::part2_GetFunctionsFromBehaviorSet(int filter_level) {
  ...
  string msgx =
      "part2_GetFunctionsFromBehaviorSet: fl=" + intToString(filter_level);
  ...
  for (bhv_ix = 0; bhv_ix < bhv_cnt; bhv_ix++) {
    if (m_bhv_set->getFilterLevel(bhv_ix) == filter_level) {
      ...
      m_ipf_timer.start();
      IvPFunction *newof =
          m_bhv_set->produceOF(bhv_ix, m_iteration, bhv_state, ipf_reuse);
      ...
      if (newof) {
        ...
        int pcs = newof->size();
        m_helm_report.addActiveBHV(descriptor, state_time_entered, pwt, pcs,
                                   of_time, upd_summary, 1);
      ...
      if (bhv_state == "running")
        m_helm_report.addRunningBHV(descriptor, state_time_entered,
                                    upd_summary);
      if (bhv_state == "idle")
        m_helm_report.addIdleBHV(descriptor, state_time_entered, upd_summary);
