  ...
  for (p = NewMail.begin(); p != NewMail.end(); p++) {
    ...
    if (m_skews_matter && (fabs(skew_time) > m_ok_skew)) {
      ...
      if (!posted_by_this_helm && !exception) {
        ...
        skew_msg +=
            " Source=" + source + " Skew=" + doubleToString(skew_time, 2);
...
void HelmIvP::postBehaviorMessages() {
  ...
  for (unsigned int i = 0; i < config_warnings.size(); i++) {
    ...
    Notify("IVPHELM_BHVSET_WARNING",
           uintToString(i) + ":" + config_warnings[i]);
  }
      ...
      // Otherwise just post to the DB directly.
        ...
        if (msg_is_string) {
          ...
          if (key_change || key_repeat) {
            string aux = intToString(m_helm_iteration) + ":" + bhv_descriptor;
...
bool HelmIvP::OnStartUp() {
  ...
  for (p = sParams.begin(); p != sParams.end(); p++) {
    ...
    else if ((param == "BHV_DIR_NOT_FOUND_OK") ||
             (param == "BHV_DIRS_NOT_FOUND_OK"))
...
bool HelmIvP::handleConfigHoldOnApp(string applist) {
  ...
  for (unsigned int i = 0; i < svector.size(); i++) {
    ...
    if (!vectorContains(m_hold_on_app_name, appname) &&
        !strContainsWhite(appname) && (appname != "")) {
...
bool HelmIvP::handleConfigPMGen(string str) { return (m_pmgen.setParams(str)); }
