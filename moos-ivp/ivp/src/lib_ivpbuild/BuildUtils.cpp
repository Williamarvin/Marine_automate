...
BoxSet *makeUniformDistro(const IvPBox &outer_box, const IvPBox &unif_box,
                          int degree) {
  BoxSet *boxset = new BoxSet;
...
IvPBox stringDiscreteToPointBox(const string &given_str,
                                const IvPDomain &domain, const char gsep,
                                const char lsep) {
  IvPBox null_box;
...
IvPBox stringNativeToPointBox(const string &given_str, const IvPDomain &domain,
                              const char gsep, const char lsep) {
  IvPBox null_box;
  if (given_str == "")
    return (null_box);
...
IvPBox stringNativeToRegionBox(const string &given_str, const IvPDomain &domain,
                               const char gsep, const char lsep) {
  IvPBox null_box;
  if (given_str == "")
    return (null_box);
...
vector<IvPBox> buildBoxesSpdAll(IvPDomain domain, double hmin, double hmax) {
  ...
  unsigned int disc_hdg_hgh =
      domain.getDiscreteVal(crs_ix, hmax, 0); // rnd down
...
vector<IvPBox> buildBoxesHdgSpd(IvPDomain domain, double hmin, double hmax,
  ...
  unsigned int disc_spd_hgh =
      domain.getDiscreteVal(spd_ix, smax, 0); // rnd down
