...
void ZAIC_Vector::addError(string error_str) { m_errors.push_back(error_str); }
...
void ZAIC_Vector::print() {
  ...
  for (j = 0; j < jsize; j++) {
    cout << "  [" << j << "]:" << m_domain_vals[j] << ", " << m_range_vals[j]
         << endl;
  }
  ...
  for (k = 0; k < ksize; k++) {
    cout << "  [" << k << "]:" << m_idomain_vals[k] << ", " << m_irange_vals[k]
         << endl;
  }
}
...
bool ZAIC_Vector::hasErrors() { return (m_errors.size() > 0); }

void ZAIC_Vector::clearErrors() { m_errors.clear(); }
...
void ZAIC_Vector::clearWarnings() { m_build_warnings.clear(); }
