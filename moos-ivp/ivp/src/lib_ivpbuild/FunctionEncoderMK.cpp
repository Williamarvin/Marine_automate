...
public:
  ...
  int doubleToString(double jwt, char *buff) {
    ...
    if (jwt == 0) {
      ...
      // long int upper = ival / 10000; // before the decimal, assume lower than
      // 10k for now long int upper = (long int)(jwt + 0.00005);
      long int upper = ival / 10000;
      long int lower =
          ival % 10000; // after the decimal, guaranteed to be withing 10k
  ...
  MK_itoa() {
    for (unsigned int i = 0; i < 10000; ++i) {
      ...
      sprintf(smaps[i].str, "%d", i);
      smaps[i].len = strlen(smaps[i].str);
    }
  }
...
string IvPFunctionToStringMK(IvPFunction *ivp_function) {
  ...
  amt = sprintf(buff + len, "H,%d,%s,%d,%d,%d,%s,D,", cstr_len, cstr.c_str(),
