...
void PDMapBuilder::setRangeVals(vector<double> vals) { m_rng_vals = vals; }
...
PDMap *PDMapBuilder::getPDMap() {
  ...
  if (m_dom_vals.size() == 1) {
    IvPBox *piece =
        buildBox(m_dom_vals[0], m_rng_vals[0], m_dom_vals[0], m_rng_vals[0]);
    if (piece)
  ...
  if (m_dom_vals.size() == 2) {
    IvPBox *piece =
        buildBox(m_dom_vals[0], m_rng_vals[0], m_dom_vals[1], m_rng_vals[1]);
    if (piece)
      m_bld_pieces.push_back(piece);
    else {
      all_pieces_built_ok = false;
  ...
  if (m_dom_vals.size() > 2) {
    ...
    for (i = 0; (i < vsize - 1) && all_pieces_built_ok; i++) {
      ...
      if (i == 0)
        piece = buildBox(m_dom_vals[0], m_rng_vals[0], m_dom_vals[1],
                         m_rng_vals[1]);
        ...
        if ((m_dom_vals[i] + 1) == m_dom_vals[i + 1]) {
          ...
          if ((i + 1) == (vsize - 1)) {
            ...
            piece = buildBox(m_dom_vals[i], m_rng_vals[i], m_dom_vals[i + 1],
                             m_rng_vals[i + 1]);
          }
        }
        // Case 5: normal case
...
void PDMapBuilder::addWarning(string warning) { m_warnings.push_back(warning); }
...
bool PDMapBuilder::hasWarnings() { return (m_warnings.size() > 0); }

//-------------------------------------------------------------
// Procedure: clearWarnings()

void PDMapBuilder::clearWarnings() { m_warnings.clear(); }
