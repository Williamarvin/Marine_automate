...
void ModelHelmScope::setDataBroker(ALogDataBroker dbroker, string vname) {
  ...
  unsigned int mix2 =
      m_dbroker.getMixFromVNameVarName(vname, "IVPHELM_MODESET");
  ...
  unsigned int mix6 =
      m_dbroker.getMixFromVNameVarName(vname, "IVPHELM_LIFE_EVENT");
...
//  Examples: iter=1, bname=loiter, btype=BHV_Loiter, event=spawn,
//  seed=helm_startup
...
vector<string> ModelHelmScope::getLifeEvents() const {
  ...
  // Example: val = iter=479, bname=avdcollision_avd_henry,
  // btype=BHV_AvoidCollision,
  //                event=spawn, seed=name=avd_henry # contact=henry
  // Example: tim = 324.981
  for (unsigned int i = 0; i < valvector.size(); i++) {
    string val = valvector[valvector.size() - 1 - i];
