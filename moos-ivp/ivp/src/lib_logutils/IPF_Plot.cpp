...
bool IPF_Plot::addEntry(double timestamp, const string &ipf_str,
                        unsigned int helm_iteration, unsigned int piece_count,
                        double priority, IvPDomain ivp_domain) {
...
void IPF_Plot::print() const {
  ...
  for (i = 0; i < m_time_stamp.size(); i++)
    cout << "time:" << m_time_stamp[i] << "  ipf:" << m_ipf_string[i] << endl;
}
