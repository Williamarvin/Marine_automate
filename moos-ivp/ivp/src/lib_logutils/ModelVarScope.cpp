...
void ModelVarScope::addVarPlot(unsigned int mix) {
  ...
  // Part 1: Get the possibly new max lengths given the new plot (for
  // formatting)
  ...
  list<VarPlotEntry> prev_entries =
      vplot.getVarPlotEntriesUpToTime(m_curr_time);
  reformat(prev_entries);
  m_prev_entries = mergePlots(m_prev_entries, prev_entries);

  // Part 3: Merge the SOON entries
  list<VarPlotEntry> soon_entries =
      vplot.getVarPlotEntriesPastTime(m_curr_time);
...
void ModelVarScope::reformat(list<VarPlotEntry> &entries) {
  ...
  for (p = entries.begin(); p != entries.end(); p++) {
    p->clearFormat();
    p->format(2, m_max_len_varname, m_max_len_vname, m_max_len_source,
              m_max_len_srcaux, m_show_vname, m_show_varname, m_show_source,
              m_show_srcaux);
  }
}
...
vector<string>
ModelVarScope::getEntries(const list<VarPlotEntry> &entries) const {
