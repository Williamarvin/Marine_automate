...
bool LogChecker::check(string alogfile, string output_file) {
  ...
  if (alogfile == output_file) {
    fprintf(stdout, "Input and output .alog files cannot be the same.\n");
    fprintf(stdout, "Exiting now.\n");
    return (false);
  }
  ...
  if (m_verbose) {
    if (m_start_flags.empty()) {
      ...
      if (start_satisfied) {
        fprintf(m_output_file, "The start consitions have been satisfied.\n");
      } else {
        fprintf(m_output_file, "The end of the log was reached and the start "
