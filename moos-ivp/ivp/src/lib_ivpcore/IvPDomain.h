...
public:
  bool operator==(const IvPDomain &) const;
...
public:
  ...
  double getNextLowerVal(unsigned int ix, double val, int snaptype,
                         bool wrap = false) const;

  // Return next higher value, if there is one
  double getNextHigherVal(unsigned int ix, double val, int snaptype,
                          bool wrap = false) const;

  // Return the discrete index into the domain given by a double
  // input value. Round up, down or closest, depending on snaptype.
  unsigned int getDiscreteVal(unsigned int ix, double val, int snaptype) const;

  double getSnappedVal(unsigned int ix, double dval, int snaptype) const;
  ...
  double coupleAux(unsigned int index, double &gval, bool up, bool wrap) const;

  // Return given val if in domain, or next higher value, if there is one
  double getEqOrHigherVal(unsigned int index, double val, int snaptype,
                          bool wrap = false) const;

  // Return given val if in domain, or next lower value, if there is one
  double getEqOrLowerVal(unsigned int index, double val, int snaptype,
                         bool wrap = false) const;
