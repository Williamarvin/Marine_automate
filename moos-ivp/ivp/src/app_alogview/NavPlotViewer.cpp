...
void NavPlotViewer::setMinimalMem() { m_draw_geo = false; }
...
void NavPlotViewer::initPlots() {
  ...
  for (unsigned int aix = 0; aix < alogs; aix++) {
    if (m_vnames[aix] == "shoreside") {
      isolate_shore = true;
      cout << "     Isolating shoreside (vehicle logged visuals skipped)"
           << endl;
    }
  }
...
double NavPlotViewer::getCurrTime() { return (m_curr_time); }
