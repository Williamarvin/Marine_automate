...
bool LogViewLauncher::handleParamsGUI(string argi) {
  ...
  if (strBegins(argi, "--seglist_viewable_all=") ||
      ...
      strBegins(argi, "--tiff_viewable=") || strBegins(argi, "--hash_delta=") ||
...
bool LogViewLauncher::configRegionInfo() {
  ...
  if (region_info == "") {
    ...
    pany_str = "-280";
    cout << termColor("red");
    cout << "WARNING: No REGION_INFO found in the log file. Defaulting "
         << endl;
    cout << "to use Forest Lake image and context. The REGION_INFO is  "
         << endl;
    cout << "normally published by pMarineViewer. If pMarineViwer was  "
         << endl;
    cout << "running, ensure that REGION_INFO is being logged. If      "
         << endl;
    cout << "pMarineViewer was not running, the content of REGION_INFO "
         << endl;
    cout << "can be passed on the command-line to alogview, or alogview"
         << endl;
    cout << "can be launch with Forest Lake, and just toggle off the   "
         << endl;
    cout << "background image if it is distracting.                    "
         << endl;
    cout << "                                                          "
         << endl;
...
bool LogViewLauncher::handleVQual(string val) {
  val = tolower(val);
  if ((val != "med") && (val != "low") && (val != "high") && (val != "max") &&
      (val != "vlow") && (val != "vvlow"))
    return (false);
