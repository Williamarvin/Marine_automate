...
void REPLAY_GUI::initWidgets() {
  ...
  m_but_zoom_in_time->callback((Fl_Callback *)REPLAY_GUI::cb_TimeZoom,
                               (void *)1);
  ...
  m_but_zoom_out_time->callback((Fl_Callback *)REPLAY_GUI::cb_TimeZoom,
                                (void *)-1);
  ...
  m_but_zoom_reset_time->callback((Fl_Callback *)REPLAY_GUI::cb_TimeZoom,
                                  (void *)0);
  ...
  m_but_hide_lp->callback((Fl_Callback *)REPLAY_GUI::cb_ButtonHideLogPlot,
                          (void *)0);
  m_but_hide_lp->clear_visible_focus();
  m_but_hide_lp->value(1);

  m_but_hide_rp = new Fl_Check_Button(0, 0, 0, 0);
  m_but_hide_rp->callback((Fl_Callback *)REPLAY_GUI::cb_ButtonHideLogPlot,
                          (void *)1);
...
void REPLAY_GUI::augmentMenu() {
  m_menubar->add("Replay/Streaming Toggle", ' ',
                 (Fl_Callback *)REPLAY_GUI::cb_Streaming, (void *)2, 0);
  m_menubar->add("Replay/Streaming Warp Faster", 'a',
                 (Fl_Callback *)REPLAY_GUI::cb_StreamSpeed, (void *)1, 0);
  m_menubar->add("Replay/Streaming Warp Slower", 'z',
                 (Fl_Callback *)REPLAY_GUI::cb_StreamSpeed, (void *)0,
                 FL_MENU_DIVIDER);
  m_menubar->add("Replay/Streaming Redraw 10x Per Logged Second", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)100,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw 4x Per Logged Second", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)250,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw 2x Per Logged Second", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)500,
                 FL_MENU_RADIO | FL_MENU_VALUE);
  m_menubar->add("Replay/Streaming Redraw Once Per 1 Logged Second", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)1000,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw Once Per 2 Logged Seconds", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)2000,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw Once Per 5 Logged Seconds", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)5000,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw Once Per 10 Logged Seconds", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)10000,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw Once Per 30 Logged Seconds", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)30000,
                 FL_MENU_RADIO | FL_MENU_RADIO);
  m_menubar->add("Replay/Streaming Redraw Once Per 60 Logged Seconds", 0,
                 (Fl_Callback *)REPLAY_GUI::cb_StreamStep, (void *)60000,
                 FL_MENU_RADIO | FL_MENU_DIVIDER);

  // m_menubar->add("Replay/Step by Seconds",  0,
  // (Fl_Callback*)REPLAY_GUI::cb_StepType, (void*)0,
  // FL_MENU_RADIO|FL_MENU_VALUE); m_menubar->add("Replay/Step by Helm
  // Iterations",  0, (Fl_Callback*)REPLAY_GUI::cb_StepType, (void*)1,
  // FL_MENU_RADIO|FL_MENU_DIVIDER);

  m_menubar->add("Replay/Jump To Start", FL_ALT + '[',
                 (Fl_Callback *)REPLAY_GUI::cb_JumpTime, (void *)0, 0);
  m_menubar->add("Replay/Jump To End", FL_ALT + ']',
                 (Fl_Callback *)REPLAY_GUI::cb_JumpTime, (void *)1,
                 FL_MENU_DIVIDER);

  m_menubar->add("Replay/Step Ahead 0.1 secs", FL_CTRL + ']',
                 (Fl_Callback *)REPLAY_GUI::cb_Step, (void *)100, 0);
  m_menubar->add("Replay/Step Back  0.1 secs", FL_CTRL + '[',
                 (Fl_Callback *)REPLAY_GUI::cb_Step, (void *)-100, 0);
  m_menubar->add("Replay/Step Ahead 1 sec", ']',
                 (Fl_Callback *)REPLAY_GUI::cb_Step, (void *)1000, 0);
  m_menubar->add("Replay/Step Back  1 sec", '[',
                 (Fl_Callback *)REPLAY_GUI::cb_Step, (void *)-1000, 0);
  m_menubar->add("Replay/Step Ahead 5 secs", '}',
                 (Fl_Callback *)REPLAY_GUI::cb_Step, (void *)5000, 0);
  m_menubar->add("Replay/Step Back  5 secs", '{',
                 (Fl_Callback *)REPLAY_GUI::cb_Step, (void *)-5000,
                 FL_MENU_DIVIDER);

  int ix1 = m_menubar->find_index("Vehicles/    Toggle Vehicle Name Mode");
  if (ix1 != -1)
    m_menubar->remove(ix1);
  int ix2 =
      m_menubar->find_index("Vehicles/    Toggle trails_connect_viewable");
  if (ix2 != -1)
    m_menubar->remove(ix2);
  int ix3 = m_menubar->find_index("Vehicles/Cycle Vehicle Focus");
  if (ix3 != -1)
    m_menubar->remove(ix3);

  // Remove some Menu items inherited from the parent class not implemented in
  // alogview

  Fl_Menu_Item *item;

  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/bearing_lines_viewable=true");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/bearing_lines_viewable=false");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/    Toggle bearing_lines_viewable");
  if (item)
    item->hide();

  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_name_mode=names+mode");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_name_mode=names+shortmode");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_name_mode=names+auxmode");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_name_mode=names+depth");
  if (item)
    item->hide();

  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_inactive_color=yellow");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_inactive_color=white");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_inactive_color=blue");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_inactive_color=green");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_inactive_color=red");
  if (item)
    item->hide();

  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/trails_connect_viewable=true");
  if (item)
    item->hide();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/trails_connect_viewable=false");
  if (item)
    item->hide();

  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/center_view = vehicle_active");
  if (item)
    item->hide();
  ...
  item =
      (Fl_Menu_Item *)m_menubar->find_item("Vehicles/vehicles_viewable=true");
  if (item)
    item->setonly();
  item =
      (Fl_Menu_Item *)m_menubar->find_item("Vehicles/vehicles_name_mode=names");
  if (item)
    item->setonly();
  item = (Fl_Menu_Item *)m_menubar->find_item(
      "Vehicles/vehicles_active_color=red");
  if (item)
    item->setonly();
...
//----------------------------------------- steptime
void REPLAY_GUI::steptime(int millisecs) { cb_Step_i(millisecs); }

//----------------------------------------- streaming
void REPLAY_GUI::streaming(int val) { cb_Streaming_i(val); }

//----------------------------------------- streaming
void REPLAY_GUI::streamspeed(bool faster) { cb_StreamSpeed_i(faster); }
...
inline void REPLAY_GUI::cb_StreamStep_i(int val) { m_replay_disp_gap = val; }
...
void REPLAY_GUI::updatePlayRateMsg() {
  ...
  if (m_stream) {
    if (m_replay_warp_ix == 0)
      m_replay_warp_msg = "(Time Warp = 0.125)";
    else if (m_replay_warp_ix == 1)
      m_replay_warp_msg = "(Time Warp = 0.25)";
    else if (m_replay_warp_ix == 2)
      m_replay_warp_msg = "(Time Warp = 0.5)";
    else if (m_replay_warp_ix == 3)
      m_replay_warp_msg = "(Time Warp = 1)";
    else if (m_replay_warp_ix == 4)
      m_replay_warp_msg = "(Time Warp = 2)";
    else if (m_replay_warp_ix == 5)
      m_replay_warp_msg = "(Time Warp = 4)";
    else if (m_replay_warp_ix == 6)
      m_replay_warp_msg = "(Time Warp = 8)";
    else if (m_replay_warp_ix == 7)
      m_replay_warp_msg = "(Time Warp = 16)";
    else if (m_replay_warp_ix == 8)
      m_replay_warp_msg = "(Time Warp = 32)";
    else if (m_replay_warp_ix == 9)
      m_replay_warp_msg = "(Time Warp = 64)";
...
void REPLAY_GUI::setVarHistMenus() {
  ...
  for (unsigned int mix = 0; mix < mix_count; mix++) {
    ...
    m_menubar->add(label.c_str(), 0, (Fl_Callback *)REPLAY_GUI::cb_VarHist,
                   (void *)ix);
...
void REPLAY_GUI::setAppLogMenus() {
  ...
  for (unsigned int alix = 0; alix < alix_count; alix++) {
    ...
    m_menubar->add(label.c_str(), 0, (Fl_Callback *)REPLAY_GUI::cb_AppLog,
                   (void *)ix);
    m_menubar->redraw();
  }
}
...
void REPLAY_GUI::setTaskDiaryMenus() {
  ...
  m_menubar->add(label.c_str(), 0, (Fl_Callback *)REPLAY_GUI::cb_TaskDiary);
  m_menubar->redraw();
}
...
void REPLAY_GUI::setIPFPlotMenus() {
  ...
  for (unsigned int aix = 0; aix < aix_count; aix++) {
    ...
    m_menubar->add(label.c_str(), 0, (Fl_Callback *)REPLAY_GUI::cb_IPF_GUI,
                   (void *)ix);
    m_menubar->redraw();
  }
}
...
void REPLAY_GUI::setEncounterPlotMenus() {
  ...
  for (unsigned int aix = 0; aix < aix_count; aix++) {
    ...
    m_menubar->add(label.c_str(), 0, (Fl_Callback *)REPLAY_GUI::cb_Encounter,
                   (void *)ix);
    m_menubar->redraw();
  }
}
...
void REPLAY_GUI::setHelmPlotMenus() {
  ...
  for (unsigned int aix = 0; aix < aix_count; aix++) {
    ...
    m_menubar->add(label.c_str(), 0, (Fl_Callback *)REPLAY_GUI::cb_Helm_GUI,
                   (void *)ix);
    m_menubar->redraw();
  }
}
