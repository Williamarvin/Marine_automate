...
int IvPFuncViewerX::handle(int event) { return (Fl_Gl_Window::handle(event)); }
...
void IvPFuncViewerX::addIPF_Plot(const IPF_Plot &g_ipf_plot) {
  ...
  if (m_vname != ipf_vname) {
    ...
    cout << "m_vname: " << m_vname << ", ipf_vname: [" << ipf_vname << "]"
         << endl;
    return;
  }
...
string IvPFuncViewerX::getCurrPieces() const { return (m_active_ipf_pieces); }
...
string IvPFuncViewerX::getCurrIteration() const { return (m_active_ipf_iter); }

//-------------------------------------------------------------
// Procedure: getCurrTime()

double IvPFuncViewerX::getCurrTime() const { return (m_curr_time); }
