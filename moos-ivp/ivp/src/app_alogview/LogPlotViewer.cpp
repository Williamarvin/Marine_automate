...
void LogPlotViewer::drawLogPlot() {
  ...
  if (m_show_left_logplot && (m_logplot1.size() != 0)) {
    ...
    if (cache_size > 75)
      glPointSize(2.0);
    else if (cache_size > 55)
      glPointSize(3.0);
    else if (cache_size > 40)
      glPointSize(4.0);
    else if (cache_size > 35)
      glPointSize(5.0);
    else
      glPointSize(6.0);
    if (color_scheme == 1)
  ...
  double scalet =
      ((m_curr_time - m_display_min_time) * m_step) + (m_margin / 2.0);
  scalet += m_lft_marg;
...
void LogPlotViewer::drawText(double px, double py, const string &text, double r,
                             double g, double b) {
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glOrtho(0, w(), 0, h(), -1, 1);
