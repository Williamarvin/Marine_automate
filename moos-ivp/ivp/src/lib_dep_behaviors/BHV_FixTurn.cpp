...
void BHV_FixTurn::onRunToIdleState() { setState("stem"); }

//-----------------------------------------------------------
// Procedure: onIdleToRunState()

void BHV_FixTurn::onIdleToRunState() { setState("stem"); }
...
bool BHV_FixTurn::handleConfigTurnSpec(string spec) {
  ...
  if (strBegins(spec, "clearall,") || strBegins(spec, "clear,")) {
    m_turn_spds.clear();
    m_turn_hdgs.clear();
    m_curr_tix = 0;
    biteString(spec, ',');
  ...
  // ===============================================
    ...
    for (unsigned int i = 0; i < svector.size(); i++) {
      ...
      if (param == "spd") {
        ...
        if ((value != "auto") && (value != "port") && (value != "star") &&
            (value != "starboard"))
          return (false);
        if (value == "starboard")
          value = "star";
