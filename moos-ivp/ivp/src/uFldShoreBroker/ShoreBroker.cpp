...
void ShoreBroker::sendAcks() {
  ...
  for (i = 0; i < vsize; i++) {
    ...
    if (elapsed_time < 10) {
      string msg_var =
          "NODE_BROKER_ACK_" + toupper(m_node_host_records[i].getCommunity());
...
void ShoreBroker::handleMailNodePing(const string &info) {
  ...
  if (m_time_warp_str != hrecord.getTimeWarp())
    status =
        "timewarp (" + m_time_warp_str + "!=" + hrecord.getTimeWarp() + ")";
...
void ShoreBroker::makeBridgeRequestAll() {
  ...
  for (i = 0; i < vsize; i++) {
    ...
    if (!m_node_bridges_made[i]) {
      ...
      for (k = 0; k < ksize; k++)
        makeBridgeRequest(m_bridge_src_var[k], hrecord, m_bridge_alias[k],
                          i + 1);
  ...
  for (i = 0; i < vsize; i++) {
    ...
    if (iroutes == "")
      ...
      for (unsigned int j = 0; j < svector.size(); j++) {
        ...
        if (j == 0)
          actab << node_name << hostip << status << s_elapsed << svector[0]
                << s_skew;
