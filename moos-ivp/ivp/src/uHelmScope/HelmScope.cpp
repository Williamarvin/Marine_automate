...
bool HelmScope::buildReport() { return (false); }
...
void HelmScope::handleNewHelmModeSet(const string &str) {
  ...
  for (i = 0; i < vsize; i++) {
    ...
    if (child == "") {
      ...
      // cout << "[" << i << "] parent:" << parent << "  child:" << child <<
      // endl;
...
void HelmScope::printReport() {
  ...
  list<string> report_lines =
      m_helm_report.formattedSummary(m_curr_time, m_concise_bhv_list);
