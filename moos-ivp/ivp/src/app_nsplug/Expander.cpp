...
bool Expander::expand() {
  ...
  vector<string> fvector = expandFile(m_infile, m_initial_macros,
                                      m_initial_filenames, m_inctag, result);
...
vector<string> Expander::expandFile(string filename,
                                    ...
                                    vector<string> filenames, string inctag,
                                    bool &result) {
  ...
  for (i = 0; i < vsize; i++) {
    ...
    //--------------------------------------------------------------
      ...
      if (!verifyInfile(full_file_str)) {
        cout << "#  Error in file " << filename << " line:" << i + 1 << endl;
        cout << "#  The #include file \"" << file_str << "\" not found."
             << endl;
        result = false;
        return (empty_vector);
      }
      ...
      vector<string> ilines =
          expandFile(full_file_str, macros, filenames, inctag, result);
      if (!result) {
    ...
    //----------------------------------------------------------------
      ...
      if ((mlen > 3) && (macro.at(0) == '$') && (macro.at(1) == '(') &&
          (macro.at(mlen - 1) == ')'))
...
bool Expander::verifyInfile() { return (verifyInfile(m_infile)); }
...
bool Expander::applyMacrosToLine(string &line, map<string, string> macros,
    ...
    if (!isCommented) {
      ...
      if (m_interactive) {
        ...
        while (!answered) {
          ...
          // cout << "char:[" << c << "]" << endl;
          if ((c == 'n') || (c == 'N') ||
              (c == ((int)(3)))) // ASCII 03 is control-c
            exit(1);
          if ((c == 'Y') || (c == 'y') ||
              (c == ((int)(10)))) // ASCII 10 is ENTER
            answered = true;
          else if (m_impatient)
            exit(1);
        }
...
void Expander::pushMode(string new_mode) { m_pmode.push_back(new_mode); }
