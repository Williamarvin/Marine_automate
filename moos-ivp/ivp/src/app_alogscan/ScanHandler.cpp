...
void ScanHandler::varStatReport() {
...
#else
  int line_digits =
      static_cast<int>(log10(static_cast<double>(m_report.getMaxLines()))) + 2;
  int char_digits =
      static_cast<int>(log10(static_cast<double>(m_report.getMaxChars()))) + 4;
  int start_digits = (int)(log10(m_report.getMaxStartTime())) + 4;
  int stop_digits = (int)(log10(m_report.getMaxStopTime())) + 4;
#endif
  ...
  printf("\n");
  printf(hformat_string.c_str(), "Variable Name", "Lines", "Chars", " Start",
         "  Stop", "Sources");
  ...
  for (i = 0; i < vsize; i++) {
    ...
    printf(bformat_string.c_str(), varname.c_str(), lcnt, schars.c_str(),
           sfirst.c_str(), slast.c_str(), varsources.c_str());
...
void ScanHandler::appStatReport() {
  ...
  for (i = 0; i < vsize; i++) {
    ...
    printf("%s           %s   %s       %s       %s\n", source.c_str(),
           s_lines.c_str(), s_chars.c_str(), s_lines_pct.c_str(),
           s_chars_pct.c_str());
  }
}
