...
void UCMD_GUI::augmentMenu() {
  m_menubar->add("File/Quit ", FL_ALT + 'q', (Fl_Callback *)UCMD_GUI::cb_Quit,
                 0, 0);
...
void UCMD_GUI::initWidgetsFolio() {
  ...
  for (list<string>::iterator p = vlist.begin(); p != vlist.end(); p++) {
    ...
    for (set<string>::iterator q = labels.begin(); q != labels.end(); q++) {
      ...
      button->callback((Fl_Callback *)UCMD_GUI::cb_ButtonCmdAction,
                       (void *)cbutton_ctr);
...
void UCMD_GUI::clearPendingCmdPosts() { m_pending_cmd_posts.clear(); }
...
void UCMD_GUI::cb_Close_i() { Fl_Widget::hide(); }
...
void UCMD_GUI::cb_Quit() { exit(0); }

//---------------------------------------------------
// Procedure: cb_ConcedeTopWindow

void UCMD_GUI::cb_ConcedeTopWindow_i() { m_concede_top = true; }
