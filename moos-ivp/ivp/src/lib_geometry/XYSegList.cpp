...
void XYSegList::mod_vertex(unsigned int ix, double x, double y, double z,
                           string vprop) {
  if (ix < size()) {
    m_vx[ix] = x;
    m_vy[ix] = y;
...
XYPoint XYSegList::get_first_point() const { return (get_point(0)); }
...
double XYSegList::max_dist_to_ctr() const {
  ...
  unsigned int i, vsize = m_vx.size();
  for (i = 0; i < vsize; i++) {
    double dist = hypot((ctr_x - m_vx[i]), (ctr_y - m_vy[i]));
    if (dist > max_dist)
      max_dist = dist;
  }
...
string XYSegList::get_spec(string param) const { return (get_spec(1, param)); }
...
double XYSegList::dist_to_point(double x, double y) const {
  ...
  double min_dist = distPointToSeg(m_vx[0], m_vy[0], m_vx[1], m_vy[1], x, y);

  for (unsigned int i = 1; i < m_vx.size() - 1; i++) {
    double dist =
        distPointToSeg(m_vx[i], m_vy[i], m_vx[i + 1], m_vy[i + 1], x, y);
    if (dist < min_dist)
      min_dist = dist;
  }
...
unsigned int XYSegList::closest_segment(double x, double y,
  ...
  // Use the distance to the first segment as initial "best-so-far"
  double dist = distPointToSeg(m_vx[0], m_vy[0], m_vx[1], m_vy[1], x, y);
  ...
  if (check_implied_seg) {
    double edist = distPointToSeg(m_vx[vsize - 1], m_vy[vsize - 1], m_vx[0],
                                  m_vy[0], x, y);
    if (edist < dist)
      ix = vsize - 1;
  }
...
void XYSegList::grow_pt_by_pct(double pct, double cx, double cy, double &px,
                               double &py) {
  px += ((px - cx) * pct);
  py += ((py - cy) * pct);
}
...
void XYSegList::grow_pt_by_amt(double amt, double cx, double cy, double &px,
                               double &py) {
...
void XYSegList::rotate_pt(double deg, double cx, double cy, double &px,
                          double &py) {
  // cout << "Rotate_pt: " << endl;
