...
/// \param ang Angle to orient the lawnmower pattern.  [0-360], 0 is North,
/// clockwise is positive \param radius 1/2 the distance between adjacent swaths
/// \param clockwise Determines the direction of the first turn, defaults to
/// true
const XYSegList generateLawnmower(const XYPolygon &poly, double px0, double py0,
                                  double ang, double radius,
                                  bool clockwise = true) {
  // General algorithm:
  // 1) Create initial segment
  // 2) Create parallel segment 2*radius away
  //    Check to see if segment intercepts, extend/contract segment to fit
  //    inside If segment doesn't intercept, quit
  ...
  if (poly.contains(px0, py0)) {
    ...
    return segList; // Return empty seglist
  ...
  // we will be reusing px0, py0, px1, py1 to represent the next two points to
  // add
  bool carryon = true;
  while (carryon) {
    ...
    if (poly.seg_intercepts(px0, py0, px1, py1)) {
      ...
      switch (situation) {
      case 1: // both inside, project 0 one way, 1 the other (segment is
              // oriented later)
      ...
      case 2: // p1 outside, project it in, project p0 same way
        if (distnormal1 == -1) { // p1 is outside, project antinormal
          ...
          // std::cout << "generateLawnmower ERROR!! both distances are
          // negative!";
        }
        break;
      ...
      // Figure out which point to add next (closest to tail of segList), then
      // add other
      if (distPointToPoint(px0, py0, segList.get_vx(segList.size() - 1),
                           segList.get_vy(segList.size() - 1)) <
          distPointToPoint(px1, py1, segList.get_vx(segList.size() - 1),
                           segList.get_vy(segList.size() - 1))) {
        segList.add_vertex(px0, py0);
        segList.add_vertex(px1, py1);
      } else {
...
/// It does so by creating two half lawnmowers starting at the center of the
/// polygon and then joining them together. \param poly Polygon string that
/// defines the boundaries \param ang Angle to orient the lawnmower pattern.
/// [0-360], 0 is North, clockwise is positive \param radius 1/2 the distance
/// between adjacent swaths
const XYSegList generateLawnmowerFull(const XYPolygon &poly, double ang,
                                      double radius) {
  ...
  XYSegList alpha =
      generateLawnmower(poly, x0, y0, angle360(ang + 180), radius, true);
...
const vector<string> generateArtifacts(double min_x, double max_x, double min_y,
                                       double max_y, double step,
                                       double num_artifacts,
  ...
  while (vArtifacts.size() < num_artifacts) {
    // RAND_MAX is defined by the math headers, refers to the maximum random
    // value
    double rand_x =
        snapToStep(min_x + rand() * (max_x - min_x) / RAND_MAX, step);
    double rand_y =
        snapToStep(min_y + rand() * (max_y - min_y) / RAND_MAX, step);
    ...
    if (poly.contains(rand_x, rand_y) &&
        !vectorContains(vArtifacts, sArtifact)) { // New artifact to add
