...
double WallEngine::getXCPA(double osh, double osv, double ttc_base,
...
void WallEngine::buildHitCacheDubins() {
  ...
  for (unsigned int i = 0; i < dsize; i++) {
    ...
    for (unsigned int j = 0; j < m_walls.size(); j++) {
      ...
      vector<double> vcpa;
      vector<double> vdcpa;
      unsigned int pts = cpasArcSegl(ax, ay, ar, lang, rang, m_walls[j], thresh,
  ...
  for (unsigned int i = 0; i < dsize; i++) {
    ...
    for (unsigned int j = 0; j < m_walls.size(); j++) {
      ...
      vector<double> vdcpa;
      unsigned int pts =
          cpasRaySegl(rx, ry, ray_angle, m_walls[j], thresh, vcpa, vdcpa);
...
void WallEngine::buildBaseProxCache() {
  ...
  for (unsigned int i = 0; i < m_walls.size(); i++) {
    ...
    unsigned int pts =
        cpasArcSegl(ax, ay, ar, lang, rang, m_walls[i], m_prox_thresh,
                    langle_is_origin, vcpa, vdcpa);
