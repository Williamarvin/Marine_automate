...
public:
  XYPolygon();
  XYPolygon(double x, double y, double rad, unsigned int pts,
            std::string label = "");
  XYPolygon(XYSegList);
  ~XYPolygon() {}
...
public:
  ...
  bool line_intersects(double x1, double y1, double x2, double y2, double &ix1,
                       double &iy1, double &ix2, double &iy2) const;
  ...
  bool closest_point_on_poly(double sx, double sy, double &rx,
                             double &ry) const;
...
protected:
  int side(double x1, double y1, double x2, double y2, double x3,
           double y3) const;
  void set_side(int);
