...
bool XYConvexGrid::initialize(const XYPolygon &poly, double cell_size,
                              double init_value) {
...
double XYConvexGrid::segIntersect(unsigned int ix, double x1, double y1,
                                  double x2, double y2) {
  if (ix >= m_elements.size())
    return (0);
...
void XYConvexGrid::setVal(unsigned int ix, double val, unsigned int cix) {
...
void XYConvexGrid::incVal(unsigned int ix, double val, unsigned int cix) {
  if ((ix >= m_elements.size()) || (cix >= m_cell_vars.size()))
    return;
...
bool XYConvexGrid::segIntersectBound(double x1, double y1, double x2,
                                     double y2) const {
...
bool XYConvexGrid::setEdgeCache(double pix_per_mtr_x, double pix_per_mtr_y) {
  if ((m_pix_per_mtr_x == pix_per_mtr_x) && (m_pix_per_mtr_y == pix_per_mtr_y))
    return (false);
