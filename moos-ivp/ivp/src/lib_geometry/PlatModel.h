...
public: // Setters (Pose)
  void setOSX(double v) {
    m_osx = v;
    m_osx_set = true;
  }
  void setOSY(double v) {
    m_osy = v;
    m_osy_set = true;
  }
...
public: // Setters (Info typically set by a PMGen)
  ...
  bool setStarSpokes(const std::vector<double> &, const std::vector<double> &);
  bool setPortSpokes(const std::vector<double> &, const std::vector<double> &);
