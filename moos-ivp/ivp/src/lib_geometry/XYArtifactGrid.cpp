...
bool XYArtifactGrid::segIntersectBound(double x1, double y1, double x2,
                                       double y2) const {
...
bool XYArtifactGrid::processDelta(const string &str) {
  ...
  for (unsigned int i = 0; i < vsize;
       i++) { // For each index,old_clearance,new_clearance
...
bool XYArtifactGrid::initialize(const XYPolygon &poly,
  ...
  // This assignment sets elements to contain only the squares in the bounding
  // poly
...
bool XYArtifactGrid::initialize(const XYSquare &outer_square,
                                const XYSquare &unit_square) {
