...
void VPlug_GeoShapes::addSeglr(const XYSeglr &new_seglr) {
  ...
  if (new_seglr.size() > 0) {
    updateBounds(new_seglr.getMinX(), new_seglr.getMaxX(), new_seglr.getMinY(),
                 new_seglr.getMaxY());
  }
...
void VPlug_GeoShapes::addVector(const XYVector &new_vect) {
  ...
  updateBounds(new_vect.xpos(), new_vect.xpos(), new_vect.ypos(),
               new_vect.ypos());
...
void VPlug_GeoShapes::addRangePulse(const XYRangePulse &new_pulse) {
  ...
  updateBounds(new_pulse.get_x(), new_pulse.get_x(), new_pulse.get_y(),
               new_pulse.get_y());
...
void VPlug_GeoShapes::addMarker(const XYMarker &new_marker) {
  ...
  updateBounds(new_marker.get_vx(), new_marker.get_vx(), new_marker.get_vy(),
               new_marker.get_vy());
...
unsigned int VPlug_GeoShapes::sizeTotalShapes() const {
  return (sizePolygons() + sizeSegLists() + sizeCircles() + sizeHexagons() +
          sizePoints() + sizeVectors() + sizeGrids() + sizeConvexGrids() +
          sizeMarkers() + sizeRangePulses() + sizeSeglrs() + sizeArrows() +
          sizeOvals() + sizeCommsPulses() + sizeTextBoxes());
}
...
void VPlug_GeoShapes::addGrid(const XYGrid &new_grid) {
  ...
  updateBounds(square.get_min_x(), square.get_max_x(), square.get_min_y(),
               square.get_max_y());
...
void VPlug_GeoShapes::addOval(const XYOval &new_oval, double draw_degs) {
  ...
  updateBounds(new_oval.get_min_x(), new_oval.get_max_x(), new_oval.get_min_y(),
               new_oval.get_max_y());
...
void VPlug_GeoShapes::addArrow(const XYArrow &new_arrow) {
  ...
  updateBounds(m_arrows[new_label].getMinX(), m_arrows[new_label].getMaxX(),
               m_arrows[new_label].getMinY(), m_arrows[new_label].getMaxY());
}
...
void VPlug_GeoShapes::addWedge(const XYWedge &new_wedge) {
  ...
  updateBounds(new_wedge.getMinX(), new_wedge.getMaxX(), new_wedge.getMinY(),
               new_wedge.getMaxY());
...
void VPlug_GeoShapes::addHexagon(const XYHexagon &hexagon) {
  ...
  updateBounds(hexagon.get_min_x(), hexagon.get_max_x(), hexagon.get_min_y(),
               hexagon.get_max_y());
...
bool VPlug_GeoShapes::addPolygon(const string &poly_str, double timestamp) {
...
bool VPlug_GeoShapes::addWedge(const string &wedge_str, unsigned int draw_pts) {
...
bool VPlug_GeoShapes::addRangePulse(const string &pulse_str, double timestamp) {
...
bool VPlug_GeoShapes::addCommsPulse(const string &pulse_str, double timestamp) {
...
bool VPlug_GeoShapes::addMarker(const string &marker_str, double timestamp) {
...
bool VPlug_GeoShapes::addTextBox(const string &tbox_str, double timestamp) {
...
bool VPlug_GeoShapes::addSegList(const string &segl_str, double timestamp) {
...
bool VPlug_GeoShapes::addPoint(const string &point_str, double timestamp) {
...
bool VPlug_GeoShapes::addCircle(const string &circle_str, unsigned int drawpts,
                                double timestamp) {
...
bool VPlug_GeoShapes::addOval(const string &oval_str, double draw_degs,
                              double timestamp) {
...
bool VPlug_GeoShapes::addArrow(const string &arrow_str, double timestamp) {
...
void VPlug_GeoShapes::updateBounds(double xl, double xh, double yl, double yh) {
  if (xl < m_xmin)
    m_xmin = xl;
  if (xh > m_xmax)
    m_xmax = xh;
...
void VPlug_GeoShapes::updateBounds() {
  ...
  for (i = 0; i < m_polygons.size(); i++) {
    if (m_polygons[i].size() > 0) {
      ...
      updateBounds(poly.get_min_x(), poly.get_max_x(), poly.get_min_y(),
                   poly.get_max_y());
    }
  }
  for (i = 0; i < m_seglists.size(); i++) {
    if (m_seglists[i].size() > 0) {
      ...
      updateBounds(segl.get_min_x(), segl.get_max_x(), segl.get_min_y(),
                   segl.get_max_y());
    }
  }
  for (i = 0; i < m_hexagons.size(); i++) {
    if (m_hexagons[i].size() > 0) {
      ...
      updateBounds(hexa.get_min_x(), hexa.get_max_x(), hexa.get_min_y(),
                   hexa.get_max_y());
    }
  }
  ...
  for (i = 0; i < m_vectors.size(); i++) {
    ...
    updateBounds(vect.xpos(), vect.xpos(), vect.ypos(), vect.ypos());
  }
  for (i = 0; i < m_range_pulses.size(); i++) {
    XYRangePulse pulse = m_range_pulses[i];
    updateBounds(pulse.get_x(), pulse.get_x(), pulse.get_y(), pulse.get_y());
  }
  ...
  for (p2 = m_markers.begin(); p2 != m_markers.end(); p2++) {
    ...
    updateBounds(marker.get_vx(), marker.get_vx(), marker.get_vy(),
                 marker.get_vy());
  }
  ...
  for (p3 = m_circles.begin(); p3 != m_circles.end(); p3++) {
    ...
    updateBounds(circle.get_min_x(), circle.get_max_x(), circle.get_min_y(),
                 circle.get_max_y());
  }
  ...
  for (p4 = m_arrows.begin(); p4 != m_arrows.end(); p4++) {
    ...
    updateBounds(arrow.getMinX(), arrow.getMaxX(), arrow.getMinY(),
                 arrow.getMaxY());
  }
  ...
  for (p6 = m_seglrs.begin(); p6 != m_seglrs.end(); p6++) {
    ...
    updateBounds(seglr.getMinX(), seglr.getMaxX(), seglr.getMinY(),
                 seglr.getMaxY());
  }
}
...
void VPlug_GeoShapes::clearSeglrs(string stype) { m_seglrs.clear(); }
