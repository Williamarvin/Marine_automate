...
void Seglr::setRayAngle(double angle) { m_ray_angle = angle; }
...
double Seglr::getRayAngle() const { return (m_ray_angle); }
...
bool Seglr::crossesLine(double x1, double y1, double x2, double y2, double &ix,
                        double &iy, bool ray_first) const {
  // Sanity check
  ...
  for (unsigned int i = 0; i < vsize; i++) {
    if ((i + 1) < vsize) {
      bool crosses = linesCross(m_vx[i], m_vy[i], m_vx[i + 1], m_vy[i + 1], x1,
                                y1, x2, y2, ix, iy);
      if (crosses)
        return (true);
    }
