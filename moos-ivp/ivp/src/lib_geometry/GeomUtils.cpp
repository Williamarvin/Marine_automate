...
double distPointToPoint(const XYPoint &pt1, const XYPoint &pt2) {
  return (hypot((pt1.get_vx() - pt2.get_vx()), (pt1.get_vy() - pt2.get_vy())));
}
...
double distPointToSeg(double x1, double y1, double x2, double y2, double px,
                      double py) {
  double ix, iy;
...
double distToSegment(double x1, double y1, double x2, double y2, double px,
                     double py) {
  double ix, iy;
  perpSegIntPt(x1, y1, x2, y2, px, py, ix, iy);
  return (distPointToPoint(px, py, ix, iy));
...
double distSegToSeg(double x1, double y1, double x2, double y2, double x3,
...
bool linesCross(double x1, double y1, double x2, double y2, double x3,
                double y3, double x4, double y4, double &ix, double &iy) {
  ix = 0;
  iy = 0;
...
bool segmentsCross(double x1, double y1, double x2, double y2, double x3,
                   double y3, double x4, double y4) {
  double ix, iy;
...
bool lineRayCross(double x1, double y1, double ray_angle, double x3, double y3,
  ...
  if (slope_a == slope_b) {   // Special case: parallel lines
    if (inter_a == inter_b) { // If identical/overlapping, just pick
      ix = x3;
      iy = y3; // any point. vertex of line is fine.
      return (true);
    } else
      return (false);
  }
...
bool lineSegCross(double sx1, double sy1, double sx2, double sy2, double lx3,
                  double ly3, double lx4, double ly4, double &ix1, double &iy1,
...
double segmentAngle(double x1, double y1, double x2, double y2, double x3,
                    double y3) {
...
void perpSegIntPt(double x1, double y1, double x2, double y2, double qx,
...
void perpLineIntPt(double x1, double y1, double x2, double y2, double qx,
                   double qy, double &rx, double &ry) {
  double xdelta = x1 - x2;
  if (xdelta < 0)
    xdelta = -xdelta;
...
void projectPoint(double degval, double dist, double cx, double cy, double &rx,
                  double &ry) {
...
bool bearingMinMaxToPoly(double osx, double osy, const XYPolygon &poly,
  ...
  double bmax_dist = 0;
  return (
      bearingMinMaxToPolyX(osx, osy, poly, bmin, bmax, bmin_dist, bmax_dist));
}
...
bool bearingMinMaxToPolyX(double osx, double osy, const XYPolygon &poly,
                          double &bmin, double &bmax, double &bmin_dist,
                          double &bmax_dist) {
  //===========================================================
...
double distCircleToLine(double cx, double cy, double radius, double px1,
...
bool crossRaySegl(double px, double py, double ph, const XYSegList &segl,
...
double segRayCPA(double rx, double ry, double ray_angle, double x1, double y1,
                 double x2, double y2, double &ix, double &iy) {
...
double seglRayCPA(double rx, double ry, double ray_angle, const XYSegList &segl,
                  double &ix, double &iy) {
...
bool crossRaySeg(double rx, double ry, double ray_angle, double x1, double y1,
                 double x2, double y2, double &ix, double &iy) {
  ix = 0;
  iy = 0;

  double kx, ky;
  bool ray_crosses_line =
      lineRayCross(rx, ry, ray_angle, x1, y1, x2, y2, kx, ky);
  if (!ray_crosses_line)
    return (false);
...
int lineCircleIntPts(double x1, double y1, double x2, double y2, double cx,
                     double cy, double crad, double &rx1, double &ry1,
...
int segCircleIntPts(double x1, double y1, double x2, double y2, double cx,
                    double cy, double crad, double &rx1, double &ry1,
                    double &rx2, double &ry2) {
  // First determine whether or where the line, defined by the
  // given segment, intersects the circle.
  int cross_pts =
      lineCircleIntPts(x1, y1, x2, y2, cx, cy, crad, rx1, ry1, rx2, ry2);
...
double distPointToLine(double px, double py, double x1, double y1, double x2,
                       double y2) {
  double ix, iy;
...
double polyRayCPA(double rx, double ry, double ray_angle, const XYPolygon &poly,
...
bool randPointOnPoly(double vx, double vy, const XYPolygon &poly, double &rx,
                     double &ry) {
  if (!poly.is_convex())
    return (false);
...
bool modSegLen(double x1, double y1, double x2, double y2, double &rx1,
               double &ry1, double &rx2, double &ry2, double newlen) {
...
bool modSegAng(double x1, double y1, double x2, double y2, double &rx1,
               double &ry1, double &rx2, double &ry2, double new_angle) {
  double curr_len = hypot(x1 - x2, y1 - y2);
  if (curr_len == 0)
    return (false);
...
bool modSegLoc(double x1, double y1, double x2, double y2, double &rx1,
               double &ry1, double &rx2, double &ry2, double cx, double cy) {
  double curr_len = hypot(x1 - x2, y1 - y2);
  if (curr_len == 0)
    return (false);
...
double distSeglToPoly(const XYSegList &segl, const XYPolygon &poly, double &rix,
                      double &riy) {
  // Sanity checks
...
double distSeglrToPoly(const XYSeglr &seglr, const XYPolygon &poly, double &rix,
                       double &riy, double &stemdist, bool verbose) {
...
double distSeglrToPoint(const XYSeglr &seglr, const XYPoint &pt, double &rix,
                        double &riy, double &stemdist, bool verbose) {
  // Sanity checks
  if (!seglr.valid()) {
    rix = 0;
...
double distSeglToPoint(const XYSegList &segl, const XYPoint &pt, double &rix,
                       double &riy) {
...
double stemDistSeglFromPoint(const XYSegList &segl, double px, double py) {
  double stemdist = 0;
  XYPoint pt(px, py);
  double rix, riy;
