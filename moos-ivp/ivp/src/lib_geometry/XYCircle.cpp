...
bool XYCircle::segIntersect(double x1, double y1, double x2, double y2) const {
...
bool XYCircle::segIntersectStrict(double x1, double y1, double x2,
                                  double y2) const {
  // First a quick way of detecting a non-intersection (based on
  // an assumption that most such queries are not-even-close false)
  if ((x1 > (m_x + m_rad)) && (x2 > (m_x + m_rad)))
...
int XYCircle::segIntersectPts(double x1, double y1, double x2, double y2,
  ...
  if ((rx1 >= x_low) && (rx1 <= x_high) && (ry1 >= y_low) && (ry1 <= y_high))
    count++;

  // If the 2nd point is in the bounding box, count it.
  if ((rx2 >= x_low) && (rx2 <= x_high) && (ry2 >= y_low) && (ry2 <= y_high)) {
    count++;
...
double XYCircle::segIntersectLen(double x1, double y1, double x2,
                                 double y2) const {
