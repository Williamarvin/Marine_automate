...
bool IvPContactBehavior::setParam(string param, string param_val) {
  ...
  else if ((param == "bearing_line_config") || (param == "bearing_lines")) {
    m_bearing_line_show = false;
...
void IvPContactBehavior::handleContactFlags() {
  ...
  for (unsigned int i = 0; i < m_cnflags.size(); i++) {
    ...
    if (strBegins(tag, "@<") || strBegins(tag, "<") || strBegins(tag, "@>") ||
        strBegins(tag, ">")) {
...
bool IvPContactBehavior::updatePlatformInfo() {
  ...
  if (ok)
    m_osx = getBufferDoubleVal("NAV_X", ok);
  if (ok)
    m_osy = getBufferDoubleVal("NAV_Y", ok);
  if (ok)
    m_osh = getBufferDoubleVal("NAV_HEADING", ok);
  if (ok)
  ...
  double cnutc = 0;
  if (ok)
    m_cnx = getBufferDoubleVal(m_contact + "_NAV_X", ok);
  if (ok)
    m_cny = getBufferDoubleVal(m_contact + "_NAV_Y", ok);
  if (ok)
    m_cnh = getBufferDoubleVal(m_contact + "_NAV_HEADING", ok);
  if (ok)
    m_cnv = getBufferDoubleVal(m_contact + "_NAV_SPEED", ok);
  if (ok)
    cnutc = getBufferDoubleVal(m_contact + "_NAV_UTC", ok);
