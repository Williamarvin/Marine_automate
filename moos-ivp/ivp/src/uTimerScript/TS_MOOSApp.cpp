...
bool TS_MOOSApp::Iterate() {
  ...
  m_elapsed_time =
      (m_curr_time - m_script_start_time) + m_skip_time - m_pause_time;
  ...
  for (p = sParams.rbegin(); p != sParams.rend(); p++) {
    ...
    if (param == "event")
      ...
      if (!strContainsWhite(value))
        ...
        reportConfigWarning(
            "The forward_var parameter given var with whitespace");
    } else if ((param == "reset_var") || (param == "reset_variable")) {
      if (!strContainsWhite(value))
        m_var_reset = value;
      else
        reportConfigWarning(
            "The reset_var parameter given var with whitespace");
      ...
      if (!strContainsWhite(value))
        ...
        reportConfigWarning(
            "The pause_var parameter given var with whitespace");
    } else if (param == "status_var") {
      if (!strContainsWhite(value))
        m_var_status = value;
      else
        reportConfigWarning(
            "The status_var parameter given var with whitespace");
...
void TS_MOOSApp::scheduleEvents(bool shuffle_requested) {
  ...
  if (shuffle_requested || unscheduled_events) {
    for (i = 0; i < vsize; i++) {
      ...
      m_ptime[i] =
          m_ptime_min[i] + (rand_pct * (m_ptime_max[i] - m_ptime_min[i]));
    }
  }
...
bool TS_MOOSApp::buildReport() {
  ...
  if (m_uts_time_warp.getMinVal() != m_uts_time_warp.getMaxVal()) {
    ...
    s_uts_time_warp +=
        "," + doubleToStringX(m_uts_time_warp.getMaxVal(), 1) + "]";
  }
