...
template <typename T> bool my_isnan(const T x) {
...
void PMV_Viewer::draw() {
  ...
  for (unsigned int i = 0; i < vnames.size(); i++) {
    ...
    const map<string, XYCircle> &circles =
        m_geoshapes_map.getCircles(vnames[i]);
    ...
    const map<string, XYMarker> &markers =
        m_geoshapes_map.getMarkers(vnames[i]);
    const map<string, XYTextBox> &textboxes =
        m_geoshapes_map.getTextBoxes(vnames[i]);
...
void PMV_Viewer::drawVehicle(string vname, bool active, string vehibody) {
  ...
  //  double shape_length = m_vehiset.getDoubleInfo(vname, "vlength") *
  //  shape_scale;
...
bool PMV_Viewer::updateScopeVariable(string varname, string value, string vtime,
                                     string vsource) {
  if (!m_scoping)
    return (false);
  ...
  if (info_type == "scope_var") {
    ...
    if (dhandled1 && dhandled2) {
      ...
      if (m_reference_point != "datum") {
        ...
        bool ok3 =
            m_vehiset.getDoubleInfo("center_vehicle", "heading", heading);
        if (ok1 && ok2 && ok3) {
          x_center = cxpos;
          y_center = cypos;
          h_heading = heading;
