...
bool FldNodeComms::Iterate() {
  ...
  for (p3 = m_map_newackmessage.begin(); p3 != m_map_newackmessage.end();
       p3++) {
  ...
  for (p = dest_names.begin(); p != dest_names.end(); p++) {
    ...
    if (msg_send) {
      ...
      if ((id != "") && listContains(m_recent_ackids, id))
        postViewCommsPulse(src_name, a_dest_name, "msg", m_msg_repeat_color,
                           0.6);
      else
...
bool FldNodeComms::buildReport() {
  ...
  m_msgs << "   Min Message Int: " << doubleToStringX(m_min_msg_interval)
         << endl;
  m_msgs << "   Min Report  Int: " << doubleToStringX(m_min_rpt_interval, 2)
         << endl;
  ...
  for (q2 = m_map_messages_sent.begin(); q2 != m_map_messages_sent.end();
       q2++) {
  ...
  unsigned int total_blk_msgs = m_blk_msg_invalid + m_blk_msg_toostale +
                                m_blk_msg_tooquick + m_blk_msg_toolong +
                                m_blk_msg_toofar + m_blk_msg_noinfo;
