...
bool BFactoryDynamic::loadDirectory(string dirname) {
  ...
  for (i = 0; i < fsize; ++i) {
    ...
    if (fname.substr(fname.length() - suffix_len, suffix_len) !=
        library_suffix) {
      continue;
    }
...
#ifdef _WIN32
    TFuncPtrCreateBehavior createFn = (TFuncPtrCreateBehavior)(GetProcAddress(
        (HMODULE)handle, "createBehavior"));
...
bool BFactoryDynamic::isKnownBehavior(string bhv_name) const {
  return (m_creation_funcs_map.find(bhv_name) != m_creation_funcs_map.end());
}
