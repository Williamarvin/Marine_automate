...
BehaviorSet::~BehaviorSet() { clearBehaviors(); }
...
bool BehaviorSet::buildBehaviorsFromSpecs() {
  ...
  for (i = 0; i < vsize; i++) {
    ...
    if (!sbuild.valid()) {
      ...
      m_helm_start_msgs.insert(m_helm_start_msgs.end(), msgs.begin(),
                               msgs.end());
...
SpecBuild BehaviorSet::buildBehaviorFromSpec(BehaviorSpec spec,
  ...
  if (bhv)
    ...
    if (bhv)
      ...
      addWarning(err);
      addWarning(
          "Check IVP_BEHAVIOR_DIRS Env Variable or ivp_behavior_dir param");
  ...
  for (i = 0; i < isize; i++) {
    ...
    for (unsigned int j = 0; j < jsize; j++) {
      ...
      if ((m_bhv_names.count(fullname) == 0) &&
          (m_bhv_names.count(update_name) == 0)) {
        SpecBuild sbuild =
            buildBehaviorFromSpec(m_behavior_specs[i], update_str);
...
vector<VarDataPair> BehaviorSet::getMessages(unsigned int ix, bool clear) {
...
void BehaviorSet::addWarning(const string &str) { m_warnings.push_back(str); }
